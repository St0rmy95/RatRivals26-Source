...
#include "AtumAdminToolDlg.h" // 2013-09-11 by jekim, 어드민 툴 해상도 수정
	...
	: CDialog(CAtumAdminToolDlg::IDD, pParent)
{
	...
	m_pButtonDlg = NULL; // 2013-09-11 by jekim, 어드민 툴 해상도 수정
...
{
	SAFE_DELETE(m_pButtonDlg); // 2013-09-11 by jekim, 어드민 툴 해상도 수정
	...
	// 2014-07-14 by bckim, 워포샵 리스트 컨트롤
	SAFE_DELETE(m_pWPShopManagementDlg);
	// End. 2014-07-14 by bckim, 워포샵 리스트 컨트롤
...
BOOL CAtumAdminToolDlg::OnInitDialog()
{
	...
	m_pButtonDlg = new CAtumAdminToolButtonDlg(this);
	CRect rc;
	...
	// 2013-09-11 by jekim, 어드민 툴 해상도 수정 - 서브 Dlg생성으로 인한 주석처리
...
//	// End. 2013-04-18 by jhseol,bckim 이달의 아머
	// end 2013-09-11 by jekim, 어드민 툴 해상도 수정 - 서브 Dlg생성으로 인한 주석처리
...
// 2013-09-11 by jekim, 어드민 툴 해상도 수정
	if(GetSystemMetrics(SM_CYSCREEN)>=1000)
	{
		CRect rect;
		GetWindowRect(rect);
		rect.bottom += 160;
...
void CAtumAdminToolDlg::OnButtonUserTool() 
{
	...
	if (m_pUserAdminDlg == NULL)
	{
		...
		m_pUserAdminDlg->Create(IDD_DIALOG_USER_ADMIN_TOOL, this);		// 2014-05-13 by bckim, IDD 리소스 에러 
	}
...
// 2013-09-11 by jekim, 어드민 툴 해상도 수정 - 서브 Dlg생성으로 인한 주석처리
	m_pButtonDlg->EnableToolControls(i_bEnable);
...
// 	GetDlgItem(IDC_BUTTON_LOG_TOOL)->EnableWindow(i_bEnable);
// 	GetDlgItem(IDC_BUTTON_HAPPY_HOUR_EVENT_TOOL)->EnableWindow(i_bEnable);
	GetDlgItem(IDC_COMBO_SERVER)->EnableWindow(!i_bEnable); // 2013-10-16 by jekim, 베트남 툴 빌드 에러 수정.
// 	GetDlgItem(IDC_BTN_GUILD_TOOL)->EnableWindow(i_bEnable);
// 	GetDlgItem(IDC_BUTTON_ITEM_EVENT)->EnableWindow(i_bEnable);
// 	GetDlgItem(IDC_BUTTON_STRATEGYPOINT)->EnableWindow(i_bEnable);
...
// end 2013-09-11 by jekim, 어드민 툴 해상도 수정 - 서브 Dlg생성으로 인한 주석처리
	...
	m_pButtonDlg->EnableToolControls(usAccountType);// 2013-09-11 by jekim, 어드민 툴 해상도 수정
}
...
//	m_TrayIcon.RemoveIcon();
	...
	if(m_pButtonDlg) { m_pButtonDlg->DestroyWindow(); } // 2013-09-11 by jekim, 어드민 툴 해상도 수정
	...
	// 2014-07-14 by bckim, 워포샵 리스트 컨트롤
	if(m_pWPShopManagementDlg){	m_pWPShopManagementDlg->DestroyWindow();}		
	// End. 2014-07-14 by bckim, 워포샵 리스트 컨트롤
...
// 2014-07-14 by bckim, 워포샵 리스트 컨트롤
void CAtumAdminToolDlg::OnBtnWPshopManagement()
{
	if (m_pWPShopManagementDlg != NULL)
	{
		SAFE_DELETE(m_pWPShopManagementDlg);
	}
	...
	if (m_pWPShopManagementDlg == NULL)
	{
		m_pWPShopManagementDlg = new CWPShopManagementDlg(this);
		if(FALSE == m_pWPShopManagementDlg->Create(IDD_DLG_WP_SHOP_MANAGEMENT, this))
		{
			return;
		}
	}
	
	m_pWPShopManagementDlg->ShowWindow(SW_SHOW);
}
// End. 2014-07-14 by bckim, 워포샵 리스트 컨트롤
...
void CAtumAdminToolDlg::ExchangeAccountDBName(CString *szTmpQuery)
{
	...
	{
		// 2013-11-12 by jhseol, 베트남 운영툴 체크 IP -> DB Name 으로 변경
		if ( strlen(m_pServerInfo4Admin->DBName) == strlen("atum2_db_15") || strlen(m_pServerInfo4Admin->DBName) == strlen("atum2_db_18") )
		{
			if( 0 == strcmp( m_pServerInfo4Admin->DBName, "atum2_db_15") || 0 == strcmp( m_pServerInfo4Admin->DBName, "atum2_db_18") )
			{
...
// 			}
// 		}
		// end 2013-11-12 by jhseol, 베트남 운영툴 체크 IP -> DB Name 으로 변경
	}
#endif
}
...
void CAtumAdminToolDlg::OnGetMinMaxInfo(MINMAXINFO FAR* lpMMI) 
{
	// TODO: Add your message handler code here and/or call default
	lpMMI->ptMinTrackSize.x=516;
	lpMMI->ptMaxTrackSize.x=516;
	lpMMI->ptMinTrackSize.y=467;
	lpMMI->ptMaxTrackSize.y=922;
	CDialog::OnGetMinMaxInfo(lpMMI);
}
// 2013-09-11 by jekim, 어드민 툴 해상도 수정
void CAtumAdminToolDlg::OnSizing(UINT fwSide, LPRECT pRect) 
{
	/*
#define MOVE_CONTROL(id,a,b) do {\
	GetDlgItem((id))->GetWindowRect(r);\
	ScreenToClient(&r);\
	r.top = win.bottom - (a);\
	r.bottom = r.top + (b);\
	GetDlgItem((id))->MoveWindow(r,FALSE); \
} while(0)

	CRect win;
	GetClientRect(win);
	CRect r;
	MOVE_CONTROL(IDC_BTN_ARENA_CHAR_SEND,32,28);
	MOVE_CONTROL(IDCANCEL,32,28);
	MOVE_CONTROL(IDC_BTN_LOCALIZATION,68,28);
	MOVE_CONTROL(IDC_EDIT_LANGUAGE,92,24);
	MOVE_CONTROL(IDC_STATIC_LANGUAGE,90,16);
	MOVE_CONTROL(IDC_STATIC_TOOLS2,114,80);
	GetDlgItem(IDC_STATIC_TOOLS)->GetWindowRect(r);
	ScreenToClient(r);
	r.bottom = win.bottom - 118;
	GetDlgItem(IDC_STATIC_TOOLS)->MoveWindow(r,FALSE);
	
	GetDlgItem(IDC_PLACEHOLDER)->GetWindowRect(r);
	ScreenToClient(r);
	r.bottom = win.bottom - 120;
	m_pButtonDlg->MoveWindow(r,FALSE);
	Invalidate(TRUE);

#undef MOVE_CONTROL	*/
}

BOOL CAtumAdminToolDlg::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt) 
{
	// TODO: Add your message handler code here and/or call default
		
	m_pButtonDlg->OnMouseWheel(nFlags,zDelta,pt);
	return FALSE;
}
// end 2013-09-11 by jekim, 어드민 툴 해상도 수정