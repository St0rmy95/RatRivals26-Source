	...
	: CDialog(CLogTabUser::IDD, pParent)
{
	...
	m_TimeEnd = 0;
	m_TimeStart = 0;
	m_bCheckUserDeleteCharacterAdmin = FALSE;
	//}}AFX_DATA_INIT
...
void CLogTabUser::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Check(pDX, IDC_CHECK_USER_DELETE_CHARACTER_BY_ADMIN, m_bCheckUserDeleteCharacterAdmin);
	//}}AFX_DATA_MAP
...
BEGIN_MESSAGE_MAP(CLogTabUser, CDialog)
	...
	ON_NOTIFY(NM_DBLCLK, IDC_GRID_USER_LOG, OnGridDClick)		// 2014-01-23 by jhseol, 어드민 툴 로그조회 캐나다 요청 편의기능 추가
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
// end 2013-01-14 by jhseol, DB 풀로그 남기기 - 문법 오류 삭제
	...
	if (bRet & m_bCheckUserDeleteCharacterAdmin)
	{
		bRet = GetLogAndInsertToGrid(T1_FL_LOG_DELETE_CHARACTER_BY_ADMIN, "atum_log_user_game_start_end","''");
		if(bRet)
		{
			bRet = GetLogAndInsertToGrid(T1_FL_LOG_DELETE_CHARACTER_BY_ADMIN, "atum_backup_log_user_game_start_end","''");
		}
	}
...
BOOL CLogTabUser::GetLogAndInsertToGrid(BYTE T1_LogType, LPCSTR szTableName, LPCSTR szContentString1, LPCSTR szContentString2 /* = NULL */)
{
	...
	if (m_bCheckAccountName)
	{
			...
			|| stricmp("atum_backup_log_user_game_start_end", szTableName) == 0)
		{
			...
			if(*((const char*)m_szAccountName) != '#') szAccountCond.Format(QUERY_080702_0013_6_A, m_szAccountName);
			else szAccountCond.Format(QUERY_131224_0001_A, ((const char*)m_szAccountName)+1);
		...
		else
		{
			...
			// 2013-12-30 by jekim, 로그 UID 검색 되게 수정.
			if(*((const char*)m_szAccountName) != '#') szAccountCond.Format(QUERY_080702_0013_6_B, m_szAccountName);		// 2008-07-02 by cmkwon, MySQL 지원 구현 - 
	...
	if (m_bCheckCharacterName)
	{
			...
			|| stricmp("atum_backup_log_user_game_start_end", szTableName) == 0)
		{
			...
			// 2013-12-30 by jekim, 로그 UID 검색 되게 수정.
			if(*((const char*)m_szCharacterName) != '#') szCharacterCond.Format(QUERY_080702_0013_7_A, m_szCharacterName);		// 2008-07-02 by cmkwon, MySQL 지원 구현 - 
			else szCharacterCond.Format(QUERY_131224_0002_A, ((const char*)m_szCharacterName)+1);
			// end 2013-12-30 by jekim, 로그 UID 검색 되게 수정.
		}
		else
		{
			...
			// 2013-12-30 by jekim, 로그 UID 검색 되게 수정.
			if(*((const char*)m_szCharacterName) != '#') szCharacterCond.Format(QUERY_080702_0013_7_B, m_szCharacterName);	// 2008-07-02 by cmkwon, MySQL 지원 구현 - 
...
void CLogTabUser::OnButtonSelectAll() 
{
	...
	m_bCheckUserDeleteCharacterAdmin = TRUE; // 2014-01-21 by jekim, 툴에서 캐릭터 삭제 가능하게 하기.
...
void CLogTabUser::OnButtonDiselectAll() 
{
	...
	m_bCheckUserCreateCharacter = FALSE;
	m_bCheckUserDeleteCharacter = FALSE;
	m_bCheckUserDeleteCharacterAdmin = FALSE; // 2014-01-21 by jekim, 툴에서 캐릭터 삭제 가능하게 하기.
	m_bCheckUserLevelChange = FALSE;
	m_bCheckUserPVPLose = FALSE;
	m_bCheckUserPVPWin = FALSE;
	m_bCheckUserQuestEnd = FALSE;
...
void CLogTabUser::OnGridDClick(NMHDR *pNotifyStruct, LRESULT* pResult)
{
	OutputDebugString("[SEOL Debuging Check] : Mouse D Click. \r\n");
	CCellID cellID = m_GridUserLog.GetFocusCell();
	if(0 > cellID.row || (FALSE == IS_IN_RANGE(0, cellID.col, NUM_COLUMNS_USER_LOG_GRID-1)) )
	...
	case 0:
		{
			...
			m_bCheckCharacterName = FALSE;
			m_szAccountName = (CString)m_GridUserLog.GetItemText(cellID.row, cellID.col);
			bDateChange = TRUE;
		}
		break;
	case 1:
		{
			...
			m_szCharacterName = (CString)m_GridUserLog.GetItemText(cellID.row, cellID.col);
			bDateChange = TRUE;
		}
		break;
	default:
