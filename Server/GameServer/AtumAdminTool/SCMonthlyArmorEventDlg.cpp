	...
	: CDialog(SCMonthlyArmorEventDlg::IDD, pParent)
{
	...
	//}}AFX_DATA_INIT
	m_nDURATION = 30;		// 2013-09-06 by jhseol, 이달의 아머 이벤트 적용 기간 변경 - 초기값 30일
}
...
void SCMonthlyArmorEventDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, IDC_EDIT_DURATION, m_nDURATION);			// 2013-09-06 by jhseol, 이달의 아머 이벤트 적용 기간 변경
...
{	
	...
	// 2014-07-16 by bckim, 이달의 아머 운영툴 설정 예외처리
	UpdateData(TRUE);
	ATUM_DATE_TIME tempStartDate,tempEndDate;
	tempStartDate.SetDateTime(m_timeStartDate.GetYear(),m_timeStartDate.GetMonth(),m_timeStartDate.GetDay(),m_timeStartTime.GetHour(),0,0);
	tempEndDate.SetDateTime(m_timeEndDate.GetYear(),m_timeEndDate.GetMonth(),m_timeEndDate.GetDay(),m_timeEndTime.GetHour(),59,59);
	tempStartDate.AddDateTime(0,0,m_nDURATION,0,0,0);

	if( tempStartDate <= tempEndDate )
	{
		MessageBox("The duration of the event and the option period is not correct ! \r\n (Option period be longer than the duration of the event) ",NULL,MB_OK);
		this->Refresh();
		return;
	}
	// End. 2014-07-16 by bckim, 이달의 아머 운영툴 설정 예외처리
	...
	NewEvent.nDuration = m_nDURATION;		// 2013-09-06 by jhseol, 이달의 아머 이벤트 적용 기간 변경
...
void SCMonthlyArmorEventDlg::Refresh()
{
	...
	m_nDURATION = 30;		// 2013-09-06 by jhseol, 이달의 아머 이벤트 적용 기간 변경
	...
	MEMSET_ZERO(&m_SelectedOption,sizeof(MONTHLY_OPTION_INFO));	
	...
	MEMSET_ZERO(&m_SelectedArmor, sizeof(MONTHLY_ARMOR_INFO));
	MEMSET_ZERO(&m_SelectedEvent, sizeof(MONTHLY_ARMOR_EVENT));

	// 2013-06-18 by bckim, 이달의 아머 운영툴 보완작업
	(CComboBox*)GetDlgItem(IDC_COMBO_OPTION_LIST)->EnableWindow(FALSE);
	(CComboBox*)GetDlgItem(IDC_EDIT_DURATION)->EnableWindow(FALSE);		// 2013-09-06 by jhseol, 이달의 아머 이벤트 적용 기간 변경
...
BOOL SCMonthlyArmorEventDlg::InsertDBMonthlyEvent(MONTHLY_ARMOR_EVNET_INFO* tmpInsertEvent )
{
	...
	SQLBindParameter(hstmt, 7, SQL_PARAM_INPUT, SQL_C_ULONG, SQL_INTEGER, 0, 0, &(tmpInsertEvent->nDuration),				0,&arrCB2[7]);	// 2013-09-06 by jhseol, 이달의 아머 이벤트 적용 기간 변경
...
void SCMonthlyArmorEventDlg::OnButtonUp() // Update !!
{
	...
	m_nDURATION = m_SelectedEvent.stMonthlyEventInfo.nDuration;
...
void SCMonthlyArmorEventDlg::Refresh_Option()
{
	...
	for(; itr != m_vectEditMonthlyOptionList.end(); itr++)
	{
#ifdef SC_PREMIUM_COLLECTION_ARMOR_YMJOO_BCKIM		// 2014-07-17 by bckim, 프리미엄 아머 컬렉션 - 운영툴 
		ITEM *pSelectedArmorItemInfo = m_mapItemInfo.findEZ_ptr(m_SelectedArmor.ItemNum);
		ITEM *pOptionItemInfo = m_mapItemInfo.findEZ_ptr(itr->ItemNum);
		
		if( pSelectedArmorItemInfo->IsExistDesParam(DES_PREMIUM_COLLECTION_ARMOR_INDEX) )
		{							
			if( IS_SAME_UNITKIND(m_SelectedArmor.ReqUnitKind,itr->ReqUnitKind) && pOptionItemInfo->IsExistDesParam(DES_PREMIUM_OPTION_ITEM_DEFAULT_DESPARAM))
			{
				char szTemp[1024];
		...
		{			
			if( IS_SAME_UNITKIND(m_SelectedArmor.ReqUnitKind,itr->ReqUnitKind) && pOptionItemInfo->IsExistDesParam(DES_OPTION_ITEM_DEFAULT_DESPARAM))
			{
				char szTemp[1024];
				sprintf(szTemp, "%s : [%d]",itr->ItemName,itr->ItemNum);
				m_comboOptionList.AddString(szTemp);
			...
			else if( IS_SAME_UNITKIND(m_SelectedArmor.ReqUnitKind,itr->ReqUnitKind) && pOptionItemInfo->IsExistDesParam(DES_OLD_OPTION_ITEM_DEFAULT_DESPARAM))
			{
				char szTemp[1024];
				sprintf(szTemp, "%s : [%d]",itr->ItemName,itr->ItemNum);
				m_comboOptionList.AddString(szTemp);
...
void SCMonthlyArmorEventDlg::_InitGrid_EventInfo(CGridCtrl *i_pGridCtrl)
{
	...
	int nRows = 1;
	int nCols = 6;	// 2013-09-06 by jhseol, 이달의 아머 이벤트 적용 기간 변경 - 5 -> 6 증가
	...
	Item.col = GRID1_COL_EVENT_INFO_EVENT_DURATION;
	Item.strText.Format("Duration");
	i_pGridCtrl->SetItem(&Item);
...
INT SCMonthlyArmorEventDlg::_AddEventInfo(CGridCtrl *i_pGridCtrl, MONTHLY_ARMOR_EVENT *i_pEvenrInfo)
{
	...
	Item.col		= GRID1_COL_EVENT_INFO_EVENT_START_DATE;
	Item.strText.Format("%s", i_pEvenrInfo->stMonthlyEventInfo.atStartDate.GetDateTimeString(STRNBUF(SIZE_MAX_ATUM_DATE_TIME_STRING)));
	Item.strText = Item.strText.Right(20);
	i_pGridCtrl->SetColumnWidth(Item.col,130);	
	i_pGridCtrl->SetItem(&Item);

	Item.col		= GRID1_COL_EVENT_INFO_EVENT_END_DATE;
	Item.strText.Format("%s", i_pEvenrInfo->stMonthlyEventInfo.atEndDate.GetDateTimeString(STRNBUF(SIZE_MAX_ATUM_DATE_TIME_STRING)));
	Item.strText = Item.strText.Right(20);
	i_pGridCtrl->SetColumnWidth(Item.col,130);	
	i_pGridCtrl->SetItem(&Item);
	...
	Item.mask		= GVIF_TEXT|GVIF_FORMAT;
	Item.nFormat	= DT_CENTER|DT_VCENTER|DT_SINGLELINE;
	Item.strText.Format("%d", i_pEvenrInfo->stMonthlyEventInfo.nDuration);
	i_pGridCtrl->SetColumnWidth(Item.col,60);
	i_pGridCtrl->SetItem(&Item);
	// end 2013-09-06 by jhseol, 이달의 아머 이벤트 적용 기간 변경
	...
	i_pGridCtrl->SetColumnWidth(Item.col,285);
	i_pGridCtrl->SetItem(&Item);
...
BOOL SCMonthlyArmorEventDlg::DBQueryGetMonthlyEventInfo(vectMONTHLY_ARMOR_EVENT_LIST *o_pVectEventList)
{
	...
	SQLBindCol(hstmt,  9, SQL_C_LONG,		&tmEventInfo.stMonthlyEventInfo.nDuration,			0,&arrCB[9]);	// 2013-09-06 by jhseol, 이달의 아머 이벤트 적용 기간 변경

	int nTempRows =0;
	do
	{
		MEMSET_ZERO(&tmEventInfo, sizeof(tmEventInfo));
...
BOOL SCMonthlyArmorEventDlg::DBQueryGetMonthlyArmorInfo(vectMONTHLY_ARMOR_INFO *o_pVectArmorList)
{
	...
	do
	{
		MEMSET_ZERO(&tmpArmorInfo, sizeof(tmpArmorInfo));
...
BOOL SCMonthlyArmorEventDlg::DBQueryGetMonthlyOptionInfo(vectMONTHLY_OPTION_INFO *o_pVectOptionList)
{
	...
	int nDesparamNum2 = DES_OPTION_ITEM_DEFAULT_DESPARAM;
	int nDesparamNum3 = DES_OPTION_ITEM_DEFAULT_DESPARAM; // 2014-12-14 by shchoi 이달의 구 아머

#ifdef	SC_PREMIUM_COLLECTION_ARMOR_YMJOO_BCKIM	// 2014-07-17 by bckim, 프리미엄 아머 컬렉션
	nDesparamNum2 = DES_PREMIUM_OPTION_ITEM_DEFAULT_DESPARAM;
#endif
#ifdef SC_OLD_COLLECTION_ARMOR_SHCHOI // 2014-12-29 by shchoi 이달의 구 아머
	nDesparamNum3 = DES_OLD_OPTION_ITEM_DEFAULT_DESPARAM;
#endif

	SQLBindParameter(hstmt, 1, SQL_PARAM_INPUT, SQL_C_LONG, SQL_INTEGER, 0, 0, &nDesparamNum, 0,&cb[1]);
	SQLBindParameter(hstmt, 2, SQL_PARAM_INPUT, SQL_C_LONG, SQL_INTEGER, 0, 0, &nDesparamNum2, 0,&cb[2]);
	SQLBindParameter(hstmt, 3, SQL_PARAM_INPUT, SQL_C_LONG, SQL_INTEGER, 0, 0, &nDesparamNum3, 0,&cb[3]); // 2014-12-29 by shchoi 이달의 구 아머
	...
	do
	{
		MEMSET_ZERO(&tmpOptionInfo, sizeof(tmpOptionInfo));
		SQLRETURN ret = SQLFetch(hstmt);
		if(SQL_ERROR == ret)
		{
			m_odbcStmt2.ProcessLogMessagesForStmt(PROCEDURE_130503_0004);	
...
void SCMonthlyArmorEventDlg::OnGrid2Click(NMHDR *pNotifyStruct, LRESULT* pResult)
{
	...
	for(; itr != m_vectCurMonthlyArmorInfo.end(); itr++)
	{
		...
		if(pGridView->iRow == pArmorInfo->TempRows)
		{
			...
			(CComboBox*)GetDlgItem(IDC_EDIT_DURATION)->EnableWindow(TRUE);		// 2013-09-06 by jhseol, 이달의 아머 이벤트 적용 기간 변경
			Refresh_Option();
			return;
		}
	}
