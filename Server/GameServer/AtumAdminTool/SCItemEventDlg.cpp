...
//	m_comboServerGroupID.SetCurSel(SERVERGROUP_START_ID);
	...
	CAtumDBHelper::LoadItemInfo(m_pODBCStmt, &m_mapItemInfo, &(m_pMainDlg->m_Localization), &m_mapItemInfoName);
	CAtumDBHelper::LoadRareItemInfo(m_pODBCStmt, &m_mapRareItemInfo, &(m_pMainDlg->m_Localization),  &m_mapRareItemInfoName);
	CAtumDBHelper::GetChannelingServerInfo(m_pODBCStmt,&m_vectCHANNELING_SERVER_INFO);				// 2014-02-10 by jhseol&bckim, 체널링

	//현재 DB에 있는 값 불러온다.
	GetItemEventInfo();

// 2009-05-11 by cmkwon, (태국요청) 운영자 AdminTool 권한 제한 요청 - GM도 아이템 이벤트 설정 가능
#if !(defined(SERVICE_TYPE_THAI_SERVER_1) || defined(SERVICE_TYPE_SINGAPORE_1))|| defined(SERVICE_TYPE_INDONESIA_SERVER_1)
	// 2007-10-02 by cmkwon, SCAdminTool 수정 권한 처리 - ItemEvent에 리로드 버튼 처리
	GetDlgItem(IDC_RELOAD_ITEM_EVENT_BUTTON)->EnableWindow(m_pMainDlg->IsManagerAdministrator());
	GetDlgItem(IDC_BUTTON_LUCKYMACHINE_INIT)->EnableWindow(m_pMainDlg->IsManagerAdministrator());	// 2009-02-04 by cmkwon, AdminTool의 GM 권한 수정 - 이벤트 관련 검색은 가능, 수정 불가
#endif
	...
	// 2014-01-21 by jhseol, 베트남 아이템 이벤트 에러 디버깅용
#ifdef S_VIE_SERVER_SETTING_HSSON
	SystemLog.InitLogManger(TRUE, "ItemEventSystemLog", "./ToolSystemLog/");
#endif
	// end 2014-01-21 by jhseol, 베트남 아이템 이벤트 에러 디버깅용
...
//	szSQLQuery.Format(QUERY_080702_0029);		// 2008-07-02 by cmkwon, MySQL 지원 구현 - 
	...
	MEMSET_ZERO(&tmItemEv, sizeof(tmItemEv));
	...
	SQL_TIMESTAMP_STRUCT tmpAtTime;				// 2013-09-30 by bckim, 특정시간 아이템지급
	MEMSET_ZERO(&tmpStartTime, sizeof(SQL_TIMESTAMP_STRUCT));
	MEMSET_ZERO(&tmpEndTime, sizeof(SQL_TIMESTAMP_STRUCT));
	MEMSET_ZERO(&tmpLastGameEndDate, sizeof(SQL_TIMESTAMP_STRUCT));
	MEMSET_ZERO(&tmpAtTime, sizeof(SQL_TIMESTAMP_STRUCT));			// 2013-09-30 by bckim, 특정시간 아이템지급
	...
	SQLBindCol(m_pODBCStmt->m_hstmt, 21, SQL_C_ULONG, &tmItemEv.UseFixedPeriod, 0,				&arrCB[21]);	// 2013-02-28 by bckim, 복귀유져 버프추가
	SQLBindCol(m_pODBCStmt->m_hstmt, 22, SQL_C_ULONG, &tmItemEv.FixedPeriod_DAY, 0,				&arrCB[22]);	// 2013-02-28 by bckim, 복귀유져 버프추가
	SQLBindCol(m_pODBCStmt->m_hstmt, 23, SQL_C_TIMESTAMP, &tmpAtTime, 0,						&arrCB[23]);	// 2013-09-30 by bckim, 특정시간 아이템지급 		
	SQLBindCol(m_pODBCStmt->m_hstmt, 24, SQL_C_ULONG, &tmItemEv.ChannelingType, 0,				&arrCB[24]);	// 2014-02-10 by jhseol&bckim, 체널링	
...
//	BOOL bRet = m_pODBCStmt->ExecuteQuery((LPCSTR)szSQLQuery);
	...
	while ( (bRet = SQLFetch(m_pODBCStmt->m_hstmt)) != SQL_NO_DATA)
	{
		...
		tmItemEv.GiveItemAtTime		= tmpAtTime;			// 2013-09-30 by bckim, 특정시간 아이템지급
	...
	for(int i=0; i < m_mtvectITEM_EVENT_INFO.size(); i++)
	{
		...
		if (ITEM_EVENT_TYPE_ONLYONE == m_mtvectITEM_EVENT_INFO[i].ItemEventType)		{	szItemEvType.Format("ONCE");			}		
		if (ITEM_EVENT_TYPE_AT_TIME == m_mtvectITEM_EVENT_INFO[i].ItemEventType)		{	szItemEvType.Format("AT TIME");			}		// 2013-09-30 by bckim, 특정시간 아이템지급 		
		if (ITEM_EVENT_TYPE_ONEDAY == m_mtvectITEM_EVENT_INFO[i].ItemEventType)			{	szItemEvType.Format("DAILY");			}	
		if (ITEM_EVENT_TYPE_INFLCHANGE == m_mtvectITEM_EVENT_INFO[i].ItemEventType)		{	szItemEvType.Format("InflChange");		}
		if (ITEM_EVENT_TYPE_LEVELUP == m_mtvectITEM_EVENT_INFO[i].ItemEventType)		{	szItemEvType.Format("LevelUp");			}
		if (ITEM_EVENT_TYPE_COUPON_ONLYONE == m_mtvectITEM_EVENT_INFO[i].ItemEventType)	{	szItemEvType.Format("Coupon Once");		}		// 2008-01-11 by cmkwon, 아이템 이벤트 시스템에 신 쿠폰 시스템 추가 - 
		if (ITEM_EVENT_TYPE_COUPON_ONEDAY == m_mtvectITEM_EVENT_INFO[i].ItemEventType)	{	szItemEvType.Format("Coupon Daily");	}		// 2008-01-11 by cmkwon, 아이템 이벤트 시스템에 신 쿠폰 시스템 추가 - 
		if (ITEM_EVENT_TYPE_LOGIN == m_mtvectITEM_EVENT_INFO[i].ItemEventType)			{	szItemEvType.Format("Login Event");		}		// 2011-08-25 by shcho, 횟수별 아이템 지급기능 구현
		if (ITEM_EVENT_TYPE_FIRST_PURCHASE == m_mtvectITEM_EVENT_INFO[i].ItemEventType)	{	szItemEvType.Format("First Purchase");	}		// 2013-03-29 by jhseol, 아이템 이벤트 - 매월 첫 결재시 선물지급
		...
		if (INFLUENCE_TYPE_NORMAL == m_mtvectITEM_EVENT_INFO[i].byInfluenceType)	{	szInfluenceType.Format("NORMAL");	}
		if (INFLUENCE_TYPE_VCN == m_mtvectITEM_EVENT_INFO[i].byInfluenceType)		{	szInfluenceType.Format("VCN");		}
		if (INFLUENCE_TYPE_ANI == m_mtvectITEM_EVENT_INFO[i].byInfluenceType)		{	szInfluenceType.Format("ANI");		}
		if (INFLUENCE_TYPE_ALL_MASK == m_mtvectITEM_EVENT_INFO[i].byInfluenceType)	{	szInfluenceType.Format("ALL");		}
		...
		Item.col++;
		int temp = m_mtvectITEM_EVENT_INFO[i].ChannelingType;	
		Item.strText.Format("%s(%d)",GetChannelingServerName(temp),temp);
		m_GridList.SetItem(&Item);
#endif
		...
		Item.col++;
		Item.strText.Format("%s", m_mtvectITEM_EVENT_INFO[i].StartTime.GetDateTimeString(STRNBUF(SIZE_MAX_ATUM_DATE_TIME_STRING)));
		m_GridList.SetItem(&Item);
		
		Item.col++;
		Item.strText.Format("%s", m_mtvectITEM_EVENT_INFO[i].EndTime.GetDateTimeString(STRNBUF(SIZE_MAX_ATUM_DATE_TIME_STRING)));
		m_GridList.SetItem(&Item);
		...
		Item.col++;
		Item.strText.Format("%s", m_mtvectITEM_EVENT_INFO[i].atLastGameEndDate.GetDateTimeString(STRNBUF(SIZE_MAX_ATUM_DATE_TIME_STRING)));
		m_GridList.SetItem(&Item);	
		...
		Item.col++;
		Item.strText.Format("%s", m_mtvectITEM_EVENT_INFO[i].GiveItemAtTime.GetAtTimeString(STRNBUF(SIZE_MAX_ATUM_DATE_TIME_STRING)) );
		m_GridList.SetItem(&Item);	
...
void CSCItemEventDlg::InitGrid()
{
	...
	m_nCols += 2;			// 2013-02-28 by bckim, 복귀유져 버프추가
	m_nCols += 1;			// 2013-09-30 by bckim, 특정시간 아이템지급

#ifdef S_ITEM_EVENT_SUB_TYPE_JHSEOL
	m_nCols += 3;
#endif

#ifdef SC_CHANNELING_JHSEOL_BCKIM_SSJUNG		// 2014-02-10 by jhseol&bckim, 체널링
	m_nCols += 1;
#endif
...
//	m_GridList.SetColumnWidth(0,0);
	...
	Item.col = m_nCols++;		Item.strText.Format("ItemEventUID");			m_GridList.SetItem(&Item);
	Item.col = m_nCols++;		Item.strText.Format("ItemEventType");			m_GridList.SetItem(&Item);		// 2006-08-25 by cmkwon
#ifdef S_ITEM_EVENT_SUB_TYPE_JHSEOL			// 2013-03-29 by jhseol, 아이템 이벤트 - 서브타입 추가구조로 변경
	Item.col = m_nCols++;		Item.strText.Format("SubParam1");				m_GridList.SetItem(&Item);		// 2013-03-29 by jhseol, 아이템 이벤트 - 서브타입 추가구조로 변경
	Item.col = m_nCols++;		Item.strText.Format("SubParam2");				m_GridList.SetItem(&Item);		// 2013-03-29 by jhseol, 아이템 이벤트 - 서브타입 추가구조로 변경
	Item.col = m_nCols++;		Item.strText.Format("SubParam3");				m_GridList.SetItem(&Item);		// 2013-03-29 by jhseol, 아이템 이벤트 - 서브타입 추가구조로 변경
#endif										// end 2013-03-29 by jhseol, 아이템 이벤트 - 서브타입 추가구조로 변경
	Item.col = m_nCols++;		Item.strText.Format("OnlyPCBang");				m_GridList.SetItem(&Item);
	Item.col = m_nCols++;		Item.strText.Format("MemberShip");				m_GridList.SetItem(&Item);
	Item.col = m_nCols++;		Item.strText.Format("NewMember");				m_GridList.SetItem(&Item);
	
#ifdef SC_CHANNELING_JHSEOL_BCKIM_SSJUNG		// 2014-02-10 by jhseol&bckim, 체널링
	Item.col = m_nCols++;		Item.strText.Format("ChannelingType");			m_GridList.SetItem(&Item);
#endif											// End. 2014-02-10 by jhseol&bckim, 체널링
		
	Item.col = m_nCols++;		Item.strText.Format("InfluenceType");			m_GridList.SetItem(&Item);
	Item.col = m_nCols++;		Item.strText.Format("ItemNumInfo");				m_GridList.SetItem(&Item);
	Item.col = m_nCols++;		Item.strText.Format("Count");					m_GridList.SetItem(&Item);			
	Item.col = m_nCols++;		Item.strText.Format("StartTime");				m_GridList.SetItem(&Item);
	Item.col = m_nCols++;		Item.strText.Format("EndTime");					m_GridList.SetItem(&Item);
	Item.col = m_nCols++;		Item.strText.Format("ReqLevel");				m_GridList.SetItem(&Item);
	Item.col = m_nCols++;		Item.strText.Format("UnitKind");				m_GridList.SetItem(&Item);
	Item.col = m_nCols++;		Item.strText.Format("Prefix");					m_GridList.SetItem(&Item);
	Item.col = m_nCols++;		Item.strText.Format("Suffix");					m_GridList.SetItem(&Item);
	Item.col = m_nCols++;		Item.strText.Format("UseLastGameEndDate");		m_GridList.SetItem(&Item);		// 2008-02-01 by cmkwon, ItemEvent 에 LastGameEndDate 체크 루틴 추가 - 
	Item.col = m_nCols++;		Item.strText.Format("LastGameEndDate");			m_GridList.SetItem(&Item);
	Item.col = m_nCols++;		Item.strText.Format("UseFixedPeriod");			m_GridList.SetItem(&Item);		// 2013-02-28 by bckim, 복귀유져 버프추가
	Item.col = m_nCols++;		Item.strText.Format("FixedPeriod_DAY");			m_GridList.SetItem(&Item);		// 2013-02-28 by bckim, 복귀유져 버프추가
	Item.col = m_nCols++;		Item.strText.Format("CheckWithCharacterUID");	m_GridList.SetItem(&Item);		// 2009-11-19 by cmkwon, 아이템 이벤트에 Account/Character 체크 기능 추가 - 
	Item.col = m_nCols++;		Item.strText.Format("iLevelMin");				m_GridList.SetItem(&Item);
	Item.col = m_nCols++;		Item.strText.Format("iLevelMax");				m_GridList.SetItem(&Item);
	Item.col = m_nCols++;		Item.strText.Format("LoginCheckNumber");		m_GridList.SetItem(&Item);		// 2011-08-25 by shcho, 횟수별 아이템 지급기능 구현 - 컬럼 추가
	Item.col = m_nCols++;		Item.strText.Format("GiveItemAtTime");			m_GridList.SetItem(&Item);		// 2013-09-30 by bckim, 특정시간 아이템지급
...
// 2009-05-11 by cmkwon, (태국요청) 운영자 AdminTool 권한 제한 요청 - GM도 아이템 이벤트 설정 가능
#if !(defined(SERVICE_TYPE_THAI_SERVER_1)||defined(SERVICE_TYPE_SINGAPORE_1)) || defined(SERVICE_TYPE_INDONESIA_SERVER_1)
	// 2007-10-02 by cmkwon, SCAdminTool 수정 권한 처리 - ItemEvent 처리
...
//		bRet = m_pODBCStmt->ExecuteQuery((char*)(PROCEDURE_080827_0265));
	...
	SQLBindParameter(m_pODBCStmt->m_hstmt, 20, SQL_PARAM_INPUT, SQL_C_LONG, SQL_INTEGER, 0, 0, &dlg.m_ItemEv.UseFixedPeriod, 0, NULL);			// 2013-02-28 by bckim, 복귀유져 버프추가	
	SQLBindParameter(m_pODBCStmt->m_hstmt, 21, SQL_PARAM_INPUT, SQL_C_LONG, SQL_INTEGER, 0, 0, &dlg.m_ItemEv.FixedPeriod_DAY, 0, NULL);			// 2013-02-28 by bckim, 복귀유져 버프추가
	char szAtTime[SIZE_MAX_SQL_DATETIME_STRING];
	dlg.m_ItemEv.GiveItemAtTime.GetSQLDateTimeString(szAtTime, SIZE_MAX_SQL_DATETIME_STRING);	
	SQLBindParameter(m_pODBCStmt->m_hstmt, 22, SQL_PARAM_INPUT, SQL_C_CHAR, SQL_VARCHAR, SIZE_MAX_SQL_DATETIME_STRING, 0, szAtTime, 0, NULL);		// 2013-09-30 by bckim, 특정시간 아이템지급 
	SQLBindParameter(m_pODBCStmt->m_hstmt, 23, SQL_PARAM_INPUT, SQL_C_ULONG, SQL_INTEGER, 0, 0, &dlg.m_ItemEv.ChannelingType, 0, NULL);				// 2014-02-10 by jhseol&bckim, 체널링
	...
	if (!bRet)
	{
		...
		SQLGetDiagRec(SQL_HANDLE_STMT, m_pODBCStmt->m_hstmt, plm_cRecNmbr, plm_szSqlState, &plm_pfNativeError, plm_szErrorMsg, 1024 - 1, &plm_pcbErrorMsg);
		char szTemp[1024];
		sprintf(szTemp, "[ERROR] CSCItemEventDlg::OnConmenuSelectedInsert _ %s\r\n", plm_szErrorMsg);
		SystemLog.WriteSystemLog(szTemp, TRUE);
#endif
		// end 2014-01-21 by jhseol, 베트남 아이템 이벤트 에러 디버깅용
		m_pODBCStmt->FreeStatement();		// cleanup
...
void CSCItemEventDlg::OnConmenuSelectedUpdate()
{
	...
	// 2014-06-26 by bckim, 운영툴 아이템 이벤트UID 재사용 버그 수정	
	ATUM_DATE_TIME tmpCurrentDate(TRUE);

	if ( pItemEv->EndTime <= tmpCurrentDate )
	{
		MessageBox("Current events have already been shut down. Please add the new event");
		return;
	}
	// End. 2014-06-26 by bckim, 운영툴 아이템 이벤트UID 재사용 버그 수정
...
//		, dlg.m_ItemEv.atLastGameEndDate.GetSQLDateTimeString(STRNBUF(SIZE_MAX_SQL_DATETIME_STRING)), pItemEv->ItemEventUID);
	...
	char GiveItemAtTime[SIZE_MAX_SQL_DATETIME_STRING];
	dlg.m_ItemEv.GiveItemAtTime.GetSQLDateTimeString(GiveItemAtTime, SIZE_MAX_SQL_DATETIME_STRING);	
	SQLBindParameter(m_pODBCStmt->m_hstmt, 23, SQL_PARAM_INPUT, SQL_C_CHAR, SQL_VARCHAR, SIZE_MAX_SQL_DATETIME_STRING, 0, GiveItemAtTime, 0, NULL);		// 2013-09-30 by bckim, 특정시간 아이템지급 
	SQLBindParameter(m_pODBCStmt->m_hstmt, 24, SQL_PARAM_INPUT, SQL_C_ULONG, SQL_INTEGER, 0, 0, &dlg.m_ItemEv.ChannelingType, 0, NULL);					// 2014-02-10 by jhseol&bckim, 체널링
...
// 2014-02-10 by jhseol&bckim, 체널링
char* CSCItemEventDlg::GetChannelingServerName( int i_ChannelingUID)
{
	if( 0 == i_ChannelingUID)
	{
		return "NONE";
	}
	...
	{
		if( i_ChannelingUID == (*itr)->nChannelingSeverUID )
		{
