...
BEGIN_MESSAGE_MAP(CLogTabConnection, CDialog)
	...
	ON_NOTIFY(NM_DBLCLK, IDC_GRID_CONNECTION_LOG, OnGridDClick)		// 2014-01-23 by jhseol, 어드민 툴 로그조회 캐나다 요청 편의기능 추가
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
BOOL CLogTabConnection::GetLogAndInsertToGrid(BYTE T1_LogType, LPCSTR szTableName)
{
	// max row count
	CString szQuery;
	if (m_bCheckMaxRowCount)
...
void CLogTabConnection::OnGridDClick(NMHDR *pNotifyStruct, LRESULT* pResult)
{
	OutputDebugString("[SEOL Debuging Check] : Mouse D Click. \r\n");
	CCellID cellID = m_GridConnectionLog.GetFocusCell();
	if(0 > cellID.row || (FALSE == IS_IN_RANGE(0, cellID.col, 4)) )
	{// 2005-12-12 by cmkwon, 선택 오류
		return;
	}

// 	char tmpostion[10] = {0,};
// 	sprintf(tmpostion, "row : %d, col : %d \r\n", cellID.row, cellID.col);
// 	OutputDebugString(tmpostion);
	...
	case 0:
		{
			...
			m_bCheckIPAddress = FALSE;
			m_szAccountName = (CString)m_GridConnectionLog.GetItemText(cellID.row, cellID.col);
			bDateChange = TRUE;
		}
		break;
	case 1:
	...
	case 3:
		{
			m_bCheckAccountName = FALSE;
			m_bCheckIPAddress = TRUE;
			m_ctlIPAddress.SetAddress(htonl(inet_addr((CString)m_GridConnectionLog.GetItemText(cellID.row, cellID.col))));   
			bDateChange = TRUE;
		}
		break;
	default:
	...
	if ( TRUE == bDateChange )
	{
		CString strDateTime = (CString)m_GridConnectionLog.GetItemText(cellID.row, 1);
		CTime tmDateStart( atoi(strDateTime.Left(4)), atoi(strDateTime.Mid(4, 2)), atoi(strDateTime.Mid(6, 2)), 0, 0, 0 );
		CTime tmDateEnd( atoi(strDateTime.Left(4)), atoi(strDateTime.Mid(4, 2)), atoi(strDateTime.Mid(6, 2)), 0, 0, 0 );
		m_dateStart = (tmDateStart - CTimeSpan(7, 0, 0, 0));
		m_dateEnd = (tmDateEnd + CTimeSpan(7, 0, 0, 0));
	}
