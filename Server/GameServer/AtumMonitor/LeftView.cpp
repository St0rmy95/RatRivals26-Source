...
// 2013-10-01 by jekim, 툴에 각 세력별 접속 유저 수 추가.
						pNode->m_nBCUClients = pMsg->NumOfBCU;
						pNode->m_nANIClients = pMsg->NumOfANI;
				...
				case T_FM_GET_MAP_USER_COUNTS_ACK:
					{
						...
						if(pPINode)
						{
							...
							pPINode->m_uiANIClients                 = pMapUserCountsAck->NumOfANI;
							pPINode->m_uiBCUClients                 = pMapUserCountsAck->NumOfBCU;
...
//						STRNCPY_MEMSET(pPreNode->m_szFtpIP, pMsg->FtpIP, SIZE_MAX_FTP_URL);
//						pPreNode->m_nFtpPort = pMsg->FtpPort;
//						STRNCPY_MEMSET(pPreNode->m_szFtpAccountName, pMsg->FtpAccountName, SIZE_MAX_ACCOUNT_NAME);
//						STRNCPY_MEMSET(pPreNode->m_szFtpPassword, pMsg->FtpPassword, SIZE_MAX_PASSWORD);
...
//						GetOnlyPathWithoutFileName(pPreNode->m_szNoticeFileUploadPath);
				...
				case T_PM_SERVER_THREAD_STATUS:
					...
					break;
				case T_PM_AUTO_UPDATE_FTP_SERVER_SETTING:
					{
						MSG_PM_AUTO_UPDATE_FTP_SERVER_SETTING *pRMsg
							= (MSG_PM_AUTO_UPDATE_FTP_SERVER_SETTING*)(pPacket + SIZE_FIELD_TYPE_HEADER);

						CTreeCtrl &tctl = GetTreeCtrl();
						CPreServerNode *pPreNode = ((CPreServerNode*)tctl.GetItemData(pMonitorWinSocket->m_hTreeItem));

						pPreNode->m_nAutoUpdateServerType		= pRMsg->nAutoUpdateServerType;
						STRNCPY_MEMSET(pPreNode->m_szFtpIP, pRMsg->FtpIP, SIZE_MAX_FTP_URL);
						pPreNode->m_nFtpPort = pRMsg->FtpPort;
						STRNCPY_MEMSET(pPreNode->m_szFtpAccountName, pRMsg->FtpAccountName, SIZE_MAX_ACCOUNT_NAME);
						STRNCPY_MEMSET(pPreNode->m_szFtpPassword, pRMsg->FtpPassword, SIZE_MAX_PASSWORD);
						pPreNode->m_szClientFTPUpdateUploadDir	= pRMsg->ClientFTPUpdateUploadDir;		// 2005-12-23 by cmkwon
						pPreNode->m_szLauncherFileUploadPath	= pRMsg->LauncherFileUploadPath;
						pPreNode->m_szDeleteFileListUploadPath	= pRMsg->DeleteFileListUploadPath;
						pPreNode->m_szNoticeFileUploadPath		= pRMsg->NoticeFileUploadPath;
						STRNCPY_MEMSET(pPreNode->m_szVersionListFileUploadPath, pRMsg->szVersionListFileUploadPath, SIZE_MAX_FTP_FILE_PATH);	// 2007-01-08 by cmkwon
				...
				case T_PM_AUTO_UPDATE_HTTP_SERVER_SETTING:		// 2007-01-06 by cmkwon
					{
						...
						pPreNode->m_nAutoUpdateServerType		= pRMsg->nAutoUpdateServerType;
						STRNCPY_MEMSET(pPreNode->m_szDownloadHttpServerIP, pRMsg->szDownloadHttpServerIP, SIZE_MAX_FTP_URL);
						pPreNode->m_nDownloadHttpServerPort		= pRMsg->nDownloadHttpServerPort;
						STRNCPY_MEMSET(pPreNode->m_szDownloadHttpServerAccountName, pRMsg->szDownloadHttpServerAccountName, SIZE_MAX_ACCOUNT_NAME);
						STRNCPY_MEMSET(pPreNode->m_szDownloadHttpServerPassword, pRMsg->szDownloadHttpServerPassword, SIZE_MAX_PASSWORD);
						...
						STRNCPY_MEMSET(pPreNode->m_szVersionListFileDownloadHttpPath, pRMsg->szVersionListFileDownloadHttpPath, SIZE_MAX_FTP_FILE_PATH);	// 2007-01-08 by cmkwon
...
// 	STRNCPY_MEMSET(szTempFileName, "tmnversion.ver", SIZE_MAX_FTP_FILE_PATH);
...
void CLeftView::OnTcmReloadVersionInfoPre()
{
	...
	switch(pNode->m_nAutoUpdateServerType)
	{
	case AU_SERVER_TYPE_HTTP:
		{
			if(FALSE == GetVersionInfoByHTTP(pNode, &mapVersionOld2New, &LatestLauncherVersion, &LatestDeleteFileListVersion, &LatestNoticeVersion))
			{
	...
	default:
		{
			if (FALSE == GetVersionInfoFromFTP(pNode, &mapVersionOld2New, &LatestLauncherVersion, &LatestDeleteFileListVersion, &LatestNoticeVersion))
			{
				MessageBox("Get version list error by FTP!");
				return;
			}
		}
	}
	...
	pSHeader->TotalNumOfClientVersions		= mapVersionOld2New.size();
...
void CLeftView::OnTcmPrintDbg()
{
	...
	*(MessageType_t*)buff = (MessageType_t)T_FM_PRINT_DEBUG_MSG;
	...
	STRNCPY_MEMSET(pMsgPrintDbg->StringParam, (LPCTSTR)dlg.m_string, 100);
...
void CLeftView::OnTcmGetCharacterInfoField()
{
	...
	STRNCPY_MEMSET(pMsgGet->CharacterName, dlg.m_string, SIZE_MAX_CHARACTER_NAME);
...
void CLeftView::OnTcmGetServerGroupInfoPre()
{
	...
	STRNCPY_MEMSET(pSendGroupInfo->ServerGroupName, pServerGroupNode->m_ServerGroupInfo.ServerGroupName, SIZE_MAX_SERVER_NAME);
...
void CLeftView::OnTcmPreSetLimitUserCount()
{
	...
	if(dlg.DoModal() == IDOK)
	{
		...
		STRNCPY_MEMSET(pSetUserCount->ServerGroupName, pServerGroupNode->m_ServerGroupInfo.ServerGroupName, SIZE_MAX_SERVER_NAME);
...
void CLeftView::OnTcmPreChangeEnableServerGroup() 
{
	...
	STRNCPY_MEMSET(pSMsg->ServerGroupName, pServerGroupNode->m_ServerGroupInfo.ServerGroupName, SIZE_MAX_SERVER_NAME);
...
void CLeftView::OnTcmPrepareShutdown() 
{
	...
	for(;itr != pPSNode->m_listServerGroupInfoNodePtr.end(); itr++)
	{
		...
		{
			STRNCPY_MEMSET(pSShutdown->ServerGroupName, pSGInfoNode->m_ServerGroupInfo.ServerGroupName, SIZE_MAX_SERVER_NAME);
...
void CLeftView::OnTcm3PrepareShutdown() 
{
	...
	INIT_MSG_WITH_BUFFER(MSG_PM_PREPARE_SHUTDOWN, T_PM_PREPARE_SHUTDOWN, pSShutdown, SendBuf);
	STRNCPY_MEMSET(pSShutdown->ServerGroupName, pServerGroupNode->m_ServerGroupInfo.ServerGroupName, SIZE_MAX_SERVER_NAME);
