...
void CAccountInfo::SetBirthday(int i_nBirthday)
{
	...
	{
	case LANGUAGE_TYPE_CHINESE:		// 2007-06-28 by cmkwon, 중국 방심취관련(출생년월일 FielServer로 가져오기) - 형식:19500625
	case LANGUAGE_TYPE_KOREAN:		// 2011-11-03 by shcho, yedang 셧다운제 구현 - 구분을 위해 필요함
		{
			int nMonth  = 0;
			int nDay	= 0;
			// int nYear	= 0;
...
CPreIOCP::~CPreIOCP()
{
	...
	SAFE_DELETE(m_pInnovaLibrary);		// 2010-04-26 by cmkwon, 러시아 Innva 인증/빌링 시스템 변경 - 
}
...
//	BOOL bRet = odbcStmt.Init((LPCSTR)g_pGlobalGameServer->GetODBCDSN(), (LPCSTR)g_pGlobalGameServer->GetODBCUID(), (LPCSTR)g_pGlobalGameServer->GetODBCPASSWORD());
	...
	BOOL bRet = odbcStmt.Init(
		...
		g_pGlobalGameServer->GetDBServerDatabaseName(),
		(char*)g_pGlobalGameServer->GetODBCUID(),
		(char*)g_pGlobalGameServer->GetODBCPASSWORD(),
		g_pGlobalGameServer->GetMainWndHandle()
	);
	...
	if (!bRet)
	{
		g_pGlobal->WriteSystemLogEX(
			...
			"IP:%s, Port:%d, DB:%s, UID:%s\n"
			"SQLSTATE:%s, NativeError:%d\n"
			"Message:%s\n",
			g_pGlobalGameServer->GetDBServerIP(),
			g_pGlobalGameServer->GetDBServerPort(),
			g_pGlobalGameServer->GetDBServerDatabaseName(),
			(char*)g_pGlobalGameServer->GetODBCUID(),
			odbcStmt.GetLastSqlState_RAT(),
			odbcStmt.GetLastNativeError_RAT(),
			odbcStmt.GetLastErrorMsg_RAT()
		);
...
BOOL CPreIOCP::DisconnectAccount(CAccountInfo *i_pAccountInfo, Err_t i_nError, CPreIOCPSocket *i_pSocketToNotify, char *reason) // // 2013-09-23 by jekim, 계정 블락시 메세지 뿌려주기.
{
	...
	pFieldServerSocket->SendErrorMessage(T_PA_ADMIN_DISCONNECT_USER, i_nError, i_pAccountInfo->CurrentClientIndex, 0, reason); // 2013-09-23 by jekim, 계정 블락시 메세지 뿌려주기.
...
BOOL CPreIOCP::INNBILL_LogOut(char *i_szAccName)
{
	...
	// 2010-04-27 by cmkwon, PreServer에서는 현재 게임 진행 시간 계산하지 않는다 필요시 구현.
