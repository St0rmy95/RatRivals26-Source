...
LONG __stdcall ExceptionHandler(_EXCEPTION_POINTERS* pExceptionInfo)
{
    ...
    char szTemp[256];
#ifdef ARENA
	wsprintf(szTemp, "ArenaFieldServer Crash !! : Create dump file (Exception 0x%08x arised)", pExceptionInfo->ExceptionRecord->ExceptionCode);
#else
...
#ifdef ARENA
	sprintf(szSystemLog, "Arena Field Server Start\r\n\r\n");
#else
...
#ifdef ARENA
	sprintf(szSystemLog, "Arena Field Server End\r\n\r\n\r\n");
#else
...
// //						DBGOUT(STRMSG_S_MF2AFCONNECT_0001);
						...
						g_pFieldGlobal->WriteSystemLogEX(FALSE, "  [Notify] connected to ArenaFieldServer(%s:%d) !, pIOCP(0x%X) \r\n", g_pFieldGlobal->m_sArenaServerInfo.ArenaFieldServerIP, g_pFieldGlobal->m_sArenaServerInfo.ArenaFieldServerPort, g_pFieldGlobal->GetGIOCP());	// 2013-10-16 by jhseol, 서버 디버깅시 불필요 DBGOUT 출력 코드 제거	// 2009-04-17 by cmkwon, 시스템 로그 수정 - 
			...
			case CWinSocket::WS_CLOSED:
				{
					...
					sprintf(szSystemLog, STRMSG_S_MF2AFCONNECT_0002,
							g_pFieldGlobal->m_sArenaServerInfo.ArenaFieldServerIP, g_pFieldGlobal->m_sArenaServerInfo.ArenaFieldServerPort);
						g_pFieldGlobal->WriteSystemLog(szSystemLog);
					//DBGOUT(szSystemLog);	// 2013-10-16 by jhseol, 서버 디버깅시 불필요 DBGOUT 출력 코드 제거
...
#if defined(TEST140)	// 2013-12-26 by jhseol, 테스트서버 로그 삭제
#else
...
#endif	//#if defined(TEST140)	// 2013-12-26 by jhseol, 테스트서버 로그 삭제
				}
				break;
			...
			case TIMERID_RECONNECT:
				{
					...
					if ( 0 != g_pFieldGlobal->m_sArenaServerInfo.ArenaServerID )
					{
						if(g_pFieldGlobal->GetField2ArenaFieldWinSocket()
