...
COutPostManager::~COutPostManager()
{
	...
	for_each(m_mtvectOutPostManager.begin(), m_mtvectOutPostManager.end(), Delete_Object());
...
void COutPostManager::OnDoMinutelyWorkOutPostManager(ATUM_DATE_TIME *i_pCurDateTime)
{
	...
	for(; itr != m_mtvectOutPostManager.end(); itr++)
	{
		...
		if ( 60 >= CheckRemainWarTimeAtMin && FALSE == (*itr)->m_bNGCTriggerOutPostStartCheckOK )
		{
			m_pFieldIOCP18->m_pMapTriggerManager->SetOutPostWarState(TRUE);
			(*itr)->m_bNGCTriggerOutPostStartCheckOK = TRUE;
		}
		...
		{// 2007-08-20 by dhjin, 전진기지 전 중이라면 체크 해야될 사항
			...
			{// 2007-08-20 by dhjin, 전진기지 전 시간이 OUTPOST_WARTIME 지났다면 전진기지 전쟁을 끝낸다.
				...
				// 2013-08-14 by jhseol, 거점전 리뉴얼 - NGC 거점
				m_pFieldIOCP18->m_pMapTriggerManager->SetOutPostWarState(FALSE);
				(*itr)->m_bNGCTriggerOutPostStartCheckOK = FALSE;
				// end 2013-08-14 by jhseol, 거점전 리뉴얼 - NGC 거점
			}
...
void COutPostManager::OutPostWarStart(COutPost * i_pOutPost)
{
	...
	g_pFieldGlobal->WriteSystemLogEX(TRUE, "  [Notify] OutPostWarNoti OutPostWarStart  OUTPOST_INIT bell1 : [%6d], bell2 : [%6d], bell3 : [%6d], NextWarTimeDB : [%s]\r\n"
		, pSummon->bell1, pSummon->bell2, pSummon->bell3, i_pOutPost->GetOutPostNextWarTime()->GetDateTimeString(STRNBUF(SIZE_MAX_ATUM_DATE_TIME_STRING)));	// 2008-06-27 by dhjin, 로그에 시작시간이 언제 였는지 추가
...
void COutPostManager::OnOutPostProtectorDestroy(int i_nMapIndex)
{
	...
	STRNCPY_MEMSET(SendMsg->GuildName, tmOutPost->GetOutPostResetPossibleGuildName(), SIZE_MAX_GUILD_NAME);
	m_pFieldIOCP18->SendMessageToAllClients(SendBuf,MSG_SIZE(MSG_FC_OUTPOST_PROTECTOR_DESTROY),INFLUENCE_TYPE_VCN|INFLUENCE_TYPE_ANI);
...
void COutPostManager::OnOutPostResetDestroy(int i_nMapIndex)
{
	...
	{// 2007-10-01 by dhjin, VCN, ANI 세력이 소유 했을 경우 그대로 둔다.
		...
		// 2014-02-06 by jhseol, 세력포인트에 따른 거점 레벨링 밸런스 패치 - 전진기지 종료기점으로 연승 확인으로 변경 위치변경 : 열외처리
...
#endif
		// end 2014-02-06 by jhseol, 세력포인트에 따른 거점 레벨링 밸런스 패치 - 전진기지 종료기점으로 연승 확인으로 변경 위치변경 : 열외처리
	}
...
void COutPostManager::OutPostResetSuccess(COutPost * i_pOutPost, vectSOUTPOSTWAR_EV_RESET_SUCCESS *io_pVectEvResetSuccessList)
{
	...
	STRNCPY_MEMSET(SendMsg->GuildName, i_pOutPost->GetOutPostGuildName(), SIZE_MAX_GUILD_NAME);
...
//	pFMChann->WarpToCityMapFieldMapChannelByInfluence(i_pOutPost->GetOutPostInfluence());
	...
	MEMSET_ZERO(&tmReset, sizeof(tmReset));
...
void COutPostManager::OutPostWarEnd(COutPost * i_pOutPost, vectSOUTPOSTWAR_EV_END *io_pEvEndList)
{
	...
	MEMSET_ZERO(&tmEvEnd, sizeof(tmEvEnd));
...
// 2013-05-09 by hskim, 세력 포인트 개선
	...
	// 2014-02-06 by jhseol, 세력포인트에 따른 거점 레벨링 밸런스 패치 - 전진기지 종료기점으로 연승 확인으로 변경 위치변경 : 코드추가
	#ifdef SC_STRATEGYPOINTWAR_RENEWAL_JHSEOL_SSJUNG
	m_pFieldIOCP18->m_InflWarManager.CalcConsecutiveVictoriesPoint(i_pOutPost->GetOutPostInfluence());
	#endif
	// end 2014-02-06 by jhseol, 세력포인트에 따른 거점 레벨링 밸런스 패치 - 전진기지 종료기점으로 연승 확인으로 변경 위치변경 : 코드추가
#endif
...
INT	COutPostManager::MakeAllOutPostInfoMsg(SCITY_WARINFO_OUTPOST *o_pOutPostWarInfo)
{
	...
	for(; itr != m_mtvectOutPostManager.end(); itr++)
	{
		...
		STRNCPY_MEMSET(o_pOutPostWarInfo[OutPostWarInfoCount].GuildName, (*itr)->GetOutPostGuildName(), SIZE_MAX_GUILD_NAME);
