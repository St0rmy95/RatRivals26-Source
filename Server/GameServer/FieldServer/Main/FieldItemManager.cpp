...
void CFieldItemManager::ResetAllItemGenerals()
{
	...
	for_each(m_mapItemGeneral.begin(), m_mapItemGeneral.end(), Delete_Object_MAP());
...
void CFieldItemManager::ResetAllItemGeneralsInStore(BOOL i_bClearEnchat/*=TRUE*/)
{
	...
	for_each(m_mapItemInStore.begin(), m_mapItemInStore.end(), Delete_Object_MAP());
...
{
	mt_auto_lock mtAuto(&m_mapItemGeneral);
	for_each(m_vectItemSkillPtr.begin(), m_vectItemSkillPtr.end(), Delete_Object());
...
// 2014-07-04 by bckim, 무료지급 입찰권 속성 변경 
ITEM_GENERAL* CFieldItemManager::GetItemGeneralByItemNumFirstCreateTimeInCharacter(INT i_nItemNum, BYTE i_enumItemStorage)
{
	mt_auto_lock mtAuto(&m_mapItemGeneral);
	ITEM_GENERAL* o_pItemG = NULL;
	...
	if (i_enumItemStorage == ITEM_IN_CHARACTER)
	{
		...
		while (itr != m_mapItemGeneral.end())
		{
			...
			{			
				...
				if( NULL != o_pItemG && itr->second->CreatedTime <= o_pItemG->CreatedTime )
				{	
					o_pItemG = itr->second ;
				}
			}
...
// End. 2014-07-04 by bckim, 무료지급 입찰권 속성 변경
...
//				, m_vectUsingGeneralItem[i]->UniqueNumber);
			...
			SAFE_DELETE(m_vectUsingGeneralItem[i]);
...
//	}
	...
	{
		TradeStateCheck(m_pFieldIOCPSocket, i_nItemUpdateType);	// 2013-11-20 by jhseol, 거래로 획득하는 아이템이 아닐경우 진행중인 거래를 강제취소한다.
		// 2013-11-22 by jhseol, 캐나다 아이템 복사 버그 수정 - 아이템 실 소유자 똑바로 적용
		if( NULL != i_pItemGeneral && IUT_DROP_ITEM == i_nItemUpdateType )
		{
			i_pItemGeneral->AccountUniqueNumber	= m_pFieldIOCPSocket->m_character.AccountUniqueNumber;
			i_pItemGeneral->Possess				= m_pFieldIOCPSocket->m_character.CharacterUniqueNumber;
		}
		// end 2013-11-22 by jhseol, 캐나다 아이템 복사 버그 수정 - 아이템 실 소유자 똑바로 적용
	}
	...
	//	if(!COMPARE_RACE(this->m_pCharacter->Race, RACE_OPERATION|RACE_GAMEMASTER|RACE_MONITOR|RACE_GUEST|RACE_DEMO))		// 2013-01-18 by khkim, GLog (일반유저만 기록)	
	{
		if( NULL != m_pFieldIOCPSocket )
		{
			...
			case IUT_RANDOMBOX:
				...
				// 2013-11-25 by jhseol&bckim, 거래소 - GLog 추가
			case IUT_MARKET:
				{
					SetGLogData(GLOG_IL_LT_MARKET,i_pItemGeneral,m_pFieldIOCPSocket->m_character.Level);
					break;
				}
				// end 2013-11-25 by jhseol&bckim, 거래소 - GLog 추가
				// 2014-01-03 by jhseol&bckim, 파괴경매장 - GLog 추가
			case IUT_AUCTION:
				{
					SetGLogData(GLOG_IL_LT_AUCTION,i_pItemGeneral,m_pFieldIOCPSocket->m_character.Level);
					break;
				}
				// end 2014-01-03 by jhseol&bckim, 파괴경매장 - GLog 추가
			default:
				{
					break;
				}
...
// 			}
	...
	if(i_bCheckGeneralItemCounts
		...
		&& FALSE == COMPARE_BIT_FLAG(i_pItemGeneral->ItemInfo->ItemAttribute, ITEM_ATTR_QUEST_ITEM)	// 2013-11-06 by jhseol, 퀘스트아이템 인벤체크 제외
		&& i_pItemGeneral->ItemInfo->Kind != ITEMKIND_QUEST )										// 2013-11-06 by jhseol, 퀘스트아이템 인벤체크 제외
	{// 인벤토리가 다 참
	...
	{// 2006-05-11 by cmkwon, DB에 업데이트 하지 않으면 바로 m_mapItemGeneral에 추가한다.
		...
		if ( IUT_MARKET == i_nItemUpdateType || IUT_AUCTION == i_nItemUpdateType )	// 2014-01-03 by jhseol&bckim, 파괴경매장 - GameLog 추가
		{
			CAtumLogSender::SendLogMessageITEMGetITEM(m_pFieldIOCPSocket, i_pItemGeneral, IS_COUNTABLE_ITEM(i_pItemGeneral->Kind)?i_pItemGeneral->CurrentCount:1, i_nItemUpdateType);
		}
...
BOOL CFieldItemManager::InsertBossRewardItemByItemNum(INT i_nRewardItemNum,BYTE i_nRewardType, CFieldMonster* i_pMoster) 
{
	...
	INT nRewardItemCount = 0;	// 2014-04-17 by bckim, 보스몬스터 보상강화 아이템 지급 수량 변경 
	MAP_CHANNEL_INDEX MosterMapChannel = i_pMoster->GetCurrentMapChannelMonster()->GetMapChannelIndex();
	if( FALSE == MosterMapChannel.IsSameMapChannelIndex(m_pFieldIOCPSocket->GetCharacter()->MapChannelIndex))
	{
		return FALSE;
	}
	if(COMPARE_BODYCON_BIT( m_pFieldIOCPSocket->GetCharacter()->BodyCondition, BODYCON_DEAD_MASK)) // 유져가 죽은 상태이면 보상 안줌 
	{
		return FALSE;
	}

	char tmpstr[1024];		
	switch (i_nRewardType)
	{
	case 1:
		{	// 보스몬스터에게 일정데미지 이상을 입힌 유져 
			sprintf(tmpstr,STRMSG_130828_0001, m_pFieldIOCPSocket->m_character.CharacterName, i_pMoster->MonsterInfoPtr->MonsterName);
			nRewardItemCount = 3;		// 2014-04-17 by bckim, 보스몬스터 보상강화 아이템 지급 수량 변경 
			break;
		}		
	case 2:
		{	// 최초로 보스몬스터에게 데미지를 입힌 유져 
			sprintf(tmpstr,STRMSG_130828_0002, m_pFieldIOCPSocket->m_character.CharacterName, i_pMoster->MonsterInfoPtr->MonsterName);
			nRewardItemCount = 1;		// 2014-04-17 by bckim, 보스몬스터 보상강화 아이템 지급 수량 변경 
			break;
		}		
	case 3:
		{	// 가장 많은 데미지를 보스 몬스터에게 입힌유져 
			sprintf(tmpstr,STRMSG_130828_0003, m_pFieldIOCPSocket->m_character.CharacterName, i_pMoster->MonsterInfoPtr->MonsterName);
			nRewardItemCount = 2;		// 2014-04-17 by bckim, 보스몬스터 보상강화 아이템 지급 수량 변경 
			break;
		}		
	case 4:
		{	// 마지막으로 보스몬스터를 죽인 유져 
			sprintf(tmpstr,STRMSG_130828_0004, m_pFieldIOCPSocket->m_character.CharacterName, i_pMoster->MonsterInfoPtr->MonsterName);
			nRewardItemCount = 2;		// 2014-04-17 by bckim, 보스몬스터 보상강화 아이템 지급 수량 변경 
			break;
		}		
	default:
	...
	if( 1 != i_nRewardType )
	{		
		mt_auto_lock mtA(&i_pMoster->m_mtvectorRewardInfoPtr);
		mtvectorActionInfoPtr::iterator itr(i_pMoster->m_mtvectorRewardInfoPtr.begin());					
		for(; itr != i_pMoster->m_mtvectorRewardInfoPtr.end(); itr++)
		{
			CFieldIOCPSocket *tmpRewardUser = ms_pFieldIOCP2->GetFieldIOCPSocket((*itr)->UnitIndex);
			if( NULL != tmpRewardUser && TRUE == tmpRewardUser->IsValidCharacter(FALSE) )
			{
				tmpRewardUser->SendString128(STRING_128_USER_NOTICE,tmpstr);
			}		
		}
	}
	else
	{
		m_pFieldIOCPSocket->SendString128(STRING_128_USER_NOTICE,tmpstr);
	}

	m_pFieldIOCPSocket->m_ItemManager.InsertItemBaseByItemNum(i_nRewardItemNum, nRewardItemCount, IUT_BONUS_ITEM);	// 아이템 지급 // 2014-04-17 by bckim, 보스몬스터 보상강화 아이템 지급 수량 변경 
...
//	///////////////////////////////////////////////////////////////////////////////
//	// 2006-11-23 by cmkwon, 베트남 5시가 제한 설정 Drop SPI 처리
//	if(LANGUAGE_TYPE_VIETNAMESE == g_pFieldGlobal->GetLanguageType()
////		&& MONEY_ITEM_NUMBER == i_nItemNum			// 2007-03-23 by dhjin, 베트남 5시간 제한 설정에 모든 아이템 적용
//		&& (IUT_DROP_ITEM == i_nItemUpdateType || IUT_QUEST == i_nItemUpdateType))	// 2007-01-29 by cmkwon, 미션 보상 SPI도 베트남 정부 정책 적용
//	{// 2006-11-23 by cmkwon, 베트남, SPI, 드랍 SPI
//		int nGameContinueTimeOfToday = m_pFieldIOCPSocket->GetCurGameContinueTimeInSecondOfToday();
...
//			i_nItemCount	= (i_nItemCount+1)/2;	// 2007-03-28 by cmkwon, 반올림으로 처리
//		}
//		if(0 >= i_nItemCount)
//		{
//			return FALSE;
...
// 2008-04-08 by cmkwon, 게임 제한 시스템 수정 - 아래의 코드는 필요 없음, 드랍율로 처리하고 습득율은 항상 100%
// 	if(IUT_DROP_ITEM == i_nItemUpdateType || IUT_QUEST == i_nItemUpdateType)
// 	{
// 		float	fApplyRate		= 0.0f;
// 		BOOL	bRestriction	= m_pFieldIOCPSocket->GPGetApplyRateByGamePlayingTime(&fApplyRate);
// 		if(bRestriction)
...
// 			}
// 
// 			i_nItemCount	= (i_nItemCount+1)*fApplyRate;
// 		}		
// 		if(0 >= i_nItemCount)
// 		{
// 			return FALSE;
...
// 	}
	...
	if (IS_GENERAL_ITEM(pItemInfo->Kind))
	{
		if (IS_COUNTABLE_ITEM(pItemInfo->Kind))
		{
			...
			if (!bRet)
			{
				SAFE_DELETE(pItemGeneral);		// 2006-06-14 by cmkwon
				return FALSE;
			}
		}
		else
		...
		//if(!COMPARE_RACE(this->m_pCharacter->Race, RACE_OPERATION|RACE_GAMEMASTER|RACE_MONITOR|RACE_GUEST|RACE_DEMO))		// 2013-01-18 by khkim, GLog (일반유저만 기록)	
		{
			if( NULL != m_pFieldIOCPSocket )
			{
				...
				{ 
					...
					case IUT_MARKET:
						{
							SetGLogData(GLOG_IL_LT_MARKET, i_pItemGeneral, m_pFieldIOCPSocket->m_character.Level, i_nChangeCount);
							break;
						}
						// end 2013-11-25 by jhseol&bckim, 거래소 - GLog 추가
						// 2014-01-03 by jhseol&bckim, 파괴경매장 - GLog 추가
					case IUT_AUCTION:
						{
							SetGLogData(GLOG_IL_LT_AUCTION, i_pItemGeneral, m_pFieldIOCPSocket->m_character.Level, i_nChangeCount);
							break;
						}
						// end 2014-01-03 by jhseol&bckim, 파괴경매장 - GLog 추가
					default:
...
//		}
		...
		case IUT_MARKET:		// 2013-11-25 by jhseol&bckim, 거래소 - GameLog 추가
		case IUT_AUCTION:		// 2014-01-03 by jhseol&bckim, 파괴경매장 - GameLog 추가
		...
		case IUT_SHOP:
			{
				// 2015-03-18 by shchoi Countable WP Item 구입로그 안남는 버그 수정
				CAtumLogSender::SendLogMessageITEMBuyITEM(m_pFieldIOCPSocket, i_pItemGeneral, i_nChangeCount, -1, FALSE, UNLIMITED_REMAIN_COUNT_FOR_LIMITED_EDITION, FALSE);		// 2011-12-12 by hskim, GLog 2차
			}
		break;
		}
...
BOOL CFieldItemManager::DeleteItemFromCharacterByPointer(ITEM_GENERAL *i_pItemGeneral,
							...
							BOOL i_bUpdateDB, BOOL i_bSendMSG, BOOL i_bDeleteMemory,
							int	 i_nGLogItemtype/*=0*/,				// 2012-11-21 by khkim, GLog 2차
							int i_bDestroyAuction/*=0*/)				// 2013-12-26 by bckim, 파괴 경매 시스템
{
	TradeStateCheck(m_pFieldIOCPSocket, i_nItemUpdateType);	// 2013-11-20 by jhseol, 거래로 획득하는 아이템이 아닐경우 진행중인 거래를 강제취소한다.
	...
	if ( IUT_MARKET == i_nItemUpdateType || IUT_AUCTION == i_nItemUpdateType )	// 2014-01-03 by jhseol&bckim, 파괴경매장 - GameLog 추가
	{
		CAtumLogSender::SendLogMessageITEMDelete(m_pFieldIOCPSocket, i_pItemGeneral, i_nItemUpdateType);
	}
	// end 2013-11-25 by jhseol&bckim, 거래소 - GameLog 추가
	...
	if (i_bUpdateDB)
	{
		...
		if( i_bDestroyAuction)				// 2013-12-26 by bckim, 파괴 경매 시스템
		{
			ms_pFieldIOCP2->m_pAtumDBManager->MakeAndEnqueueQuery(QT_DestroyAuction_AddItem, m_pFieldIOCPSocket, m_pFieldIOCPSocket->m_character.AccountUniqueNumber, pQParam);
		}
		else
		{
		ms_pFieldIOCP2->m_pAtumDBManager->MakeAndEnqueueQuery(QT_DeleteStoreItem, m_pFieldIOCPSocket, m_pFieldIOCPSocket->m_character.AccountUniqueNumber, pQParam);
													 ...
													 
{
	...
	return DeleteItemFromCharacterByPointer(i_pItemGeneral, i_nItemUpdateType, i_enumStorage, i_bUpdateDB, i_bSendMSG, i_bDeleteMemory, i_nGLogItemtype,i_bDestroyAuction);	// 2013-12-26 by bckim, 파괴 경매 시스템
	//End. 2013-12-26 by bckim, 파괴 경매 시스템
}
...
void CFieldItemManager::RearrangeCountableItems(BYTE i_enumStorage, int i_nItemNum/*=0*/ , BYTE i_byItemUpdateType /*= IUT_GENERAL*/ )
{
	...
	if (i_enumStorage == ITEM_IN_CHARACTER)
	{
		...
		while (itrItemGeneral != m_mapItemGeneral.end())
		{
			...
			// 2014-02-19 by jhseol, 카운터블 아이템 UID:0 예외처리
			if ( FALSE == IS_VALID_UID64(pItemGeneral->UniqueNumber) )
			{
				g_pFieldGlobal->WriteSystemLogEX(TRUE, "[ERROR] CFieldItemManager::RearrangeCountableItems_ITEM_IN_CHARACTER, UID is zero %s %s [CreatedTime:%s]\r\n",
					GetCharacterString(&(m_pFieldIOCPSocket->m_character), string()),
					GetItemGeneralString(pItemGeneral, string()), pItemGeneral->CreatedTime.GetDateTimeString(STRNBUF(SIZE_MAX_SQL_DATETIME_STRING)));
				itrItemGeneral++;
				continue;
			}
			// end 2014-02-19 by jhseol, 카운터블 아이템 UID:0 예외처리
	...
	else if (i_enumStorage == ITEM_IN_STORE)
	{
		...
		while (itrItemInStore != m_mapItemInStore.end())
		{
			...
			// 2014-02-19 by jhseol, 카운터블 아이템 UID:0 예외처리
			if ( FALSE == IS_VALID_UID64(pItemInStore->UniqueNumber) )
			{
				g_pFieldGlobal->WriteSystemLogEX(TRUE, "[ERROR] CFieldItemManager::RearrangeCountableItems_ITEM_IN_STORE, UID is zero %s %s [CreatedTime:%s]\r\n",
					GetCharacterString(&(m_pFieldIOCPSocket->m_character), string()),
					GetItemGeneralString(pItemInStore, string()), pItemInStore->CreatedTime.GetDateTimeString(STRNBUF(SIZE_MAX_SQL_DATETIME_STRING)));
				itrItemInStore++;
				continue;
			}
			// end 2014-02-19 by jhseol, 카운터블 아이템 UID:0 예외처리
...
void CFieldItemManager::DeleteItemSkillBySkillBaseNum(INT i_nSkillItemNum, BOOL i_bSendMsg)
{
	...
	while (itr != m_vectItemSkillPtr.end())
	{
		...
		{			
			...
			SAFE_DELETE(pItemSkill);			// release memory
...
#if defined(S_CAN_SERVER_SETTING_HSSON) || defined(S_140_SERVER_SETTING_HSSON)
...
#else
		pSUsingItem->nRemainSecond			= CAtumSJ::GetTimeSecondByItemKind8ItemAttribute(m_vectUsingGeneralItem[i]->ItemInfo) - m_vectUsingGeneralItem[i]->UsingTimeStamp;
#endif	// #ifdef S_CAN_SERVER_SETTING_HSSON
	...
	while (itr != m_vectUsingGeneralItem.end())
	{
		...
		
		{
			...
			if(WEAR_ATTACHED == pItemGen->Wear)
			{
				...
				MEMSET_ZERO(&tmItemChangePos, sizeof(MSG_FC_ITEM_CHANGE_WINDOW_POSITION));
...
//			if(pItemGen->ItemInfo->Kind == ITEMKIND_CARD && COMPARE_BIT_FLAG(pItemGen->ItemInfo->ItemAttribute, ITEM_ATTR_TIME_LIMITE|ITEM_ATTR_DELETED_TIME_LIMITE_AFTER_USED)) {
			if(IS_ATTACHABLE_ITEM(pItemGen->ItemInfo)) {
				...
				// 사용시 바로 삭제되는 시간제 아이템
				CIOCPSocket	*	MFSSock;
				MFSSock = this->ms_pFieldIOCP2->GetMFSSockForArenaServer(m_pFieldIOCPSocket->m_AfsNeedMfsInfo.MFSServerID);
...
void CFieldItemManager::CashInitPremiumCardInfo(SCASH_PREMEIUM_CARD_INFO *i_pCardInfo)
{
	...
	m_cashPremiumCardInfo.ResetAllPlusRateByPremiumCard(g_pFieldGlobal->GetLanguageType());		// 2006-10-11 by cmkwon
}
...
// 		}
		...
		if(FALSE == i_bOnlyCheck)
		{
			...
			MEMSET_ZERO(pQParam, sizeof(QPARAM_CASH_INSERT_PREMIUM_CARD));
	...
	{// 이미 카드가 존재한다 - 
		...
		if(FALSE == i_bOnlyCheck)
		{
			...
			MEMSET_ZERO(pQParam, sizeof(QPARAM_CASH_UPDATE_PREMIUM_CARD));
...
void CFieldItemManager::CashInsertBonusItemPremiumCard(ITEM *i_pItemInfo)
{
	...
	///////////////////////////////////////////////////////////////////////////////
	// 추가 아이템이 설정되어있으면 아이템을 추가한다
	// ParameterValue1	<== ItemNum
	// DestParameter1	<== Counts
// 2009-04-21 by cmkwon, ITEM에 DesParam 필드 개수 8개로 늘리기 - 아래와 같이 수정
// 	if(0.0f < i_pItemInfo->ParameterValue1
// 		&& 0 < i_pItemInfo->DestParameter1)
// 	{
// 		ITEM *pLinkItemInfo = ms_pFieldIOCP2->GetItemInfo(i_pItemInfo->ParameterValue1);
// 		if (pLinkItemInfo)
// 		{
// 			this->InsertItemBaseByItemNum(pLinkItemInfo->ItemNum, i_pItemInfo->DestParameter1, IUT_SHOP);
// 		}
// 	}
// 	if(0.0f < i_pItemInfo->ParameterValue2
// 		&& 0 < i_pItemInfo->DestParameter2)
// 	{
// 		ITEM *pLinkItemInfo = ms_pFieldIOCP2->GetItemInfo(i_pItemInfo->ParameterValue2);
// 		if (pLinkItemInfo)
// 		{
// 			this->InsertItemBaseByItemNum(pLinkItemInfo->ItemNum, i_pItemInfo->DestParameter2, IUT_SHOP);
// 		}
// 	}
// 	if(0.0f < i_pItemInfo->ParameterValue3
// 		&& 0 < i_pItemInfo->DestParameter3)
// 	{
// 		ITEM *pLinkItemInfo = ms_pFieldIOCP2->GetItemInfo(i_pItemInfo->ParameterValue3);
// 		if (pLinkItemInfo)
// 		{
// 			this->InsertItemBaseByItemNum(pLinkItemInfo->ItemNum, i_pItemInfo->DestParameter3, IUT_SHOP);
// 		}
// 	}
// 	if(0.0f < i_pItemInfo->ParameterValue4
// 		&& 0 < i_pItemInfo->DestParameter4)
// 	{
// 		ITEM *pLinkItemInfo = ms_pFieldIOCP2->GetItemInfo(i_pItemInfo->ParameterValue4);
// 		if (pLinkItemInfo)
// 		{
// 			this->InsertItemBaseByItemNum(pLinkItemInfo->ItemNum, i_pItemInfo->DestParameter4, IUT_SHOP);
// 		}
// 	}	
...
///////////////////////////////////////////////////////////////////////////////
/// \fn			BOOL CFieldItemManager::IsUsePremiumCardAdvantage(void)
/// \brief		
/// \author		cmkwon
/// \date		2006-06-14 ~ 2006-06-14
/// \warning	
...
/// \return		
///////////////////////////////////////////////////////////////////////////////
BOOL CFieldItemManager::IsUsePremiumCardAdvantage(void)
{
...
void CFieldItemManager::ItemInsertByInfinityPlaying(ITEM_GENERAL *i_pItemGeneral)
{
	...
	MEMSET_ZERO(&InsertItemInfo, sizeof(INFINITY_INSERT_ITEM_INFO));
...
//				, m_pFieldIOCPSocket->m_InfinityPlayingInfo.InfinityCreateUID, GetCharacterString(m_pFieldIOCPSocket->GetCharacter(), string()), GetItemGeneralString(i_pItemGeneral, string()), i_pItemGeneral->Wear, i_pItemGeneral->UsingTimeStamp, i_pItemGeneral->CoolingTimeStamp, i_pItemGeneral->MainSvrItemUID);
		...
		MEMSET_ZERO(&UpdateItemInfo, sizeof(INFINITY_UPDATE_ITEM_INFO));
...
BOOL CFieldItemManager::CheckBuyLimitedEditionShopItem(INT *o_pRemainCount, INT i_nItemNum)
{
	...
	BOOL bRet = mainDBOdbcStmt.Init(g_pGlobalGameServer->GetDBServerIP(), g_pGlobalGameServer->GetDBServerPort(), g_pGlobalGameServer->GetDBServerDatabaseName(), (char*)g_pGlobalGameServer->GetODBCUID(), (char*)g_pGlobalGameServer->GetODBCPASSWORD(), g_pGlobalGameServer->GetMainWndHandle());
	if (FALSE == bRet)
	{		
...
BOOL CFieldItemManager::AddInvokingItemList(ItemUID_t i_ItemUID)
{
	...
	{// 2010-03-04 by cmkwon, 이미 리스트에 존재
		...
		g_pFieldGlobal->WriteSystemLogEX(TRUE, "[ERROR] CFieldItemManager::AddInvokingItemList# %s %s %d %s\r\n"
			, GetCharacterString(m_pCharacter, string()), GetItemString(pItemG, string()), pItemG->CoolingTimeStamp, pItemG->CoolingStartTime.GetDateTimeString(STRNBUF(SIZE_MAX_SQL_DATETIME_STRING)));
		return FALSE;
	}
...
BOOL CFieldItemManager::InfinityInsertItemInfoFromDeleteItemList(ItemUID_t i_ItemUID4MFS)
{
	...
	INFINITY_DELETE_ITEM_INFO tmItemInfo;	MEMSET_ZERO(&tmItemInfo, sizeof(tmItemInfo));
...
// 						}
				...
				m_pFieldIOCPSocket->m_ArmorCollection.CheckMonthlyArmorEvent(pItemG);	// 2013-09-06 by jhseol, 이달의 아머 이벤트 적용 기간 변경
#endif
...
void CFieldItemManager::SetGLogData(BYTE i_nLogType, ITEM_GENERAL *i_pItemGeneral, BYTE i_nCharLv, int i_nChangeCount, int i_nCharSerial)
{
	if(NULL != i_pItemGeneral && NULL != m_pFieldIOCPSocket )	// 2013-05-30 by jhseol, GLog 시스템 보완
	{
		...
		MEMSET_ZERO(&LogItemParameter, sizeof(LogItemParameter));	// 2013-05-30 by jhseol, GLog 시스템 보완 - 초기화
...
// 2013-11-20 by jhseol, 거래로 획득하는 아이템이 아닐경우 진행중인 거래를 강제취소한다.
void CFieldItemManager::TradeStateCheck(CFieldIOCPSocket *i_pFieldIOCPSocket, BYTE i_nItemUpdateType)
{
	if ( NULL != i_pFieldIOCPSocket && IUT_TRADE != i_nItemUpdateType)
	{
		if (IS_VALID_UNIQUE_NUMBER(i_pFieldIOCPSocket->m_peerTraderCharacterUniqueNumber))	// 거래중이다.
		{
			i_pFieldIOCPSocket->SendTradeCanceledAndReset(NULL, TRUE);	// 거래 강제취소.
		}
	}
}
// end 2013-11-20 by jhseol, 거래로 획득하는 아이템이 아닐경우 진행중인 거래를 강제취소한다.


// 2014-07-17 by bckim, 프리미엄 아머 컬렉션
INT CFieldItemManager::ReturnItemNumWithShapeNum(USHORT i_UniKind)
{
	if( IS_BGEAR(i_UniKind) )	return PREMIUM_COLLECTION_MAX_ENCHANT_RETURN_ITEMNUM_B;
	else if( IS_MGEAR(i_UniKind) )		return PREMIUM_COLLECTION_MAX_ENCHANT_RETURN_ITEMNUM_M;
	else if( IS_AGEAR(i_UniKind) )		return PREMIUM_COLLECTION_MAX_ENCHANT_RETURN_ITEMNUM_A;
	else if( IS_IGEAR(i_UniKind) )		return PREMIUM_COLLECTION_MAX_ENCHANT_RETURN_ITEMNUM_I;
	else return 0;
}
