...
#include "InnovaLibrary.h"						// 2010-04-26 by cmkwon, 러시아 Innva 인증/빌링 시스템 변경 - 
...
:CIOCP(CLIENT_INDEX_START_NUM, SIZE_MAX_FIELDSERVER_SESSION, nPort, szLocalIP, ST_FIELD_SERVER)
{
	...
	CFieldIOCPSocket * pFieldIOCPSocket = new CFieldIOCPSocket[m_dwArrayClientSize];
	m_pArrayIOCPSocket = pFieldIOCPSocket;
	for(int i = 0; i < m_dwArrayClientSize; i++)
	{
		pFieldIOCPSocket[i].InitFieldIOCPSocket();
		m_ArrayClient[i] = &pFieldIOCPSocket[i];
...
//	m_pCouponManager				= NULL;
	...
	m_vectCHANNELING_SERVER_INFO.clear();			// 2014-02-10 by jhseol&bckim, 체널링
	...
	{// 2006-11-17 by cmkwon, 베트남은 기본 적용
		m_bUseTimeLimiteSysteOfToday	= TRUE;
	}
	...
	m_mtRandManager.seed();		// 2007-12-10 by cmkwon, 랜덤 시스템 수정 - 

	m_mtvectSARENA_USE_FIELD_SERVER_INFO.clear();		// 2007-12-27 by dhjin, 아레나 통합 - 아레나 서버에서 사용하는 필드 서버 정보

	//////////////////////////////////////////////////////////////////////////
	// 2008-08-25 by dhjin, 태국 PC방 IP정보 로딩
	m_nPCBangMinTick = 0;			
	m_nMaxPCBangMinTick = MaxPCBangMinTick;		// 2008-08-25 by dhjin, 24 * 60 = 1440분
	if(LANGUAGE_TYPE_THAI == g_pFieldGlobal->GetLanguageType())
	{// 2008-08-25 by dhjin, 태국은 15분 마다.
		m_nMaxPCBangMinTick = 15;
	}
...
//	m_TenderInfoList.clear();			// 2009-09-09 ~ 2010 by dhjin, 인피니티 - Tender
	...
	m_pInnovaLibrary					= NULL;		// 2010-04-26 by cmkwon, 러시아 Innva 인증/빌링 시스템 변경 - 
	...
	m_pDestroyAuctionManager			= NULL;
	m_bDestroyAuctionInProgress			= FALSE;
	m_bDestroyAuctionContents			= TRUE;		// 경매장 ON/OFF 명령어 처리
	// end 2014-01-03 by jhseol&bckim, 파괴경매장

	m_bMarketActivation= TRUE;			// 2014-03-18 by bckim, 마켓 ON/OFF
}
...
CFieldIOCP::~CFieldIOCP()
{
	...
	SAFE_DELETE_DESTROY(m_pMapTriggerManager);		// 2011-10-28 by hskim, EP4 [트리거 시스템] - 크리스탈 시스템

	SAFE_DELETE(m_pHappyHourEventManager);
// 2008-01-10 by cmkwon, 아이템 이벤트 시스템에 신 쿠폰 시스템 추가 - 삭제됨
//	SAFE_DELETE(m_pCouponManager);
	SAFE_DELETE(m_pRacingDemon);
	SAFE_DELETE(m_pAtumDBManager);
	SAFE_DELETE(m_pFieldMapWorkspace);
	SAFE_DELETE(m_pFieldTickManager);
	SAFE_DELETE(m_pSecurityManager);		// 2011-06-22 by hskim, 사설 서버 방지
	SAFE_DELETE(m_pStatisticsManager);		// 2012-01-16 by hskim, 통계 - 화패
	SAFE_DELETE(m_pGiveItemManager);
	SAFE_DELETE(m_pAllGuildItemManager);	// 2006-09-20 by dhjin

	SAFE_DELETE(m_pFieldODBC);

	SAFE_DELETE(m_pMonthlyArmorManager);		// 2013-04-18 by jhseol,bckim 이달의 아머 - m_pMonthlyArmorManager 삭제
	SAFE_DELETE(m_pMarketDataManager);		// 2013-11-25 by jhseol&bckim, 거래소 - m_pMarketDataManager 삭제
}
...
BOOL CFieldIOCP::LoadFieldServerDataDebug(void)
{
	...
	bRet = odbcStmt.Init(g_pGlobalGameServer->GetDBServerIP(), g_pGlobalGameServer->GetDBServerPort(), g_pGlobalGameServer->GetDBServerDatabaseName(), (LPCSTR)g_pGlobalGameServer->GetODBCUID(), (LPCSTR)g_pGlobalGameServer->GetODBCPASSWORD(), g_pFieldGlobal->GetMainWndHandle());
	if (!bRet)
	{
	...
	// Load Map Data...
	// 2008-09-10 by cmkwon, 통합아레나 맵설정(map.cfg) 관련 수정 - 
	//bRet = m_pFieldMapWorkspace->LoadAllProject((string(CONFIG_ROOT)+string("./map.cfg")).c_str());
#ifdef ARENA
	bRet = m_pFieldMapWorkspace->LoadAllProject((string(CONFIG_ROOT) + string("./arenamap.cfg")).c_str(), g_pFieldGlobal->IsArenaServer(), &vectArenaMapIndexList);
#else
	bRet = m_pFieldMapWorkspace->LoadAllProject((string(CONFIG_ROOT)+string("./map.cfg")).c_str(), g_pFieldGlobal->IsArenaServer(), &vectArenaMapIndexList);
#endif
	...
	// Load ItemInfo...
	LoadItemInfo(&(g_pFieldGlobal->m_Localization));
	...
	// Load RareItemInfo...
	bRet = CAtumDBHelper::LoadRareItemInfo(&odbcStmt, &m_mapRareItemInfo, &(g_pFieldGlobal->m_Localization), NULL, &m_mapLevel2RareItemInfo);
	if (!bRet)
	{
...
BOOL CFieldIOCP::IOCPInit(void)
{
	...
	ShowLoadingProgressbar(); // 2014-03-10 by jekim, 서버 로딩 프로그래스바 창
	SetProgressBar(1); // 2014-03-10 by jekim, 서버 로딩 프로그래스바 창
...
//	}
	...
	SetProgressBar(2); // 2014-03-10 by jekim, 서버 로딩 프로그래스바 창
...
//		, g_pFieldGlobal->GetODBCPASSWORD()))
	if(FALSE == m_pFieldODBC->InitFieldODBC(this, g_pGlobalGameServer->GetDBServerIP(), g_pGlobalGameServer->GetDBServerPort(), g_pGlobalGameServer->GetDBServerDatabaseName(), (char*)g_pGlobalGameServer->GetODBCUID()
			, (char*)g_pGlobalGameServer->GetODBCPASSWORD(), g_pGlobalGameServer->GetMainWndHandle()))
	{
		// 2008-09-05 by cmkwon, DBServer 연결 실패 시스템 로그 남기기 - 
	...
	SetProgressBar(3); // 2014-03-10 by jekim, 서버 로딩 프로그래스바 창
	CODBCStatement odbcStmt;
	// 2008-07-11 by cmkwon, MySQL 지원을 위해 수정(IP,Port 로 DB 서버 연결) - 아래와 같이 수정 함
	//bRet = odbcStmt.Init((LPCSTR)g_pFieldGlobal->GetODBCDSN(), (LPCSTR)g_pFieldGlobal->GetODBCUID(), (LPCSTR)g_pFieldGlobal->GetODBCPASSWORD());
	bRet = odbcStmt.Init(g_pGlobalGameServer->GetDBServerIP(), g_pGlobalGameServer->GetDBServerPort(), g_pGlobalGameServer->GetDBServerDatabaseName(), (LPCSTR)g_pGlobalGameServer->GetODBCUID(), (LPCSTR)g_pGlobalGameServer->GetODBCPASSWORD(), g_pGlobalGameServer->GetMainWndHandle());
	if (!bRet)
	{
		// 2008-09-05 by cmkwon, DBServer 연결 실패 시스템 로그 남기기 - 
	...
	SetProgressBar(4); // 2014-03-10 by jekim, 서버 로딩 프로그래스바 창
	// Load AtumSJ Data
	if (!CAtumSJ::LoadData(&odbcStmt, &(g_pFieldGlobal->m_Localization)))
	{
		MessageBox(NULL, "CAtumSJ::LoadData() fail", "ERROR - SpaceCowboy FieldServer", NULL);
		return FALSE;
	}
	SetProgressBar(5); // 2014-03-10 by jekim, 서버 로딩 프로그래스바 창
...
// 	this->Send2IMServer((BYTE*)&msgTy, SIZE_FIELD_TYPE_HEADER);
	this->Send2PreAndIMOfAlivePacket();		// 2008-02-25 by cmkwon, Pre,IM Server 로 Alive Packet 전송하는 함수 추가 - 
	SetProgressBar(6); // 2014-03-10 by jekim, 서버 로딩 프로그래스바 창
	///////////////////////////////////////////////////////////////////////////////
	// 2008-09-10 by cmkwon, 통합아레나 맵설정(map.cfg) 관련 수정 - 
	vectMapIndexList		vectArenaMapIndexList;
	bRet = CAtumDBHelper::LoadArenaMapList(&vectArenaMapIndexList, &odbcStmt);
	...
	SetProgressBar(7); // 2014-03-10 by jekim, 서버 로딩 프로그래스바 창
	////////////////////////////////////////////////////////////////////////////////
	...
	SetProgressBar(8); // 2014-03-10 by jekim, 서버 로딩 프로그래스바 창
	if(g_pFieldGlobal->IsArenaServer()) {
		// 아레나 서버만 Tick시작
		this->m_InfinityManager.StartTick();
	}
	SetProgressBar(9); // 2014-03-10 by jekim, 서버 로딩 프로그래스바 창
	vectorCinemaInfo			tmVectCinemaInfo;
	bRet = CAtumFieldDBManager::DBLoadCinemaInfo(&odbcStmt, &tmVectCinemaInfo, &(g_pFieldGlobal->m_Localization));	// 2009-09-09 ~ 2010-02-26 by dhjin, 인피니티 - *.cfg파일 추가
	if(FALSE == bRet) {
	...
	SetProgressBar(10); // 2014-03-10 by jekim, 서버 로딩 프로그래스바 창
	...
	SetProgressBar(11); // 2014-03-10 by jekim, 서버 로딩 프로그래스바 창
	...
	SetProgressBar(12); // 2014-03-10 by jekim, 서버 로딩 프로그래스바 창
	...
	SetProgressBar(13); // 2014-03-10 by jekim, 서버 로딩 프로그래스바 창
	///////////////////////////////////////////////////////////////////////////////
	...
	SetProgressBar(14); // 2014-03-10 by jekim, 서버 로딩 프로그래스바 창
	...
	///////////////////////////////////////////////////////////////////////////////
	SetProgressBar(16); // 2014-03-10 by jekim, 서버 로딩 프로그래스바 창
	...
	SetProgressBar(17); // 2014-03-10 by jekim, 서버 로딩 프로그래스바 창
	// start 2011-10-28 by hskim, EP4 [트리거 시스템] - 크리스탈 시스템
	m_pMapTriggerManager = new CMapTriggerManager;
	m_pMapTriggerManager->InitOdbcStmt();		// 2013-08-14 by jhseol, 거점전 리뉴얼 - NGC 거점
	...
	// 2009-09-09 ~ 2010 by dhjin, 인피니티 -
	SetProgressBar(18); // 2014-03-10 by jekim, 서버 로딩 프로그래스바 창
	// Load Map Data...
	// 2008-09-10 by cmkwon, 통합아레나 맵설정(map.cfg) 관련 수정 - 
	//bRet = m_pFieldMapWorkspace->LoadAllProject((string(CONFIG_ROOT)+string("./map.cfg")).c_str());
#ifdef ARENA
	...
	SetProgressBar(40); // 2014-03-10 by jekim, 서버 로딩 프로그래스바 창
	m_pDefaultMapProject = (CFieldMapProject*)m_pFieldMapWorkspace->GetMapProjectByMapIndex(m_DefaultMapIndex);
	if (NULL == m_pDefaultMapProject)
	...
	SetProgressBar(41); // 2014-03-10 by jekim, 서버 로딩 프로그래스바 창
...
// 	this->Send2IMServer((BYTE*)&msgTy, SIZE_FIELD_TYPE_HEADER);
	this->Send2PreAndIMOfAlivePacket();		// 2008-02-25 by cmkwon, Pre,IM Server 로 Alive Packet 전송하는 함수 추가 - 
	SetProgressBar(44); // 2014-03-10 by jekim, 서버 로딩 프로그래스바 창
	///////////////////////////////////////////////////////////////////////////////
	...
	SetProgressBar(45); // 2014-03-10 by jekim, 서버 로딩 프로그래스바 창
	...
	SetProgressBar(46); // 2014-03-10 by jekim, 서버 로딩 프로그래스바 창
	// Load ItemInfo...
	LoadItemInfo(&(g_pFieldGlobal->m_Localization));
	...
	// 2009-08-03 by cmkwon, EP3-4 편대 대형 스킬 구현 - 
	this->ArrangePartyFormationSkill(&m_PartyFormationItemSkillList, &m_mapItemInfo);
	SetProgressBar(47); // 2014-03-10 by jekim, 서버 로딩 프로그래스바 창
// 2008-02-25 by cmkwon, Pre,IM Server 로 Alive Packet 전송하는 함수 추가 - 아래와 같이 Send2PreAndIMOfAlivePacket() 로 대체
// 	// 2008-02-14 by cmkwon, Pre,IM Server로 Alive 패킷을 전송한다.
// 	msgTy		= T_FP_CONNECT_ALIVE;
// 	this->Send2PreServerW((BYTE*)&msgTy, SIZE_FIELD_TYPE_HEADER);
...
// 	this->Send2IMServer((BYTE*)&msgTy, SIZE_FIELD_TYPE_HEADER);
	this->Send2PreAndIMOfAlivePacket();		// 2008-02-25 by cmkwon, Pre,IM Server 로 Alive Packet 전송하는 함수 추가 - 
	SetProgressBar(48); // 2014-03-10 by jekim, 서버 로딩 프로그래스바 창
	///////////////////////////////////////////////////////////////////////////////
	// 2006-08-10 by cmkwon, 아이템 체크, 랜덤박스 지급 아이템 추가, 
	ITEM * pErrItemInfo = CheckAllItemInfo();
	if(NULL != pErrItemInfo)
	...
	SetProgressBar(49); // 2014-03-10 by jekim, 서버 로딩 프로그래스바 창
	///////////////////////////////////////////////////////////////////////////////
	...
	SetProgressBar(50); // 2014-03-10 by jekim, 서버 로딩 프로그래스바 창
	
	// 2014-02-10 by jhseol&bckim, 체널링
	if(FALSE == CAtumDBHelper::GetChannelingServerInfo(&odbcStmt, &m_vectCHANNELING_SERVER_INFO))
	{
// 		char szSystemLog[1024];
// 		sprintf(szSystemLog, "[ERROR] GetChannelingServerInfo Error, vectorCnt(%d)", tmItemOverlapList.size());
// 		g_pFieldGlobal->WriteSystemLogEX(TRUE, "%s \r\n", szSystemLog);
// 		MessageBox(NULL, szSystemLog, "ERROR - SpaceCowboy FieldServer", NULL);
		return FALSE;
	}
	...
	SetProgressBar(51); // 2014-03-10 by jekim, 서버 로딩 프로그래스바 창
	// 2006-08-24 by dhjin
	...
	SetProgressBar(52); // 2014-03-10 by jekim, 서버 로딩 프로그래스바 창
	...
	SetProgressBar(53); // 2014-03-10 by jekim, 서버 로딩 프로그래스바 창
	// 2006-09-20 by dhjin
	...
	SetProgressBar(54); // 2014-03-10 by jekim, 서버 로딩 프로그래스바 창
// 2008-02-25 by cmkwon, Pre,IM Server 로 Alive Packet 전송하는 함수 추가 - 아래와 같이 Send2PreAndIMOfAlivePacket() 로 대체
//	// 2008-02-14 by cmkwon, Pre,IM Server로 Alive 패킷을 전송한다.
//	msgTy		= T_FP_CONNECT_ALIVE;
//	this->Send2PreServerW((BYTE*)&msgTy, SIZE_FIELD_TYPE_HEADER);
...
//	this->Send2IMServer((BYTE*)&msgTy, SIZE_FIELD_TYPE_HEADER);
	this->Send2PreAndIMOfAlivePacket();		// 2008-02-25 by cmkwon, Pre,IM Server 로 Alive Packet 전송하는 함수 추가 - 
	SetProgressBar(55); // 2014-03-10 by jekim, 서버 로딩 프로그래스바 창
	// Load EnchantInfo...
...
//	this->Send2IMServer((BYTE*)&msgTy, SIZE_FIELD_TYPE_HEADER);
	this->Send2PreAndIMOfAlivePacket();		// 2008-02-25 by cmkwon, Pre,IM Server 로 Alive Packet 전송하는 함수 추가 - 
	SetProgressBar(56); // 2014-03-10 by jekim, 서버 로딩 프로그래스바 창
	// Load EnchantInfo...
	...
	SetProgressBar(57); // 2014-03-10 by jekim, 서버 로딩 프로그래스바 창
	...
	SetProgressBar(58); // 2014-03-10 by jekim, 서버 로딩 프로그래스바 창
	// 2007-05-11 by cmkwon, 미스테리아이템캡슐 드랍 아이템 리스트 로딩
	bRet = CAtumDBHelper::LoadMysteryItemDropInfo(&m_mtmapMysteryItemDropList, &odbcStmt, &m_mapItemInfo, g_pFieldGlobal->GetMGameServerID());
	if(FALSE == bRet)
	{
	...
	SetProgressBar(59); // 2014-03-10 by jekim, 서버 로딩 프로그래스바 창
	...
	SetProgressBar(60); // 2014-03-10 by jekim, 서버 로딩 프로그래스바 창
	...
	SetProgressBar(61); // 2014-03-10 by jekim, 서버 로딩 프로그래스바 창
	///////////////////////////////////////////////////////////////////////////////
	...
	{
		if(g_pGlobalGameServer->InMasangServer())
	...
	SetProgressBar(62); // 2014-03-10 by jekim, 서버 로딩 프로그래스바 창
	// 2013-08-14 by jhseol, 거점전 리뉴얼 - MonsterGradeList 로드
#ifdef SC_STRATEGYPOINTWAR_RENEWAL_JHSEOL_SSJUNG	// - MonsterGradeList 로드
	m_MonsterGradeList.clear();
	bRet = CAtumDBHelper::LoadMonsterGradeList(&odbcStmt, &m_MonsterGradeList);
	if(FALSE == bRet)
	{
		MessageBox(NULL, "CAtumDBHelper::LoadMonsterGradeList_ fail", "ERROR - AceOnline FieldServer", NULL);
		return FALSE;
	}
	
	// 2009-07-02 by cmkwon, 서버 실행시 실행 관련 로그 추가 - 
	g_pFieldGlobal->WriteSystemLogEX(TRUE, "  [Notify] CFieldIOCP::IOCPInit# success to LoadMonsterGradeList(%d) \r\n", m_MonsterGradeList.size());
#endif
	// end 2013-08-14 by jhseol, 거점전 리뉴얼 - MonsterGradeList 로드
	SetProgressBar(63); // 2014-03-10 by jekim, 서버 로딩 프로그래스바 창
...
//	}
	SetProgressBar(64); // 2014-03-10 by jekim, 서버 로딩 프로그래스바 창
	...
	SetProgressBar(65); // 2014-03-10 by jekim, 서버 로딩 프로그래스바 창
	...
	SetProgressBar(66); // 2014-03-10 by jekim, 서버 로딩 프로그래스바 창
	///////////////////////////////////////////////////////////////////////////////
...
//	if (!m_GuildWarManager.InitGuildWarMananger(this, g_pFieldGlobal->GetODBCDSN(), g_pFieldGlobal->GetODBCUID(), g_pFieldGlobal->GetODBCPASSWORD()))
	if (!m_GuildWarManager.InitGuildWarMananger(this, g_pGlobalGameServer->GetDBServerIP(), g_pGlobalGameServer->GetDBServerPort(), g_pGlobalGameServer->GetDBServerDatabaseName(), (char*)g_pGlobalGameServer->GetODBCUID(), (char*)g_pGlobalGameServer->GetODBCPASSWORD(), g_pGlobalGameServer->GetMainWndHandle()))
	{
		// 2008-09-05 by cmkwon, DBServer 연결 실패 시스템 로그 남기기 - 
	...
	SetProgressBar(67); // 2014-03-10 by jekim, 서버 로딩 프로그래스바 창
	...
	SetProgressBar(68); // 2014-03-10 by jekim, 서버 로딩 프로그래스바 창
	//////////////////////////////////////////////////////////////////////////
	...
	SetProgressBar(69); // 2014-03-10 by jekim, 서버 로딩 프로그래스바 창
	...
	SetProgressBar(70); // 2014-03-10 by jekim, 서버 로딩 프로그래스바 창
	//////////////////////////////////////////////////////////////////////////
	...
	SetProgressBar(71); // 2014-03-10 by jekim, 서버 로딩 프로그래스바 창
	...
	// 2014-03-24 by bckim, 거래소 시스템 ( 거래완료된 목록 DB 로그로 따로 옮기기) 
#ifdef SC_MARKET_JHSEOL_BCKIM_SSJUNG
	if ( FALSE == g_pFieldGlobal->IsArenaServer() )
	{
		// 2014-04-22 by bckim, 거래소 관련 로그를 같은 테이블에서 확인하기 위해 주석 처리함
// 		bRet = CAtumFieldDBManager::DBRemoveMarketInfoCloseOfTrade(&odbcStmt);
// 		if(FALSE == bRet)
// 		{
// 			MessageBox(NULL, "CAtumFieldDBManager::DBRemoveMarketInfoCloseOfTrade_fail", "ERROR - SpaceCowboy FieldServer", NULL);
// 			return FALSE;
// 		}
		// End. 2014-04-22 by bckim, 거래소 관련 로그를 같은 테이블에서 확인하기 위해 주석 처리함
	}
#endif
	// End. 2014-03-24 by bckim, 거래소 시스템 ( 거래완료된 목록 DB 로그로 따로 옮기기) 

	SetProgressBar(72); // 2014-03-10 by jekim, 서버 로딩 프로그래스바 창
	// 2009-07-02 by cmkwon, 서버 실행시 실행 관련 로그 추가 - 
	...
	SetProgressBar(73); // 2014-03-10 by jekim, 서버 로딩 프로그래스바 창
	// 2009-07-02 by cmkwon, 서버 실행시 실행 관련 로그 추가 - 
	...
	SetProgressBar(74); // 2014-03-10 by jekim, 서버 로딩 프로그래스바 창
	// 2009-07-02 by cmkwon, 서버 실행시 실행 관련 로그 추가 - 
	...
	SetProgressBar(75); // 2014-03-10 by jekim, 서버 로딩 프로그래스바 창

	///////////////////////////////////////////////////////////////////////////////
	// 2007-05-28 by cmkwon
	this->LoadResObjCheckList();
	SetProgressBar(95); // 2014-03-10 by jekim, 서버 로딩 프로그래스바 창
	///////////////////////////////////////////////////////////////////////////////
	// 2008-04-29 by cmkwon, 서버군 정보 DB에 추가(신규 계정 캐릭터 생성 제한 시스템추가) - 
	SDBSERVER_GROUP tmDBServGroup;
	MEMSET_ZERO(&tmDBServGroup, sizeof(SDBSERVER_GROUP));
	...
	if(FALSE == bRet)
	{
		STRNCPY_MEMSET(tmDBServGroup.ServerGroupName, g_pFieldGlobal->GetServerGroupName(), SIZE_MAX_SERVER_NAME);
	...
	SetProgressBar(96); // 2014-03-10 by jekim, 서버 로딩 프로그래스바 창
	// 2009-07-02 by cmkwon, 서버 실행시 실행 관련 로그 추가 - 
	...
	SetProgressBar(97); // 2014-03-10 by jekim, 서버 로딩 프로그래스바 창
	///////////////////////////////////////////////////////////////////////////////
	// 2010-04-09 by cmkwon, 인피2차 추가 수정(단계별 보상 추가) - 
	...
	SetProgressBar(98); // 2014-03-10 by jekim, 서버 로딩 프로그래스바 창
	// 2013-11-25 by jhseol&bckim, 거래소 - 매니저 생성
#ifdef SC_MARKET_JHSEOL_BCKIM_SSJUNG
	if ( FALSE == g_pFieldGlobal->IsArenaServer() )
	{
		m_pMarketDataManager = new CMarketDataManager(this);
		bRet = CAtumFieldDBManager::DBLoadMarketList(&odbcStmt, m_pMarketDataManager);
		if (FALSE == bRet)
		{
			SAFE_DELETE(m_pMarketDataManager);	
			g_pGlobal->WriteSystemLogEX(TRUE, "[ERROR] CMarketManager Create fail !! \r\n");
			MessageBox(NULL, "CMarketManager fail !!", "ERROR - FieldServer", NULL);
			return FALSE; 
		}
	}
#endif 
	// end 2013-11-25 by jhseol&bckim, 거래소 - 매니저 생성

	// 2014-01-03 by jhseol&bckim, 파괴경매장 - 매니저 생성
#ifdef SC_DESTROY_AUCTION_JHSEOL_BCKIM_SSJUNG	// - 매니저 생성
	m_pDestroyAuctionManager = new CDestroyAuctionManager(this);
#endif	//#ifdef SC_DESTROY_AUCTION_JHSEOL_BCKIM_SSJUNG
	// end 2014-01-03 by jhseol&bckim, 파괴경매장 - 매니저 생성
...
// 	if (!m_MapOwnershipManager.InitMapOwnershipManager(this, g_pGlobalGameServer->GetDBServerIP(), g_pGlobalGameServer->GetDBServerPort(), g_pGlobalGameServer->GetDBServerDatabaseName(), (char*)g_pGlobalGameServer->GetODBCUID(), (char*)g_pGlobalGameServer->GetODBCPASSWORD(), g_pGlobalGameServer->GetMainWndHandle())
...
//		, g_pFieldGlobal->GetODBCPASSWORD(), CAtumSJ::GetVectPtrALL_MAP_INFO()))
	if(FALSE == m_cityWarManager.InitCityWarManager(this, g_pGlobalGameServer->GetDBServerIP(), g_pGlobalGameServer->GetDBServerPort(), g_pGlobalGameServer->GetDBServerDatabaseName(), (char*)g_pGlobalGameServer->GetODBCUID(), (char*)g_pGlobalGameServer->GetODBCPASSWORD(), g_pGlobalGameServer->GetMainWndHandle(), CAtumSJ::GetVectPtrALL_MAP_INFO()))
	{
		// 2008-09-05 by cmkwon, DBServer 연결 실패 시스템 로그 남기기 - 
...
//		return FALSE;
//	}
	SetProgressBar(99); // 2014-03-10 by jekim, 서버 로딩 프로그래스바 창
...
// 	m_pAtumDBManager->MakeAndEnqueueQuery(QT_LoadTenderInfo, NULL, 0, NULL);
	...
	///////////////////////////////////////////////////////////////////////////////
	// 2010-07-20 by hslee, Skip Execute Is ArenaServer.
	if ( !g_pFieldGlobal->IsArenaServer() )
	{
		///////////////////////////////////////////////////////////////////////////////
		...
		if(FALSE == this->INNBILL_Init())
		{
			...
			MessageBox(NULL, "INNBILL_Init fail !!", "ERROR - SpaceCowboy FieldServer", NULL);
			return FALSE;
		}
	}
	...
	SetProgressBar(100); // 2014-03-10 by jekim, 서버 로딩 프로그래스바 창
	CloseLoadingProgressbar(); // 2014-03-10 by jekim, 서버 로딩 프로그래스바 창
...
//	if(m_pCouponManager){				m_pCouponManager->CleanCouponManager();}
	...
	if(m_pFieldTickManager){			m_pFieldTickManager->CleanTickManager();}
	if(m_pSecurityManager){				m_pSecurityManager->CleanTickManager();} // 2011-06-22 by hskim, 사설 서버 방지
	if(m_pStatisticsManager){			m_pStatisticsManager->CleanTickManager();} // 2011-06-22 by hskim, 사설 서버 방지
	...
	///////////////////////////////////////////////////////////////////////////////
	// 2009-08-03 by cmkwon, EP3-4 편대 대형 스킬 구현 - 
	for_each(m_PartyFormationItemSkillList.begin(), m_PartyFormationItemSkillList.end(), Delete_Object_MAP());
...
//	m_MapOwnershipManager.CleanMapOwnershipManager();
	...
	for_each(m_mtvectorDeletedFieldParties.begin(), m_mtvectorDeletedFieldParties.end(), Delete_Object());
	...
	for (int i = (CLIENT_INDEX_START_NUM); i < m_dwArrayClientSize; i++)
	{
			...
			&& COMPARE_BIT_FLAG(pSock->GetCharacter()->UnitKind, i_usReqUnitKind))	// 2008-09-09 by cmkwon, /세력소환 명령어 인자 리스트에 기어타입 추가 - 기어타입 인자 추가
		{
			if(i_pExcludeMapChannel
				&& i_pExcludeMapChannel->IsSameMapChannelIndex(pSock->m_character.MapChannelIndex))
...
BOOL CFieldIOCP::OpenUDPPortForOtherServer(void)
{
	...
	for(int i=0; i < m_pFieldMapWorkspace->GetMapProjectCounts(); i++)
	{
		...
		if(NULL == hret )
		{
			...
			SetLastError(0);
			char	szError[1024];
			sprintf(szError, "CNPCIOCP::OpenUDPPortForOtherServer CreateIoCompletionPort() LastError[%d] hCompletionPort[0x%X] pIOCPSocket[0x%X]\r\n"
...
void CFieldIOCP::LoadItemInfo(CLocalization *i_pLocalization, BOOL bReload)
{
...
//	ret = SQLExecDirect(hstmt, (UCHAR*)QUERY_080702_0246, SQL_NTS);
	...
	if (!bReload)
	{
		...
		while ( (ret = SQLFetch(hstmt)) != SQL_NO_DATA)
		{
			...
			i_pLocalization->ItemLocalization(item.ItemNum, item.ItemName, item.Description);
...
// 2008-07-11 by cmkwon, MySQL 지원을 위해 수정(IP,Port 로 DB 서버 연결) - 아래와 같이 수정 함
//	odbcStmt.Init((char*)g_pGlobalGameServer->GetODBCDSN(), (char*)g_pGlobalGameServer->GetODBCUID(), (char*)g_pGlobalGameServer->GetODBCPASSWORD());
	...
	CAtumDBHelper::LoadItemInfo(&odbcStmt, &tmpMapItemInfo, &(g_pFieldGlobal->m_Localization));
...
//		}
	...
	hSearch = FindFirstFile((char*)(string(CONFIG_ROOT)+string("/quest/q_*.cfg")).c_str(), &FileData);
	if (hSearch == INVALID_HANDLE_VALUE)
	{
		DBGOUT("No quest files found.\n");
		sprintf(szSysLog, "    CFieldIOCP::LoadQuestInfo() _ No quest files found. File Root(%s) GetLastError(%d)\r\n", (char*)(string(CONFIG_ROOT)+string("/quest/q_*.cfg")).c_str(), GetLastError());
		g_pFieldGlobal->WriteSystemLog(szSysLog);		// 2013-12-12 by jhseol, 퀘스트 로딩시 Error SystemLog 추가
		return TRUE;
	}
	...
	while (!fFinished)
	{
		...
		{
			vecQuestFiles.push_back((char*)(string(CONFIG_ROOT)+string("./quest/")+string(FileData.cFileName)).c_str());
		}
		...
		if (!FindNextFile(hSearch, &FileData))
		{
			...
			else
			{
				...
				sprintf(szSysLog, "    CFieldIOCP::LoadQuestInfo() _ Could not find next file. File Root(%s) GetLastError(%d)\r\n", (char*)(string(CONFIG_ROOT)+string("./quest/")+string(FileData.cFileName)).c_str(), GetLastError());
				g_pFieldGlobal->WriteSystemLog(szSysLog);		// 2013-12-12 by jhseol, 퀘스트 로딩시 Error SystemLog 추가
				return FALSE;
			}
		}
	...
	if (!QuestLoaderParser::ParseFiles((char*)(string(CONFIG_ROOT)+string("./quest/")+string(QUEST_USER_DEFINED_VARIABLES_FILE_NAME)).c_str(), &vecQuestFiles, &m_mapQuestInfoFromScript))
	{
		sprintf(szSysLog, "    CFieldIOCP::LoadQuestInfo() _ ParseFiles() file. File Root(%s) GetLastError(%d)\r\n", (char*)(string(CONFIG_ROOT)+string("./quest/")+string(QUEST_USER_DEFINED_VARIABLES_FILE_NAME)).c_str(), GetLastError());
		g_pFieldGlobal->WriteSystemLog(szSysLog);		// 2013-12-12 by jhseol, 퀘스트 로딩시 Error SystemLog 추가
		return FALSE;
	}
...
BOOL CFieldIOCP::CheckAndInitQuestInfo(CQuest *i_pQuestInfo)
{
	...
	for (i = 0; i < i_pQuestInfo->vecQuestDropItem.size(); i++)
	{
		...
		MONSTER2ITEM *pM2I = new MONSTER2ITEM; MEMSET_ZERO(pM2I, sizeof(MONSTER2ITEM));
...
//	BOOL bIsNoDropItem		= FALSE;
//	if(LANGUAGE_TYPE_VIETNAMESE == g_pFieldGlobal->GetLanguageType()
//		&& FALSE == COMPARE_MPOPTION_BIT(i_pDeadFMon->MonsterInfoPtr->MPOption, MPOPTION_BIT_BOSS_MONSTER))
//	{
//		int nGameContinueTimeOfToday = i_pFISockTopDamage->GetCurGameContinueTimeInSecondOfToday();
//		if(nGameContinueTimeOfToday > TERM_GAME_PLAYTIME_LIMIT_LAST_SEC)
//		{// 2006-12-01 by cmkwon, 경험치/SPI/DropItem 습득 없음
//			bIsNoDropItem	= TRUE;;
//		}
//	}

// 2008-04-08 by cmkwon, 게임 제한 시스템 수정 - 아래와 같이 수정
// 	if(FALSE == COMPARE_MPOPTION_BIT(i_pDeadFMon->MonsterInfoPtr->MPOption, MPOPTION_BIT_BOSS_MONSTER))
// 	{
// 		float	fApplyRate		= 0.0f;
// 		BOOL	bRestriction	= i_pFISockTopDamage->GPGetApplyRateByGamePlayingTime(&fApplyRate);
// 		if(bRestriction
// 			&& fApplyRate <= 0.0f)
// 		{
// 			bIsNoDropItem	= TRUE;;
...
// 	}
	...
	///////////////////////////////////////////////////////////////////////////////
	// 2008-04-08 by cmkwon, 게임 제한 시스템 수정 - 드랍시 체크
	//							일반몹 - 드랍율 100% -->  50% -->   0%
	//							보스몹 - 드랍율 100% --> 100% --> 100%
	...
	if(FALSE == COMPARE_MPOPTION_BIT(i_pDeadFMon->MonsterInfoPtr->MPOption, MPOPTION_BIT_BOSS_MONSTER))
	{// 2008-04-08 by cmkwon, 보스 몬스터는 드랍율 100%, 퀘템 포함
		if(i_pFISockTopDamage
			&& i_pFISockTopDamage->GPGetApplyRateByGamePlayingTime(&fDropRateOfGameRestriction)
			&& fDropRateOfGameRestriction <= 0.0f)
		{
			return;		// 2008-04-08 by cmkwon, 일반몹 드랍율 0% 일때 그냥 리턴
		}
	}
	else
	{
		// 2008-04-03 by cmkwon, 핵쉴드 서버 연동 시스템 수정 - 보스몬스터는 50% 일때에도 드랍아이템이나 SPI 드랍량은 100%로처리
		//							0%일때에는 드랍아이템은 100%, 자동습득아이템(SPI,퀘템,...)은 드랍하지 않는다.
		float tmDropRate	= 0.0f;		// 2008-04-08 by cmkwon, 실제 드랍에서 처리하지는 않는다. 임시용임
		if(i_pFISockTopDamage
			&& i_pFISockTopDamage->GPGetApplyRateByGamePlayingTime(&tmDropRate)
			&& tmDropRate <= 0.0f)
		{
			bNoDropAutoPickUpItem		= TRUE;
		}
	}

// 2008-04-08 by cmkwon, 게임 제한 시스템 수정 - 아래와 같이 게임 제한 시스템 드랍율 적용
// 	float fPlusSPI			= i_pFISockTopDamage->GetPlusRateSPI() + i_fPartyBonus;
// 	float fPlusDropItem		= i_pFISockTopDamage->GetPlusRateDropItem() - 1.0f + i_fPartyBonus;
// 	float fPlusDropRareItem	= i_pFISockTopDamage->GetPlusRateDropRareItem() - 1.0f;

// 2010-04-09 by cmkwon, 인피2차 추가 수정(단계별 보상 추가) - 
// 	// 2008-04-08 by cmkwon, 게임 제한 시스템 수정 - 게임 제한 시스템 드랍율 적용
//  	float fPlusSPI			= (i_pFISockTopDamage->GetPlusRateSPI() + i_fPartyBonus) * fDropRateOfGameRestriction;
//  	float fPlusDropItem		= (i_pFISockTopDamage->GetPlusRateDropItem() - 1.0f + i_fPartyBonus) * fDropRateOfGameRestriction;
//  	float fPlusDropRareItem	= i_pFISockTopDamage->GetPlusRateDropRareItem() - 1.0f;
	///////////////////////////////////////////////////////////////////////////////
	// 2010-04-09 by cmkwon, 인피2차 추가 수정(단계별 보상 추가) - 
...
int CFieldIOCP::InsertTimeLimiteItemAllCharacterSameMapChannel(ITEM *i_pTimeLimiteItemInfo, MAP_CHANNEL_INDEX i_mapChanIdx, BYTE* i_pMsgData, int i_nMsgSize, BYTE i_byInflMask/*=INFLUENCE_TYPE_ALL_MASK*/)
{
	...
	for (int i = (CLIENT_INDEX_START_NUM); i < m_dwArrayClientSize; i++)
	{
		...
		if (pSock
			...
			&& i_mapChanIdx.IsSameMapChannelIndex(pSock->GetCharacter()->MapChannelIndex)
...
ITEM * CFieldIOCP::CheckAllItemInfo(void)
{
	...
	for(;itr != m_mapItemInfo.end(); itr++)
	{
		...
		if(FALSE == this->CheckItemInfo(pItemInfo))
		{
			return pItemInfo;
		}
...
// 2010-03-12 by cmkwon, CFieldIOCP::CheckItemInfo#에서 릴리즈모드일때문만 FALSE를 리턴 - 
	...
	int nBitsCnt = GetNumberOfBits(i_pItemInfo->ItemAttribute&(ITEM_ATTR_TIME_LIMITE|ITEM_ATTR_DELETED_TIME_LIMITE_AFTER_USED));
	if(1 < nBitsCnt)
	{
...
// 				|| 0.0f >= i_pItemInfo->DestParameter1+i_pItemInfo->DestParameter2+i_pItemInfo->DestParameter3+i_pItemInfo->DestParameter4)
			if(0.0f > i_pItemInfo->ArrDestParameter[0]
				...
				|| 0.0f >= i_pItemInfo->ArrDestParameter[0]+i_pItemInfo->ArrDestParameter[1]+i_pItemInfo->ArrDestParameter[2]+i_pItemInfo->ArrDestParameter[3])			
			{
				// 2009-08-07 by cmkwon, 아이템 에러 시스템 로그 추가 - 
	...
	{
		if(FALSE == IS_IN_RANGE(1, i_pItemInfo->GetParameterValue(DES_SKILL_HALLUCINATION), 20))
		{
			// 2009-08-07 by cmkwon, 아이템 에러 시스템 로그 추가 - 
	...
	if(i_pItemInfo->IsExistDesParam(DES_SKILL_CHAFF_HP))
	{
		...
		if(FALSE == IS_IN_RANGE(1, i_pItemInfo->GetParameterValue(DES_SKILL_CHAFF_HP), 3000))
		{
			// 2009-08-07 by cmkwon, 아이템 에러 시스템 로그 추가 - 
...
//	mmapINT2MYSTERY_ITEM_DROP::iterator	itr	= m_mmapMysteryItemDropList.find(i_nMysteryItemDropNum);
	...
	// 2013-10-18 by jekim, 이달의 아머 30%
	vectMYSTERY_ITEM_DROPPtr    vectMyteryDropItemPtrListMonthlyArmor;
	int nTotalProbabilityMonthlyArmor = 0;
	BOOL bMonthlyArmor = FALSE;
#ifdef S_MONTHL_ARMOR_EVENT_JHSEOL_BCKIM		// 이달의 아머 30%
	vector<ItemNum_t> vectMonthlyArmorList;
	if( FALSE == g_pFieldGlobal->IsArenaServer() )		//	2013-12-06 by bckim, 아레나서버 다운(외형캡슐사용)
	{
		ATUM_DATE_TIME atCurrentDate(TRUE);
		mtvectMONTHLY_ARMOR_EVNET_INFO *mtMonthlyArmorEventList = m_pMonthlyArmorManager->GetVectMonthlyArmorEventListPtr();
		mtvectMONTHLY_ARMOR_EVNET_INFO::iterator itrEvent = mtMonthlyArmorEventList->begin();
		for ( ; itrEvent != mtMonthlyArmorEventList->end() ; itrEvent++ )
		{
			if ( atCurrentDate >= itrEvent->atStartDate && atCurrentDate < itrEvent->atEndDate )
			{
				if(itrEvent->nOptionItemNum == MONTHLY_OLD_ARMOR_KIT_NUMBER_1
				|| itrEvent->nOptionItemNum == MONTHLY_OLD_ARMOR_KIT_NUMBER_2) continue; // 2014-12-29 by shchoi 이달의 구 아머
				vectMonthlyArmorList.push_back(itrEvent->nArmorSourceIndex);
			}
		}
	}

#endif
	// end 2013-10-18 by jekim, 이달의 아머 30%
	for(; itr != m_mtmapMysteryItemDropList.upper_bound(i_nMysteryItemDropNum); itr++)
	{
...
#ifdef S_MONTHL_ARMOR_EVENT_JHSEOL_BCKIM		// 이달의 아머 30%
		bMonthlyArmor = FALSE;
		for(int i=0;i<vectMonthlyArmorList.size();i++)
		{
			if(vectMonthlyArmorList[i] == pItemInfo->LinkItem)
			{
				bMonthlyArmor=TRUE;
				break;
			}
		...
		if(bMonthlyArmor)
		{
			nTotalProbabilityMonthlyArmor += pMysteryItemDropInfo->Probability;
			vectMyteryDropItemPtrListMonthlyArmor.push_back(pMysteryItemDropInfo);
		}
		else
		{
	...
	// 2013-10-18 by jekim, 이달의 아머 30%
#ifdef S_MONTHL_ARMOR_EVENT_JHSEOL_BCKIM		// 이달의 아머 30%
	if(!vectMyteryDropItemPtrListMonthlyArmor.empty() && RANDI(1,100) <= 30)
	{
		if(0 >= nTotalProbabilityMonthlyArmor)
		{
			return FALSE;
		}
		int nSumProbability	= 0;
		int nDropProValue	= RANDI(0, nTotalProbabilityMonthlyArmor-1);	// 2008-02-13 by cmkwon, 수정함
		int nCnt			= vectMyteryDropItemPtrListMonthlyArmor.size();
		for(int i=0; i < nCnt; i++)
		{
			if(nDropProValue < nSumProbability + vectMyteryDropItemPtrListMonthlyArmor[i]->Probability)
			{
				o_pMysteryDropItemList->push_back(vectMyteryDropItemPtrListMonthlyArmor[i]);
				return TRUE;
			}
			nSumProbability		+= vectMyteryDropItemPtrListMonthlyArmor[i]->Probability;
		}
		
		g_pFieldGlobal->WriteSystemLogEX(TRUE, "[ERROR] CFieldIOCP::GetMysteryDropItem_ error !!, vectMyteryDropItemPtrListMonthlyArmor MysteryItemDropNum(%8d) TotalProbability(%8d)\r\n"
		, i_nMysteryItemDropNum, nTotalProbabilityMonthlyArmor);
		return FALSE;
	}
#endif
	// end 2013-10-18 by jekim, 이달의 아머 30%
...
//	///////////////////////////////////////////////////////////////////////////////
//	// 2008-09-01 by cmkwon, 한국 Yedang_Kor 21시, 22시에 이벤트 아이템 지급 - 예당만 처리
//	//						매일 저녁
//	//							21 : 00 / 7016160 EPISODE4 완성형 문자 1개
//	//							22 : 00 / 7004540 고대의 얼음 1개
//#if defined(SERVICE_TYPE_KOREAN_SERVER_2) || defined(SERVICE_TYPE_KOREAN_SERVER_1)
//	switch(pDateTime->Hour)
//	{
//	case 21:
//		{
//			this->InsertItemAllCharacter(7016160, 1, INFLUENCE_TYPE_ALL_MASK);
//		}
//		break;
//	case 22:
//		{
//			this->InsertItemAllCharacter(7004540, 1, INFLUENCE_TYPE_ALL_MASK);
//		}
//		break;
//	}
//#endif
	...
	// 2008-12-19 by cmkwon, QT_UpdatePCBangList->DQT_UpdatePCBangList 변경 - Masang140 서버를 테스트로 60분에 한번씩 로딩되게 처리
	m_pAtumDBManager->MakeQueryWithNewThread(DQT_UpdatePCBangList, NULL, NULL);		// 2008-12-19 by cmkwon, QT_UpdatePCBangList->DQT_UpdatePCBangList 변경 - 
#endif
	...
	if(CFieldIOCPSocket::ms_bUsingNProtect
		&& 0 == pDateTime->Hour%3)
	{
		UINT32 uiRet = GGAuthUpdateTimer();
		if(ERROR_SUCCESS != uiRet)
		{
			g_pFieldGlobal->WriteSystemLogEX(TRUE, "\r\n[Error] CFieldIOCP::OnDoHourlyWorkIOCP# GGAuthUpdateTimer() error !! Ret(%ld) \r\n", uiRet);
		}
	}
#endif
...
void CFieldIOCP::OnDoMinutelyWorkIOCP(ATUM_DATE_TIME *pDateTime)
{
	...
	// 2008-08-25 by dhjin, 태국 PC방 IP정보 로딩
	if(LANGUAGE_TYPE_THAI == g_pFieldGlobal->GetLanguageType()
		&& m_nMaxPCBangMinTick <= m_nPCBangMinTick)
	{
		// 2008-12-19 by cmkwon, QT_UpdatePCBangList->DQT_UpdatePCBangList 변경 - 아래와 같이 수정
		//m_pAtumDBManager->MakeAndEnqueueQuery(QT_UpdatePCBangList, NULL, 0, NULL);
		m_pAtumDBManager->MakeQueryWithNewThread(DQT_UpdatePCBangList, NULL, NULL);		// 2008-12-19 by cmkwon, QT_UpdatePCBangList->DQT_UpdatePCBangList 변경 - 
...
//	m_cityWarManager.OnDoMinutelyWorkCityWarManager(pDateTime);
	...
	///////////////////////////////////////////////////////////////////////////////
	// 2006-04-14 by cmkwon
	m_InflWarManager.OnDoMinutelyWarkInflWarManager(pDateTime);
...
#if defined(SERVICE_TYPE_KOREAN_SERVER_2) || defined(SERVICE_TYPE_KOREAN_SERVER_1)
	// 2012-10-04 by hskim, 한국 자체 서비스 (가비아 IDC)

	#ifdef S_GIVE_EVENT_ITEM_AT_TIME_BCKIM				// 2013-09-30 by bckim, 특정시간 아이템지급
// 	if( (18 == pDateTime->Hour || 21 == pDateTime->Hour || 0 == pDateTime->Hour) && 0 == pDateTime->Minute)		// 아이템 지급 
// 	{
// 		this->InsertItemAllCharacter(7039590, 100, INFLUENCE_TYPE_ALL_MASK);		// S급수리키트
// 		this->InsertItemAllCharacter(7039600, 100, INFLUENCE_TYPE_ALL_MASK);		// S급쉴드키트
// 		this->InsertItemAllCharacter(7039610, 50, INFLUENCE_TYPE_ALL_MASK);			// C급SP회복키트
// 	}	// 우선 살려 놓고 운영팀에서 툴 사용이 익숙해지면 삭제하자
// 2013-12-16 by jhseol, 기획팀 요청 삭제
	#else
	// 오후 6시, 9시, 12시에 아이템 지급
	if( (18 == pDateTime->Hour || 21 == pDateTime->Hour || 0 == pDateTime->Hour) && 0 == pDateTime->Minute)		// 아이템 지급 
	{
		this->InsertItemAllCharacter(7039590, 100, INFLUENCE_TYPE_ALL_MASK);		// S급수리키트
		this->InsertItemAllCharacter(7039600, 100, INFLUENCE_TYPE_ALL_MASK);		// S급쉴드키트
	...
	// 정시에 해피 캡슐, PC방 유저만 지급
	if( 0 == pDateTime->Minute )
	{
		this->InsertItemAllCharacter(7038790, 1, INFLUENCE_TYPE_ALL_MASK, FALSE, TRUE);		// 해피 캡슐, PC방 유저만 1시간마다 지급
		//this->InsertItemAllCharacter(7037030, 1, INFLUENCE_TYPE_ALL_MASK, FALSE, TRUE);		// 해피 캡슐, PC방 유저만 1시간마다 지급 - 미스터리 주화 => 이벤트 끝 주석 처리 (hskim 2012-12-11)
	}
	// end 2012-10-04 by hskim, 한국 자체 서비스 (가비아 IDC)

	// start 2011-11-03 by shcho, yedang 셧다운제 구현 - 미성년자 체크 함수 
	this->FindShutdownUserAndStartShutDownRoutin(pDateTime);
	// end 2011-11-03 by shcho, yedang 셧다운제 구현 - 미성년자 체크 함수 
#endif

#if defined(SERVICE_TYPE_KOREAN_SERVER_1)
	// 2009-05-04 by cmkwon, 메모리풀 상태정보 로그 남기기 - 현재는 내부 서버만 체크한다.
	//CVMemPool::vmPoolSaveCurrentStatus();
#endif

	
#ifdef S_GIVE_EVENT_ITEM_AT_TIME_BCKIM				// 2013-09-30 by bckim, 특정시간 아이템지급
	vectItemEventInfo	EventItemList;
	if( NULL != m_pGiveItemManager)
	{
		m_pGiveItemManager->OnMinutecheckItemEventAtTime(&EventItemList);
	}

	int v_size = EventItemList.size();
	for (int id = 0; id < v_size; id++)
	{
		SITEM_EVENT_INFO *pItemEvInfo = &(EventItemList[id]);
		if( NULL != pItemEvInfo )
		{
			this->InsertItemAllCharacter(pItemEvInfo->ItemNum,pItemEvInfo->Count,pItemEvInfo->byInfluenceType);
		}	
	}	
#endif

	// 2014-01-03 by jhseol&bckim, 파괴경매장 - 경매 시작 및 종료
#ifdef SC_DESTROY_AUCTION_JHSEOL_BCKIM_SSJUNG	// - 경매 시작 및 종료
	if ( NULL != m_pDestroyAuctionManager && FALSE == g_pFieldGlobal->IsArenaServer() && TRUE == m_bDestroyAuctionContents )
	{
		///////////////////////////////////////////////
		// 경매 시작
		if ( FALSE == m_bDestroyAuctionInProgress )	// 경매가 종료상태 일때
		{
			///////////////////////////////////////////////
			// 경매 시작 준비 단계 : 등록할 파괴 아이템 시간과 경매의 시작과 종료 시간 셋팅
#if defined(TEST140)
			if ( 0 == (pDateTime->Minute) )		// 매시간 마다 
#else
			if ( 0 == (pDateTime->Hour%DESTROY_AUCTION_RUN_GAP_TIME_HOUR) )		// 3시간 마다 
#endif			
			{
//				if ( (*pDateTime) >= m_pDestroyAuctionManager->GetDestroyAuctionEndTime() )
				if ( 0 <= pDateTime->GetTimeDiffTimeInSeconds(m_pDestroyAuctionManager->GetDestroyAuctionEndTime()) )		// 처음 서버 올라갔을떄, 무조건 TRUE 아닌가 ?
				{
					m_pDestroyAuctionManager->SetItemDestroyTimeFilter();
					m_pDestroyAuctionManager->SetDestroyAuctionStartEndTime();
				}
			}

			///////////////////////////////////////////////
			// 경매 시작 실행
			int tempTimeGap = pDateTime->GetTimeDiffTimeInMinutes(m_pDestroyAuctionManager->GetDestroyAuctionStartTime());  // 임시 데이터 확인용 

			if ( *pDateTime <= m_pDestroyAuctionManager->GetDestroyAuctionStartTime())
//			if ( DESTROY_AUCTION_RUN_READY_TIME_MINUTE <= pDateTime->GetTimeDiffTimeInMinutes(m_pDestroyAuctionManager->GetDestroyAuctionStartTime()))	// 현재시간 - 입력시간 
			{
				CODBCStatement odbcStmt;
				...
				if ( FALSE == bRet )
				{
					g_pFieldGlobal->WriteSystemLogEX(TRUE, "[ERROR] CFieldIOCP::OnDoMinutelyWorkIOCP, DestroyAuction Start (CFieldIOCP::OnDoMinutelyWorkIOCP) !! %s,%d %s %s\r\n"
						, g_pGlobalGameServer->GetDBServerIP(), g_pGlobalGameServer->GetDBServerPort(), g_pGlobalGameServer->GetDBServerDatabaseName(), (CHAR*)g_pGlobalGameServer->GetODBCUID());
				}
				else
				{
					bRet = CAtumFieldDBManager::DBLoadDestroyAuctionList(&odbcStmt, m_pDestroyAuctionManager, m_pDestroyAuctionManager->GetItemDestroyTimeFilterBegin(), m_pDestroyAuctionManager->GetItemDestroyTimeFilterEnd());
					if (FALSE == bRet)
					{
						g_pFieldGlobal->WriteSystemLogEX(TRUE, "[ERROR] CFieldIOCP::OnDoMinutelyWorkIOCP, DBLoadDestroyAuctionList fail !! \r\n");
					}
					else
					{
						m_pDestroyAuctionManager->DestroyItemListUP();

						m_bDestroyAuctionInProgress = TRUE;		// 경매 시작함 진행상태로 변경
						
						// 2014-07-07 by bckim, 파괴경매가 시작되어도 등록된 물품이 없으면 시작 메세지를 보내지 않음. 
						DESTROY_AUCTION_LIST* temp = m_pDestroyAuctionManager->GetDestroyAuctionListPtr();
						if( NULL != temp && FALSE == temp->empty() )		// 경매물품이 있으면. 
						{
						MessageType_t msgTy = T_FC_DESTROY_AUCTION_NOTI_START;
							SendMessageToAllClients((BYTE*)&msgTy, SIZE_FIELD_TYPE_HEADER);
						}
						// End. 2014-07-07 by bckim, 파괴경매가 시작되어도 등록된 물품이 없으면 시작 메세지를 보내지 않음. 
						
						g_pFieldGlobal->WriteSystemLogEX(TRUE, "[NOTIFY] CFieldIOCP::OnDoMinutelyWorkIOCP, DestroyAuction Begin (%s ~ %s)\r\n"
							,m_pDestroyAuctionManager->GetDestroyAuctionStartTime().GetDateTimeString(STRNBUF(SIZE_MAX_SQL_DATETIME_STRING))
							,m_pDestroyAuctionManager->GetDestroyAuctionEndTime().GetDateTimeString(STRNBUF(SIZE_MAX_SQL_DATETIME_STRING)));

					// 2014-05-12 by bckim, 파괴경매장 운영툴  // 경매물품 리스트 업 운영툴테이블 insert 
					bRet = CAtumFieldDBManager::DBInsertDestroyAuctionListUp(&odbcStmt, m_pDestroyAuctionManager);
					if (FALSE == bRet)
					{
						g_pFieldGlobal->WriteSystemLogEX(TRUE, "[ERROR] CFieldIOCP::OnDoMinutelyWorkIOCP, DBLoadDestroyAuctionList fail !! \r\n");
					}
					// End. 2014-05-12 by bckim, 파괴경매장 운영툴 
					}
				}
			}
//			g_pFieldGlobal->WriteSystemLogEX(TRUE, "[ERROR] CFieldIOCP::OnDoMinutelyWorkIOCP, DestroyAuction Not Runing !! \r\n");
		}
		///////////////////////////////////////////////
		// 경매 종료
		else if ( TRUE == m_bDestroyAuctionInProgress )	// 경매가 진행 상태 일때
		{
			if ( (*pDateTime) >= m_pDestroyAuctionManager->GetDestroyAuctionEndTime() )
			{
				// 진행중인 경매 종료
				...
				if ( FALSE == bRet )
				{
					g_pFieldGlobal->WriteSystemLogEX(TRUE, "[ERROR] CFieldIOCP::OnDoMinutelyWorkIOCP, DestroyAuction End (CFieldIOCP::OnDoMinutelyWorkIOCP) !! %s,%d %s %s\r\n"
						, g_pGlobalGameServer->GetDBServerIP(), g_pGlobalGameServer->GetDBServerPort(), g_pGlobalGameServer->GetDBServerDatabaseName(), (CHAR*)g_pGlobalGameServer->GetODBCUID());
				}
				else
				{
					bRet = CAtumFieldDBManager::DBSetClearDestroyAuctionList(&odbcStmt, m_pDestroyAuctionManager, m_pDestroyAuctionManager->GetItemDestroyTimeFilterBegin(), m_pDestroyAuctionManager->GetItemDestroyTimeFilterEnd());
					if (FALSE == bRet)
					{
						g_pFieldGlobal->WriteSystemLogEX(TRUE, "[ERROR] CFieldIOCP::OnDoMinutelyWorkIOCP, DBSetClearDestroyAuctionList fail !! \r\n");
					}
					else
					{
						m_bDestroyAuctionInProgress = FALSE;	// 경매 끝남 종료상태로 변경
						MessageType_t msgTy = T_FC_DESTROY_AUCTION_NOTI_END;
						SendMessageToAllClients((BYTE*)&msgTy, SIZE_FIELD_TYPE_HEADER);

						g_pFieldGlobal->WriteSystemLogEX(TRUE, "[NOTIFY] CFieldIOCP::OnDoMinutelyWorkIOCP, DestroyAuction End (%s ~ %s)\r\n"
							,m_pDestroyAuctionManager->GetDestroyAuctionStartTime().GetDateTimeString(STRNBUF(SIZE_MAX_SQL_DATETIME_STRING))
							,m_pDestroyAuctionManager->GetDestroyAuctionEndTime().GetDateTimeString(STRNBUF(SIZE_MAX_SQL_DATETIME_STRING)));

						///////////////////////////////////////////////
						// 낙찰 로그 남기기
						DESTROY_AUCTION_LIST pDAuctionList;
						pDAuctionList.clear();
						m_pDestroyAuctionManager->CopyDestroyAuctionList(&pDAuctionList);
						DESTROY_AUCTION_ITR itr = pDAuctionList.begin();
						for ( ; itr != pDAuctionList.end() ; itr++ )
						{
							if ( 0 != (*itr)->CharacterUID )
							{
								INIT_MSG_WITH_BUFFER(MSG_FL_LOG_DESTROY_AUCTION_FIX, T_FL_LOG_DESTROY_AUCTION_FIX, pLogMsg, SendLogBuf);
								memcpy(&pLogMsg->DAuctionInfo, (*itr), sizeof(DESTROY_AUCTION_INFO));
								pLogMsg->DAuctionInfo.DAuctionState = AUCTION_ITEM_STATE_END;
								g_pFieldGlobal->SendLogFieldServer2LogServer(SendLogBuf, MSG_SIZE(MSG_FL_LOG_DESTROY_AUCTION_FIX));
							}
						}
					}
				}
			}
//			g_pFieldGlobal->WriteSystemLogEX(TRUE, "[ERROR] CFieldIOCP::OnDoMinutelyWorkIOCP, DestroyAuction Runing !! \r\n");
		}
	}
#endif	//#ifdef SC_DESTROY_AUCTION_JHSEOL_BCKIM_SSJUNG
	// end 2014-01-03 by jhseol&bckim, 파괴경매장 - 경매 시작 및 종료

// 2014-03-14 by jekim, 전투력 로그 추가
#ifdef S_COMBAT_POWER_LOGGING
	if((pDateTime->Minute%5) == 0 && !g_pFieldGlobal->IsArenaServer())
	{
		int totalANI=0,totalBCU=0;
#ifdef S_COMBAT_POWER_LOGGING_DEBUG
		g_pFieldGlobal->WriteSystemLogEX(TRUE, "[NOTIFY] CFieldIOCP::OnDoMinutelyWorkIOCP, CombatPower Log Start\r\n");
#endif
		for (int i = (CLIENT_INDEX_START_NUM); i < m_dwArrayClientSize; i++) // 2014-03-21 by jekim, 전투력 로그 추가 - 수정
		{
			CFieldIOCPSocket *pFISock = this->GetFieldIOCPSocket(i);
			if (pFISock
				&& pFISock->IsValidCharacter(FALSE, TRUE)		// 2010-04-22 by cmkwon, 아레나 플레이 상태도 유효한 캐릭터로 처리 - 게임가드 처리는 
				&& pFISock->IsCheckInfluenceTypeANDSocketType(INFLUENCE_TYPE_ALL_MASK, ST_CLIENT_TYPE, FALSE))
			{
				int power=pFISock->GetCombatPower();
				if(TRUE == COMPARE_INFLUENCE(pFISock->m_character.InfluenceType,INFLUENCE_TYPE_VCN) )
				{
#ifdef S_COMBAT_POWER_LOGGING_DEBUG
					g_pFieldGlobal->WriteSystemLogEX(TRUE, "[NOTIFY] CFieldIOCP::OnDoMinutelyWorkIOCP, CombatPower[BCU] username:%s power:%d\r\n",pFISock->m_character.CharacterName,power);
#endif
					totalBCU+=power;
				}
				else if(TRUE == COMPARE_INFLUENCE(pFISock->m_character.InfluenceType,INFLUENCE_TYPE_ANI) )
				{
#ifdef S_COMBAT_POWER_LOGGING_DEBUG
					g_pFieldGlobal->WriteSystemLogEX(TRUE, "[NOTIFY] CFieldIOCP::OnDoMinutelyWorkIOCP, CombatPower[ANI] username:%s power:%d\r\n",pFISock->m_character.CharacterName,power);
#endif
					totalANI+=power;
				}
			}
		}
		g_pFieldGlobal->WriteSystemLogEX(TRUE, "[NOTIFY] CFieldIOCP::OnDoMinutelyWorkIOCP, CombatPower Log End! (ANI:%d BCU:%d)\r\n",totalANI,totalBCU); // 2014-03-21 by jekim, 전투력 로그 추가 - 수정
		QPARAM_INSERT_COMBATPOWER_LOG *pQParam = new QPARAM_INSERT_COMBATPOWER_LOG;
		pQParam->time = *pDateTime;
		pQParam->totalANI = totalANI;
		pQParam->totalBCU = totalBCU;
		m_pAtumDBManager->MakeAndEnqueueQuery(QT_InsertCombatPowerLog, NULL, 0, pQParam);
	}
#endif
// end 2014-03-14 by jekim, 전투력 로그 추가
...
CFieldIOCPSocket* CFieldIOCP::GetFieldIOCPSocketByCharacterName(char* i_szCharacterName)
{
	if(NULL == i_szCharacterName
		|| 0 == strncmp("", i_szCharacterName, SIZE_MAX_CHARACTER_NAME))
	{
		return NULL;
	}
...
{
	INT iCheckSourceItemKind[COUNT_ITEM_FIX_MIXING_SOURCE] = { 0, };
	INT iCheckSourceReqUnitKind[COUNT_ITEM_FIX_MIXING_SOURCE] = { 0, };

	memset(iCheckSourceItemKind, -1, sizeof(iCheckSourceItemKind));
	memset(iCheckSourceReqUnitKind, -1, sizeof(iCheckSourceReqUnitKind));

	// 현재 고정 조합식은 2개를 가지고 있음
	// 그리고 고정 조합식은 한번에 하나씩 만 지원

	if( COUNT_ITEM_FIX_MIXING_SOURCE != i_nItemWithCount || 1 != i_nMixCnts || NULL == i_pItemMixingInfo )
	{
		return NULL;
	}
	...
	for(int i=0; i < i_nItemWithCount; i++)
	{
		...
		ITEM *pItemInfo	= GetItemInfo(tmpSItemWCountCheck.ItemNum);

		if( NULL == pItemInfo || 1 != tmpSItemWCountCheck.Count )
		{
			return NULL;
		}
		...
		// 2013-05-31 by jhseol,bckim 아머 컬렉션 - 컬렉션용 외변킷은 조합이 되면 안된다.
#ifdef SC_COLLECTION_ARMOR_JHSEOL_BCKIM	// - 컬렉션용 외변킷은 조합이 되면 안된다.
		if ( TRUE == pItemInfo->IsExistDesParam(DES_COLLECTION_ARMOR_INDEX) )
		{
			return NULL;
		}
#endif
		// end 2013-05-31 by jhseol,bckim 아머 컬렉션 - 컬렉션용 외변킷은 조합이 되면 안된다.

		if( pItemInfo->IsExistDesParam(DES_SHAPE_ITEM) && pItemInfo->IsExistDesParam(DES_EFFECT_ITEM) )
		{
			float fParameterValue = pItemInfo->GetParameterValue(DES_SHAPE_ITEM);
			float fParameterValue2 = pItemInfo->GetParameterValue(DES_EFFECT_ITEM);
			
			if( fParameterValue == fParameterValue2 )
			{
				iCheckSourceItemKind[INDEX_ITEM_FIX_MIXING_EFFECT_CARD] = (int)fParameterValue;
		...
		{
			iCheckSourceItemKind[INDEX_ITEM_FIX_MIXING_EFFECT_CARD] = (int)pItemInfo->GetParameterValue(DES_SHAPE_ITEM);
			iCheckSourceReqUnitKind[INDEX_ITEM_FIX_MIXING_EFFECT_CARD] = pItemInfo->ReqUnitKind;
			i_pItemMixingInfo->TargetItemNum = pItemInfo->ItemNum;
		...
		{
			iCheckSourceItemKind[INDEX_ITEM_FIX_MIXING_EFFECT_CARD] = (int)pItemInfo->GetParameterValue(DES_EFFECT_ITEM);
			iCheckSourceReqUnitKind[INDEX_ITEM_FIX_MIXING_EFFECT_CARD] = pItemInfo->ReqUnitKind;
			i_pItemMixingInfo->TargetItemNum = pItemInfo->ItemNum;
		...
		{
			iCheckSourceItemKind[INDEX_ITEM_FIX_MIXING_EFFECT_CARD] = (int)pItemInfo->GetParameterValue(DES_INIT_EFFECT_ITEM);
			iCheckSourceReqUnitKind[INDEX_ITEM_FIX_MIXING_EFFECT_CARD] = pItemInfo->ReqUnitKind;
			i_pItemMixingInfo->TargetItemNum = pItemInfo->ItemNum;
		...
		{
			iCheckSourceItemKind[INDEX_ITEM_FIX_MIXING_EFFECT_CARD] = (int)pItemInfo->GetParameterValue(DES_INIT_SHAPE_ITEM);
			iCheckSourceReqUnitKind[INDEX_ITEM_FIX_MIXING_EFFECT_CARD] = pItemInfo->ReqUnitKind;
			i_pItemMixingInfo->TargetItemNum = pItemInfo->ItemNum;
	...
	if( TRUE != IS_GENERAL_ITEM(iCheckSourceItemKind[INDEX_ITEM_FIX_MIXING_EQUIPMENT]) || TRUE != IS_GENERAL_ITEM(iCheckSourceItemKind[INDEX_ITEM_FIX_MIXING_EFFECT_CARD]) )
	{
		return NULL;
	}
	...
	if( IS_PRIMARY_WEAPON(iCheckSourceItemKind[INDEX_ITEM_FIX_MIXING_EQUIPMENT])		// 무기 외형 - 기본 무기 A 기어
		...
		&& KIND_ITEM_FIX_MIXING_SHAPE_PRIMARY_WEAPON_A == iCheckSourceItemKind[INDEX_ITEM_FIX_MIXING_EFFECT_CARD] )
	{
	...
	if( IS_PRIMARY_WEAPON(iCheckSourceItemKind[INDEX_ITEM_FIX_MIXING_EQUIPMENT])		// 무기 외형 - 기본 무기 BMI 기어
		...
		&& KIND_ITEM_FIX_MIXING_SHAPE_PRIMARY_WEAPON_BMI == iCheckSourceItemKind[INDEX_ITEM_FIX_MIXING_EFFECT_CARD] )
	{
		return i_pItemMixingInfo;
	}
	...
	if( IS_SECONDARY_WEAPON(iCheckSourceItemKind[INDEX_ITEM_FIX_MIXING_EQUIPMENT])		// 무기 외형 - 고급 무기 A 기어
		&& IS_AGEAR(iCheckSourceReqUnitKind[INDEX_ITEM_FIX_MIXING_EQUIPMENT])
		&& KIND_ITEM_FIX_MIXING_SHAPE_SECOND_WEAPON_A == iCheckSourceItemKind[INDEX_ITEM_FIX_MIXING_EFFECT_CARD] )
	{
		return i_pItemMixingInfo;
	}
	...
	if( IS_SECONDARY_WEAPON(iCheckSourceItemKind[INDEX_ITEM_FIX_MIXING_EQUIPMENT])		// 무기 외형 - 고급 무기 BMI 기어
		&& (IS_BGEAR(iCheckSourceReqUnitKind[INDEX_ITEM_FIX_MIXING_EQUIPMENT]) || IS_MGEAR(iCheckSourceReqUnitKind[INDEX_ITEM_FIX_MIXING_EQUIPMENT]) || IS_IGEAR(iCheckSourceReqUnitKind[INDEX_ITEM_FIX_MIXING_EQUIPMENT]))
		&& KIND_ITEM_FIX_MIXING_SHAPE_SECOND_WEAPON_BMI == iCheckSourceItemKind[INDEX_ITEM_FIX_MIXING_EFFECT_CARD] )
	{
		return i_pItemMixingInfo;
	}

	if( IS_SECONDARY_WEAPON(iCheckSourceItemKind[INDEX_ITEM_FIX_MIXING_EQUIPMENT])		// 무기 외형 - 고급 무기 이펙트
		&& KIND_ITEM_FIX_MIXING_EFFECT_SECOND_WEAPON == iCheckSourceItemKind[INDEX_ITEM_FIX_MIXING_EFFECT_CARD] )
	{
		return i_pItemMixingInfo;
	}
	...
	if( ITEMKIND_DEFENSE == iCheckSourceItemKind[INDEX_ITEM_FIX_MIXING_EQUIPMENT]		// 아머 외형 - B 기어
		...
		&& KIND_ITEM_FIX_MIXING_SHAPE_DEFENSE_B == iCheckSourceItemKind[INDEX_ITEM_FIX_MIXING_EFFECT_CARD] )
	{
		return i_pItemMixingInfo;
	}
	...
	if( ITEMKIND_DEFENSE == iCheckSourceItemKind[INDEX_ITEM_FIX_MIXING_EQUIPMENT]		// 아머 외형 - M 기어
		...
		&& KIND_ITEM_FIX_MIXING_SHAPE_DEFENSE_M == iCheckSourceItemKind[INDEX_ITEM_FIX_MIXING_EFFECT_CARD] )
	{
		return i_pItemMixingInfo;
	}
	...
	if( ITEMKIND_DEFENSE == iCheckSourceItemKind[INDEX_ITEM_FIX_MIXING_EQUIPMENT]		// 아머 외형 - A 기어
		&& IS_AGEAR(iCheckSourceReqUnitKind[INDEX_ITEM_FIX_MIXING_EQUIPMENT])
		&& KIND_ITEM_FIX_MIXING_SHAPE_DEFENSE_A == iCheckSourceItemKind[INDEX_ITEM_FIX_MIXING_EFFECT_CARD] )
	{
		return i_pItemMixingInfo;
	}
	...
	if( ITEMKIND_DEFENSE == iCheckSourceItemKind[INDEX_ITEM_FIX_MIXING_EQUIPMENT]		// 아머 외형 - I 기어
		...
		&& KIND_ITEM_FIX_MIXING_SHAPE_DEFENSE_I == iCheckSourceItemKind[INDEX_ITEM_FIX_MIXING_EFFECT_CARD] )
	{
		return i_pItemMixingInfo;
	}

	if( ITEMKIND_DEFENSE == iCheckSourceItemKind[INDEX_ITEM_FIX_MIXING_EQUIPMENT]		// 아머 외형 - 초기화
		&& KIND_ITEM_FIX_MIXING_SHAPE_DEFENSE_INIT == iCheckSourceItemKind[INDEX_ITEM_FIX_MIXING_EFFECT_CARD] )
	{
		return i_pItemMixingInfo;
	}

	if( IS_SECONDARY_WEAPON(iCheckSourceItemKind[INDEX_ITEM_FIX_MIXING_EQUIPMENT]) 		// 이펙트 외형 - 고급 무기 초기화
		&& KIND_ITEM_FIX_MIXING_SHAPE_SECOND_EFFECT_INIT == iCheckSourceItemKind[INDEX_ITEM_FIX_MIXING_EFFECT_CARD] )
	{
		return i_pItemMixingInfo;
	}		
	...
	if( ITEMKIND_PET_ITEM == iCheckSourceItemKind[INDEX_ITEM_FIX_MIXING_EQUIPMENT] 		// 파트너 외형
		&& KIND_ITEM_FIX_MIXING_SHAPE_PET_ITEM == iCheckSourceItemKind[INDEX_ITEM_FIX_MIXING_EFFECT_CARD] )
	{
		return i_pItemMixingInfo;
	}
#endif
...
{	
	...
	STRNCPY_MEMSET(pS2CStartWar->szOccupyGuildName3, i_pCityWar->m_OccupyCITY_WAR_DATAWhenWarStarted.GuildName1, SIZE_MAX_GUILD_NAME);
...
void CFieldIOCP::OnCityWarEndFieldIOCP(CCityWar *i_pCityWar, vectCITYWAR_QUEST *i_pCityWarQuestResultList)
{
	...
	STRNCPY_MEMSET(pS2CEndWar->szOccupyGuildName3, i_pCityWar->m_OccupyCITY_WAR_DATAWhenWarStarted.GuildName1, SIZE_MAX_GUILD_NAME);
...
void CFieldIOCP::OnCityWarQuestMonsterDead(CCityWar *i_pCityWar)
{
	...
	STRNCPY_MEMSET(pSMonDead->szOccupyGuildName3, i_pCityWar->m_OccupyCITY_WAR_DATAWhenWarStarted.GuildName1, SIZE_MAX_GUILD_NAME);
...
BOOL CFieldIOCP::GetP2PPKFamePoint(const char *szAttackCharName, const char *szDeadCharName)
{
	...
	MEMSET_ZERO(szTemp, SIZE_MAX_CHARACTER_NAME*2+1);
...
void CFieldIOCP::AddInfluenceTypeCount(BYTE i_byInflType)
{
	...
	g_pGlobal->WriteSystemLogEX(TRUE, "[TEMP] 090331 CFieldIOCP::AddInfluenceTypeCount# %s Normal(%d) BCU(%d) ANI(%d) \r\n", CAtumSJ::GetInfluenceTypeString(i_byInflType), m_nNormalInfluenceCount, m_nVCNInfluenceCount, m_nANIInfluenceCount);
#endif
}
...
ITEM * CFieldIOCP::GetKillMarkItemPtr(BYTE i_byCharInflTy, int i_byCharLevel)
{
	...
	for(int i=0; i < m_mtvectItemPtrKillMarks.size(); i++)
	{
		...
		if(COMPARE_INFLUENCE(pITEM->ReqRace, i_byCharInflTy)
			&& IS_IN_RANGE(pITEM->ReqMinLevel, i_byCharLevel, pITEM->ReqMaxLevel))
		{
			return pITEM;
		}
	}
...
void CFieldIOCP::SendSignBoardStringList(CFieldIOCPSocket *i_pFISocket, int i_nReqStringIndex/*=-1*/)
{
	...
	MSG_FC_WAR_SIGN_BOARD_INSERT_STRING *pInsertString;
	int	 nSendBytes = 0;
	...
	for(int i=0; i < m_mtvectSignBoardStringList.size(); i++)
	{
		if(SIZE_MAX_PACKET <= nSendBytes + sizeof(MSG_FC_WAR_SIGN_BOARD_INSERT_STRING))
		{
			i_pFISocket->SendAddData(SendBuf, nSendBytes);
			INIT_MSG(MSG_FC_WAR_REQ_SIGN_BOARD_STRING_LIST_OK, T_FC_WAR_REQ_SIGN_BOARD_STRING_LIST_OK, pSStringList, SendBuf);
			nSendBytes		= MSG_SIZE(MSG_FC_WAR_REQ_SIGN_BOARD_STRING_LIST_OK);
			pInsertString	= (MSG_FC_WAR_SIGN_BOARD_INSERT_STRING*)(SendBuf+nSendBytes);
		}
			...
			&& i_nReqStringIndex < m_mtvectSignBoardStringList[i].StringIndex)
		{
			...
			pInsertString->SignBoardExprieATime	= m_mtvectSignBoardStringList[i].SignBoardExprieATime;
			STRNCPY_MEMSET(pInsertString->SingBoardString, m_mtvectSignBoardStringList[i].SingBoardString, SIZE_MAX_SIGN_BOARD_STRING);			
			pSStringList->nStringCount++;
			nSendBytes		+= sizeof(MSG_FC_WAR_SIGN_BOARD_INSERT_STRING);
			pInsertString	= (MSG_FC_WAR_SIGN_BOARD_INSERT_STRING*)(SendBuf+nSendBytes);
		}
	}
	if(0 < pSStringList->nStringCount)
	{
		i_pFISocket->SendAddData(SendBuf, nSendBytes);
	}
}
...
// 2008-04-03 by cmkwon, 핵쉴드 서버 연동 시스템 수정 - 사용하지 않음
// ///////////////////////////////////////////////////////////////////////////////
// /// \fn			BOOL CFieldIOCP::SecurityInit(void)
// /// \brief		
// /// \author		cmkwon
...
// /// \return		
// ///////////////////////////////////////////////////////////////////////////////
// BOOL CFieldIOCP::SecurityInit(void)
// {
...
// 			wsprintf(szFullDir, "%s\\%s", szCurDir, SECURITY_HACKSHIELD_DIRECTORY_PATH);
// 			DWORD dwRet = _AntiCpSvr_Initialize(SECURITY_HACKSHIELD_DIRECTORY_PATH);
// 			if(dwRet != ERROR_SUCCESS)
// 			{
// 				char szSys[1024];
// 				wsprintf(szSys, "[Error] CFieldIOCP::SecurityInit Error(%d)!!\r\n", dwRet);
// 				g_pFieldGlobal->WriteSystemLog(szSys);
// 				DbgOut(szSys);
// 				return FALSE;
// 			}
// 			m_bInitHackShield		= TRUE;
// 		}
// 		break;
// 	}
// 
...
// 
// ///////////////////////////////////////////////////////////////////////////////
// /// \fn			void CFieldIOCP::SecurityClean(void)
// /// \brief		
// /// \author		cmkwon
// /// \date		2006-06-05 ~ 2006-06-05
// /// \warning	
...
// /// \return		
// ///////////////////////////////////////////////////////////////////////////////
// void CFieldIOCP::SecurityClean(void)
// {
// 	switch(g_pFieldGlobal->GetLanguageType())
// 	{
// 	case LANGUAGE_TYPE_KOREAN:
...
// 	case LANGUAGE_TYPE_CHINESE:				// 2007-11-23 by cmkwon, 중국 핵쉴드 버그 수정
// 		{
// 			if(m_bInitHackShield)
// 			{// 2006-06-29 by cmkwon
// 				m_bInitHackShield	= FALSE;
// 				_AntiCpSvr_Finalize();
// 			}
// 		}
// 		break;
...
int CFieldIOCP::GetPCBangUserCount()
{
	...
	for (int i = (CLIENT_INDEX_START_NUM); i < m_dwArrayClientSize; i++)
	{
		...
		if (pSock->IsValidCharacter(FALSE))
		{
			if (TRUE == pSock->m_bIsPCBangClient)
			{
				nPCBangUserCount++;
			}
		}
...
BOOL CFieldIOCP::LoadResObjCheckList(BOOL i_bReloadOnlyRexTexDirectory/*=FALSE*/)
{
	...
	///////////////////////////////////////////////////////////////////////////////
	// 2009-10-06 by cmkwon, 베트남 게임 가드 X-TRAP으로 변경 - CFieldIOCP::LoadResObjCheckList#, 리로드 map 파일
	char szPrefixFilename[MAX_PATH];
	STRNCPY_MEMSET(szPrefixFilename, CLIENT_EXEUTE_FILE_NAME, min(MAX_PATH, strlen(CLIENT_EXEUTE_FILE_NAME)-3));	
	if(FALSE == CFieldIOCPSocket::XTrapLoadMapFile(SECURITY_XTRAP_DIRECTORY_PATH, szPrefixFilename))
	{
		char szCurDir[MAX_PATH];
		GetCurrentDirectory(MAX_PATH, szCurDir);
		g_pFieldGlobal->WriteSystemLogEX(TRUE, "\r\n[Error] CFieldIOCP::LoadResObjCheckList# load map error !! Dir(%s), PrefixFileName(%s), CurDir(%s)\r\n", SECURITY_XTRAP_DIRECTORY_PATH, szPrefixFilename, szCurDir);
		return FALSE;
	}
#endif
...
SRESOBJ_CHECKSUM * CFieldIOCP::GetResObjCheckSum(char *i_szFileName)
{
	...
	STRNCPY_MEMSET(szTmResObjFileName, i_szFileName, SIZE_MAX_RESOBJ_FILE_NAME);
...
{
	// 2012-11-21 by hskim, PrepareShutdown 기능 삭제
	g_pFieldGlobal->WriteSystemLogEX(TRUE, "[Notify] CFieldIOCP::SetPrepareShutdown(%d)\r\n", i_bPrepareShutdown);

#ifdef S_DISABLE_PREPARE_SHUTDOWN
	return ;
#endif //S_DISABLE_PREPARE_SHUTDOWN
	// end 2012-11-21 by hskim, PrepareShutdown 기능 삭제
...
// 2013-05-09 by hskim, 세력 포인트 개선
	...
	// 2013-08-14 by jhseol, 거점전 리뉴얼 - 세력포인트 비율 변경
	BYTE nMSLevel = 1;
#ifdef SC_STRATEGYPOINTWAR_RENEWAL_JHSEOL_SSJUNG
	nMSLevel = 5;		// 버그수정 해야함 우선 5랩으로 고정
#endif
	m_InflWarManager.CalcContributionPoint(i_byInfluence, CONTRIBUTIONPOINT_BOSS_MONSTER_WINNER*nMSLevel, CONTRIBUTIONPOINT_BOSS_MONSTER_LOSER*nMSLevel);
	// end 2013-08-14 by jhseol, 거점전 리뉴얼 - 세력포인트 비율 변경
...
int CFieldIOCP::GetRandomRareCode(ITEM *i_pTargetItemInfo, BOOL i_bIsPrefix, int i_nReqUseTypeMask, CFieldIOCPSocket *i_pFISoc/*=NULL*/, RARE_ITEM_INFO *i_pLastRareItemInfo/*=NULL*/)
{
	...
	for (i = 0; i < pRareFixVector->size(); i++)
	{
		...
		if (IS_IN_RANGE(nCurSumProb, prob, nCurSumProb+pRareItemInfo->Probability))
...
{
	mt_auto_lock mtArenaServerInfo(&m_mtvectSARENA_USE_FIELD_SERVER_INFO);
	mtvectSARENA_USE_FIELD_SERVER_INFO::iterator itr = m_mtvectSARENA_USE_FIELD_SERVER_INFO.begin();
	for(; itr != m_mtvectSARENA_USE_FIELD_SERVER_INFO.end(); itr++)
	{
		if(i_pFieldServerInfo->MFS_ID == itr->MFS_ID
			&& i_pFieldServerInfo->MFS_Port == itr->MFS_Port
//			&& 0 == strcmp(i_pFieldServerInfo->MFS_IP, itr->MFS_IP)		// 2013-06-12 by hskim, 하나의 OS 에서 서버 동시 실행
			&& 0 == strcmp(i_pFieldServerInfo->MFS_Name, itr->MFS_Name)
			)
		{
			itr = m_mtvectSARENA_USE_FIELD_SERVER_INFO.erase(itr);
			m_mtvectSARENA_USE_FIELD_SERVER_INFO.push_back(*i_pFieldServerInfo);
			return TRUE;
		}
...
/// \return		
///////////////////////////////////////////////////////////////////////////////
CIOCPSocket * CFieldIOCP::GetMFSSockForArenaServer(USHORT i_MFSID)
{
	mt_auto_lock mtArenaServerInfo(&m_mtvectSARENA_USE_FIELD_SERVER_INFO);
	mtvectSARENA_USE_FIELD_SERVER_INFO::iterator itr = m_mtvectSARENA_USE_FIELD_SERVER_INFO.begin();
	for(; itr != m_mtvectSARENA_USE_FIELD_SERVER_INFO.end(); itr++)
	{
		if(i_MFSID == itr->MFS_ID)
		{
			return itr->Socket;
		}
	}
...
/// \return		
///////////////////////////////////////////////////////////////////////////////
void CFieldIOCP::SendArenaServerPay(USHORT i_MFSID, BYTE i_byInfluence, INT i_nPayInfluencePoint)
{
	CIOCPSocket * MainFieldServer = this->GetMFSSockForArenaServer(i_MFSID);
	if(NULL == MainFieldServer)
	{
		return;
	}

	INIT_MSG_WITH_BUFFER(MSG_FtoA_ARENA_SERVER_PAY, T_FtoA_ARENA_SERVER_PAY, pSendMsg, SendBuf);
	pSendMsg->Influence					= i_byInfluence;
	pSendMsg->PayInfluencePoint			= i_nPayInfluencePoint;
	MainFieldServer->SendAddData(SendBuf, MSG_SIZE(MSG_FtoA_ARENA_SERVER_PAY));
...
void CFieldIOCP::SendAllLetterByInfluence(BYTE i_byInfluence, SLETTER_INFO * i_pAllLetterInfo)
{
	...
	for (int i = (CLIENT_INDEX_START_NUM); i < m_dwArrayClientSize; i++)
	{
			...
			&& pSock->IsCheckInfluenceTypeANDSocketType(i_byInfluence, ST_CLIENT_TYPE))
		{
			...
			STRNCPY_MEMSET(pSMsg->SendCharacterName, i_pAllLetterInfo->SendCharacterName, SIZE_MAX_CHARACTER_NAME);
...
// 			return FALSE;
// 		}
// 		if(IS_IN_RANGE(49, nInflPercent, 51))
// 		{
...
/// \return		
///////////////////////////////////////////////////////////////////////////////
BOOL CFieldIOCP::INNBILL_Init(void)
{
	if(m_pInnovaLibrary)
	{
		this->INNBILL_UnInit();
...
/// \return		
///////////////////////////////////////////////////////////////////////////////
BOOL CFieldIOCP::INNBILL_UnInit(void)
{
...
/// \return		
///////////////////////////////////////////////////////////////////////////////
BOOL CFieldIOCP::INNBILL_GetMoney(char *i_szAccName, int *o_pnTotalMoney)
{
	if(NULL == m_pInnovaLibrary){					return FALSE;}

#if defined(SERVICE_TYPE_RUSSIAN_SERVER_1)
	Err_t errCode = m_pInnovaLibrary->INNBILL_check_user_status_a(i_szAccName, o_pnTotalMoney);
	if(ERR_NO_ERROR != errCode)
	{
		g_pGlobalGameServer->WriteSystemLogEX(TRUE, "[ERROR] innBill CFieldIOCP::INNBILL_GetMoney# AccName(%s) Err(%d:%s) \r\n"
			, i_szAccName, errCode, GetErrorString(errCode));
		return FALSE;
	}
	return TRUE;
#endif
...
/// \return		
///////////////////////////////////////////////////////////////////////////////
BOOL CFieldIOCP::INNBILL_BuyItem(char *i_szAccName, int *o_pnTotalMoney, ITEM *i_pBuyItemInfo, char *i_szCharName, uint_64* o_pbillTranID)
{
	if(NULL == m_pInnovaLibrary){					return FALSE;}

#if defined(SERVICE_TYPE_RUSSIAN_SERVER_1)
	Err_t errCode = m_pInnovaLibrary->INNBILL_buy_item_aia(i_szAccName, o_pnTotalMoney, i_pBuyItemInfo, g_pFieldGlobal->GetServerGroupName(), i_szCharName, o_pbillTranID);
	if(ERR_NO_ERROR != errCode)
	{
		g_pGlobalGameServer->WriteSystemLogEX(TRUE, "[ERROR] innBill CFieldIOCP::INNBILL_BuyItem# AccName(%s) Err(%d:%s) \r\n"
			, i_szAccName, errCode, GetErrorString(errCode));
		return FALSE;
	}
...
void CFieldIOCP::WarContributionCalculation(CFieldIOCPSocket * i_pFISoc, UID32_t i_nValue)
{
	...
	if(m_InflWarManager.CheckSummonBoss())
	{
		...
		if(IS_VALID_UNIQUE_NUMBER(i_pFISoc->m_character.GuildUniqueNumber))
		{
			...
			MEMSET_ZERO(&tmWarContributionCalculationGuild, sizeof(SWarContributionCalculationGuild));
			tmWarContributionCalculationGuild.GuildUID	= i_pFISoc->m_character.GuildUniqueNumber;
			STRNCPY_MEMSET(tmWarContributionCalculationGuild.GuildName, i_pFISoc->m_character.GuildName, SIZE_MAX_GUILD_NAME);
...
void CFieldIOCP::PayCBossWarContributionCalculation(MapIndex_t i_nMapIndex, BYTE i_byWinInfl)
{
		...
		; itr != m_mtvectCBossWarContributionCalculation.end(); itr++)
	{
			...
			&& PAY_MINIMUN_COUNT <= (*itr)->GetSizeCharacterUIDContribution())
		{
			...
			MEMSET_ZERO(&tmContributionGaer, sizeof(MSG_FL_LOG_WAR_CONTRIBUTION_GEAR));
...
void CFieldIOCP::SendBossGLogEventParticipationRate(MapIndex_t i_nMapIndex, INT i_nCnt, ATUM_DATE_TIME * i_pWarStartTime)
{
	...
	MEMSET_ZERO(&EventMsg, sizeof(MSG_FL_LOG_EVENT_PARTICIPATION_RATE));
	STRNCPY_MEMSET(EventMsg.Description, szDes, SIZE_MAX_GLOG_EVENT_DESCRIPTION);
...
void CFieldIOCP::PaySendBossWarContributionGuild(CBossWarContributionCalculation * i_pBossWarContribution, BYTE i_byWinInfl)
{
		...
		; itrG != tmVectWarContributionCalculationGuild->end(); itrG++)
	{
		...
		if(1 == GuildCount)
		{
			...
			STRNCPY_MEMSET(pMsg->ContributionGuldInfo[0].GuildName, itrG->GuildName, SIZE_MAX_GUILD_NAME);
			pMsg->ContributionGuldInfo[0].GuildUID	= itrG->GuildUID;
		...
		else if(2 == GuildCount)
		{
			...
			STRNCPY_MEMSET(pMsg->ContributionGuldInfo[1].GuildName, itrG->GuildName, SIZE_MAX_GUILD_NAME);
			pMsg->ContributionGuldInfo[1].GuildUID	= itrG->GuildUID;
		...
		else if(3 == GuildCount)
		{
			...
			STRNCPY_MEMSET(pMsg->ContributionGuldInfo[2].GuildName, itrG->GuildName, SIZE_MAX_GUILD_NAME);
			pMsg->ContributionGuldInfo[2].GuildUID	= itrG->GuildUID;
...
#if defined(SERVICE_TYPE_KOREAN_SERVER_1)
			///////////////////////////////////////////////////////////////////////////////
			// 2009-07-09 by cmkwon, 마상 내부만 로그 남김
			int nA = ( (itrC->second.Contribution * i_nDefinePayWarType * i_pWarContribution->GetInflUserCount(PayFISoc->m_character.InfluenceType) ) / TotalContribution );
			int nB = ( ((itrC->second.OtherInflStayTime + itrC->second.DeathCount) * i_nDefinePayWarType) / (TotalWarTime + i_pWarContribution->GetInflDeathCount(PayFISoc->m_character.InfluenceType)) );
			g_pFieldGlobal->WriteSystemLogEX(TRUE, "[TEMP] 090708 %s SelfInfl(%s) WinInfl(%s) %I64d = A(%d) + B(%d), A= %I64d * %d * %d / %I64d, B= ((%d+%d)*%d)/(%d+%d) \r\n"
				, GetCharacterString(PayFISoc->GetCharacter(), string()), CAtumSJ::GetInfluenceTypeString(PayFISoc->GetCharacter()->InfluenceType)
				, CAtumSJ::GetInfluenceTypeString(PayFISoc->m_character.InfluenceType), tmPay, nA, nB
				, itrC->second.Contribution, i_nDefinePayWarType, i_pWarContribution->GetInflUserCount(PayFISoc->m_character.InfluenceType), TotalContribution
				, itrC->second.OtherInflStayTime, itrC->second.DeathCount, i_nDefinePayWarType, TotalWarTime, i_pWarContribution->GetInflDeathCount(PayFISoc->m_character.InfluenceType));
#endif
...
// 			if(0 == tmPay)
// 			{
// 				if(PAY_TYPE_OUTPOST == i_byWarType)
// 				{
// 					tmPay = PAY_MINIMUN_BY_OUTPOST;
// 				}
// 				else if(PAY_TYPE_STRATEGYPOINT == i_byWarType)
// 				{
// 					tmPay = PAY_MINIMUN_BY_STRATEGYPOINT;
// 				}
// 			}
// 			else if(MAX_ITEM_COUNTS <= tmPay)
// 			{
...
// 2014-03-12 by bckim, 버닝타임(전쟁지역)
BOOL CFieldIOCP::IsBurningMap(MapIndex_t i_nMapIdx)
{
	BurningMapInfoList::iterator itr(m_BurningMapList.begin());
	for(; itr != m_BurningMapList.end(); itr++)
	{	
...
// End. 2014-03-12 by bckim, 버닝타임(전쟁지역)
...
BYTE CFieldIOCP::ReversalMapWarpPossible(int i_nMapindex)
{
	...
	{
		mt_auto_lock mta(&m_lockRandManager);		// 이미 있는 Lock를 활용하자~
...
//			g_pFieldGlobal->WriteSystemLogEX(TRUE, "[S_ITEM_EVENT_SUB_TYPE_JHSEOL][Notify] ItemEventSubTypeCheck Step.End_3 : AUID(%8d) CUID(%8d) EventInfo(%3d|%d)\r\n",i_nAUID, i_nCUID, i_SITEM_EVENT_INFO.ItemEventUID, i_SITEM_EVENT_INFO.ItemEventType);
			return FALSE; // 틱에 추가가 됬든 안됬든 상관없이 플레이타임 이벤트 타입이 걸리면 아직 지급해 주면 안됨으로 무조건 FALSE 리턴
		}
...
// 2014-06-17 by bckim, 세력 불균형 ( 세력정보 및 우세세력 표시 )	
BOOL CFieldIOCP::SendInfluenceShowInfo(BYTE i_byInfluenceMask)
{
	CFieldIOCPSocket *pSock = NULL;
...
BOOL CFieldIOCP::InsertBuffSkillAllCharacter(BYTE i_byInfluenceMask, INT nItemNum)
{
	...
	for (int i = (CLIENT_INDEX_START_NUM); i < GetArrayClientSize(); i++)
	{
			...
			&& pSock->IsCheckInfluenceTypeANDSocketType(i_byInfluenceMask, ST_CLIENT_TYPE) )
		{
			...
			if (TURN_AROUND_BUFF_SKILL_1ST == nItemNum || TURN_AROUND_BUFF_SKILL_2ND == nItemNum || TURN_AROUND_BUFF_SKILL_3RD == nItemNum)
			{
				...
				// 2014-03-25 by jekim, 승리의 환호
				if(FALSE == m_InflWarManager.GetPVPBuffPercent(i_byInfluenceMask,&pInflPoint->PVPBuffPercent))
				{
					pInflPoint->PVPBuffPercent=0.0f;
				}
				// end 2014-03-25 by jekim, 승리의 환호
...
void CFieldIOCP::LoadItemSecurityMaximunUID()
{
	...
	MEMSET_ZERO(pTempSystem, sizeof(QPARAM_TEMPORARY_SYSTEM_INFOMATION));
...
// 2013-08-14 by jhseol, 거점전 리뉴얼 - CFieldIOCP MonsterGradeList 관련 함수 추가
INT CFieldIOCP::GetMonsterGeade_SummonMonsterNumByLevel(BYTE i_Type, BYTE i_InflType, BYTE i_Level)
{
	// 타입과 세력과 level로 소환해야 하는 몬스터 번호를 가져오는 함수
	MONSTER_GRADE_LIST::iterator itr = m_MonsterGradeList.begin();
	for( ; itr != m_MonsterGradeList.end() ; itr++ )
	{
		if (itr->MonsterGeadeType == i_Type && itr->InflType == i_InflType && itr->Level == i_Level)
		{
			return itr->MonsterNum;
		}
	}
	return 0;
}

BYTE CFieldIOCP::GetMonsterGeade_SummonMonsterLevelBySummonMonsterNum(BYTE i_Type, BYTE i_InflType, INT i_SummonMosterNum)
{
	// 타입과 세력과 몬스터 번호로 소환해야 하는 몬스터 level을 가져오는 함수
	MONSTER_GRADE_LIST::iterator itr = m_MonsterGradeList.begin();
	for( ; itr != m_MonsterGradeList.end() ; itr++ )
	{
		if (itr->MonsterGeadeType == i_Type && itr->InflType == i_InflType && itr->MonsterNum == i_SummonMosterNum)
		{
			return itr->Level;
		}
	}
...
// end 2013-08-14 by jhseol, 거점전 리뉴얼 - CFieldIOCP MonsterGradeList 관련 함수 추가

// 2014-02-10 by jhseol&bckim, 체널링
int CFieldIOCP::GetChannellingServerUIDbyToken(char cToken)
{	
	vectChannelingServerInfo::iterator itr = m_vectCHANNELING_SERVER_INFO.begin();
	
	for ( ; itr != m_vectCHANNELING_SERVER_INFO.end() ; itr++ )
	{
		if( cToken == (*itr)->cChannelingServerToken )
		{
