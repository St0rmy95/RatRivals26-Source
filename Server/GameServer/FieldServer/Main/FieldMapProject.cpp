...
// 2006-10-20 by cmkwon, 아래와 같이 수정함
//	RETCODE		ret;
//	SQLINTEGER	cb1, cb2, cb3, cb4, cb5, cb6;
//	SHOP_ITEM	shopItem;
//	UINT		BuildingIndex;
//
//	/*[Stored Query Definition]************************************************
//	CREATE PROCEDURE atum_GetAllShopItems
//		@MapIndex SMALLINT
//	AS
...
//	GO
//	**************************************************************************/
//	SQLBindParameter(hstmt, 1,SQL_PARAM_INPUT, SQL_C_USHORT, SQL_SMALLINT
//		, 0, 0, &m_nMapIndex, 0, 0);
//
//	ret = SQLExecDirect(hstmt, (UCHAR*)"{call dbo.atum_GetAllShopItems(?)}", SQL_NTS);
//
//	if ( ret!=SQL_SUCCESS && ret!=SQL_SUCCESS_WITH_INFO )
//	{
...
//	}
//
//	SQLBindCol(hstmt, 1, SQL_C_ULONG, &BuildingIndex, 0, &cb1);
//	SQLBindCol(hstmt, 2, SQL_C_ULONG, &shopItem.ItemNum, 0, &cb2);
//	cb3 = SQL_NTS;
//	SQLBindCol(hstmt, 3, SQL_C_CHAR, shopItem.ItemName, SIZE_MAX_ITEM_NAME, &cb3);
//	SQLBindCol(hstmt, 4, SQL_C_USHORT, &shopItem.MinTradeQuantity, 0, &cb4);
//	SQLBindCol(hstmt, 5, SQL_C_LONG, &shopItem.Price, 0, &cb5);
//	SQLBindCol(hstmt, 6, SQL_C_UTINYINT, &shopItem.ItemKind, 0, &cb6);
//
//	// initialize to 0x00
//	memset(&shopItem, 0x00, sizeof(SHOP_ITEM));
//	while ( (ret = SQLFetch(hstmt)) != SQL_NO_DATA)
//	{
//		// 2005-05-23 by cmkwon, 현지화
//		if(FALSE == i_pLocalization->ItemLocalization(shopItem.ItemNum, shopItem.ItemName))
//		{
//// 2006-06-09 by cmkwon
////			SQLFreeStmt(hstmt, SQL_CLOSE);
////			return FALSE;
//		}
//
...
//			itr_shop = m_mapShopInfo.find(BuildingIndex);
//		}
//// 2006-10-20 by cmkwon, 상점 아이템 리스트 컨테이너 변경, 아래와 같이 수정함
////		itr_shop->second->m_mapShopItem.insert(pair<UINT, SHOP_ITEM>(shopItem.ItemNum, shopItem));
//		itr_shop->second->m_vectShopItemList.push_back(shopItem);
//		if(BuildingIndex == 9999)
//		{
//			DBGOUT(" ItemInfo(%d: %s)\r\n", shopItem.ItemNum, shopItem.ItemName);
//		}
//
//		// initialize to 0x00
//		memset(&shopItem, 0x00, sizeof(SHOP_ITEM));
	...
	for(int i=0; i < vectBuildingIndexList.size(); i++)
	{
		...
		if (itr_shop != m_mapShopInfo.end())
		{
			...
			**************************************************************************/
			...
			arrCB[1]=arrCB[2]=arrCB[3]=arrCB[4]=arrCB[5]=arrCB[6]=arrCB[7]=arrCB[8]=SQL_NTS;		// 2009-01-28 by cmkwon, 캐쉬샾 수정(추천탭,신상품 추가) - 초기화 추가  // 2013-09-24 by bckim, 캐시샵 추천탭 정렬순서부여 보완
			...
			SQLBindCol(hstmt, 9, SQL_C_LONG, &shopItem.RecommandedIdx, 0,			&arrCB[9]);		// 2013-09-24 by bckim, 캐시샵 추천탭 정렬순서부여 보완
...
// 2006-10-20 by cmkwon, 상점 아이템 리스트 컨테이너 변경, 아래와 같이 수정함
//		map<UINT, SHOP_ITEM>::iterator itr_item = itr_shop->second->m_mapShopItem.begin();
//		while (itr_item != itr_shop->second->m_mapShopItem.end())
//		{
...
//			}
//
//			*(SHOP_ITEM*)(SendBuf + offset) = itr_item->second;
//			offset += sizeof(SHOP_ITEM);
//			nBytesUsedForPutItem += sizeof(SHOP_ITEM);
//
////			g_pFieldGlobal->WriteSystemLogEX(TRUE, "\t%04d %d %d(%s): %d spi\r\n", m_nMapIndex, itr_shop->first, itr_item->second.ItemNum, itr_item->second.ItemName, itr_item->second.Price);
//
//			NumOfItem++;
//			itr_item++;
//		}
...
#ifdef SC_WORLDCUP_EVENT_MAP_CHANNEL_INCREASES 
		|| 4190 == this->m_nMapIndex						// 2014-06-13 by bckim, 월드컵이벤트맵 멀티채널가능하게 수정 
#endif
		)
