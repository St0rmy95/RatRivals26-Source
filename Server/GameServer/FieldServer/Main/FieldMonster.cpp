...
CFieldMonster::CFieldMonster()
{
	...
	MEMSET_ZERO(&m_bBarrierInfo, sizeof(BARRIER_SKILL_INFO));	// 2009-09-09 ~ 2010 by dhjin, 인피니티 - 배리어
	MEMSET_ZERO(&m_ImmediateBarrier, sizeof(IMMEDIATE_BARRIER_SKILL_INFO));	// 2009-09-09 ~ 2010-01-15 by dhjin, 인피니티 - 몬스터 즉시 시전 배리어 스킬(고급무기만발동)
...
BOOL CFieldMonster::SetAttackerCliIdx(ClientIndex_t i_cliIdx)
{
	...
	// 2013-08-14 by jhseol, 거점전 리뉴얼 - 전략포인트도 공격 가능하도록 수정
#ifdef SC_STRATEGYPOINTWAR_RENEWAL_JHSEOL_SSJUNG	//  - 전략포인트도 공격 가능하도록 수정
		)
#else
		|| IS_STRATEGYPOINT_MONSTER(this->MonsterInfoPtr->Belligerence))	// 2006-12-13 by cmkwon, 전략포인트 몬스터도 절대 공격하지 않는다
#endif
	// end 2013-08-14 by jhseol, 거점전 리뉴얼 - 전략포인트도 공격 가능하도록 수정
...
#ifdef SC_STRATEGYPOINTWAR_RENEWAL_JHSEOL_SSJUNG	// - 거점 몬스터 HP변화에 따른 정보 업데이트
	if(this->MSMonsterCheckWithoutMSBossMonster())
	{
		if (m_pCurrentFieldMapChannelMonster->m_pFieldIOCPMapChannel->m_InflWarManager.CheckSummonBoss())
		{
			this->HPRateCalcByMSWarInfoDisPlay ( this->MonsterInfoExtend.fMaxHP , BeforeHP , AfterHP , WARINFO_DISPLAY_TYPE_MSWAR);	// 모선전
		}
		else
		{
			this->HPRateCalcByMSWarInfoDisPlay ( this->MonsterInfoExtend.fMaxHP , BeforeHP , AfterHP , WARINFO_DISPLAY_TYPE_SPWAR);	// 일반 거점전
		}
	}
	else if (this->IsTriggerFunction())
	{
		if ( NULL != this->MonsterInfoPtr)
		{
			...
			for(int i=0; i<m_mtVectTriggerFunctionPtr.size(); i++)
			{
				...
				pTriggerFunction->OnStrategyPointDisPlay(this->MonsterInfoPtr->MonsterUnitKind, this->MonsterInfoExtend.fMaxHP , BeforeHP , AfterHP, NGCSP_PLAYING);
			}
		}
	}
#else
...
void CFieldMonster::ProcessingDamagedMonsterByCharacter(float i_fValue, CFieldIOCPSocket *i_AttackFISock, vectActionInfo *i_pAInfoAggroList)
{
	...
	if( 0.0f < i_fValue )
	{
		this->SetFirstUserAttack2Monster(i_AttackFISock);
	}
...
void CFieldMonster::InserttoAttackedInfoList(ActionInfo *i_pAttackedInfo)
{
	...
	if( FALSE == g_pFieldGlobal->IsArenaServer() 
		&& TRUE == COMPARE_MPOPTION_BIT(this->MonsterInfoPtr->MPOption, MPOPTION_BIT_BOSS_MONSTER) )
	{
		this->InserttoBossRewardInfoList(i_pAttackedInfo);
	}
	// End. 2013-08-28 by bckim, 보스몬스터 보상강화
}
...
// 2013-08-28 by bckim, 보스몬스터 보상강화
//////////////////////////////////////////////////////////////////////
//	보스몬스터를 공격할때 공격정보 누적 저장 및 새로운 공격정보 추가   
//////////////////////////////////////////////////////////////////////
void CFieldMonster::InserttoBossRewardInfoList(ActionInfo *i_pAttackedInfo)
{
	if( NULL == i_pAttackedInfo) {	return; }

	DWORD nRetainTimeOfSumDamage =  (DWORD)(this->MonsterInfoPtr->MonsterRewardInfo.nTime * 1000);

	mt_auto_lock mtAuto_(&m_mtvectorRewardInfoPtr);
	mtvectorActionInfoPtr::iterator it_Reward = find_if(m_mtvectorRewardInfoPtr.begin(), m_mtvectorRewardInfoPtr.end(),
														listActionInfoPtr_UnitIndex_Find_Object(i_pAttackedInfo->UnitIndex));
	if(it_Reward != m_mtvectorRewardInfoPtr.end())
	{	// 리스트에 UnitIndex가 같은 정보가 있음		
		
		DBGOUT("######공격정보 갱신(OLD) BEFORE###### ClientIDX:[%d] dwCurTick:[%d],dwStartTick:[%d],fSumDamage:[%f],fSumDamageForReward:[%f]\n\r"
			,(*it_Reward)->UnitIndex,(*it_Reward)->dwCurTick,(*it_Reward)->dwStartTick,(*it_Reward)->fSumDamage,(*it_Reward)->fSumDamageForReward);	
		
		// 현재 틱이랑 비교후에 제한시간이 넘어서면 누적데미지 초기화 
		DWORD dwTickTimeGap	= i_pAttackedInfo->dwCurTick - (*it_Reward)->dwStartTick;
		if( 0 != nRetainTimeOfSumDamage && dwTickTimeGap > nRetainTimeOfSumDamage )		// 임시 15초로 
		{
			(*it_Reward)->fSumDamageForReward = 0.0f;
			
			DBGOUT("######누적데미지 초기화 ###### ClientIDX:[%d] dwCurTick:[%d],dwStartTick:[%d],fSumDamage:[%f],fSumDamageForReward:[%f]\n\r"
				,(*it_Reward)->UnitIndex,(*it_Reward)->dwCurTick,(*it_Reward)->dwStartTick,(*it_Reward)->fSumDamage,(*it_Reward)->fSumDamageForReward);	
		}
		
		(*it_Reward)->dwStartTick = timeGetTime();
		(*it_Reward)->dwCurTick = i_pAttackedInfo->dwCurTick;
		(*it_Reward)->fSumDamage += i_pAttackedInfo->fSumDamage;
		(*it_Reward)->fSumDamageForReward += i_pAttackedInfo->fSumDamageForReward;	
		
		DBGOUT("######공격정보 갱신(OLD) AFTER###### ClientIDX:[%d] dwCurTick:[%d],dwStartTick:[%d],fSumDamage:[%f],fSumDamageForReward:[%f]\n\r"
			,(*it_Reward)->UnitIndex,(*it_Reward)->dwCurTick,(*it_Reward)->dwStartTick,(*it_Reward)->fSumDamage,(*it_Reward)->fSumDamageForReward);	
	}
	else
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		// 새로운 클라이언트 정보가 추가 될떄 해당 리스트에서 해당시간동안 공격이 없었던 유져 삭제 
		INT RemoveListCount =  CheckInvalidAttackerbyBossReward();
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		
		ActionInfo *ptmAInfo = new ActionInfo;
		*ptmAInfo = *i_pAttackedInfo;
		m_mtvectorRewardInfoPtr.push_back(ptmAInfo);
			
		DBGOUT("######공격정보 갱신(NEW2222) ###### ClientIDX:[%d] dwCurTick:[%d],dwStartTick:[%d],fSumDamage:[%f],fSumDamageForReward:[%f]\n\r"
			,ptmAInfo->UnitIndex,ptmAInfo->dwCurTick,ptmAInfo->dwStartTick,ptmAInfo->fSumDamage,ptmAInfo->fSumDamageForReward);	
	}
}


//////////////////////////////////////////////////////////////////////
//	보스몬스터 보상정보 리스트에서 기준 미달인 유져정보 삭제 
//////////////////////////////////////////////////////////////////////
INT CFieldMonster::CheckInvalidAttackerbyBossReward()
{
	// 새로운 클라이언트 정보가 추가 될떄 해당 리스트에서 해당시간동안 공격이 없었던 유져 삭제 
	// 호출단에서 Lock 하고 있음. 
	ActionInfo *tmp = NULL;
	INT RemoveCount = 0;
	DWORD i_dwTick = timeGetTime();		// 현재시간 
	mtvectorActionInfoPtr::iterator it_remove = m_mtvectorRewardInfoPtr.begin();
	while( it_remove != m_mtvectorRewardInfoPtr.end() )	
	{
		tmp = (*it_remove);
		DWORD dwTimeGap = i_dwTick - tmp->dwCurTick;
		DWORD nRetainTimeOfSumDamage =  (DWORD)(this->MonsterInfoPtr->MonsterRewardInfo.nTime * 1000);

		if( (nRetainTimeOfSumDamage*2) < dwTimeGap ) // 데미지초기화 시간의 2배의 시간이 넘어가면 리스트에서 삭제.
		{
			SAFE_DELETE(tmp);
			it_remove = m_mtvectorRewardInfoPtr.erase(it_remove);
			RemoveCount++;
			continue;
		}
		it_remove++;			
...
//////////////////////////////////////////////////////////////////////
//	몬스터에게 처음으로 데미지를 준 유져 인덱스 저장 
//////////////////////////////////////////////////////////////////////
BOOL CFieldMonster::SetFirstUserAttack2Monster(CFieldIOCPSocket *i_AttackFISock)
{
	if( NULL != i_AttackFISock && 0 == this->m_nFirstUserAttacks			// 최초데미지 등록된 유저가 없고 
		&& TRUE == IS_VALID_CLIENT_INDEX(i_AttackFISock->m_character.ClientIndex))
	{
		this->m_nFirstUserAttacks = i_AttackFISock->m_character.ClientIndex;
		return TRUE;
	}
	return FALSE;
}
...
/// \return		
///////////////////////////////////////////////////////////////////////////////
void CFieldMonster::HPRateCalcByMSWarInfoDisPlay(float i_fMonsterMaxHP, float i_fMonsterBeforeHP, float i_fMonsterAfterHP, BYTE i_nDisplayType)	// 2013-08-14 by jhseol, 거점전 리뉴얼 - 출력 타입 (0:모선전, 1:일반 거점전) BYTE i_nDisplayType 추가
{
	...
	{
		m_pCurrentFieldMapChannelMonster->m_pFieldIOCPMapChannel->m_InflWarManager.SetMSWarInfo(m_pCurrentFieldMapChannelMonster->GetMapChannelIndex().MapIndex, T_MSWARINFO_MODIFY_HPRATE, AfterHPRate, STATE_ERROR, STATE_ERROR, i_nDisplayType);	// 2013-08-14 by jhseol, 거점전 리뉴얼 - 출력 타입 (0:모선전, 1:일반 거점전) BYTE i_nDisplayType 추가
	}
}
...
//	Prob100_t nRand = RANDI(1, PROB100_MAX_VALUE+1);
	Prob100_t nRand = RANDI(0, PROB100_MAX_VALUE);
	if(m_bBarrierInfo.HitRate < nRand) {
		// 스킬 발동 체크
		return FALSE;
	}
...
void CFieldMonster::ProcessingInserttoAttackedInfoList(float i_fValue, CFieldIOCPSocket *i_AttackFISock, vectActionInfo *i_pAInfoAggroList)
{
	...
	// 2013-08-28 by bckim, 보스몬스터 보상강화
	float fRateOfUnitKind = 1.0f;
	if( IS_BGEAR(i_AttackFISock->m_character.UnitKind) ){ fRateOfUnitKind = 1.0f; }
	if( IS_MGEAR(i_AttackFISock->m_character.UnitKind) ){ fRateOfUnitKind = 2.0f; }
	if( IS_AGEAR(i_AttackFISock->m_character.UnitKind) ){ fRateOfUnitKind = 1.2f; }
	if( IS_IGEAR(i_AttackFISock->m_character.UnitKind) ){ fRateOfUnitKind = 1.4f; }

	float fValueForReward = i_fValue * fRateOfUnitKind;

	this->InserttoAttackedInfoList(&ActionInfo(i_AttackFISock->GetCharacter()->ClientIndex, timeGetTime(), i_fValue, 0.0F, fValueForReward ));
	//this->InserttoAttackedInfoList(&ActionInfo(i_AttackFISock->GetCharacter()->ClientIndex, timeGetTime(), i_fValue));
	// End. 2013-08-28 by bckim, 보스몬스터 보상강화
