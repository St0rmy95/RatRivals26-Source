...
typedef QPARAM_DELETESTOREITEM QPARAM_DESTROYAUCTIONADDITEM;	// 2013-12-26 by bckim, 파괴 경매 시스템
...
struct FIELD_DB_QUERY
{
	...
	FIELD_DB_QUERY(const DB_QUERY& rhs)
	{
		...
		this->enumQuryType		= rhs.enumQuryType;
...
struct QPARAM_CHECK_EVENTITEM
{
	...
	int		CheckWithCharacterUID; // 2014-03-12 by jekim, 주민번호당 아이템 지급
...
{// 2008-08-28 by dhjin, 버그 수정, 게임DB에 남겨야 Admintool로 초기화가 가능하다.
	...
	BYTE			BCUNextSPLevel;		// 2013-08-14 by jhseol, 거점전 리뉴얼 - 다음 생성될 BCU 거점 lv
	BYTE			ANINextSPLevel;		// 2013-08-14 by jhseol, 거점전 리뉴얼 - 다음 생성될 ANI 거점 lv
};
...
struct QPARAM_INSERT_STOREITEM_FROM_XMLRPC
{
	UID32_t		AccountUID;
	UID32_t		CharacterUID;
	INT			ItemNum;
...
struct QPARAM_DELETE_STOREITEM_FROM_XMLRPC
{
	UID32_t		AccountUID;
	UID32_t		CharacterUID;
	INT			ItemNum;
...
struct QPARAM_GET_CHARACTERINFO_FROM_XMLRPC
{
	UID32_t		AccountUID;
	UID32_t		CharacterUID;
	UID32_t		GiftCharacterUID;

	INT			ReturnValue;
	char		AccountName[SIZE_MAX_ACCOUNT_NAME];
	char		CharacterName[SIZE_MAX_CHARACTER_NAME];
	USHORT		Race;
	UID32_t		GiftAccountUID;
};
...
struct QPARAM_INSERT_COMBATPOWER_LOG
{
	ATUM_DATE_TIME time;
	INT			totalANI;
	INT			totalBCU;
...
class CCityLeader;
class CMarketDataManager;		// 2013-11-25 by jhseol&bckim, 거래소
class CDestroyAuctionManager;	// 2014-01-03 by jhseol&bckim, 파괴경매장
...
public:
	...
	static BOOL DBRemoveMarketInfoCloseOfTrade(CODBCStatement *i_pODBCStmt);	
	...
	////////////////////////////////////////////////////////////////////////////////
	// 2009-09-09 ~ 2010 by dhjin, 인피니티 - 	
	static BOOL DBLoadCinemaInfo(CODBCStatement *i_pODBCStmt, vectorCinemaInfo * o_pvectCinemaInfo, CLocalization *i_pLocalization);	// 2009-09-09 ~ 2010-02-26 by dhjin, 인피니티 - *.cfg파일 추가
	...
	BOOL DQP_UpdatePCBangList(DB_QUERY *i_qQuery, CODBCStatement *i_pODBC);		// 2008-12-19 by cmkwon, QT_UpdatePCBangList->DQT_UpdatePCBangList 변경 - 
	BOOL DQP_ReloadWRKServiceList(DB_QUERY *i_qQuery, CODBCStatement *i_pODBC);	// 2009-02-12 by cmkwon, EP3-3 월드랭킹시스템 구현 - 
	BOOL DQP_ReloadWRKLevel(DB_QUERY *i_qQuery, CODBCStatement *i_pODBC);	// 2009-02-12 by cmkwon, EP3-3 월드랭킹시스템 구현 - 
	BOOL DQP_ReloadWRKFame(DB_QUERY *i_qQuery, CODBCStatement *i_pODBC);	// 2009-02-12 by cmkwon, EP3-3 월드랭킹시스템 구현 - 
	BOOL DQP_ReloadWRKPVP(DB_QUERY *i_qQuery, CODBCStatement *i_pODBC);		// 2009-02-12 by cmkwon, EP3-3 월드랭킹시스템 구현 - 
	...
	static BOOL DBLoadMarketList(CODBCStatement *i_pODBCStmt, CMarketDataManager *i_pMarketManager);	// 2013-11-25 by jhseol&bckim, 거래소
	static BOOL DBLoadDestroyAuctionList(CODBCStatement *i_pODBCStmt, CDestroyAuctionManager *i_pMarketManager,ATUM_DATE_TIME i_StartTime, ATUM_DATE_TIME i_EndTime );	// 2013-12-26 by bckim, 파괴 경매 시스템 
	static BOOL DBSetClearDestroyAuctionList(CODBCStatement *i_pODBCStmt, CDestroyAuctionManager *i_pMarketManager,ATUM_DATE_TIME i_StartTime, ATUM_DATE_TIME i_EndTime );	// 2013-12-26 by bckim, 파괴 경매 시스템 

	// 2014-05-12 by bckim, 파괴경매장 운영툴  // 경매물품 리스트 업 운영툴테이블 insert 
	static BOOL DBInsertDestroyAuctionListUp(CODBCStatement *i_pODBCStmt, CDestroyAuctionManager *i_pMarketManager);	// 2013-12-26 by bckim, 파괴 경매 시스템 
...
//	void QP_GetAllQuest(FIELD_DB_QUERY q, SQLHSTMT hstmt);		// 사용자의 진행중이거나 완료된 모든 quest를 loading
	...
	void QP_InsertCombatPowerLog(FIELD_DB_QUERY q, SQLHSTMT hstmt); // 2014-03-14 by jekim, 전투력 로그 추가
	...
	void QP_JamboreeEntrants(FIELD_DB_QUERY q, SQLHSTMT hstmt);				// 2007-04-10 by cmkwon
...
//	void QP_LoadTenderInfo(FIELD_DB_QUERY q, SQLHSTMT hstmt);					// 2009-09-09 ~ 2010 by dhjin, 인피니티 - Tender
	...
	void QP_GetNumberOfChangeInflCount(FIELD_DB_QUERY q, SQLHSTMT hstmt);
	void QP_SetNumberOfChangeInflCount(FIELD_DB_QUERY q, SQLHSTMT hstmt);
	...
	void QP_InsertStoreItemFromXMLRPC(FIELD_DB_QUERY q, SQLHSTMT hstmt);		// 2013-03-13 by hskim, 웹 캐시 상점
	void QP_DeleteStoreItemFromXMLRPC(FIELD_DB_QUERY q, SQLHSTMT hstmt);		// 2013-03-13 by hskim, 웹 캐시 상점
	void QP_GetCharacterInfoFromXMLRPC(FIELD_DB_QUERY q, SQLHSTMT hstmt);		// 2013-03-13 by hskim, 웹 캐시 상점
	...
	void QP_InsertFirstPurchaseFromXMLRPC(FIELD_DB_QUERY q, SQLHSTMT hstmt); // 2014-04-21 by jekim, 웹 캐시 상점 보완 - 첫구매
	void QP_GetFirstPurchaseFromXMLRPC(FIELD_DB_QUERY q, SQLHSTMT hstmt); // 2014-04-21 by jekim, 웹 캐시 상점 보완 - 첫구매
	...
	void QP_LoadSPWarInfoResult(FIELD_DB_QUERY q, SQLHSTMT hstmt);				// 2013-08-14 by jhseol, 거점전 리뉴얼 - 지난 거점전정보 로드

	
	// 2013-11-25 by jhseol&bckim, 거래소
	void QP_Market_ItemRegistration(FIELD_DB_QUERY q,SQLHSTMT hstmt);			// 2013-11-26 by bckim, 거래소시스템
	void QP_Market_Buying(FIELD_DB_QUERY q,SQLHSTMT hstmt);						// 2013-11-26 by bckim, 거래소시스템
	void QP_Market_GetTradeItem(FIELD_DB_QUERY q,SQLHSTMT hstmt);				// 2013-11-26 by bckim, 거래소시스템
	void QP_Market_GetSaleList(FIELD_DB_QUERY q,SQLHSTMT hstmt);				// 2013-11-26 by bckim, 거래소시스템
	// End. 2013-11-25 by jhseol&bckim, 거래소

	// 2013-12-16 by jhseol, 역전의 기회 버프 리뉴얼
	void QP_GetTurnAroundBuffApplyInfo(FIELD_DB_QUERY q,SQLHSTMT hstmt);
	void QP_UpdateTurnAroundBuffApplyInfo(FIELD_DB_QUERY q,SQLHSTMT hstmt);
	// end 2013-12-16 by jhseol, 역전의 기회 버프 리뉴얼

	void QP_PvpBuffUpdate(FIELD_DB_QUERY q,SQLHSTMT hstmt); // 2014-03-26 by jekim, 역전의 버프 전투력

	
	// 2013-12-26 by bckim, 파괴 경매 시스템
	void QP_DestroyAuction_AddItem(FIELD_DB_QUERY q, SQLHSTMT hstmt);			// 인챈트 파괴시 아이템 등록 
	void QP_DestroyAuction_UpdateBiddingInfo(FIELD_DB_QUERY q, SQLHSTMT hstmt);	// 입찰정보 업데이트 
	void QP_DestroyAuction_GetAuctionItem(FIELD_DB_QUERY q, SQLHSTMT hstmt);	// 낙찰된 물품 회수 
	void QP_DestroyAuction_GetBiddingInfo(FIELD_DB_QUERY q, SQLHSTMT hstmt);	// 클라이언트 입찰참여중인 리스트
