...
//		if(IS_SAME_CHARACTER_8_MAP_INFLUENCE(i_byMonsterInfluence, itr->MapInfluenceType))
		if(CAtumSJ::IsSameCharacterInfluence8MapInfluence(i_byMonsterInfluence, itr->MapInfluenceType) && 0 == itr->DisplatType)	// 2013-08-14 by jhseol, 거점전 리뉴얼 - 출력 타입 (0:모선전, 1:일반 거점전) 모선전 정보만 삭제		// 2009-10-12 by cmkwon, 프리스카 제거 방안 적용 - 
		{
...
// 2013-08-14 by jhseol, 거점전 리뉴얼 - 출력 타입 (0:모선전, 1:일반 거점전)
...
// end 2013-08-14 by jhseol, 거점전 리뉴얼 - 출력 타입 (0:모선전, 1:일반 거점전)
...
void CMSWarInfoDisPlay::SetMSWarInfoDisPlay(MSWARINFO_DISPLAY * i_pMSWARINFO_DISPLAY)
{
	...
	{
		if(i_pMSWARINFO_DISPLAY->MapIndex == itr->MapIndex && itr->DisplatType == i_pMSWARINFO_DISPLAY->DisplatType)	// 2013-08-14 by jhseol, 거점전 리뉴얼 - 출력 타입 (0:모선전, 1:일반 거점전)
		{
...
void CMSWarInfoDisPlay::SetMSWarInfoDisPlayByHPRate(MapIndex_t i_nMapindex, INT i_nHPRate, BYTE i_nDisplayType)	// 2013-08-14 by jhseol, 거점전 리뉴얼 - 출력 타입 (0:모선전, 1:일반 거점전) BYTE i_nDisplayType 추가
{
	mt_auto_lock mta(&m_mtvectMSWARINFO_DISPLAY);
	...
	for(; itr != m_mtvectMSWARINFO_DISPLAY.end(); itr++)
	{
		if(i_nMapindex == itr->MapIndex && i_nDisplayType == itr->DisplatType)	// 2013-08-14 by jhseol, 거점전 리뉴얼 - 출력 타입 (0:모선전, 1:일반 거점전) BYTE i_nDisplayType 추가
		{
...
void CMSWarInfoDisPlay::SetMSWarInfoDisPlayByCreateTeleport(MapIndex_t i_nMapindex, INT i_nHPRate, INT i_nTelePortState, BYTE i_byBelligerence)
{
	...
	for(; itr != m_mtvectMSWARINFO_DISPLAY.end(); itr++)
	{
		if(i_nMapindex == itr->MapIndex && WARINFO_DISPLAY_TYPE_MSWAR == itr->DisplatType)	// 2013-08-14 by jhseol, 거점전 리뉴얼 - 출력 타입 (0:모선전, 1:일반 거점전) 포탈은 모선전에만 나옴으로 모선전 정보만 삭제
		{
			itr->HPRate			= i_nHPRate;
...
BOOL CDeclarationOfWar::CheckSPSummonPossibleTime()
{
	...
	// end 2013-08-27 by jhseol, 모선시간 체크
	
	if(m_pCurrentBCUWarStepInfo->MSWarStartTime <= CheckTime
		&& BCUCheckTime <= (STRATEGYPOINT_SUMMONTIME_BY_INFLUENCEBOSS/2 + 3)	// 2013-08-27 by jhseol, 모선시간 체크
		&& FALSE == m_pCurrentBCUWarStepInfo->GiveUp
	...
	if(m_pCurrentANIWarStepInfo->MSWarStartTime <= CheckTime
		&& ANICheckTime <= (STRATEGYPOINT_SUMMONTIME_BY_INFLUENCEBOSS/2 + 3)	// 2013-08-27 by jhseol, 모선시간 체크
		&& FALSE == m_pCurrentANIWarStepInfo->GiveUp
...
void CInflWarManager::InitCInflWarManager(CFieldIOCP *i_pFIOCP)
{
	...
	// 2013-08-14 by jhseol, 거점전 리뉴얼 - 거점전 로그 로드 함수 LoadSPWarInfoResult() call
#ifdef SC_STRATEGYPOINTWAR_RENEWAL_JHSEOL_SSJUNG	// - 거점전 로그 로드 함수 LoadSPWarInfoResult() call
	LoadSPWarInfoResult();
#endif
	// end 2013-08-14 by jhseol, 거점전 리뉴얼 - 거점전 로그 로드 함수 LoadSPWarInfoResult() call
	...
	// 2013-12-16 by jhseol, 역전의 기회 버프 리뉴얼 - Init
#ifdef S_UPGRADE_INFLUENCE_POINT_RENEWAL_JHSEOL		// - Init
	m_nBeforeContributionPointGap = 0;
	m_nContributionPointGapOnDeadSP = 0; // 2014-03-25 by jekim, 승리의 환호
...
#endif
	// end 2013-12-16 by jhseol, 역전의 기회 버프 리뉴얼 - Init
}
...
void CInflWarManager::OnDoMinutelyWarkInflWarManager(ATUM_DATE_TIME *pDateTime)
{
	...
	if(FALSE == g_pFieldGlobal->IsArenaServer()	// 2008-09-10 by cmkwon, 통합아레나 서버에서는 체크 필요 없음
	...
	if ( CurrentTime > m_atBrforeCheckTime && CurrentTime.Day != m_atBrforeCheckTime.Day )	// 2013-01-14 by jhseol, 전쟁 시스템 리뉴얼 - 거점전, 날짜 체크 조건문 수정
	{
		...
		{
			if ( (SHORT)localStTime.tm_wday == itr_renewal_strategypoint->DayOfWeek )		// 오늘과 같은 요일의 정보를 기준으로 
			{
				...
				BYTE	tmTodayBCUSummonCount = itr_renewal_strategypoint->CountBCU;	// 다음의 락 해제를 위해 임시변수에 값 저장
				BYTE	tmTodayANISummonCount = itr_renewal_strategypoint->CountANI;	// 다음의 락 해제를 위해 임시변수에 값 저장
	...
	// 2013-08-14 by jhseol, 거점전 리뉴얼 - 소환 재한시간 삭제
	BOOL tmIsNGCSPWaring = FALSE;
#ifdef SC_STRATEGYPOINTWAR_RENEWAL_JHSEOL_SSJUNG	// - 소환 재한시간 삭제
	int gap;
...
// 		if ( RenewalStrategyPointSummonTimeTermMin >= gap || TRUE == m_pFieldIOCP14->m_pMapTriggerManager->GetNGCSPWarState() )
		// 2014-02-12 by jhseol, 일반거점에서 NGC거점 UI보여지는 현상 수정 - NGC거점 1시간(일반거점의 진행시간)전부터 일반거점의 소환을 막는다.
		...
		// end 2014-02-12 by jhseol, 일반거점에서 NGC거점 UI보여지는 현상 수정 - NGC거점 1시간(일반거점의 진행시간)전부터 일반거점의 소환을 막는다.
		{
			tmIsNGCSPWaring = TRUE;
		}
	}
	if ( TRUE == m_DeclarationOfWar.CheckSPSummonPossibleTime() && FALSE == m_pFieldIOCP14->m_OutPostManager.CheckALLOutPostWaring() && FALSE == tmIsNGCSPWaring  && FALSE == CheckSummonBoss() )	// 2014-02-26 by jhseol, 모선이 종료되지 않아도 1시간 후에 거점이 생성되는 현상 수정
#else
	if ( TRUE == m_DeclarationOfWar.CheckSPSummonPossibleTime() && FALSE == CheckSummonBoss() )	// 2014-02-26 by jhseol, 모선이 종료되지 않아도 1시간 후에 거점이 생성되는 현상 수정
#endif
	// end 2013-08-14 by jhseol, 거점전 리뉴얼 - 소환 재한시간 삭제
	{
...
// 						, tmStartTime.GetDateTimeString(STRNBUF(SIZE_MAX_ATUM_DATE_TIME_STRING)), tmEndTime.GetDateTimeString(STRNBUF(SIZE_MAX_ATUM_DATE_TIME_STRING)));
					...
					{
						if ( FALSE == itr->SummonAttribute || 0 >= itr->SummonCount)		// 소환하지 않는 거점 skip
						{
							itr++;
							continue;
						}
						if ( CurrentTime < itr->SummonTime )								// 아직 소환이 안됐다면 skip
						{
...
// 								, itr->MapIndex, itr->SummonTime.GetDateTimeString(STRNBUF(SIZE_MAX_ATUM_DATE_TIME_STRING)));
						...
						// 2013-08-14 by jhseol, 거점전 리뉴얼 - 소환 재한시간 삭제
#ifdef SC_STRATEGYPOINTWAR_RENEWAL_JHSEOL_SSJUNG	// - 소환 재한시간 삭제
						if ( CurrentTime >= itr->SummonTime )	// 소환 시간이 지났다면 무조건 소환
#else
						if ( 300 > CurrentTime.GetTimeDiffTimeInSeconds(itr->SummonTime) )	// 현제 시간과 소환 해야할 시간의 차이가 5분 이내면 소환을 한다.
#endif
						// end 2013-08-14 by jhseol, 거점전 리뉴얼 - 소환 재한시간 삭제
						{
							if ( NULL == m_pFieldIOCP14)
							...
							itr->SummonCount = 0;
							MapIndex_t	tmMapIndex = itr->MapIndex;			// 다음의 락 해제를 위해 임시변수에 값 저장
							...
							BOOL bSummonCheck = FALSE;						// 이미 소환중인 거점맵인지 체크할 변수
							...
							if( TRUE != m_mtvectSummonStrategyPointInfo.empty() )	// 거점이 소환된 맵이 있다면
							{
								...
								{
									if(tmMapIndex == itr->MapIndex)			// 지금 소환할 거점맵인지 확인
									{
										bSummonCheck = TRUE;				// 맞으면 이미 거점이 소환중이다.
									}
									itr++;
								}
							}
							mtc.auto_unlock_cancel();						// 거점 조회가 끝났음로 락 헤제.

							// 2013-08-14 by jhseol, 거점전 리뉴얼 - 소환 재한시간 삭제
#ifdef SC_STRATEGYPOINTWAR_RENEWAL_JHSEOL_SSJUNG	// - 소환 재한시간 삭제
							if ( FALSE == bSummonCheck)	// 소환가능 여부만 체크
#else
							if ( FALSE == m_pFieldIOCP14->m_OutPostManager.CheckALLOutPostWaring() && FALSE == bSummonCheck)	// 전진기지전이 아니고 거점 생성이 가능하다면 거점소환
#endif
							// end 2013-08-14 by jhseol, 거점전 리뉴얼 - 소환 재한시간 삭제
							{
								...
								if(pFMapChann)
								{
									...
									// 2013-08-14 by jhseol, 거점전 리뉴얼 - 일반 거점전 거점 소환
#ifdef SC_STRATEGYPOINTWAR_RENEWAL_JHSEOL_SSJUNG	// - 일반 거점전 거점 소환
...
#endif
									// end 2013-08-14 by jhseol, 거점전 리뉴얼 - 일반 거점전 거점 소환
...
// 		g_pFieldGlobal->WriteSystemLogEX(FALSE, "  [Notify] CInflWarManager::OnDoMinutelyWarkInflWarManager() - #Summon Step.Finish - MSWar is underway BCU(%s), ANI(%s)\r\n", cBCUMswarStartTime, cANIMswarStartTime);
		...
		// end 2013-02-26 by jhseol, 전쟁 시스템 리뉴얼 부가옵션 - 모선전이 있는 날에도 거점 생성이 가능
	}
	// end 2012-11-13 by jhseol, 전쟁 시스템 리뉴얼 - 거점전

	
	// 2013-12-16 by jhseol, 역전의 기회 버프 리뉴얼 - 버프 갱신 체크
#ifdef S_UPGRADE_INFLUENCE_POINT_RENEWAL_JHSEOL		// - 버프 갱신 체크
	if ( m_atBeforeTurnAroundBuffApplyTime < (*pDateTime))	// 버프가 유지 되는 시간보다 현제 시간이 크면 버프 정보를 갱신시킨다. 통상 AM 6시가 기준
	{
		...
		if ( 6 <= pDateTime->Hour )	// AM 6시 이후이면 다음 역전의 버프 체크 시간에 하루를 더해준다
		{
			m_atBeforeTurnAroundBuffApplyTime.AddDateTime(0,0,1,0,0,0);
		}
// 2014-03-26 by jekim, 역전의 버프 전투력
...
#endif
// end 2014-03-26 by jekim, 역전의 버프 전투력
	}
#endif
	// end 2013-12-16 by jhseol, 역전의 기회 버프 리뉴얼 - 버프 갱신 체크
...
//	CalcConsecutiveVictoriesPoint(pNewInfo->InfluenceType);		// 2013-05-09 by hskim, 세력 포인트 개선
	// 2013-11-18 by jekim, 모선 파괴를 못했지만 양 세력 모두 200+1000 습득한 이슈 수정
	...
	CalcConsecutiveVictoriesPoint(byWinInflTy);		// 2013-05-09 by hskim, 세력 포인트 개선
	// end 2013-11-18 by jekim, 모선 파괴를 못했지만 양 세력 모두 200+1000 습득한 이슈 수정
#else
...
BOOL CInflWarManager::CalcContributionPoint(BYTE i_byWinnerInflTy, INT i_nWinnerValue, INT i_nLoserValue)
{
	...
	// 2013-12-16 by jhseol, 역전의 기회 버프 리뉴얼 - 패배세력 세력포인트 감소(승리세렉 세력포인트의 1/10)
#ifdef S_UPGRADE_INFLUENCE_POINT_RENEWAL_JHSEOL		// - 패배세력 세력포인트 감소(승리세렉 세력포인트의 1/10)
	i_nLoserValue = i_nWinnerValue/10;
#endif
	// end 2013-12-16 by jhseol, 역전의 기회 버프 리뉴얼 - 패배세력 세력포인트 감소(승리세렉 세력포인트의 1/10)
	...
	// 2013-12-16 by jhseol, 역전의 기회 버프 리뉴얼 - 기존코드 사용 안함
#ifdef S_UPGRADE_INFLUENCE_POINT_RENEWAL_JHSEOL		// - 기존코드 사용 안함
	...
	return TRUE;
#endif
	// end 2013-12-16 by jhseol, 역전의 기회 버프 리뉴얼 - 기존코드 사용 안함
...
{
#ifdef SC_UPGRADE_INFLUENCE_POINT_HSKIM_BHSOHN
	// 2013-12-16 by jhseol, 역전의 기회 버프 리뉴얼 - AM 6시에 체크한 세력포인트 차이를 리턴
#ifdef S_UPGRADE_INFLUENCE_POINT_RENEWAL_JHSEOL		// - AM 6시에 체크한 세력포인트 차이를 리턴
	return m_nBeforeContributionPointGap;
#endif
	// end 2013-12-16 by jhseol, 역전의 기회 버프 리뉴얼 - AM 6시에 체크한 세력포인트 차이를 리턴
...
BOOL CInflWarManager::SetSubLeader(BYTE i_byInflTy, UID32_t i_CharacterUID, CHAR * i_pCharacterName, BYTE SubLeaderNum)
{
	...
	if(1 == SubLeaderNum)
	{
		...
		STRNCPY_MEMSET(pInflWarData->InflSubLeader1CharacterName, i_pCharacterName, SIZE_MAX_CHARACTER_NAME);	// 2008-01-03 by dhjin, 전장 정보에 부지도자 안보이는 버그로 수정
	}
	else if(2 == SubLeaderNum)
	{
		pInflWarData->InflSub2LeaderCharacterUID = i_CharacterUID;
		STRNCPY_MEMSET(pInflWarData->InflSubLeader2CharacterName, i_pCharacterName, SIZE_MAX_CHARACTER_NAME);	// 2008-01-03 by dhjin, 전장 정보에 부지도자 안보이는 버그로 수정
	}
	else 
	{
		return FALSE;
...
void CInflWarManager::SetDoingInfluenceWar()
{
	...
	// 2013-03-25 by jhseol, 비전쟁시 트리거 이용 불가능 현상관련 추가로그
	char szSysLog[1024];
...
void CInflWarManager::LoadStrategyPointSummonInfo()
{
	...
	m_atBrforeCheckTime.SetCurrentDateTime();	// 로드한 시간 저장
...
/// \return		
///////////////////////////////////////////////////////////////////////////////
void CInflWarManager::InsertMSWarInfoByMSWarStart(MapIndex_t i_nStrategypointMapindex, INT i_nStrategypointMapInfluence, BYTE i_byStrategypointBelligerence, BYTE i_nDisplayType)	// 2013-08-14 by jhseol, 거점전 리뉴얼 - 출력 타입 (0:모선전, 1:일반 거점전) BYTE i_nDisplayType 추가
{
	...
	SMSWarInfoDisPlay.DisplatType		= i_nDisplayType;		// 2013-08-14 by jhseol, 거점전 리뉴얼 - 출력 타입 (0:모선전, 1:일반 거점전)
...
INT CInflWarManager::MakeMSG_FC_INFO_MSWARINFO_DISPLAY(SMSWARINFO_DISPLAY *o_pMSWarInfoDisPlay, vectMSWARINFO_DISPLAY * i_pVectMSWarInfoDisPlay)
{
	...
	for(; itr != i_pVectMSWarInfoDisPlay->end(); itr++)
	{
		...
		o_pMSWarInfoDisPlay[MSWarInfoDisPlayListCount].DisplatType				= itr->DisplatType;	// 2013-08-14 by jhseol, 거점전 리뉴얼 - 출력 타입 (0:모선전, 1:일반 거점전)
...
/// \return		
///////////////////////////////////////////////////////////////////////////////	
void CInflWarManager::SetMSWarInfo(MapIndex_t i_nMapindex, INT i_nMSWarInfoModifyType, INT i_nHPRate/*= STATE_ERROR*/, INT i_nTelePortState/*= STATE_ERROR*/, BYTE i_byBelligerence/*= STATE_ERROR*/, BYTE i_nDisplayType)	// 2013-08-14 by jhseol, 거점전 리뉴얼 - 출력 타입 (0:모선전, 1:일반 거점전) BYTE i_nDisplayType 추가
{
	switch(i_nMSWarInfoModifyType)
	{
		...
		case T_MSWARINFO_MODIFY_HPRATE:
			{// HP등급 변경
				...
				{
					this->m_MSWarInfoDisPlay.SetMSWarInfoDisPlayByHPRate(i_nMapindex, i_nHPRate, i_nDisplayType);	// 2013-08-14 by jhseol, 거점전 리뉴얼 - 출력 타입 (0:모선전, 1:일반 거점전) BYTE i_nDisplayType 추가
				}
			}
			break;
...
{
	m_MSWarInfoDisPlay.MSWarInfoDisPlayClearByMapindex(i_nSPSummonMapIndex, WARINFO_DISPLAY_TYPE_SPWAR);	// 2013-08-14 by jhseol, 거점전 리뉴얼 - 출력 타입 (0:모선전, 1:일반 거점전)
...
void CInflWarManager::SendLogMSWarInfoResult(INT i_nMonsterUID)
{
	...
	for(; itr != m_vectMSWarInfoResult.end(); itr++)
	{
		if(i_nMonsterUID == itr->MonsterUID)
		{
			...
			//CAtumLogSender::SendLogMessageMSWar(&MSWarLog);		// 2013-08-19 by jhseol, 모선전 로그 두번 남기는 현상 수정. 주석처리함.
...
/// \return		
///////////////////////////////////////////////////////////////////////////////
void CInflWarManager::SetSPWarInfoResultByWarStart(BYTE i_byAttInfluence, INT i_nSPSummonMapIndex, BYTE i_nSPLevel)	// 2013-08-14 by jhseol, 거점전 리뉴얼 - BYTE i_nSPLevel 추가
{
	...
	SPWarInfoResult.SPLevel				= i_nSPLevel;	// 2013-08-14 by jhseol, 거점전 리뉴얼 - BYTE i_nSPLevel 추가
...
void CInflWarManager::SendLogSPWarInfoResult(INT i_nSPSummonMapIndex)
{
	...
	for(; itr != m_vectSPWarInfoResult.end(); itr++)
	{
		if(i_nSPSummonMapIndex == itr->SPSummonMapIndex)
		{
			// 2013-08-14 by jhseol, 거점전 리뉴얼 - 최근 거점전 결과 메모리에 저장
#ifdef SC_STRATEGYPOINTWAR_RENEWAL_JHSEOL_SSJUNG	// - 최근 거점전 결과 메모리에 저장
			// 2014-03-25 by jekim, 승리의 환호
			BYTE MapInflu = INFLUENCE_TYPE_VCN == itr->AttInfluence ? INFLUENCE_TYPE_ANI : INFLUENCE_TYPE_VCN; // 공격 세력이 BCU이면 ANI거점맵이다
			SetNextSummonSPLevel(MapInflu,
				itr->AttInfluence == itr->WinInfluence ? FALSE : TRUE);		// 공격 세력과 승리 세력이 같으면 거점을 파괴한것 고로 결과적으론 해당 세력이 패배한 것임!.
#ifdef S_ACCLAIM_OF_VICTORY_JEKIM
			int CurrentDiffContributionPoint;
			BYTE BuffInflu = GetPVPBuffInfluence(&CurrentDiffContributionPoint);
			if(BuffInflu != 0 && BuffInflu == MapInflu)
			{
				if(GetNextStrategyPointLevel(MapInflu)>11) // 11 레벨 거점을 깬 이후부터 버프
				{
			...
			// end 2014-03-25 by jekim, 승리의 환호
#endif
			// end 2013-08-14 by jhseol, 거점전 리뉴얼 - 최근 거점전 결과 메모리에 저장
			// 2012-08-30 by jhseol, 풀로그 남기기
#ifdef S_FULL_LOG_JHSEOL
			// 2008-08-28 by dhjin, 버그 수정, 게임DB에 남겨야 Admintool로 초기화가 가능하다.
			...
			pQParam->SPLevel			= itr->SPLevel;		// 2013-08-14 by jhseol, 거점전 리뉴얼 - 거점전 결과 로그때 남길 거점 레벨
			pQParam->BCUNextSPLevel		= GetNextStrategyPointLevel(INFLUENCE_TYPE_VCN);	// 2013-08-14 by jhseol, 거점전 리뉴얼 - 다음 생성될 거점 lv
			pQParam->ANINextSPLevel		= GetNextStrategyPointLevel(INFLUENCE_TYPE_ANI);	// 2013-08-14 by jhseol, 거점전 리뉴얼 - 다음 생성될 거점 lv
			
			// 2014-05-30 by bckim, 거점정보 시스템 로그 남기기 ( 일본 요청 )
			...
			// End. 2014-05-30 by bckim, 거점정보 시스템 로그 남기기 ( 일본 요청 )
...
// 2013-08-14 by jhseol, 거점전 리뉴얼 - CInflWarManager SPWarInfoResult_BackUp 관련 함수추가
///////////////////////////////////////////////////////////////////////////////
/// \fn			void CInflWarManager::LoadSPWarInfoResult()
/// \brief		벡터 초기화 및 지난 거점전 로그 로드 호출함수
/// \author		jhseol
/// \date		2013-08-14
/// \warning	
///
...
/// \brief		거점전 결과에 따른 다음 거점 level 셋팅
/// \author		jhseol
/// \date		2013-08-14
/// \warning	
///
...
{
#ifdef SC_STRATEGYPOINTWAR_RENEWAL_JHSEOL_SSJUNG	// - SetNextSummonSPLevel 함수 무효화 구분
	if ( 0 < i_nFixedLevel )		// 0 보다 그면 강제 고정시키는 것임으로
	{
		if ( INFLUENCE_TYPE_VCN == i_nInflType )		// 세력확인하구 바로 적용
		{
	...
	{
		if ( INFLUENCE_TYPE_VCN == i_nInflType )	// BCU세력
		{
			if ( TRUE == i_bVictory )		// BCU세력이 이겼으면 거점 방어에 성공한 것 이므로 ANI세력이 파괴 해야할 다음 BCU세력 거점의 레벨 감소
			{
				if ( SPLEVEL_INIT < m_nBCU_NextSummonSPLevel )		// 상위랩 거점이면
				{
					m_nBCU_NextSummonSPLevel = SPLEVEL_INIT;		// 바로 5랩으루 강등
				}
				else									// 그렇지 않으면
				{
			...
			else		// 젔으면 거점이 파괴된 것 이므로
			{
				m_nBCU_NextSummonSPLevel = min(m_nBCU_NextSummonSPLevel+1, SPLEVEL_MAX);	// ANI세력이 파괴 해야할 다음 BCU세력 거점의 레벨 증가

				// 2014-02-06 by jhseol, 세력포인트에 따른 거점 레벨링 밸런스 패치 - 방어 실패시 조건 삭제
				// 방어 실패시 BCU가 파괴해야 하는 다음 ANI 거점 레벨도 강등
// 				if ( SPLEVEL_INIT < m_nANI_NextSummonSPLevel )		// 상위랩 거점이면
// 				{
// 					m_nANI_NextSummonSPLevel = SPLEVEL_INIT;		// 바로 5랩으루 강등
// 				}
// 				else									// 그렇지 않으면
// 				{
// 					m_nANI_NextSummonSPLevel = max(m_nANI_NextSummonSPLevel-1, SPLEVEL_MIN);	// -1
// 				}
				// end 2014-02-06 by jhseol, 세력포인트에 따른 거점 레벨링 밸런스 패치 - 방어 실패시 조건 삭제
			}
			return;
		}
		
		if ( INFLUENCE_TYPE_ANI == i_nInflType )	// ANI세력
		{
			if ( TRUE == i_bVictory )		// ANI세력이 이겼으면 거점 방어에 성공한 것 이므로 BCU세력이 파괴 해야할 다음 ANI세력 거점의 레벨 감소
			{
				if ( SPLEVEL_INIT < m_nANI_NextSummonSPLevel )		// 상위랩 거점이면
				{
					m_nANI_NextSummonSPLevel = SPLEVEL_INIT;		// 바로 5랩으루 강등
			...
			{
				m_nANI_NextSummonSPLevel = min(m_nANI_NextSummonSPLevel+1, SPLEVEL_MAX);	// BCU세력이 파괴 해야할 다음 ANI세력 거점의 레벨 증가

				// 2014-02-06 by jhseol, 세력포인트에 따른 거점 레벨링 밸런스 패치 - 방어 실패시 조건 삭제
				// 방어 실패시 ANI가 파괴해야 하는 다음 BCU 거점 레벨도 강등
// 				if ( SPLEVEL_INIT < m_nBCU_NextSummonSPLevel )		// 상위랩 거점이면
// 				{
// 					m_nBCU_NextSummonSPLevel = SPLEVEL_INIT;		// 바로 5랩으루 강등
// 				}
...
/// \brief		지난 거점 로그를 확인하여 다음에 생성될 거점의 lv을 결정하는 함수
/// \author		jhseol
/// \date		2013-08-14
/// \warning	
///
...
{
#ifdef SC_STRATEGYPOINTWAR_RENEWAL_JHSEOL_SSJUNG	// - GetNextStrategyPointLevel 함수 무효화 구분
	if ( INFLUENCE_TYPE_VCN == i_AttInflType )		// 공격 세력이 BCU 이면 ANI 거점기록을 확인하여 생성할 거점의 lv을 결정한다.
	{
	...
	else if ( INFLUENCE_TYPE_ANI == i_AttInflType )	// 공격 세력이 ANI 이면 BCU 거점기록을 확인하여 생성할 거점의 lv을 결정한다.
	{
...
// end 2013-08-14 by jhseol, 거점전 리뉴얼 - CInflWarManager SPWarInfoResult_BackUp 관련 함수추가
