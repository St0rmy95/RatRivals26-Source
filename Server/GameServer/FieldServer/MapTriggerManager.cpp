...
void CMapTriggerManager::Clear()
{
	...
	m_nCountOutPostWar = 0;								// 진행중인 전전기기전 카운팅
	m_nCountMSWar = 0;										// 진행중인 모선전 카운팅
...
#ifdef SC_STRATEGYPOINTWAR_RENEWAL_JHSEOL_SSJUNG	// - NGC거점전 생성시간 가져오기
...
void CMapTriggerManager::CountChangeNGCSPWar(BOOL i_bCountPlus/* = FALSE*/)
{
	...
	{
		// 모든 NGC거점전이 종료된것을 의미
		m_bPlayingNGCSPWar = FALSE;		// 2013-10-24 by jhseol, NGC거점전 종료후 크리스탈 파괴안되는 버그 수정 - m_bPlayingNGCSPWar 체크 포인트 위치 변경
		m_pFieldIOCP->m_InflWarManager.SetDoingInfluenceWar();
		SetNGCSPCreateTime();
		//m_bPlayingNGCSPWar = FALSE;		// 2013-10-24 by jhseol, NGC거점전 종료후 크리스탈 파괴안되는 버그 수정 - m_bPlayingNGCSPWar 체크 포인트 위치 변경 (주석처리)
...
void CMapTriggerManager::SetNGCSPCreateTime()
{
	...
	INT nAddDay = 0;
	INT nAddMinute = RANDI(0, 120);		// 2시간중 랜덤 (저녁 7시 ~ 9시 까지 랜덤 : 2 시간 * 60 분  = 120 분)
	CurrentTime.SetCurrentDateTime();
	CurrentTime.Convert(localStTime);

	switch(localStTime.tm_wday)					// 일요일 부터 시작하고 시작값 0
	{
	case 0:	//일
	case 1:	//월
	case 2:	//화
		nAddDay = 3 - (INT)localStTime.tm_wday;	// 차이나는 일수만큼 더하면 수요일
		break;
	case 3:	//수
		nAddDay = 3;							// 3일을 더하면 토요일
		break;
	case 4:	//목
	case 5:	//금
		nAddDay = 6 - (INT)localStTime.tm_wday;	// 차이나는 일수만큼 더하면 토요일
		break;
	case 6:	//토
		nAddDay = 4;							// 4일을 더하면 수요일
		break;
	}

	m_atNGCSPCreateTime.SetDateTime(CurrentTime.Year, CurrentTime.Month, CurrentTime.Day, 19, 0, 0);
	m_atNGCSPCreateTime.AddDateTime(0, 0, nAddDay, 0, nAddMinute, 0);			// 차이나는 일수만큼 더하구 오후 7시 부터 랜덤 시간 후에 다음 NGC거점 생성
