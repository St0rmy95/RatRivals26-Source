...
:CIOCP(SIZE_MAX_UDP_CHANNEL_SESSION, SIZE_MAX_NPCSERVER_SESSION, nPort, szLocalIP, ST_NPC_SERVER)	// 2008-08-29 by cmkwon, FieldServ<->NPCServ 가 UDP 통신 할 채널수 - 
	, m_vectorClientInfo(SIZE_MAX_FIELDSERVER_SESSION)		// 2008-12-02 by cmkwon, NPCServer 클라이언트 관리 구조 수정 - CNPCMapChannel 에 있던 것을 이동 함.
{
...
#ifdef ARENA
	bRet = m_pNPCMapWorkspace->LoadAllProject((string(CONFIG_ROOT)+string("./ArenaMap.cfg")).c_str(), g_pNPCGlobal->GetIsArenaServer(), &vectArenaMapIndexList);
#else
