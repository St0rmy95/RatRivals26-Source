...
// end 2011-10-10 by hskim, EP4 [버그 디버깅]
...
#ifdef SC_ANTI_HACKING_CODE_BCKIM_JWLEE			
		
		if( m_PeerSocketType == ST_INVALID_TYPE )
		{
			//g_pIMGlobal->WriteSystemLogEX(TRUE, "[ HACK socket_type] Type(%s) IP(%15s) socket_type(%d) \r\n", GetProtocolTypeString(nRecvType), this->GetPeerIP(), m_PeerSocketType );
		}
		...
		case T_NM_CONNECT_ALIVE:
			{
				//g_pIMGlobal->WriteSystemLogEX(TRUE, "[ANTI_HACKING_CODE_1] Type(%s) IP(%15s) socket_type(%d) \r\n", GetProtocolTypeString(nRecvType), this->GetPeerIP(), m_PeerSocketType );
				break;	// 확인하지 않음. 
			}
		...
		case T_IA_ADMIN_GET_SERVER_INFO:
			{
				if( FALSE == g_pIMGlobal->CheckAllowedToolIP( GetPeerIP()) )
				{
					g_pIMGlobal->WriteSystemLogEX(TRUE, "[ANTI_HACKING_CODE_2_Abnormal] Type(%s) IP(%15s) socket_type(%d) \r\n", GetProtocolTypeString(nRecvType), this->GetPeerIP(), m_PeerSocketType );
					return FALSE ;
				}
				else
				{
					// g_pIMGlobal->WriteSystemLogEX(TRUE, "[ANTI_HACKING_CODE_2_Normal] Type(%s) IP(%15s) socket_type(%d) \r\n", GetProtocolTypeString(nRecvType), this->GetPeerIP(), m_PeerSocketType );
					break;		// 허용아이피 체크(운영툴) 
				}
			}
		...
		case T_IM_GET_TOTAL_USER_COUNTS:
			{
				...
				{
					g_pIMGlobal->WriteSystemLogEX(TRUE, "[ANTI_HACKING_CODE_3_Abnormal] Type(%s) IP(%15s) socket_type(%d) \r\n", GetProtocolTypeString(nRecvType), this->GetPeerIP(), m_PeerSocketType );
					return FALSE ;
				}
				else
				{
					g_pIMGlobal->WriteSystemLogEX(TRUE, "[ANTI_HACKING_CODE_3_Normal] Type(%s) IP(%15s) socket_type(%d) \r\n", GetProtocolTypeString(nRecvType), this->GetPeerIP(), m_PeerSocketType );
					break;		// 허용아이피 체크(운영툴) 
				}
			}
		...
		case T_IC_GUILD_APPLICANT_REJECT_INVITE:		// 2008-06-12 by dhjin, EP3 - 여단 수정 사항 - 지원자 관리에서 여단원 초대 거부 
			{
				if( FALSE == CheckClientType(nRecvType) )
				{
					g_pIMGlobal->WriteSystemLogEX(TRUE, "[ANTI_HACKING_CODE_4_Abnormal] Type(%s) IP(%15s) socket_type(%d) \r\n", GetProtocolTypeString(nRecvType), this->GetPeerIP(), m_PeerSocketType );
						Close(0x12000);
					return FALSE ;
				}
				else
				{
					g_pIMGlobal->WriteSystemLogEX(TRUE, "[ANTI_HACKING_CODE_4_Normal] Type(%s) IP(%15s) socket_type(%d) \r\n", GetProtocolTypeString(nRecvType), this->GetPeerIP(), m_PeerSocketType );
					break;		// 허용아이피 체크(운영툴) 
				}
			}
		...
		case T_IC_ADMIN_GET_SERVER_STAT:
			{
				if( FALSE == CheckClientType(nRecvType) )
				{
					g_pIMGlobal->WriteSystemLogEX(TRUE, "[ANTI_HACKING_CODE_5_Abnormal] Type(%s) IP(%15s) socket_type(%d) \r\n", GetProtocolTypeString(nRecvType), this->GetPeerIP(), m_PeerSocketType );
					return FALSE ;
				}
				else
				{
				//	g_pIMGlobal->WriteSystemLogEX(TRUE, "[ANTI_HACKING_CODE_5_Normal] Type(%s) IP(%15s) socket_type(%d) \r\n", GetProtocolTypeString(nRecvType), this->GetPeerIP(), m_PeerSocketType );
					break;		// 허용아이피 체크(운영툴) 
				}
			}
		...
		case T_IC_CHATROOM_OTHER_MEMBER_INFO:			// I -> C, 2008-06-25 by dhjin, EP3 채팅방 - 채팅방 다른 맴버 정보 전송		
			{
				if( FALSE == CheckClientType(nRecvType) )
				{
					g_pIMGlobal->WriteSystemLogEX(TRUE, "[ANTI_HACKING_CODE_6_Abnormal] Type(%s) IP(%15s) socket_type(%d) \r\n", GetProtocolTypeString(nRecvType), this->GetPeerIP(), m_PeerSocketType );
					Close(0x12000);
					return FALSE ;
				}
				else
				{
					g_pIMGlobal->WriteSystemLogEX(TRUE, "[ANTI_HACKING_CODE_6_Normal] Type(%s) IP(%15s) socket_type(%d) \r\n", GetProtocolTypeString(nRecvType), this->GetPeerIP(), m_PeerSocketType );
					break;		// 허용아이피 체크(운영툴) 
				}
			}
		...
		case T_IC_VOIP_ERROR:
			{
				g_pIMGlobal->WriteSystemLogEX(TRUE, "[ANTI_HACKING_CODE_7] Type(%s) IP(%15s) socket_type(%d) \r\n", GetProtocolTypeString(nRecvType), this->GetPeerIP(), m_PeerSocketType );
				// 확인하지 않음. 
				break;
			}
		...
		case T_FI_INFO_DECLARATION_MSWAR_SET_OK:	// 2009-01-12 by dhjin, 선전 포고 - 선전포고 시간 및 포기 설정 F->I
			{
				if( FALSE == CheckFieldServerGroupIP( this->GetPeerIP() ) )
				{
					g_pIMGlobal->WriteSystemLogEX(TRUE, "[ANTI_HACKING_CODE_8_Abnormal] Type(%s) IP(%15s) socket_type(%d) \r\n", GetProtocolTypeString(nRecvType), this->GetPeerIP(), m_PeerSocketType );
					Close(0x12000);
					return FALSE ;
				}
				else
				{
					g_pIMGlobal->WriteSystemLogEX(TRUE, "[ANTI_HACKING_CODE_8_Normal] Type(%s) IP(%15s) socket_type(%d) \r\n", GetProtocolTypeString(nRecvType), this->GetPeerIP(), m_PeerSocketType );
					break;	
				}
			}
		}	
#endif  // SC_ANTI_HACKING_CODE_BCKIM_JWLEE
...
//				msgAdminConnectOK->AuthOK = bRet;
				...
				else
				{
					...
					STRNCPY_MEMSET(m_character.AccountName, msgAdminConnect->UID, SIZE_MAX_ACCOUNT_NAME);	// 2007-10-02 by cmkwon, 수정
		...
		case T_IA_ADMIN_CHAT_ALL:
			{
				...
				if (strncmp(chatString, "/", 1) == 0)
				{
					...
					if( TRUE == g_pIMGlobal->CheckWriteLog(chatString) || ms_pIMIOCP->m_pSecurityManager->CheckCommand(chatString) )		// 2011-06-22 by hskim, 사설 서버 방지
...
ProcessResult CIMIOCPSocket::Process_IC_CONNECT_LOGIN(const char* pPacket, int nLength, int &nBytesUsed)
{
	...
	// 2009-01-23 by cmkwon, 터키 i,I 문자 문제 처리 - T_IC_CONNECT_LOGIN
...
ProcessResult CIMIOCPSocket::Process_IC_CHAT_REGION(const char* pPacket, int nLength, int &nBytesUsed, SThreadInfo *i_pThreadInfo)
{
	...
	if( TRUE == g_pIMGlobal->CheckWriteLog(chatString) && FALSE == ms_pIMIOCP->m_pSecurityManager->CheckCommand(chatString) )		// 2011-06-22 by hskim, 사설 서버 방지
	g_pIMGlobal->WriteChattingLog(szChatting, m_character.Race);
...
// End. 2013-05-21 by bckim, 캐나다 채팅로그 정보 추가
	...
	//////////////////////////////////////////////////////////////////////////
	// 2009-03-03 by cmkwon, 아래와 같이 Gameforge4D 모든 서비스는 귓말, 여단, 편대 채팅 로그를 남기지 않는다.
	// 2009-01-06 by dhjin, Gameforge4D 요청으로 영국, 독일은 귓말, 여단, 편대 채팅 로그를 남기지 않는다.
	// 2007-11-13 by cmkwon, 관리자, 운영자 채팅 로그 저장하기 - CIMGlobal::WriteChattingLog() 함수 인자 추가
//	g_pIMGlobal->WriteChattingLog(szChatting, m_character.Race);
...
ProcessResult CIMIOCPSocket::Process_IC_CHAT_PTOP(const char* pPacket, int nLength, int &nBytesUsed, SThreadInfo *i_pThreadInfo)
{
	...
	// 2009-01-23 by cmkwon, 터키 i,I 문자 문제 처리 - T_IC_CHAT_PTOP
...
ProcessResult CIMIOCPSocket::Process_IC_CHAT_FRIENDLIST_INSERT(const char* pPacket, int nLength, int &nBytesUsed)
{
	...
	// 2009-01-23 by cmkwon, 터키 i,I 문자 문제 처리 - T_IC_CHAT_FRIENDLIST_INSERT
...
ProcessResult CIMIOCPSocket::Process_IC_CHAT_FRIENDLIST_DELETE(const char* pPacket, int nLength, int &nBytesUsed)
{
	...
	// 2009-01-23 by cmkwon, 터키 i,I 문자 문제 처리 - T_IC_CHAT_FRIENDLIST_DELETE
	LowercaseOnly_i_forTurkish(pNewMsg->szCharacterName, SIZE_MAX_CHARACTER_NAME);
...
ProcessResult CIMIOCPSocket::Process_IC_CHAT_REJECTLIST_INSERT(const char* pPacket, int nLength, int &nBytesUsed)
{
	...
	// 2009-01-23 by cmkwon, 터키 i,I 문자 문제 처리 - T_IC_CHAT_REJECTLIST_INSERT
	LowercaseOnly_i_forTurkish(pNewMsg->szCharacterName, SIZE_MAX_CHARACTER_NAME);
...
ProcessResult CIMIOCPSocket::Process_IC_CHAT_REJECTLIST_DELETE(const char* pPacket, int nLength, int &nBytesUsed)
{
	...
	// 2009-01-23 by cmkwon, 터키 i,I 문자 문제 처리 - T_IC_CHAT_REJECTLIST_DELETE
	LowercaseOnly_i_forTurkish(pNewMsg->szCharacterName, SIZE_MAX_CHARACTER_NAME);
...
// 2006-04-12 by cmkwon, 수정작업으로 임시 주석처리함
// 	int nHeaderSize = 0;
// 	INIT_MSG_WITH_BUFFER(MSG_IC_STRING_128, T_IC_STRING_128, MsgStr, SendBuf);
// 	MsgStr->PrintType = STR128_PRINT_TYPE_CHAT;
// 	if (string128_type == STRING_128_DEBUG_L1)
...
ProcessResult CIMIOCPSocket::Process_IC_GUILD_CREATE(const char* pPacket, int nLength, int &nBytesUsed)
{
	...
	// 2009-01-23 by cmkwon, 터키 i,I 문자 문제 처리 - 여단생성시 여단명 처리
...
ProcessResult CIMIOCPSocket::Process_IC_GUILD_REQUEST_INVITE(const char* pPacket, int nLength, int &nBytesUsed)
{
	...
	// 2009-01-23 by cmkwon, 터키 i,I 문자 문제 처리 - T_IC_GUILD_REQUEST_INVITE
...
ProcessResult CIMIOCPSocket::Process_IC_GUILD_CHANGE_GUILD_NAME(const char *pPacket, int nLength, int &nBytesUsed)
{
	...
	// 2009-01-23 by cmkwon, 터키 i,I 문자 문제 처리 - 여단명 변경 카드 사용시 여단명 처리
...
ProcessResult CIMIOCPSocket::Process_IC_CHATROOM_JOIN(const char* pPacket, int nLength, int &nBytesUsed)
{
	...
	for(; itr != ms_pIMIOCP->m_ChatRoomManager.m_mtlistChatRoom.end(); itr++)
	{
		...
		{	
			...
			msg->CharacterLevel	= this->m_character.Level;		// 2014-01-27 by jhseol, 채팅방 lv 정보 추가
...
BOOL CIMIOCPSocket::CheckCommandExecutability(USHORT i_nAllowedRace, BOOL i_bAllowAdminTool /* = FALSE */)
{
	...
	// 2015-07-07 by bckim, 게임포지 해킹보완 소켓타입확인
	// if( bRet && FALSE==g_pIMGlobal->CheckAllowedToolIP(this->GetPeerIP()))
	// {
	//	g_pIMGlobal->WriteSystemLogEX(TRUE, "HACKUSER!! Connect CheckCommandExecutability Command Using: HackingIP(%15s)\r\n", this->GetPeerIP());
	//	bRet = FALSE;
	// }	
...
//		SendString128(STRING_128_ADMIN_CMD, "서버 그룹 \'%s\'의 전체 접속자: %d명", g_szServerGroupName, ms_pIMIOCP->m_MapChactarterUniqueNumber.size());
		...
		|| 0 == stricmp(token, STRCMD_CS_COMMAND_WHOAREYOU_1))
	{
		...
		{// 2006-05-09 by cmkwon, 데모/일반사용자 처리

			// 2009-02-04 by cmkwon, 태국에만 "/WhoareYou" 명령어 일반 유저 사용 불가 - 
			// 2009-07-31 by cmkwon, (러시아 요청) 일반 유저 /누구 명령어 사용 막기 -
...
//#if defined(S_ARG_SERVER_SETTING_JHAHN)	// 2012-07-04 by hskim, 아르헨티나 Command 권한 변경 작업 - // 2009-04-21 by cmkwon, 스펠링 수정함, // 2009-04-20 by cmkwon, Gameforge4D "/누구" 명령어 제거하기 - 
//			return TRUE;				// 2009-04-20 by cmkwon, Gameforge4D "/누구" 명령어 제거하기 - 일반 유저는 바로 return 처리한다.
//#endif
#if defined(S_ARG_SERVER_SETTING_JHAHN) || defined(S_DEU_SERVER_SETTING_JHAHN) || defined(S_ENG_SERVER_SETTING_JHAHN) || defined(S_ESP_SERVER_SETTING_JHAHN) || defined(S_ITA_SERVER_SETTING_JHAHN) || defined(S_TUR_SERVER_SETTING_JHAHN) || defined(S_FRA_SERVER_SETTING_JHAHN) || defined(S_POL_SERVER_SETTING_JHAHN)
			return TRUE;				// 2012-10-15 by mspark, 게임포지 /whoareyou 명령어 일반 유저 사용불가 요청 추가			
#endif
...
// 2007-11-06 by cmkwon, 한국도 도우미 시스템 사용
//		if(LANGUAGE_TYPE_KOREAN == g_pIMGlobal->GetLanguageType())
//		{// 2006-10-17 by cmkwon, 한글 버젼에서는 막음
//			return FALSE;
//		}
	...
	else if ( stricmp(token, STRCMD_CS_COMMAND_PARTYMEMBER_INFO_0) == 0
		|| 0 == stricmp(token, STRCMD_CS_COMMAND_PARTYMEMBER_INFO_1))
	{
		if ( !CheckCommandExecutability(RACE_OPERATION) )
		{
			return FALSE;
		...
		if(NULL == token)
		{
			this->SendString128(STRING_128_USER_NOTICE, STRCMD_CS_COMMAND_PARTYMEMBER_INFO_HELP);
			return TRUE;
		}
		...
		{
			CIMIOCPSocket *pIISock = ms_pIMIOCP->GetIMIOCPSocketByCharacterName(token);
			if (pIISock == NULL || FALSE == pIISock->IsUsing())
			{
				SendString128(STRING_128_ADMIN_CMD, STRMSG_S_F2NOTIFY_0102, token);
				return FALSE;
			}

			CIMParty* ptmParty = pIISock->GetIMParty();
			if (NULL == ptmParty)
			{
				SendString128(STRING_128_ADMIN_CMD, STRMSG_C_080703_0202);
				return FALSE;
			}
			...
			ptmParty->LockIMParty();	
			{
				...
				sprintf(sztemptitle, "== formationinfo PartyUID:[%I64d] ==\r\n",ptmParty->m_PartyID);
				SendString128(STRING_128_ADMIN_CMD, sztemptitle);

				for (int i=0 ; i < SIZE_MAX_PARTY_MEMBER; i++)
				{
					if(ptmParty->m_arrPartyMember[i].IsUsingMember())
					{
						if( ptmParty->m_arrPartyMember[i].IsOnlineMember() )
						{
							BOOL bMaster = FALSE;
							CIMIOCPSocket* ptmIIsoc = ptmParty->m_arrPartyMember[i].PM_pIMIOCPSocket;
							if( NULL == ptmIIsoc) 	{	return FALSE; }

							if( ptmParty->m_pMasterIMPartyMember->PM_CharacterUniqueNumber == ptmIIsoc->m_character.CharacterUniqueNumber)
							{
								bMaster = TRUE;
							}
							
							char UnitKindInitial;
							ptmIIsoc->GetUnitkind2Initial(&UnitKindInitial);
							sprintf(sztemp, "[%s%c %d %c %s %s]\r\n",ptmIIsoc->m_character.CharacterName, bMaster?'*':' ',ptmIIsoc->m_character.MapChannelIndex.MapIndex,UnitKindInitial, ptmIIsoc->m_szPeerIP, "ON");
							SendString128(STRING_128_ADMIN_CMD, sztemp);
						}
						else
						{
							sprintf(sztemp, "[%s %s]\r\n",ptmParty->m_arrPartyMember[i].PM_CharacterName,"OFF");
							SendString128(STRING_128_ADMIN_CMD, sztemp);
						}
					}					
				}
			}
			SendString128(STRING_128_ADMIN_CMD, "======= formationinfo End =======");
			ptmParty->UnlockIMParty();
			return TRUE;
		}
	}
	...
	// 필드서버에도 전송해야 하므로 FALSE를 return한다.
	// 해당 명령어가 없는 경우 --> Field 서버가 나머지를 받음
	return FALSE;
}
...
// 2013-11-19 by bckim, 파티원 정보 보기 명령어 추가
void CIMIOCPSocket::GetUnitkind2Initial(CHAR *o_pCharInitial)
{
	switch(m_character.UnitKind)
	{
	case UNITKIND_BGEAR:	*o_pCharInitial = 'B';	break;	
	case UNITKIND_MGEAR:	*o_pCharInitial = 'M';	break;
	case UNITKIND_AGEAR:	*o_pCharInitial = 'A';	break;
	case UNITKIND_IGEAR:	*o_pCharInitial = 'I';	break;
	default:				*o_pCharInitial = '_';	break;
	}
}
// End. 2013-11-19 by bckim, 파티원 정보 보기 명령어 추가

// 2015-07-07 by bckim, 게임포지 해킹보완 소켓타입확인
BOOL CIMIOCPSocket::CheckClientType(MessageType_t i_nRecvType)
{
	//m_PeerSocketType = ST_CLIENT_TYPE;

	if(ST_INVALID_TYPE == m_PeerSocketType )
	{
		// g_pIMGlobal->WriteSystemLogEX(TRUE, "HACKUSER!! The socket type does not match : HackingIP(%15s) Type(%s) SocketType(%d) \r\n", this->GetPeerIP(), GetProtocolTypeString(i_nRecvType), m_PeerSocketType );
		return FALSE;
	}				
	return TRUE;
}

BOOL CIMIOCPSocket::CheckFieldServerGroupIP(const char * i_szPeerIP)
{
	// ms_pIMIOCP->m_FieldServerInfo.pSocket->GetPeerIP()

	// if (strnicmp(  ms_pIMIOCP->m_pFieldServerSocket->GetPeerIP() , i_szPeerIP, SIZE_MAX_IPADDRESS) == 0)
	if (strnicmp( ms_pIMIOCP->m_FieldServerInfo.pSocket->GetPeerIP(), i_szPeerIP, SIZE_MAX_IPADDRESS) == 0)
	{
		//g_pIMGlobal->WriteSystemLogEX(TRUE, "[Notify:CheckIMServerGroupIP] IMserverIP(%15s), InputIP(%15s) \r\n",ms_pIMIOCP->m_pFieldServerSocket->GetPeerIP(), i_szPeerIP);
		//g_pIMGlobal->WriteSystemLogEX(TRUE, "[Notify:CheckFieldServerGroupIP] IMserverIP(%15s), InputIP(%15s) \r\n",ms_pIMIOCP->m_FieldServerInfo.pSocket->GetPeerIP(), i_szPeerIP);
		return TRUE;
	}
	//g_pIMGlobal->WriteSystemLogEX(TRUE, "[Notify:CheckFieldServerGroupIP HACKUSER!!] Input_IP(%15s)\r\n", i_szPeerIP);
	return FALSE;
}
