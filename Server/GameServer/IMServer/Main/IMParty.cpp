...
{
	ResetIMPartyMember();
	STRNCPY_MEMSET(PM_CharacterName, i_szCharName, SIZE_MAX_CHARACTER_NAME);
				   ...
				   )
{
	...
	MEMSET_ZERO(&m_SPartyInfo, sizeof(SPARTY_INFO));
...
void CIMParty::SetPartyInfo(SPARTY_INFO * i_pPartyInfo)
{
	...
	STRNCPY_MEMSET(m_SPartyInfo.PartyName, i_pPartyInfo->PartyName, SIZE_MAX_PARTY_NAME);
	STRNCPY_MEMSET(m_SPartyInfo.PartyPW, i_pPartyInfo->PartyPW, SIZE_MAX_TEAM_PW);
...
INT CIMParty::MakeMSG_IC_PARTY_RECOMMENDATION_MEMBER_OK(SRECOMMENDATION_MEMBER_INFO * o_pRecommendationMember, CIMIOCPSocket * i_pPartyMasterSocket, INT i_nMinLevel, INT i_nMaxLevel)
{
	...
	for(i = RandomCount; i < nArrSize; i++)
	{
			...
			)
		{
			...
			STRNCPY_MEMSET(o_pRecommendationMember[nListCount].CharacterName, pSocket->m_character.CharacterName, SIZE_MAX_CHARACTER_NAME);
			nListCount++;
		}
	}
