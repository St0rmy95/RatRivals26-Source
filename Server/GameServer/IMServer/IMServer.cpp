...
LONG __stdcall ExceptionHandler(_EXCEPTION_POINTERS* pExceptionInfo)
{
    ...
    char szTemp[256];
#ifdef ARENA
	wsprintf(szTemp, "ArenaIMServer Crash !! : Create dump file (Exception 0x%08x arised)", pExceptionInfo->ExceptionRecord->ExceptionCode);
#else
...
// end 2012-04-20 by hskim 미니덤프 기능 추가
...
#ifdef ARENA
	sprintf(szSystemLog, "Arena IM Server Start\r\n\r\n");
#else
	...
	CIOCP::SocketClean();
#ifdef ARENA
	sprintf(szSystemLog, "Arena IM Server End\r\n\r\n\r\n");
#else
