...
CChatRoom::CChatRoom(CIMIOCPSocket* i_pIISoc, INT i_nChatRoomNum, MSG_IC_CHATROOM_CREATE * i_pChatRoomCreateInfo)
{
	...
	STRNCPY_MEMSET(m_szChatRoomName, i_pChatRoomCreateInfo->ChatRoomName, SIZE_MAX_CHATROOM_NAME);
	STRNCPY_MEMSET(m_szChatRoomPW, i_pChatRoomCreateInfo->ChatRoomPW, SIZE_MAX_TEAM_PW);
...
void CChatRoom::SendChatRoomMemberInfo(CIMIOCPSocket* i_pIISoc)
{
	...
	STRNCPY_MEMSET(pSMsg->ChatRoomName, m_szChatRoomName, SIZE_MAX_CHATROOM_NAME);
	pSMsg->ChatRoomLock				= m_bChatRoomLock;
	STRNCPY_MEMSET(pSMsg->ChatRoomPW, m_szChatRoomPW, SIZE_MAX_TEAM_PW);
...
INT	CChatRoom::MakeMSG_IC_CHATROOM_MEMBER_INFO_OK(SCHATROOM_MEMBER_INFO * o_pChatRoomMemberInfo)
{
	...
	for(; itr != m_mtpvectChatRoomMember.end(); itr++)
	{
		...
		STRNCPY_MEMSET(o_pChatRoomMemberInfo[nChatRoomMemberCount].CharacterName, (*itr)->m_character.CharacterName, SIZE_MAX_CHARACTER_NAME);
		o_pChatRoomMemberInfo[nChatRoomMemberCount].CharacterLevel				=   (*itr)->m_character.Level;		// 2014-01-27 by jhseol, 채팅방 lv 정보 추가
...
{
	STRNCPY_MEMSET(m_szChatRoomName, i_szNewChatRoomName, SIZE_MAX_CHATROOM_NAME);
}
...
void CChatRoom::UpdateChatRoomLock(BOOL i_bNewChatRoomLock, char * i_szNewChatRoomPW)
{
	...
	STRNCPY_MEMSET(m_szChatRoomPW, i_szNewChatRoomPW, SIZE_MAX_TEAM_PW);
}
