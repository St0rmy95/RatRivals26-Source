...
//		paramFactor.pfp_DODGE_PART += fDestValue;
	...
	case DES_ATTACKPROBABILITY_01:		// 공격확률 01
		...
		//paramFactor.pfp_ATTACKPROBABILITY_01 += (Prob256_t)fDestValue;
		paramFactor.pfp_ATTACKPROBABILITY_01 += fDestValue;
		break;
	case DES_ATTACKPROBABILITY_02:		// 공격확률 02
		// 2010-07-28 by dgwoo 확률 수식 변경 (255 => 100%)
		//paramFactor.pfp_ATTACKPROBABILITY_02 += (Prob256_t)fDestValue;
	...
	case DES_SKILLPROBABILITY_02	:	// 스킬공격확률 02
		paramFactor.pfp_SKILLPROBABILITY += (Prob256_t)fDestValue;
		break;
	case DES_FACTIONRESISTANCE_01:		// 속성저항력 01
	case DES_FACTIONRESISTANCE_02:		// 속성저항력 02
		paramFactor.pfp_FACTIONRESISTANCE += (Prob256_t)fDestValue;
		break;
	case DES_SPEED		:				// 이동속도
	...
	case DES_ABRASIONRATE_01	:		// 마모율 01
		paramFactor.pfp_ABRASIONRATE_01 += (Prob256_t)fDestValue;
		break;
	case DES_ABRASIONRATE_02	:		// 마모율 02
		paramFactor.pfp_ABRASIONRATE_02 += (Prob256_t)fDestValue;
		break;
	...
	case DES_FACTIONPROBABILITY_02:		// 속성에 대한 방어확률 02
		paramFactor.pfp_FACTIONPROBABILITY += (Prob256_t)fDestValue;
		break;
...
{
	FLOG( "Random(float max, float min)" );
	float fTerm = max - min;
...
//	fTerm = ((float)nRnd) / 100.0f;
	...
	if(nTerm == 0)
		return min;
	int nRnd = rand()%(nTerm*100);
	return ((float)nRnd)/100.0f + min;
}
...
///////////////////////////////////////////////////////////////////////////////
/// \fn			UINT GetUnitNum(int nDegree, int nDefenseItemNum, int nUnitKind )
/// \brief		렌더링할 유닛의 번호(오브젝트 인덱스, 이펙트(캐릭터)인덱스)
/// \author		dhkwon
/// \date		2004-07-28 ~ 2004-07-28
/// \warning	
///
/// \param		
/// \return		
...
UINT GetUnitNum(int nDegree, int nDefenseItemNum, int nUnitKind, BOOL bCharacter)
{
	...
	else
	{
		...
		{
			int nTemp = nDefenseItemNum - 8000000;	// 아머는 8에서 시작하는 7자리수이다.
			// 2006-02-17 by ispark, 5단계 이후는 Low 렌더링이 없다
			if(nTemp > 100000 && nDegree == 0)
			{
				nDegree = 2;
			}
...
BOOL IsInfluenceCharacter(USHORT nMyInfluenceType, USHORT nEnemyInfuenceType, BYTE byType)
{
	// byType
	// 0 : 모두 다른 세력, 
	// 1 : 미세력과 바이제니유는 같은 세력 (기본 타입)
	if(byType == 0)
	{
		if(!COMPARE_INFLUENCE(nMyInfluenceType, nEnemyInfuenceType))
		{
			return FALSE;
		}
	}
...
int GetMusicVolume(int nRate)
{
	...
	float fPercent = (float)nRate*0.01f;
...
#endif
// end 2013-10-17 by ssjung 폰트 색생 추가
	default:
...
BOOL StringCullingUserDataEx(char *pLineString, int nCullStringNum, vector<string> *vecStorage, CD3DHanFont* pHanFont, BOOL bChRt /*= FALSE*/, BOOL bShop/*= FALSE*/)
{
	...
	BOOL bSetChar = FALSE;
	BOOL bShopNextLine = FALSE;				// 샾일 때 아이템 이름이 다음라인에 걸쳐져 있습니까 ?
	ZERO_MEMORY(strNPCTalk);
	
	// 2015-07-29 by jwlee 컬링시 스트링 사이즈 관련 오류 수정
	int nBufColorSize = 0;
	// end 2015-07-29 by jwlee 컬링시 스트링 사이즈 관련 오류 수정
	
	while(TRUE)
	{
		bShopNextLine = FALSE; // 2013-04-05 by ssjung 샾 일때 아이템 이름 글자 수 넘어 갈때 처리 보정
		bSetChar = FALSE;
		char strBuf[1024] = {0,};
		...
		// 2015-07-29 by jwlee 컬링시 스트링 사이즈 관련 오류 수정
		int nColorlen = strlen(strBuf) - 1;
		if (nColorlen > 0)
		{
			if(strBuf[nColorlen - 1] == '\\')
			{
				SIZE ColorSize;
				GetTextExtentPoint32(g_pApp->GetHDC(), &strBuf[nColorlen - 1], 2, &ColorSize);
				nBufColorSize += ColorSize.cx;
			}
		}
		nBufSize -= nBufColorSize;
		// end 2015-07-29 by jwlee 컬링시 스트링 사이즈 관련 오류 수정

		// 2014-05-12 by ymjoo 스트링 컬링 무한 루프 현상 수정
		if('\0' == *pStr && strlen(pLineString) == nOldPoint)
		{
			if(nPoint == 0)
			{
				nLine--;
			...
			{
				memcpy(strNPCTalk[nLine], pLineString + nCheckPoint, nPoint);
			}
			break;
		}
		// END 2014-05-12 by ymjoo 스트링 컬링 무한 루프 현상 수정

// 2013-04-05 by ssjung 샾 일때 아이템 이름 글자 수 넘어 갈때 처리 보정
		if(bShop)
		{
...
// 2013-04-05 by ssjung 샾 일때 아이템 이름 글자 수 넘어 갈때 처리 보정
		...
		if(nBufSize >= nCullStringNum || bShopNextLine)
		{
			...
			if('\\' != strBuf[strlen(strBuf)] && '\\' != strBuf[strlen(strBuf)-1] && '\\' != strBuf[strlen(strBuf)-2])	// 2014-02-21 by ymjoo 글자 컬링 개행 개선
			{
				if(chColorSave)
				{				
					char strNPCTalkTemp[1024] = {0, };
				...
				{
					if(bShopNextLine) ++nLineBreakPoint;									 // 2013-04-05 by ssjung 샾 일때 아이템 이름 글자 수 넘어 갈때 처리 보정
					memcpy(strNPCTalk[nLine], pLineString + nCheckPoint, nLineBreakPoint);
				}
...
//			continue;
		...
		if(nLastCount > 0 && strBuf[nLastCount - 1] == '\\')
		// end 2015-07-29 by jwlee 컬링시 스트링 사이즈 관련 오류 수정
		{
...
BOOL StringCullingUserData_ToBlank( char* strTalk, int nCullPixel, vector<string> *vecDstString, CD3DHanFont* pFont, BOOL bChRt /*= FALSE*/)
{
	...
	while(TRUE)
	{
		...
		else if(NULL == pNextStr[0] || (NULL == pNextStr))
		{
			bFindBlank = TRUE;			
			bBreak = TRUE;			
		}		
...
/// \return		
///////////////////////////////////////////////////////////////////////////////
int GetStringBuffPos(char* pTxt, int i_nCurSelPos)
{
	int nCheckPoint = 0;
	int nStringPos = 0;
	int nCnt = 0;
...
/// \return		
///////////////////////////////////////////////////////////////////////////////
int GetStringBuffLen(char* pTxt)
{
	int nCheckPoint = 0;
	int nStringPos = 0;
	int nCnt = 0;
