...
// 2013-10-15 by bckim, 국가별 공용으로 쓰는 스트링 구분 COMMON_STRMSG_REG_KEY_NAME_WINDOWDEGREE_NEW
// exe 및 런쳐에서 사용 
...
#ifdef SERVICE_TYPE_GLOBAL_SERVER_1							// 2013-09-04 by jhseol, 글로벌 서버 빌드옵션 추가
...
#define SIZE_MAX_ITEM_DESCRIPTION				300		// 2015-02-12 SIZE_MAX_ITEM_DESCRIPTION 값 200에서 300으로 수정
#define SIZE_MAX_NUM_CHARACTER					3
#define SIZE_MAX_CHAT_MESSAGE					100			// 2006-05-04 by cmkwon, 60-->100
#define SIZE_MAX_ERROR_STRING					512	//60	// 2013-09-23 by jekim, 계정 블락시 메세지 뿌려주기. 60->512 변경
...
// 2014-02-06 by jhseol, 세력포인트에 따른 거점 레벨링 밸런스 패치 - 거점 세력포인트 원복
...
// end 2014-02-06 by jhseol, 세력포인트에 따른 거점 레벨링 밸런스 패치 - 거점 세력포인트 원복

// 2013-12-16 by jhseol, 역전의 기회 버프 리뉴얼 - 보상 WP 증가
#ifdef S_UPGRADE_INFLUENCE_POINT_RENEWAL_JHSEOL		// - 보상 WP 증가
#define ADD_WARPOINT_BOSS_MONSTER							(1000+400)	// 모선 폭파/방어 성공시 증가 WARPOINT
#define ADD_WARPOINT_BOSS_MONSTER_BY_LOSE					(400+400)	// 모선 폭파/방어 패배시 증가 WARPOINT
#define ADD_WARPOINT_STRATEGYPOINT_MONSTER					(600+400)	// 전략포인트 폭파/방어 성공시 증가 WARPOINT 
#define ADD_WARPOINT_STRATEGYPOINT_MONSTER_BY_LOSE			(200+400)	// 전략포인트 폭파/방어 패배시 증가 WARPOINT 
#else
...
#endif
// end 2013-12-16 by jhseol, 역전의 기회 버프 리뉴얼 - 보상 WP 증가
...
// 2009-01-28 by cmkwon, 캐쉬샾 수정(추천탭,신상품 추가) - 아래의 컬러 탭만 고정하고 다른 탭은 의미를 두지 않는다. 단지 탭의 순서만을 의미하게 수정함.
...
#define CASH_ITEMKIND_RECOMMEND_ITEM				((BYTE)0x00)	// 2009-01-28 by cmkwon, 캐쉬샾 수정(추천탭,신상품 추가) - 첫번째 탭은 추천탭
#define CASH_ITEMKIND_COLOR_ITEM					((BYTE)0x09)	// 2009-01-28 by cmkwon, 캐쉬샾 수정(추천탭,신상품 추가) - 컬러탭은 고정 해야 하므로 일단 제일 뒤로 지정함.

#define CASHSHOP_BIT_TAPINDEX_MASK					((BYTE)0x0F)	// 2009-01-28 by cmkwon, 캐쉬샾 수정(추천탭,신상품 추가) - 
#define CASHSHOP_BIT_RECOMMEND						((BYTE)0x10)	// 2009-01-28 by cmkwon, 캐쉬샾 수정(추천탭,신상품 추가) - 
#define CASHSHOP_BIT_NEW							((BYTE)0x20)	// 2009-01-28 by cmkwon, 캐쉬샾 수정(추천탭,신상품 추가) - 
#define GET_TABINDEX_BY_CASHSHOPINDEX(idx)				((idx)&CASHSHOP_BIT_TAPINDEX_MASK)				// 2009-01-28 by cmkwon, 캐쉬샾 수정(추천탭,신상품 추가) - 0~15까지가 탭인덱스임
#define GET_CASHSHOP_BIT_BY_CASHSHOPINDEX(idx)			((idx)&((BYTE)(~CASHSHOP_BIT_TAPINDEX_MASK)))	// 2009-01-28 by cmkwon, 캐쉬샾 수정(추천탭,신상품 추가) - 16~255까지가 캐쉬아이템 비트플래그
#define IS_NEW_CASHSHOPITEM_BY_CASHSHOPINDEX(idx)		(0 != ((idx)&CASHSHOP_BIT_NEW))				// 2009-01-28 by cmkwon, 캐쉬샾 수정(추천탭,신상품 추가) - 신상품 체크
#define IS_RECOMMEND_CASHSHOPITEM_BY_CASHSHOPINDEX(idx)	(0 != ((idx)&CASHSHOP_BIT_RECOMMEND))		// 2009-01-28 by cmkwon, 캐쉬샾 수정(추천탭,신상품 추가) - 추천상품 체크
...
#define BUILDINGKIND_DESTROY_AUCTION	(BYTE)40	// 2014-01-03 by jhseol&bckim, 파괴경매장 - 건물 종류 추가
...
#define ITEM_IN_MARKET					3   // 2013-11-25 by jhseol&bckim, 거래소 - 거래중개소에 등록된 아이템
#define ITEM_IN_DESTROY_AUCTION			4   // 2014-01-03 by jhseol&bckim, 파괴경매장 - 파괴경매장에 등록된 아이템
...
#define DES_CASH_CHANGE_INFLUENCE			120		// 세력 변경 카드 
...
// 2014-06-17 by bckim, 세력 불균형 ( 초심자 아이템 전투력 계산 제외 )
#define DES_BEGINNER_ITEM							249		// 2014-06-17 by bckim, 세력 불균형 ( 초심자 아이템 전투력 계산 제외 )
// End. 2014-06-17 by bckim, 세력 불균형 ( 초심자 아이템 전투력 계산 제외 )
// 2014-06-17 by bckim, 세력 불균형 ( 역전의 기회 WP 획득량 증가 )
#define DES_PLUS_WARPOINT_RATE_SHOW_TOOLTIP			250		// 서버에서 사용하지 않음. 툴팁용 데스파람 추가 - 역전의 기회 
// End. 2014-06-17 by bckim, 세력 불균형 ( 역전의 기회 WP 획득량 증가 )
...
#define DES_PREMIUM_COLLECTION_ARMOR_INDEX			391		// 프리미엄 아머 컬렉션에서 사용될 인덱스
// End. 2014-07-17 by bckim, 프리미엄 아머 컬렉션

// 2013-08-07 by bckim, 미션프리패스 아이템
#define DES_QUEST_MISSION_FREE_PASS					410		// 프리패스 아이템 구분용 데스파람
#define DES_QUEST_MISSION_FREE_PASS_MIN_LV			411		// 프리패스 아이템 사용 퀘스트 최소 레벨
#define DES_QUEST_MISSION_FREE_PASS_MAX_LV			412		// 프리패스 아이템 사용 퀘스트 최대 레벨 
// End. 2013-08-07 by bckim, 미션프리패스 아이템

// 2013-08-30 by bckim, 필드이동금지아이템
#define DES_MOVEMENT_RESTRICTION_GIVE				420		// 지도자가 사용하는 카드 아이템 
#define DES_MOVEMENT_RESTRICTION_TAKE				421		// 케릭터에게 부여되는 스킬	
#define DES_MOVEMENT_RESTRICTION_TAKE_ITEMNUM		7044190	// 케릭터에게 부여되는 스킬아이템 넘버 
// End. 2013-08-30 by bckim, 필드이동금지아이템
...
#define DES_WEBPAGE_OPEN							505		// 아이템 사용시 웹페이지를 뛰운다 // 2013-09-27 by jhseol, 1주년 이벤트
...
// 2014-07-17 by bckim, 프리미엄 아머 컬렉션
#define DES_PREMIUM_OPTION_ITEM_DEFAULT_DESPARAM	601		// 이달의 프리미엄 아머에 적용될 옵션 아이템인지 구분할 DesParam
// End. 2014-07-17 by bckim, 프리미엄 아머 컬렉션
// 2014-12-29 by shchoi 이달의 구 아머
#define DES_OLD_OPTION_ITEM_DEFAULT_DESPARAM		602		// 이달의 구 아머에 적용될 옵션 아이템인지 구분할 DesParam
...
{
#ifdef _RAT_RANK_SYSTEM
	int					RankID;							// 2012-04-12 by jhseol, 아레나 추가개발 - 아레나 Rank ID
#endif // _RAT_RANK_SYSTEM
...
struct SITEM_EVENT_INFO
{
	...
	int				OnlyPCBang;
	int				ChannelingType;			// 2014-02-10 by jhseol&bckim, 체널링		
	...
	INT				CheckWithCharacterUID;	// 0 =  Account UID , 1 = 캐릭터 UID , 2 = CI값 체크. // 2014-03-12 by jekim, 주민번호당 아이템 지급
	INT				iLevelMin;
	INT				iLevelMax;
	...
	ATUM_DATE_TIME  GiveItemAtTime;			// 2013-09-30 by bckim, 특정시간 아이템지급 
...
#define ITEM_EVENT_CHECKWITH_ACCOUNTEXT_UID 2 // CI값 체크
...
#define ITEM_EVENT_TYPE_AT_TIME					9	// 2013-09-30 by bckim, 특정시간 아이템지급 
...
struct MONTHLY_ARMOR_EVNET_INFO
{
	...
	INT				nDuration;			// 옵션 유지 기간(일)	// 2013-09-06 by jhseol, 이달의 아머 이벤트 적용 기간 변경
};
	...
	int GetMaxBulletCount(BOOL i_bIsMembershipUser, BOOL i_bIsEffectInWingout = FALSE)
	{
		...
		if (FALSE == i_bIsMembershipUser)
		{
			if (TRUE == i_bIsEffectInWingout)
			{
				...
				// 2015-02-23 by shchoi 이펙트 장착 고급무기 탄약이 비정상적으로 많이 충전되는 버그 수정
			}
...
struct SCASH_PREMEIUM_CARD_INFO
{
	...
	void ResetAllPlusRateByPremiumCard(int i_nLangTy)
	{
		...
		case LANGUAGE_TYPE_CHINESE:
			{// 2007-11-06 by cmkwon, 중국에도 멤버쉽 혜택 추가 - 경험치 15%
				fExpRate			= 0.15f;
			}
			break;
		case LANGUAGE_TYPE_TAIWANESE:
			{// 2008-12-15 by cmkwon, 대만 Netpower_tpe 에 멤버쉽 추가 혜택(경험치 20%) 추가 - 
				fExpRate			= 0.2f;
			}
			break;
		}
// 2007-11-06 by cmkwon, 중국에도 멤버쉽 혜택 추가 - 위와 같이 수정 함
...
// 2013-12-16 by jhseol, 역전의 기회 버프 리뉴얼 - 전진 기지전 보상 WP 추가
#ifdef S_UPGRADE_INFLUENCE_POINT_RENEWAL_JHSEOL		// - 전진 기지전 보상 WP 추가
#define	OUTPOST_PAY_WARPOINT			(600+400)	// 400WP 추가
#define	OUTPOST_PAY_WARPOINT_BY_LOSE	(200+400)	// 400WP 추가
#else
...
#endif
// end 2013-12-16 by jhseol, 역전의 기회 버프 리뉴얼 - 전진 기지전 보상 WP 추가
...
{// 2008-04-01 by dhjin, 거점전 전장정보
	...
	BYTE			WinInfluence;
	BYTE			SPLevel;			// 2013-08-14 by jhseol, 거점전 리뉴얼 - 거점전 결과 로그때 남길 거점 레벨
};
...
#define WPUT_MARKET					9		// 2013-11-25 by jhseol&bckim, 거래소
...
///////////////////////////////////////////////////////////////////////////////
// 2015-01-07 by jwLee 게임포지 nProtector 적용
...
// 2013-10-09 by jhseol, X-Trap + VTC 게임가드
...
// end 2013-10-09 by jhseol, X-Trap + VTC 게임가드

//#if defined(SERVICE_TYPE_VIETNAMESE_SERVER_1)		// 2012-02-09 by hskim, 베트남 X-Trap -> VTC 가드로 변경
...
#endif
// End. 2015-07-21 by bckim, 만렙 확장 ( 110 >> 115 )
...
#endif
// end 2013-08-14 by jhseol, 거점전 리뉴얼 - 세력포인트 비율 변경
// 2013-12-16 by jhseol, 역전의 기회 버프 리뉴얼 - 디파인 수정
#ifdef S_UPGRADE_INFLUENCE_POINT_RENEWAL_JHSEOL		// - 디파인 수정
#define TURN_AROUND_BUFF_SKILL_MAXIMUM_POINT_GAP	5000	// 버프 적용 최대 세력포인트차 2만점, 버프 비율로 환산하면 [0.006*5000 = 30%]
#else
...
#endif
// end 2013-12-16 by jhseol, 역전의 기회 버프 리뉴얼 - 디파인 수정
#define TURN_AROUND_BUFF_SKILL_100P_VALUE			0.006f	// 2013-10-31 by jhseol, 역전의 버프 너프	// 2013-08-01 by jhseol, 역전의 버프 리뉴얼 - 세력포인트 1포인트당 버프 증가 폭
// end 2013-05-09 by hskim, 세력 포인트 개선
...
struct COLLECTION_INFO
{
	...
	ItemNum_t		nOptionItemNum;		// 2013-09-06 by jhseol, 이달의 아머 이벤트 적용 기간 변경 - 이달의 아머 옵션 아이템 번호
	ATUM_DATE_TIME	DurationTime;		// 2013-09-06 by jhseol, 이달의 아머 이벤트 적용 기간 변경 - 이달의 아머 옵션 종료시간
...
// 2013-08-14 by jhseol, 거점전 리뉴얼 - [ti_MonsterGrade] 테이블 구조체 선언
#define MONSTER_GRADE_TYPE_STRATEGYPOINT		1
#define WARINFO_DISPLAY_TYPE_MSWAR				0	// 전쟁정보 출력타입 (0:모선)
#define WARINFO_DISPLAY_TYPE_SPWAR				1	// 전쟁정보 출력타입 (1:일반 거점)
#define WARINFO_DISPLAY_TYPE_NGCSPWAR			2	// 전쟁정보 출력타입 (2:NGC 거점)
#define FIEXD_MSWAR_SPLEVEL						6	// 모선전 시 소환되는 거점 lv
#define SPLEVEL_INIT							5	// 거점전 초기화 레벨
#define SPLEVEL_MIN								1	// 거점전 최소 레벨
// 2014-03-25 by jekim, 승리의 환호
#ifdef S_ACCLAIM_OF_VICTORY_JEKIM
#define SPLEVEL_MAX								20	// 거점전 최대 레벨
#else
	#ifdef SC_UNBALANCED_FORCES_ADJUST_SPLEVEL_MAX		// 2014-07-10 by bckim, 세력 불균형(승리의환호제거시 거점레벨 MAX 조정
		#define SPLEVEL_MAX								20	// 거점전 최대 레벨
	#else 
        #define SPLEVEL_MAX								10	// 거점전 최대 레벨
	#endif
#endif
// end 2014-03-25 by jekim, 승리의 환호
#define TRIGGER_FIXEDTIMETYPR_NOMAL				0	// 트리거 시간 (일반)
#define TRIGGER_FIXEDTIMETYPR_FIXED				1	// 트리거 시간 (시간대 고정 오후6~12시)
#define TRIGGER_FIXEDTIMETYPR_SPTIME			2	// 트리거 시간 (거점용 소환시간)
struct MONSTER_GRADE_INFO
{
	BYTE MonsterGeadeType;	// 타입 (1. 거점몬스터)
	BYTE InflType;			// 세력
	BYTE Level;				// 레벨 
	INT MonsterNum;			// 해당 레벨의 몬스터넘버
	INT BeforeMonsterNum;	// 이전 레벨의 몬스터넘버
	INT AfterMonsterNum;	// 다음 레벨의 몬스터넘버
	MONSTER_GRADE_INFO()
	{
...
// end 2013-08-14 by jhseol, 거점전 리뉴얼 - [ti_MonsterGrade] 테이블 구조체 선언


// 2013-11-25 by jhseol&bckim, 거래소 - 해더 추가
#define MARKET_VECTOR_SIZE								400000	// 계정당 보유캐릭터 4게(확장을 고려함), 보유가능 계정 5게(부계정의 계념), 유저 1000명 기준 하루에 20게씩 올릴수 있는 수 4*5*1000*20=40만
#define MARKET_VIEW_LIST_COUNT							8		// 한페이지에 보여주는 리스트
#define MARKET_SELL_ITEM_COUNT							10		// 등록가능한 아이템의 수
#define MARKET_MONEY_TYPE_SPI							0		// SPI 판매
#define MARKET_MONEY_TYPE_WP							1		// WP 판매
#define MARKET_ITEM_FULL_NAME							140		// 접두, 점미 포함된 아이템 이름
#define MARKET_PACKET_RECV_TICK_BUY_PAGE				1000	// 거래소 관련 연속 패킷 제한 텀 - 구매페이지
#define MARKET_PACKET_RECV_TICK_SELL_PAGE				1000	// 거래소 관련 연속 패킷 제한 텀 - 등록페이지
#define MARKET_PACKET_RECV_TICK_MY_LIST_PAGE			1000	// 거래소 관련 연속 패킷 제한 텀 - 현황페이지
#define MARKET_PACKET_RECV_TICK_GET_REQUEST				1000	// 거래소 관련 연속 패킷 제한 텀 - 회수요청
#define MARKET_PACKET_RECV_TICK							1000	// 거래소 관련 연속 패킷 제한 텀

#if defined(TEST140) || defined(_DEBUG)
#define MARKET_SELLING_MINUTES							(30)	// 테스트 30분 
#else
#define MARKET_SELLING_MINUTES							(10080)	// 거래소 판매 유지시간 기간 7일 60분*24시간*7일 = 10080
#endif

#define MARKET_SELL_CHARGE_SPI							0.005f	// 거래소 판매 SPI 수수료 0.5%
#define MARKET_SELL_CHARGE_WP							0.005f	// 거래소 판매 WP 수수료 0.5%

inline INT GetMarketSellChargeSPI(INT i_nPrice)	{return max(1, i_nPrice*MARKET_SELL_CHARGE_SPI);}	// SPI 수수료를 계산하는 함수
inline INT GetMarketSellChargeWP(INT i_nPrice)	{return max(1, i_nPrice*MARKET_SELL_CHARGE_WP);}	// WP 수수료를 계산하는 함수
// MARKET_STATE_TYPE									// 상태 타입
#define MARKET_STATE_SELL					1			// 판매중
#define MARKET_STATE_SELL_DONE				2			// 판매완료
#define MARKET_STATE_TIME_OUT				3			// 기간만료
#define MARKET_STATE_DELETE					4			// 거래완료(삭제대상)
#define MARKET_GET_SELLING					5			// 거래회수 (판매중)	// 클라이언트 응답용 
#define MARKET_GET_EXPIRATION				6			// 거래회수 (기간만료)	// 클라이언트 응답용 
#define MARKET_GET_SOLD						7			// 거래회수 (판매완료)	// 클라이언트 응답용 

// MARKET_SORT_TYPE										// 정렬 타입
...
// MARKET_ITEM_KIND
#define MARKET_ITEM_KIND_ALL							0	//	전체
#define		MARKET_ITEM_KIND_WEAPON						1	//	무기
#define			MARKET_ITEM_KIND_PRIMARY_WEAPON			2	//		기본무기
#define				MARKET_ITEM_KIND_VULCAN				3	//			발칸
#define				MARKET_ITEM_KIND_CANNON				4	//			캐논
#define				MARKET_ITEM_KIND_GATLING			5	//			개틀링
#define				MARKET_ITEM_KIND_RIFLE				6	//			라이플
#define				MARKET_ITEM_KIND_AUTOMATIC			7	//			오토매틱
#define				MARKET_ITEM_KIND_DUALIST			8	//			듀얼리스트
#define				MARKET_ITEM_KIND_MASSDRIVE			9	//			메스드라이브
#define			MARKET_ITEM_KIND_SECONDARY_WEAPON		10	//		고급무기
#define				MARKET_ITEM_KIND_MISSILE			11	//			미사일
#define				MARKET_ITEM_KIND_BUNDLE				12	//			번들
#define		MARKET_ITEM_KIND_ARMOR						13	//	아머
#define			MARKET_ITEM_KIND_VEIL					14	//		베일
#define			MARKET_ITEM_KIND_DEFENDER				15	//		디펜더
#define			MARKET_ITEM_KIND_GUARDER				16	//		가더
#define			MARKET_ITEM_KIND_BINDER					17	//		바인더
#define		MARKET_ITEM_KIND_RADAR						18	//	레이더
#define			MARKET_ITEM_KIND_RADAR_B				19	//		레이더 B-Gear
#define			MARKET_ITEM_KIND_RADAR_M				20	//		레이더 M-Gear
#define			MARKET_ITEM_KIND_RADAR_A				21	//		레이더 A-Gear
#define			MARKET_ITEM_KIND_RADAR_I				22	//		레이더 I-Gear
#define		MARKET_ITEM_KIND_SUPPORT					23	//	보조장비
#define			MARKET_ITEM_KIND_UNLIMITED_ACCESSORY	24	//		무제한 악세사리
#define			MARKET_ITEM_KIND_TIMELIMIT_ACCESSORY	25	//		시간제 악세사리
#define			MARKET_ITEM_KIND_COMPUTER				26	//		컴퓨터(CPU)
#define			MARKET_ITEM_KIND_MARK					27	//		마크
#define		MARKET_ITEM_KIND_ENGIN						28	//	엔진
#define			MARKET_ITEM_KIND_ENGIN_B				29	//		엔진 B-Gear
#define			MARKET_ITEM_KIND_ENGIN_M				30	//		엔진 M-Gear
#define			MARKET_ITEM_KIND_ENGIN_A				31	//		엔진 A-Gear
#define			MARKET_ITEM_KIND_ENGIN_I				32	//		엔진 I-Gear
#define		MARKET_ITEM_KIND_CONSUMABLE					33	//	소모품
#define			MARKET_ITEM_KIND_ENERGY					34	//		회복키트
#define			MARKET_ITEM_KIND_GAMBLE					35	//		갬블카드
#define			MARKET_ITEM_KIND_ENCHANT				36	//		인첸카드
#define			MARKET_ITEM_KIND_CARD					37	//		소모품카드
#define			MARKET_ITEM_KIND_RANDOMBOX				38	//		행운의상자
#define		MARKET_ITEM_KIND_ETC						39	//	기타
#define			MARKET_ITEM_KIND_MATERIAL				40	//		조합재료
...
inline BYTE GetMarketItemKindByOriginItemKind(ITEM i_itemInfo)
{
	...
	case ITEMKIND_PREVENTION_DELETE_ITEM:			return MARKET_ITEM_KIND_ENCHANT;	// 인첸 파방, 확률 카드
	case ITEMKIND_ENCHANT:							return MARKET_ITEM_KIND_ENCHANT;	// 인첸 카드
...
struct MARKET_ENCHANT_INFO // 인첸트 세부 정보
{
	ItemNum_t		ItemNum;			// 인첸카드 아이템넘버
	BYTE			Count;				// 인첸 횟수
};
struct MARKET_INFO // 거래소 등록 아이템 정보
{
	UID64_t			MarketUID;			// 거래소 고유번호
	UID64_t			ItemUID;			// 아이템 고유번호
	UID32_t			AccountUID;			// 소유자 계정 UID
	UID32_t			CharacterUID;		// 소유자 캐릭터 UID
	BYTE			ItemKind;			// 아이템 종류
	char			ItemName[MARKET_ITEM_FULL_NAME];	// 아이템 이름(접두, 접미 포함)
	ItemNum_t		ItemNum;			// 아이템 번호
	ItemNum_t		ItemCount;			// 판매 수량(무기일 경우 총알 수량)
	ItemNum_t		PrefixCodeNum;		// 접두 옵션(없으면 0)
	ItemNum_t		SuffixCodeNum;		// 접미 옵션(없으면 0)
	BYTE			EnchantCount;		// 인첸트 수(없으면 0)
	MARKET_ENCHANT_INFO	Enchant[8];		// 인첸트 세부 정보
	ItemNum_t		ShapeItemNum;		// 아머는 외형 정보, 고급무기는 이펙트 정보
	INT				ColorCode;			// 무기 이펙트 정보
	BYTE			ItemLevel;			// 사용가능 레벨
	USHORT			ItemGear;			// 사용가능 기어
	ATUM_DATE_TIME	RegistTime;			// 거래소 등록시간
	BYTE			MoneyType;			// 화폐 타입(0=SPI, 1=WP)
	INT				Price;				// 판매금액
	BYTE			MarketState;		// 아이템에 대한 거래소 상태(판매중, 판매완료, 기간만료 등등)
	BYTE			FindNameIndex;		// 이름검색시 최근 검색한 이름의 인덱스 위치(서버에서만 검색할때 사용)
	
	BYTE			MarketItemInfl;		// 2014-06-17 by bckim, 세력 불균형 ( 거래소 동일세력간에만 거래가 이루어지게 수정 )			
...
struct MARKET_LIST_SORT_PRICE_UP
{
	...
	{
		return op1.Price < op2.Price;		// 오름차순 정렬
	}
};
struct MARKET_LIST_SORT_PRICE_DOWN
{
	bool operator()(MARKET_INFO op1, MARKET_INFO op2)
	{
		return op1.Price > op2.Price;		// 내림차순 정렬
	}
};
//////////////////////////////////
// 레벨별 정렬
struct MARKET_LIST_SORT_LEVEL_UP
{
	bool operator()(MARKET_INFO op1, MARKET_INFO op2)
	{
		return op1.ItemLevel < op2.ItemLevel;		// 오름차순 정렬
	}
};
struct MARKET_LIST_SORT_LEVEL_DOWN
{
	bool operator()(MARKET_INFO op1, MARKET_INFO op2)
	{
		return op1.ItemLevel > op2.ItemLevel;		// 내림차순 정렬
	}
};
//////////////////////////////////
// 인첸트별 정렬
struct MARKET_LIST_SORT_ENCHANT_UP
{
	bool operator()(MARKET_INFO op1, MARKET_INFO op2)
	{
		return op1.EnchantCount < op2.EnchantCount;		// 오름차순 정렬
	}
};
struct MARKET_LIST_SORT_ENCHANT_DOWN
{
	bool operator()(MARKET_INFO op1, MARKET_INFO op2)
	{
		return op1.EnchantCount > op2.EnchantCount;		// 내림차순 정렬
	}
};
//////////////////////////////////
// 시간별 정렬
struct MARKET_LIST_SORT_TIME_UP
{
	bool operator()(MARKET_INFO op1, MARKET_INFO op2)
	{
		return op1.MarketUID < op2.MarketUID;		// 오름차순 정렬
	}
};
struct MARKET_LIST_SORT_TIME_DOWN
{
	bool operator()(MARKET_INFO op1, MARKET_INFO op2)
	{
		return op1.MarketUID > op2.MarketUID;		// 내림차순 정렬
	}
};
//////////////////////////////////
// 이름별 정렬		strcmp 리턴 결과 : [ 0 : s1 = s2 ] , [ -1 : s1 < s2 ] , [ 1 : s1 > s2 ]
struct MARKET_LIST_SORT_NAME_UP
{
	bool operator()(MARKET_INFO op1, MARKET_INFO op2)
	{
		return 0 > strcmp(op1.ItemName, op2.ItemName);	// 오름차순 정렬
	}
};
struct MARKET_LIST_SORT_NAME_DOWN
{
	bool operator()(MARKET_INFO op1, MARKET_INFO op2)
	{
		return 0 < strcmp(op1.ItemName, op2.ItemName);	// 내림차순 정렬
	}
};
//////////////////////////////////
// 문자열에서 단어 검색
		...
		if (string[i] == word[0])
		{
			if ( 1 < Lw )
			{
				for(j=1; j<Lw; j++)
				{
					...
					if(j == Lw-1)				{ return i+j+1;	}    //마지막글자의 인덱스 리턴 
				} 
			}
			else	{ return i+1; }
		}
...
// end 2013-11-25 by jhseol&bckim, 거래소 - 해더 추가

// 2014-01-03 by jhseol&bckim, 파괴경매장 - 해더 추가

// DESROY_AUCTION_ITEM_STATE							// 상태 타입
#define AUCTION_ITEM_STATE_INIT				1			// 최초 초기 단계 
#define AUCTION_ITEM_STATE_START			2			// 경매 시작되면 변경 >> 2
#define AUCTION_ITEM_STATE_3				3			// 한번이라도 입찰이 진행되었다니 ? ( DB에서만 쓸듯 )
#define AUCTION_ITEM_STATE_END				4			// 경매종료 (낙찰) : 회수 가능 상태  
#define AUCTION_ITEM_STATE_COMPLETE			5			// 회수 완료 : 정보 삭제 대상 	
#define AUCTION_ITEM_STATE_6              	6			// 	

#if defined(TEST140)
	#define DESTROY_AUCTION_RUN_TIME_MINUTE				30			// 경매 진행 시간(분)
	#define DESTROY_AUCTION_RUN_READY_TIME_MINUTE		1			// 경매 시작 전 준비 시간
	#define DESTROY_AUCTION_RUN_GAP_TIME_HOUR			1			// 경매 시작시간 텀
	#define DESTROY_AUCTION_PACKET_RECV_TICK			200			// 거래소 관련 연속 패킷 제한 텀
	#define DESTROY_AUCTION_INIT_PRICE					100000		// 경매 시작가 10만SPI
	#define DESTROY_AUCTION_GET_POSSIBLE_TIME_HOUR		2			// 낙찰품 회수 가능 시간
	#define DESTROY_AUCTION_TENDET_ITEM_NUM_LIMITED		7045530		// 기간제 입찰권
	#define DESTROY_AUCTION_TENDET_ITEM_NUM_UNLIMITED	7045540		// 무제한 임찰권
	#define DESTROY_AUCTION_LISTUP_ITEM_ENCHANT_TYPE_1	3			// 엔진, 레이더, 보조장비 리스트업 인챈 최소값
	#define DESTROY_AUCTION_LISTUP_ITEM_ENCHANT_TYPE_2	10			// 기본, 고급무기, 아머 리스트업 인챈 최소값
#else
	#define DESTROY_AUCTION_RUN_TIME_MINUTE				60			// 경매 진행 시간(분)
	#define DESTROY_AUCTION_RUN_READY_TIME_MINUTE		1			// 경매 시작 전 준비 시간
	#define DESTROY_AUCTION_RUN_GAP_TIME_HOUR			3			// 경매 시작시간 텀
	#define DESTROY_AUCTION_PACKET_RECV_TICK			200			// 거래소 관련 연속 패킷 제한 텀
	#define DESTROY_AUCTION_INIT_PRICE					100000		// 경매 시작가 10만SPI
	#define DESTROY_AUCTION_GET_POSSIBLE_TIME_HOUR		24			// 낙찰품 회수 가능 시간
	#define DESTROY_AUCTION_TENDET_ITEM_NUM_LIMITED		7045530		// 기간제 입찰권
	#define DESTROY_AUCTION_TENDET_ITEM_NUM_UNLIMITED	7045540		// 무제한 임찰권
	#define DESTROY_AUCTION_LISTUP_ITEM_ENCHANT_TYPE_1	3			// 엔진, 레이더, 보조장비 리스트업 인챈 최소값
	#define DESTROY_AUCTION_LISTUP_ITEM_ENCHANT_TYPE_2	10			// 기본, 고급무기, 아머 리스트업 인챈 최소값
...
struct DESTROY_AUCTION_INFO
{
	...
	BYTE			MoneyType;			// 화폐 타입(0=SPI, 1=WP)
	INT				Price;				// 최고입찰금액
	INT				MyTenderPrice;		// 내가 입찰한 금액(입찰현황에서만 사용)
	ATUM_DATE_TIME	DestroyedTime;		// 아이템 파괴(등록)시간 
	BYTE			DAuctionState;		// 아이템에 대한 경매 상태(경매중, 유찰, 낙찰 등등)
	UID32_t			AccountUID;			// 최고입찰자 계정 UID
	UID32_t			CharacterUID;		// 최고입찰자 캐릭터 UID
	ATUM_DATE_TIME	DAuctionEndTime;	// 낙찰품 회수 가능 시간
...
// end 2014-01-03 by jhseol&bckim, 파괴경매장 - 해더 추가
