...
CWinSocket::CWinSocket(HWND hWnd, UINT wmSock, UINT wmNotify, BOOL bEncoding, char* szNPlayServerIP_Real/* = ""*/, char* szNPlayServerIP_Real_BackUp/* = ""*/) // 2012-10-05 by jhseol, NPlay pc방 모듈 - NPlay 서버 IP 인자 추가
{
	m_hMsgWnd			= hWnd;
	...
	STRNCPY_MEMSET(m_szNPlayServerIP_Real, szNPlayServerIP_Real, sizeof(m_szNPlayServerIP_Real));
	STRNCPY_MEMSET(m_szNPlayServerIP_BackUp, szNPlayServerIP_Real_BackUp, sizeof(m_szNPlayServerIP_BackUp));
...
//	DbgOut("[TEMP] 090710 Frost frostWSARecv After  Peer(%s:%d) nRet(%d) ReceivedBytes(%d) \r\n", inet_ntoa(m_SockAddrRemote.sin_addr), ntohs(m_SockAddrRemote.sin_port), nRet, nBytesRecvd);
	...
	while(nBytesRecvd > 0)
	{
		// 2012-10-05 by jhseol, NPlay pc방 모듈 - NPlay 소켓이면 해더 테일 제거하고 리시브 받기
#ifdef S_NPLAY_PCBANG_MODULE_JHSEOL
		char destIParrd[SIZE_MAX_IPADDRESS] = {0,};
		STRNCPY_MEMSET(destIParrd, inet_ntoa(m_SockAddrRemote.sin_addr), sizeof(destIParrd));
		if( strlen(m_szNPlayServerIP_Real) == strlen(destIParrd) || strlen(m_szNPlayServerIP_BackUp) == strlen(destIParrd) )
		{
			if( 0 == strncmp(m_szNPlayServerIP_Real, destIParrd, strlen(m_szNPlayServerIP_Real)) || 
				0 == strncmp(m_szNPlayServerIP_BackUp, destIParrd, strlen(m_szNPlayServerIP_BackUp))) 
			{
				nRet = OnRecvdPacket(wsaBuf.buf, wsaBuf.len, 0);
				if(nRet == FALSE)
				{	// Protocol Error
					CloseSocket(0x10002);
...
#endif
		// end 2012-10-05 by jhseol, NPlay pc방 모듈 - NPlay 소켓이면 해더 테일 제거하고 리시브 받기
