...
#define PROCEDURE_080822_0247 (UCHAR*)"{call dbo.atum_log_insert_connection(?,?,?,?,?, ?,?)}"				// 2010-11 by dhjin, 아라리오 채널링 로그인.
...
#define PROCEDURE_080822_0264 (UCHAR*)"{call dbo.atum_log_Insert_SPWar(?,?,?,?,?,?,?,?)}"		// 2013-08-14 by jhseol, 거점전 리뉴얼 - 거점 로그 저장
...
#define PROCEDURE_080827_0026 (UCHAR*)"{call dbo.atum_PROCEDURE_080827_0026(?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,? ,?)}"	// 2013-08-12 by bckim, 신규해피아워
#define PROCEDURE_080827_0027 (UCHAR*)"{call dbo.atum_PROCEDURE_080827_0027(?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?)}"	// 2013-08-12 by bckim, 신규해피아워
...
#define PROCEDURE_080827_0264 (UCHAR*)"{call dbo.atum_PROCEDURE_080827_0264(?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?)}"		// 2013-08-12 by bckim, 신규해피아워
#define PROCEDURE_080827_0265 (UCHAR*)"{call dbo.atum_PROCEDURE_080827_0265(?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?)}"		// 2013-08-12 by bckim, 신규해피아워	

#define PROCEDURE_080827_0266 (UCHAR*)"{call dbo.atum_PROCEDURE_080827_0266(?,?,?,?,?,?,?,?,?,?, ?,?,?,?,?,?,?,?,?,? ,?,?,?)}"		
// 2013-02-28 by bckim, 복귀유져 버프추가 // 인자추가		// 2013-09-30 by bckim, 특정시간 아이템지급 // 인자추가	// 2014-02-10 by jhseol&bckim, 체널링
...
///////////////////////////////////////////////////////////////////////////////
// 2009-01-28 by cmkwon, 캐쉬샾 수정(추천탭,신상품 추가) - 프로시저 2개 추가
...
#define PROCEDURE_120921_0001 (UCHAR*)"{call dbo.arena_CopyDBInfo_EX_GAMEFORGE_VERSION(?,?)}"	// 2012-06-08 by jhseol, 아레나 추가개발part2 - 케릭터 : 타입 복사 파람 추가
...
///////////////////////////////////////////////////////////////////////////////
#define PROCEDURE_130228_0001 (UCHAR*)"{call dbo.atum_update_Log_ItemEvent_fixed_period(?, ?, ?, ?, ?)}"	// 2014-03-12 by jekim, 주민번호당 아이템 지급	// 2013-02-28 by bckim, 복귀유져 버프추가
	   
//#define PROCEDURE_GLOG_3ND_130313_0001 (UCHAR*)"{call dbo.GLog_Account_Info_New_Insert(?, ?, ?, ?)}"		// 2013-03-14 by bckim, Glog 기능추가
#define PROCEDURE_GLOG_3ND_130313_0001 (UCHAR*)"{call dbo.GLog_Account_Info_New_Insert(?, ?, ?, ?, ?)}"		// 2014-02-10 by jhseol&bckim, 체널링 - 한국 유저 정보 추가저장
...
#define PROCEDURE_130503_0001 (UCHAR*)"{call dbo.atum_InsertMonthlyArmorEvent(?,?,?,?,?,?,?)}"		// 2013-09-06 by jhseol, 이달의 아머 이벤트 적용 기간 변경 - 파라미터 추가
...
#define PROCEDURE_130503_0005 (UCHAR*)"{call dbo.atum_GetMonthlyOptionList(?,?,?)}" // 2014-12-29 by shchoi 이달의 구 아머
...
#define PROCEDURE_130531_0002 (UCHAR*)"{call dbo.atum_UpdateCollectionInfo(?,?,?,?,?, ?,?,?,?)}"		// 2013-09-06 by jhseol, 이달의 아머 이벤트 적용 기간 변경
...
// 2013-08-14 by jhseol, 거점전 리뉴얼
#define PROCEDURE_130814_0001 (UCHAR*)"{call dbo.atum_Get_Summon_SPLevel(?)}"
#define PROCEDURE_130814_0002 (UCHAR*)"{call dbo.atum_Load_ti_MonsterGrade}"
#define PROCEDURE_130814_0003 (UCHAR*)"{call dbo.atum_delete_ti_MonsterGrade}"
#define PROCEDURE_130814_0004 (UCHAR*)"{call dbo.atum_import_ti_MonsterGrade(?,?,?,?,?,?)}"
#define PROCEDURE_130814_0005 (UCHAR*)"{call dbo.atum_LoadNGCSPCreateTime}"
#define PROCEDURE_130814_0006 (UCHAR*)"{call dbo.atum_InsertNGCSPCreateTime(?)}"
// end 2013-08-14 by jhseol, 거점전 리뉴얼


// 2013-08-28 by bckim, 보스몬스터 보상강화
#define PROCEDURE_130828_0001 (UCHAR*)"{call dbo.atum_Load_ti_BossReward}"
#define PROCEDURE_130828_0002 (UCHAR*)"{call dbo.atum_delete_ti_BossReward}"					// 모니터툴 : DELETE 
#define PROCEDURE_130828_0003 (UCHAR*)"{call dbo.atum_insert_ti_BossReward(?,?,?,?,?)}"			// 모니터툴 : INSERT
// End. 2013-08-28 by bckim, 보스몬스터 보상강화

#define PROCEDURE_131119_0001 (UCHAR*)"{call dbo.atum_log_insert_item_mix(?, ?, ?, ?, ?, ?, ?)}" // 2013-11-20 by jekim, 조합, 융해 툴에서 볼 수 있게 수정.


// 2013-11-25 by jhseol&bckim, 거래소
#define PROCEDURE_131126_0001 (UCHAR*)"{call dbo.atum_Market_Insert_ItemRegist(?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,? )}"	// 등록			// 2013-11-26 by bckim, 거래소시스템			// 2014-06-17 by bckim, 세력 불균형 ( 거래소 동일세력간에만 거래가 이루어지게 수정 )
#define PROCEDURE_131126_0002 (UCHAR*)"{call dbo.atum_Market_Get_Enchant_Info(?)}"						// 등록_ 마켓용 인챈트 관련 
#define PROCEDURE_131126_0003 (UCHAR*)"{call dbo.atum_Market_Get_td_Enchant(?)}"						// 구매시 아이템용 인챈트정보 
#define PROCEDURE_131126_0004 (UCHAR*)"{call dbo.atum_Market_BuyTradeItem(?,?,?,?,?, ?,? )}"			// 구매 
#define PROCEDURE_131126_0005 (UCHAR*)"{call dbo.atum_Market_GetTradeItem(?,?,?,?,?, ?,?)}"				// 회수
#define PROCEDURE_131126_0006 (UCHAR*)"{call dbo.atum_Market_Get_SaleList(?)}"							// 현황조회 
#define PROCEDURE_131126_0007 (UCHAR*)"{call dbo.atum_Market_Get_MarketList}"							// 전체리스트  
#define PROCEDURE_131126_0008 (UCHAR*)"{call dbo.atum_Market_RemoveInfo}"								// 완료된 거래소 정보 처리 
#define PROCEDURE_140423_0001 (UCHAR*)"{call dbo.atum_Market_Get_LogData(?,?,?,?,?, ?,?,?,?,?, ?,?,?)}"				// 운영툴 거래소 로그 

// End. 2013-11-25 by jhseol&bckim, 거래소

// 2013-12-16 by jhseol, 역전의 기회 버프 리뉴얼
#define PROCEDURE_131216_0001 (UCHAR*)"{call dbo.atum_GetTurnAroundBuffApplyInfo}"
#define PROCEDURE_131216_0002 (UCHAR*)"{call dbo.atum_UpdateTurnAroundBuffApplyInfo(?,?,?)}" // 2014-03-25 by jekim, 승리의 환호
// end 2013-12-16 by jhseol, 역전의 기회 버프 리뉴얼

// 2013-12-26 by bckim, 파괴 경매 시스템
#define PROCEDURE_131226_0001 (UCHAR*)"{call dbo.atum_DestroyAuction_AddItem(?, ?, ?)}"		// 파괴시 아이템 등록 
#define PROCEDURE_131226_0002 (UCHAR*)"{call dbo.atum_DestroyAuction_GetItemList(?, ?, ?)}"	// 경매 리스트 

#define PROCEDURE_131226_0003 (UCHAR*)"{call dbo.atum_DestroyAuction_AddItem(?, ?)}"
#define PROCEDURE_131226_0004 (UCHAR*)"{call dbo.atum_DestroyAuction_GetAuctionItem(?, ?, ?, ?, ?, ?)}"				// 낙찰물품 회수 
#define PROCEDURE_131226_0005 (UCHAR*)"{call dbo.atum_DestroyAuction_UpdateBiddingInfo(?, ?, ?, ?)}"	// 입찰 
#define PROCEDURE_131226_0006 (UCHAR*)"{call dbo.atum_DestroyAuction_GetBiddingInfo(?,?)}"					// 입찰한 물품 리스트 
#define PROCEDURE_131226_0007 (UCHAR*)"{call dbo.atum_DestroyAuction_SetClearDestroyAuction(?, ?, ?)}"		// 경매종료시..처리해야하는것들...
// End. 2013-12-26 by bckim, 파괴 경매 시스템

// 2014-05-12 by bckim, 파괴경매장 운영툴  // 경매물품 리스트 업 운영툴테이블 insert 
#define PROCEDURE_140512_0001 (UCHAR*)"{call dbo.atum_DestroyAuction_InsertDestroyAuctionListup(?, ?)}"		
#define PROCEDURE_140512_0002 (UCHAR*)"{call dbo.atum_DestroyAuction_Get_LogData_byItem(?,?,?,?)}"		
#define PROCEDURE_140512_0003 (UCHAR*)"{call dbo.atum_DestroyAuction_Get_LogData_byCondition(?,?,?,?,? ,?,?,?,?)}"		
// End. 2014-05-12 by bckim, 파괴경매장 운영툴

#define PROCEDURE_140127_0001 (UCHAR*)"{call dbo.GLog_BugTrap_Insert_Log(?,?,?,?,? ,?,?,?,?,? ,?,?,?,?,? ,?,?)}"		// 2014-01-27 by bckim, 버그 트랩

#define PROCEDURE_140206_0001 (UCHAR*)"{call dbo.atum_InitSPLevel(?,?)}"		// 2014-02-06 by jhseol, 세력포인트에 따른 거점 레벨링 밸런스 패치- 세력포인트 초기화 시 거점 레밸도 초기화

#define PROCEDURE_140217_0001 (UCHAR*)"{call dbo.atum_GetChannelingServerInfo}"			// 2014-02-10 by jhseol&bckim, 체널링

#define PROCEDURE_140312_0001 (UCHAR*)"{call dbo.atum_GetCashBuyDateByAccountExtUID(?)}" // 2014-03-12 by jekim, 주민번호당 아이템 지급

#define PROCEDURE_140314_0001 (UCHAR*)"{call dbo.atum_Insert_CombatPowerLog(?,?,?)}" // 2014-03-14 by jekim, 전투력 로그 추가

#define PROCEDURE_140326_0001 (UCHAR*)"{call dbo.atum_GetCombatPowerLatest}" // 2014-03-26 by jekim, 역전의 버프 전투력

#define PROCEDURE_140421_0001 (UCHAR*)"{call dbo.atum_InsertFirstPurchaseFromXMLRPC(?)}" // 2014-04-21 by jekim, 웹 캐시 상점 보완 - 첫구매
#define PROCEDURE_140421_0002 (UCHAR*)"{call dbo.atum_GetFirstPurchaseFromXMLRPC(?)}" // 2014-04-21 by jekim, 웹 캐시 상점 보완 - 첫구매

#define PROCEDURE_140425_0001 (UCHAR*)"{? = call dbo.atum_GetCombatPowerFromTOOL(?,?)}" // 2014-04-25 by jekim, 전투력 툴 작업

// 2014-06-10 by bckim, 세력변경 아이템
#define PROCEDURE_140610_0001 (UCHAR*)"{call dbo.atum_GetNumberOfChangeInfl(?)}"		// get
#define PROCEDURE_140610_0002 (UCHAR*)"{call dbo.atum_SetNumberOfChangeInfl(?,?)}"		// set
// End. 2014-06-10 by bckim, 세력변경 아이템

// 2014-07-14 by bckim, 워포샵 리스트 컨트롤
#define PROCEDURE_140714_0001 (UCHAR*)"{call dbo.atum_LoadWPShopItemList}"			
#define PROCEDURE_140714_0002 (UCHAR*)"{call dbo.atum_DeleteWPShopItemList}"		
#define PROCEDURE_140714_0003 (UCHAR*)"{call dbo.atum_InsertWPShopItemList(?)}"		
