...
CIOCPSocket::~CIOCPSocket()
{
	...
	for_each(m_mtlistWriteBuf.begin(), m_mtlistWriteBuf.end(), Delete_Object());
	m_mtlistWriteBuf.clear();
...
///////////////////////////////////////////////////////////////////////////////
/// \fn			BOOL CIOCPSocket::SendAddData(BYTE* pData, int nSize, EN_PACKET_PRIORITY i_enPacketPriority, BOOL bRawSend, , BOOL bSessionEnd)
...
/// \return
///////////////////////////////////////////////////////////////////////////////
BOOL CIOCPSocket::SendAddData(BYTE* pData, int nSize, EN_PACKET_PRIORITY i_enPacketPriority, BOOL bRawSend, BOOL bSessionEnd)		// 2013-03-13 by hskim, 웹 캐시 상점 - RawData 전송 기능 추가
{
	if(m_bUsing == FALSE 
...
//		DBGOUT(szSystemLog);
	...
	MEMSET_ZERO(&tmSendedOverInfo, sizeof(SSendedOverlappedInfo));
	...
	if( FALSE == bRawSend && FALSE == this->PreSendAddData(pData, nSize, m_mtlistWriteBuf.size(), &m_vectSendedOverlappedInfoList) )		// 2013-03-13 by hskim, 웹 캐시 상점 - RawData 전송 기능 추가
	{
		m_mtlistWriteBuf.unlock();
		return TRUE;
	}
	...
	// 2013-03-13 by hskim, 웹 캐시 상점 - RawData 전송 기능 추가
	BOOL bEncoding = TRUE;
	if( TRUE == m_bUDPFlag || TRUE == bRawSend ) bEncoding = FALSE;

	COverlapped::ENDataType DataType = COverlapped::ENDataType::EN_DATA_TYPE_ACEONLINE;
	if( TRUE == bRawSend ) DataType = COverlapped::ENDataType::EN_DATA_TYPE_RAW;
	// end 2013-03-13 by hskim, 웹 캐시 상점 - RawData 전송 기능 추가

	if(m_mtlistWriteBuf.empty() == true)
	{
		pOver = new COverlapped(COverlapped::EN_OPERATION_MODE_WRITE,
			COverlapped::EN_OVERLAPPED_TYPE_DYNAMIC, bEncoding, DataType);			// 2013-03-13 by hskim, 웹 캐시 상점 - RawData 전송 기능 추가
		...
		pOver->SetSessionEnd(bSessionEnd);		// 2013-03-13 by hskim, 웹 캐시 상점 - RawData 전송 기능 추가
	...
	if( FALSE == bRawSend )	this->OnSendAddData(&tmSendedOverInfo, nSize, m_mtlistWriteBuf.size());		// 2013-03-13 by hskim, 웹 캐시 상점 - RawData 전송 기능 추가
...
// 2013-03-13 by hskim, 웹 캐시 상점 - RawData 전송 기능 추가
BOOL CIOCPSocket::SendAddRawData(BYTE* pData, int nSize, BOOL bSessionEnd)
{
	return SendAddData(pData, nSize, EN_PACKET_PRIORITY_HIGHEST, TRUE, bSessionEnd);
}
// end 2013-03-13 by hskim, 웹 캐시 상점 - RawData 전송 기능 추가
...
void CIOCPSocket::OnReceive(char* pBlock, int length, ENServerType ServerType, char* pPeerIP, int nPeerPort, SThreadInfo *i_pThreadInfo)
{
	...
	if(this->m_bUDPFlag)
	{
		...
		case ST_NPC_SERVER:
			...
			break;
		case ST_AUTHENTICATION_SERVER:		// 2011-01-26 by hskim, 인증 서버 구현
			OnRecvdPacketAuthenticationServer(pBlock+SIZE_PACKET_HEADER, length - SIZE_PACKET_HEADER, bySeq, pPeerIP, nPeerPort, i_pThreadInfo);
			break;
		}
		return;
	}
	...
	if( ST_FIELD_WEB_SERVER == ServerType && this->IsUsing() && m_bFlagDelayClose == FALSE && nBytesRecvd > 0 )
	{
		// 여기부터 HTTP 레벨

		bRet = m_RecvHTTPPacket.AddTail(pBlock, nBytesRecvd, &nBytesUsed);

		if( bRet < 0 )
		{
			g_pGlobal->WriteSystemLogEX(TRUE, "[HTTP Error] Recved (Len : %d) ==========>\r\n%s\r\n<==========", m_RecvHTTPPacket.GetPacketLength(), m_RecvHTTPPacket.GetPacket());

			// HTTP 에러 메시지 전송
			const char *pSendHttpMessage = "<HEAD><TITLE>Internal Server Error</TITLE></HEAD>\r\n<BODY BGCOLOR=\"white\" FGCOLOR=\"black\">\r\n<FONT FACE=\"Helvetica,Arial\"><B>Internal Server Error</B></FONT>\r\n<!-- default \"Internal Server Error\" response (500) -->\r\n</BODY>\r\n";
			SendAddRawData((unsigned char *)pSendHttpMessage, strlen(pSendHttpMessage), TRUE);
			m_RecvHTTPPacket.Init();

			return;
		}

		if( m_RecvHTTPPacket.RecvCompleted() )
		{
			// 여기부터 XML 레벨 
			
			bRet = OnRecvdPacketFieldWebServer(m_RecvHTTPPacket.GetHttpText(), m_RecvHTTPPacket.GetHttpLength(), 0, NULL, 0, i_pThreadInfo);
			m_RecvHTTPPacket.Init();
		}
	...
	while(this->IsUsing() && m_bFlagDelayClose == FALSE && nBytesRecvd > 0)
	{
		...
		if(m_RecvPacket.RecvCompleted() == TRUE)
		{
...
#endif
			// start 2011-12-12 by shcho, 암호화 되지 않은 패킷 처리 안되도록 처리 - 이미 아래에서 암호화 구분하고 있음 로그만 찍도록 함.
			if(FALSE == m_RecvPacket.IsPacketEncoded())
			{	// 암호화 되지 않은 패킷이면 
			...
			// end 2011-12-12 by shcho, 암호화 되지 않은 패킷 처리 안되도록 처리 - 이미 아래에서 암호화 구분하고 있음 로그만 찍도록 함.

			if(m_RecvPacket.IsPacketEncoded() == FALSE) // 여기서 암호화된 패킷을 거른다.
			{
				switch(ServerType)
				{
				case ST_PRE_SERVER:
				...
				case ST_NORMAL_SERVER:
					...
					break;
				case ST_AUTHENTICATION_SERVER:		// 2011-01-26 by hskim, 인증 서버 구현
					bRet = OnRecvdPacketAuthenticationServer(m_RecvPacket.GetPacket(), m_RecvPacket.GetLength(), m_byPeerSequenceNumber, NULL, 0, i_pThreadInfo);
					break;
				}
			}
			else if(m_RecvPacket.IsValidPacket() == TRUE)
			{
				...
				case ST_NORMAL_SERVER:
					...
					break;
				case ST_AUTHENTICATION_SERVER:		// 2011-01-26 by hskim, 인증 서버 구현
					bRet = OnRecvdPacketAuthenticationServer(m_RecvPacket.GetPacket(), m_RecvPacket.GetLength(), m_RecvPacket.GetSequenceNumber(), NULL, 0, i_pThreadInfo);
					break;
				}
			}
			else
...
BOOL CIOCPSocket::OnRecvdPacketAuthenticationServer(const char* pPacket, int nLength, BYTE nSeq, char* pPeerIP, int nPeerPort, SThreadInfo *i_pThreadInfo)
{
	// This function should not be called.
	// Instead, the function of child class should be called.
	DBGOUT("Implementation ERROR! The function of child class should be called!!!");
...
// 2013-03-13 by hskim, 웹 캐시 상점
BOOL CIOCPSocket::OnRecvdPacketFieldWebServer(const char* pPacket, int nLength, BYTE nSeq, char* pPeerIP, int nPeerPort, SThreadInfo *i_pThreadInfo)
{
	// This function should not be called.
	// Instead, the function of child class should be called.
	DBGOUT("Implementation ERROR! The function of child class should be called!!!");
...
void CIOCPSocket::OnConnect()
{
	...
	m_RecvHTTPPacket.Init();		// 2013-03-13 by hskim, 웹 캐시 상점
	m_dwCountClientCheck			= 0;
	m_byHostSequenceNumber = rand()%SEQNO_VAR_C;								// TCP 초기값을 SEQNO_VAR_C 값보다 작게 설정
...
{
	STRNCPY_MEMSET(m_szPeerIP, pIP, SIZE_MAX_IPADDRESS);
	m_nPeerPort = nPort;
}
