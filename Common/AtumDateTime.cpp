...
{
	char buf[512];		MEMSET_ZERO(buf, 512);
	///////////////////////////////////////////////////////////////////////////////
...
#ifdef C_DATE_SHORT_FORM
			strftime(buf, 512, "%d-%b-%Y", &tmTM);			// 2013-10-14 by ssjung 날짜 형식 수정 (ex> Oct-) 달을 약자로 쓰도록 수정
#else
			strftime(buf, 512, "%d-%B-%Y", &tmTM);			// 2008-03-27 by cmkwon, 날짜 형식 수정(ex> March-01-2008 20:00:00) - March-10-2008			
#endif
...
#ifdef C_DATE_SHORT_FORM
			strftime(buf, 512, "%b-%Y", &tmTM);		// 2013-10-14 by ssjung 날짜 형식 수정 (ex> Oct-) 달을 약자로 쓰도록 수정
#else
...
#ifdef C_DATE_SHORT_FORM
			strftime(buf, 512, "%d-%b", &tmTM);			// 2013-10-14 by ssjung 날짜 형식 수정 (ex> Oct-) 달을 약자로 쓰도록 수정
#else
			strftime(buf, 512, "%d-%B", &tmTM);		// 2008-03-27 by cmkwon, 날짜 형식 수정(ex> March-01-2008 20:00:00) - March-10
#endif
		}
		break;
	default:
...
#ifdef C_DATE_SHORT_FORM //Inetpub edited - first day then month then year as in normal human callendar
				strftime(buf, 512, "%d-%b-%Y %H:%M:%S", &tmTM);			// 2013-10-14 by ssjung 날짜 형식 수정 (ex> Oct-) 달을 약자로 쓰도록 수정
#else
				strftime(buf, 512, "%d-%B-%Y %H:%M:%S", &tmTM);		// 2008-03-27 by cmkwon, 날짜 형식 수정(ex> March-01-2008 20:00:00) - March-10-2008 15:30:20
#endif
			}
			else
			{
...
#ifdef C_DATE_SHORT_FORM
				strftime(buf, 512, "%d-%b-%Y %H:%M", &tmTM);			// 2013-10-14 by ssjung 날짜 형식 수정 (ex> Oct-) 달을 약자로 쓰도록 수정
#else
				strftime(buf, 512, "%d-%B-%Y %H:%M", &tmTM);			// 2008-03-27 by cmkwon, 날짜 형식 수정(ex> March-01-2008 20:00:00) - March-10-2008 15:30
#endif
			}
			o_szStrBuf = buf;
		}
...
void ATUM_DATE_TIME::SetDateTime(const char *i_szDateTimeString)
{
	...
	STRNCPY_MEMSET(buffer, i_szDateTimeString, SIZE_MAX_SQL_DATETIME_STRING);
...
void ATUM_DATE_TIME::AddDateTime(INT i_nYear, INT i_nMonth, INT i_nDay, INT i_nHour/*=0*/, INT i_nMinute/*=0*/, INT i_nSecond/*=0*/)
{
	...
	// start 2012-02-28 by hskim, 시간 추가할 때 예외 사항시 처리
	if( -1 == ltime )
	{
		SetDateTime(2037, 12, 31, 23, 59, 59);		// 2038. 1. 19 일까지 밖에 사용 못함

		return ;
	}
	// end 2012-02-28 by hskim, 시간 추가할 때 예외 사항시 처리
...
const char* ATUM_DATE_TIME::GetAtTimeString(STRNBUF &i_szBuffer, BOOL i_bExcludeSecond/*=FALSE*/) const
{
	if(FALSE == i_bExcludeSecond)
	{
		sprintf(i_szBuffer.GetBuffer(), "%02d:%02d:%02d", Hour, Minute, Second);
	}
	else
	{
		sprintf(i_szBuffer.GetBuffer(), "%02d:%02d", Hour, Minute);
	}
	return i_szBuffer.GetBuffer();
}
...
{
	MEMSET_ZERO(&o_tm, sizeof(tm));		// 2008-01-09 by cmkwon, Gameforge4D 버그 수정 - 초기화 처리	
