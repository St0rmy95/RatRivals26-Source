...
void SFrame::SetTime(float fGlobalTime)
{
	...
	else
	{
		...
		if (m_pRotateKeys)
		{
			int i1 = 0;
			int i2 = 0;
			DWORD dwTimeTmp = m_pRotateKeys[m_cRotateKeys - 1].dwTime;
			if (dwTimeTmp != 0)//by Inet - fix for random crashes when opening shops - cannot divide by 0
			{
...
HRESULT CSkinnedMesh::DeleteDeviceObjects()
{
	...
	//	if(m_pDecalTextures)
	//	{
	//		int i; for(i=0; i<MAX_DECAL_NUM; i++)
	//		{		
...
HRESULT CSkinnedMesh::LoadMesh(LPD3DXFILEDATA pxofobjCur, DWORD options, DWORD fvf, LPDIRECT3DDEVICE9 pD3DDevice, SFrame* pframeParent)
{
	...
	{
		ID3DXMesh* temp;
		ID3DXBuffer* remap = nullptr;

		if (FAILED(pmcMesh->m_pSkinMesh->CloneMeshFVF(D3DXMESH_MANAGED, pmcMesh->m_pSkinMesh->GetFVF(), m_pd3dDevice, &temp)))
		{
			goto e_Exit;
		}

		SAFE_RELEASE(pmcMesh->m_pSkinMesh);

		if (!HasNormals(temp))
		{
			if (FAILED(D3DXComputeNormals(temp, nullptr)))
			{
				goto e_Exit;
			}
		}

		hr = temp->Optimize(D3DXMESH_MANAGED | D3DXMESHOPT_COMPACT | D3DXMESHOPT_ATTRSORT | D3DXMESHOPT_VERTEXCACHE, static_cast<LPDWORD>(pbufAdjacency->GetBufferPointer()), pmcMesh->m_rgiAdjacency, nullptr, &remap, &pmcMesh->m_pSkinMesh);
		...
		hr = pmcMesh->m_pSkinMeshInfo->Remap(pmcMesh->m_pSkinMesh->GetNumVertices(), (LPDWORD)remap->GetBufferPointer());
	...
	{
		if (!HasNormals(pmcMesh->m_pSkinMesh))
		{
			if (FAILED(D3DXComputeNormals(pmcMesh->m_pSkinMesh, nullptr)))
			{
				goto e_Exit;
			}
		}

		hr = pmcMesh->m_pSkinMesh->Optimize(D3DXMESH_MANAGED | D3DXMESHOPT_COMPACT | D3DXMESHOPT_ATTRSORT | D3DXMESHOPT_VERTEXCACHE, static_cast<LPDWORD>(pbufAdjacency->GetBufferPointer()), pmcMesh->m_rgiAdjacency, nullptr, nullptr, &pmcMesh->pMesh);
...
BOOL CSkinnedMesh::HasNormals(ID3DXMesh* mesh)
{
	D3DVERTEXELEMENT9 elems[MAX_FVF_DECL_SIZE];
	if (FAILED(mesh->GetDeclaration(elems)))
	{
		return false;
	}
...
HRESULT CSkinnedMesh::GenerateMesh(SMeshContainer* pmcMesh)
{
	...
	else if (m_method == D3DINDEXED)
	{
		...
		{
			pmcMesh->m_paletteSize = pmcMesh->m_pSkinMeshInfo->GetNumBones(); // min(m_d3dCaps.MaxVertexBlendMatrixIndex + 1, pmcMesh->m_pSkinMeshInfo->GetNumBones());
			pmcMesh->m_bUseSW = false;
			flags |= D3DXMESH_MANAGED;
		}
...
HRESULT CSkinnedMesh::UpdateFrames(SFrame* pframeCur, const D3DXMATRIX& matCur)
{
	...
	for (auto* pframeChild = pframeCur->pframeFirstChild; pframeChild; pframeChild = pframeChild->pframeSibling)
	{
		...
		if (g_bDetailDrawFrame == TRUE && pmcMesh != nullptr && m_bProgressiveMesh == false)
		{
			...
			if (bResult && D3DXVec3Length(&(vMeshPos - g_pCamera->m_vCamCurrentPos)) - pmcMesh->fRadius > g_pScene->m_fFogEndValue)
			{
				bResult = false;
			}
		}
...
void CSkinnedMesh::ReleaseDeviceDependentMeshes(SFrame* pframe)
{
	...
	if (pframe->pmcMesh != nullptr)
	{
		for (SMeshContainer* pmcCurr = pframe->pmcMesh; pmcCurr != nullptr; pmcCurr = pmcCurr->pmcNext)
		{
			...
			{
				SAFE_RELEASE(pmcCurr->pMesh);
				pmcCurr->m_Method = NONE;
			}
			/*			for(i=0;i<5;i++)
						{
						if(pmcCurr->pMeshes[i])
						SAFE_RELEASE(pmcCurr->pMeshes[i]);
						}
						*/
		}
	}
...
HRESULT CSkinnedMesh::LoadMeshHierarchyFromMemLoadStep4(structLoadingGameData* pGameData)
{
	...
	if (totalNum > 1)
	{
		...
		for (auto i = cnt; i < totalNum; i++)
		{
			...
			HRESULT hrTex = D3DXCreateTextureFromFileInMemory(m_pd3dDevice, (LPCVOID)m_pDataHeader[i].m_pData,
				m_pDataHeader[i].m_DataSize, &(m_pTexture[i - 1]));
			// hrTexŔÇ żˇ·ŻÄÚµĺ
			// D3DERR_NOTAVAILABLE 2154
			// D3DERR_OUTOFVIDEOMEMORY	380
			// D3DERR_INVALIDCALL 2156
			// D3DXERR_INVALIDDATA 2905
			// E_OUTOFMEMORY 0x80070002L or 0x8007000EL
			if (FAILED(hrTex))
				// END 2014-07-30 by ymjoo ¸®ĽŇ˝ş ĆÄŔĎ ·Îµů ą®Á¦ ¸®ĽŇ˝ş ŔĚ¸§ Ăâ·Â
			{
				if (g_pD3dApp && g_pD3dApp->m_pChat)
				{
					// 2014-07-30 by ymjoo ¸®ĽŇ˝ş ĆÄŔĎ ·Îµů ą®Á¦ ¸®ĽŇ˝ş ŔĚ¸§ Ăâ·Â
#if defined(STRMSG_C_140730_0000)
					char buff[256];
					wsprintf(buff, STRMSG_C_140730_0000, m_pDataHeader[i].m_FileName, hrTex);		// "¸®ĽŇ˝ş ĆÄŔĎ ·Îµůżˇ ą®Á¦°ˇ ŔÖľîĽ­ żŔşęÁ§Ć® »ýĽşŔĚ Áß´ÜµĘ. [%s][%d]"
					g_pD3dApp->m_pChat->CreateChatChild(buff, COLOR_ERROR);
					...
					wsprintf(sMsg.Message,
						"%s \"%s\" (%d)\"%s\" Data:%X, Size:%d, ErrCode:%d, Map:%d, ResTotalNum:%d, VGA:%s",
						"[ResErr]",
						m_pDataHeader[0].m_FileName,											// obj ŔĚ¸§
						i,																		// ąřČŁ
						m_pDataHeader[i].m_FileName,											// ¸®ĽŇ˝ş ŔĚ¸§
						m_pDataHeader[i].m_pData,												// µĄŔĚĹÍ ł»żë
						m_pDataHeader[i].m_DataSize,											// µĄŔĚĹÍ Ĺ©±â
						hrTex,																	// żˇ·Ż ÄÚµĺ
						g_pShuttleChild->m_myShuttleInfo.MapChannelIndex.MapIndex,				// ¸Ę ąřČŁ
						totalNum,																// ¸®ĽŇ˝ş ĆÄŔĎ °łĽö
						id.Description															// VGA
					);
					g_pFieldWinSocket->SendMsg(T_FC_CLIENT_REPORT, (char*)&sMsg, sizeof(sMsg));
#else
					g_pD3dApp->m_pChat->CreateChatChild(STRERR_C_RESOURCE_0012, COLOR_ERROR);// "¸®ĽŇ˝ş ĆÄŔĎ ·Îµůżˇ ą®Á¦°ˇ ŔÖľîĽ­ żŔşęÁ§Ć® »ýĽşŔĚ Áß´ÜµĘ."
#endif
					// END 2014-07-30 by ymjoo ¸®ĽŇ˝ş ĆÄŔĎ ·Îµů ą®Á¦ ¸®ĽŇ˝ş ŔĚ¸§ Ăâ·Â
