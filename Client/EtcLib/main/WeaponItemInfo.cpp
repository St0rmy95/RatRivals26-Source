...
//	}
	...
	int i; for(i=0;i<nMultiNum;i++)
	{
		...
		{
			vTempPos.x = rand()%nReactionRange - (m_pItemInfo->GetRealItemInfo()->ReactionRange)/2;
			vTempPos.y = rand()%nReactionRange - (m_pItemInfo->GetRealItemInfo()->ReactionRange)/2;
			vTempPos.z = rand()%nReactionRange - (m_pItemInfo->GetRealItemInfo()->ReactionRange)/2;
...
void CWeaponItemInfo::CheckInNet()
{
	if(IS_SECONDARY_WEAPON(m_pItemInfo->Kind))
	{
		...
		int i; for(i=0; i< m_vecTargetIndexData.size(); i++)
		{
					...
					if(ATTACK_AVAILABLE_STATE((*it)->m_dwState))
					{
						...
						if( IS_PRIMARY_WEAPON(m_pItemInfo->Kind) )
						{
							...
							if( length != -1 )
							{
								...
								nTotalMultiTarget > 1)			// nTotalMultiTarget이 1이면 이곳에서 계산하지 않고 밑에서 shuttle의 타겟 지시기로 한다.
						{
				...
				!IS_SAME_CHARACTER_MONSTER_INFLUENCE(g_pShuttleChild->m_myShuttleInfo.InfluenceType, (*itMon)->m_pMonsterInfo->Belligerence))
			{
				...
				// end 2010-06-15 by shcho&hslee 펫시스템 - 펫 무기 생성
				{
					...
					if((*itMon)->m_nindexSize)
					{
						...
						int i; for(i=0; i< (*itMon)->m_nindexSize; i++)
						{
...
float CWeaponItemInfo::GetStatPierce()
{
	...
	if(pItem)	
	{
		...
		fResistanceRate = ((float)fResistance / (float)PROB100_MAX_VALUE *100.0f);	
		fResistanceRate = FloatSecRangeSharp(fResistanceRate);// 2008-03-19 by bhsohn FLOAT형 재정렬 하여 사용
...
float CWeaponItemInfo::GetPreSufFixItemInfoRate(int nDesParameter)
{
	...
	if(fPreSufFixPierce > 0)
	{
		...
		fPreSufFixPierceRate = ((float)fPreSufFixPierce / (float)PROB100_MAX_VALUE *100.0f);
		fPreSufFixPierceRate = FloatSecRangeSharp(fPreSufFixPierceRate);// FLOAT형 재정렬 하여 사용
...
float CWeaponItemInfo::GetPrimaryEnchantPierce()
{
	...
	fPierce = ((float)fPierce / (float)PROB100_MAX_VALUE *100.0f);
...
{	
	if(NULL == m_pCharacterParamFactor)
	{
		return 0.0f;
	}

	
	float fTotalPeierceRate = 0.0f;

	float fStatPierceRate = GetStatPierce();
	float fEqPierce = 0.0f;
	float fEqPierceRate = 0.0f;
	if(bIsPrimary)
	{
		fEqPierce = g_pShuttleChild->GetShuttleParamFactor()->pfm_PIERCE_UP_01;								
	}
	else
	{
		fEqPierce = g_pShuttleChild->GetShuttleParamFactor()->pfm_PIERCE_UP_02;
	}
	// 2010-07-28 by dgwoo 확률 수식 변경 (255 => 100%)
	//fEqPierceRate = ((float)fEqPierce / (float)PROB256_MAX_VALUE *100.0f);
	fEqPierceRate = ((float)fEqPierce / (float)PROB100_MAX_VALUE *100.0f);
	

	// 아이템 장착 피어스 율
	float fItemPierce = 0.0f;
	float fItemPierceRate = 0.0f;
	{		
		if(m_pItemInfo)
		{
			CItemInfo* pSelectItem = g_pStoreData->FindItemInInventoryByUniqueNumber( m_pItemInfo->UniqueNumber);
			if(pSelectItem)
			{
				...
				{		
					if(bIsPrimary)
					{
						fItemPierce = pRefEnchant->pfm_PIERCE_UP_01;
					}
					else
					{
						fItemPierce = pRefEnchant->pfm_PIERCE_UP_02;
					}
					// 2010-07-28 by dgwoo 확률 수식 변경 (255 => 100%)
					//fItemPierceRate = ((float)fItemPierce / (float)PROB256_MAX_VALUE *100.0f);
					fItemPierceRate = ((float)fItemPierce / (float)PROB100_MAX_VALUE *100.0f);
					// 2008-03-19 by bhsohn FLOAT형 재정렬 하여 사용							
	...
	float fPreSufPierceRate = 0.0f;
	if(bIsPrimary)
	{
		fPreSufPierceRate = GetPreSufFixItemInfoRate(DES_PIERCE_UP_01);
	}
	else
	{
		fPreSufPierceRate = GetPreSufFixItemInfoRate(DES_PIERCE_UP_02);
	}	
	fTotalPeierceRate = fStatPierceRate+fEqPierceRate+fItemPierceRate+fPreSufPierceRate;
