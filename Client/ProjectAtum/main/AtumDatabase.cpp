...
void CAtumDatabase::ReleaseMixingItemSearchTable()
{
	...
	SAFE_DELETE(m_pMapMixingSearchTable);
}
...
//		SAFE_DELETE(it_map->second);
	...
	SAFE_DELETE(m_pMapMapResultMixingInfo);
}
...
int CAtumDatabase::GetFindLookChangeIdx(ITEM* pItem)
{
	...
	else if(pItem->IsExistDesParam(DES_SHAPE_ITEM))
	{
		...
		switch(nParmaVale)
		{		
		case KIND_ITEM_FIX_MIXING_SHAPE_PRIMARY_WEAPON_A:		// 무기 외형 - 기본 무기 A 기어
			{
				nItemShapeType = LOOK_MIXING_SHAPE_PRIMARY_WEAPON_A;
			}
			break;
		case KIND_ITEM_FIX_MIXING_SHAPE_PRIMARY_WEAPON_BMI:		// 무기 외형 - 기본 무기 BMI 기어
			{
				nItemShapeType = LOOK_MIXING_SHAPE_PRIMARY_WEAPON_BMI;
			}
			break;
		case KIND_ITEM_FIX_MIXING_SHAPE_SECOND_WEAPON_A:		// 무기 외형 - 고급 무기 A 기어
			{
				nItemShapeType = LOOK_MIXING_SHAPE_SECOND_WEAPON_A;
			}
			break;
		case KIND_ITEM_FIX_MIXING_SHAPE_SECOND_WEAPON_BMI:		// 무기 외형 - 고급 무기 BMI 기어
			{
				nItemShapeType = LOOK_MIXING_SHAPE_SECOND_WEAPON_BMI;
			}
			break;
		case KIND_ITEM_FIX_MIXING_EFFECT_PRIMARY_WEAPON:		// 무기 외형 - 기본 무기 이펙트
			{
				nItemShapeType = LOOK_MIXING_SHAPE_EFFECT_PRIMARY_WEAPON;
			}
			break;
		case KIND_ITEM_FIX_MIXING_EFFECT_SECOND_WEAPON:			// 무기 외형 - 고급 무기 이펙트
			{
				nItemShapeType = LOOK_MIXING_SHAPE_EFFECT_SECOND_WEAPON;
			}
			break;
		case KIND_ITEM_FIX_MIXING_SHAPE_DEFENSE_B:				// 아머 외형 - B 기어
			{
				nItemShapeType = LOOK_MIXING_SHAPE_DEFENSE_B;
			}
			break;
		case KIND_ITEM_FIX_MIXING_SHAPE_DEFENSE_M:				// 아머 외형 - M 기어
			{
				nItemShapeType = LOOK_MIXING_SHAPE_DEFENSE_M;
			}
			break;
		case KIND_ITEM_FIX_MIXING_SHAPE_DEFENSE_A:				// 아머 외형 - A 기어
			{
				nItemShapeType = LOOK_MIXING_SHAPE_DEFENSE_A;
			}
			break;
		case KIND_ITEM_FIX_MIXING_SHAPE_DEFENSE_I:				// 아머 외형 - I 기어
			{
				nItemShapeType = LOOK_MIXING_SHAPE_DEFENSE_I;
			}
			break;
		case KIND_ITEM_FIX_MIXING_SHAPE_DEFENSE_INIT:			// 아머 외형 - 초기화
			{
				nItemShapeType = LOOK_MIXING_SHAPE_DEFENSE_INIT;
			}
			break;		
		case KIND_ITEM_FIX_MIXING_SHAPE_PET_ITEM:				// 파트너 외형 - // 2012-12-03 by hskim, 파트너 기간제 외형 변경
			{
				nItemShapeType = LOOK_MIXING_SHAPE_PET_ITEM;
			}
			break;		
		case KIND_ITEM_FIX_MIXING_SHAPE_SECOND_EFFECT_INIT:		// 이펙트 외형 - 고급 무기 초기화 
			{
				nItemShapeType = LOOK_MIXING_SHAPE_SECOND_EFFECT_INIT;
			}
			break;
		case KIND_ITEM_FIX_MIXING_SHAPE_PRIMARY_EFFECT_INIT:	// 이펙트 외형 - 기본 무기 초기화 
			{
...
void CAtumDatabase::ReleaseMixingItemListIndex()
{
	...
	for(nCnt = 0;nCnt < MAX_LOOK_MIXING_ITEM;nCnt++)
	{
		...
		SAFE_DELETE(m_pMapItemShapeType[nCnt]);
	}
}
///////////////////////////////////////////////////////////////////////////////
/// \fn			
...
ITEM * CAtumDatabase::FindBulletItemToDB(BYTE bCaliber)
{
	CMapItemInfoIterator itBullet = m_mapItemInfoTemp.begin();
	while(itBullet != m_mapItemInfoTemp.end())
	{
		ITEM* pBullet = itBullet->second;
		if(ITEMKIND_BULLET == pBullet->Kind && bCaliber == pBullet->Caliber)
		{
			return pBullet;
		}
		++itBullet;
...
{	
	...
	while(it != m_mapBuildingNPCInfoTemp.end())
	{
		...
		{
			BOOL bExist = FALSE;
			int i; for(i=0; i < pVecBuildingNPC->size(); i++)
			{
...
MONSTER_MULTI_TARGET CAtumDatabase::GetMultiTargetPosion(int nMonsterIndex, int nPosion)
{
	...
	memset(&tempInfo, 0x0, sizeof(MONSTER_MULTI_TARGET));
