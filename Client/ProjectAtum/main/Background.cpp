...
//	SAFE_DELETE_ARRAY(m_pVBRender);
	SAFE_DELETE_ARRAY(m_pVBWater);
	SAFE_DELETE_ARRAY(m_bWaterRender);
...
//	SAFE_DELETE(m_pMeshMinimap);				// 2005-09-12 by ispark 별도 처리
//	SAFE_DELETE(m_pMiniBigMapImage);
	int i;
	for(i=0;i<WATER_TEXTURE_COUNT;i++)
		SAFE_RELEASE(m_pWaterTexture[i]);

	SAFE_DELETE_ARRAY(m_bTileRender);

	map<INT,EFFECTINFOCLIENT *>::iterator itEff = m_mapEffectList.begin();
	while(itEff != m_mapEffectList.end())
	{
		SAFE_DELETE(itEff->second);
		itEff++;
	}
	m_mapEffectList.clear();

	SAFE_DELETE(m_pBigObject);
	SAFE_DELETE(m_pWaterObject);// 2005-02-11 by jschoi  물 오브젝트 타입 추가
	SAFE_DELETE(m_pObjectMonster);
	SAFE_DELETE(m_pObjectEvent);
	...
	SAFE_DELETE(m_pMiniMapDataHeader);
	SAFE_DELETE(m_pMiniMapCityDataHeader);
...
//				m_pTileInfo[i].useWater = FALSE;
	...
	// 2014-12-03 by jwLee m_ppObjectList 변수 할당 방법 변경
...
//	}
	for(i=0; i < m_NUM_BLOCK_X; i++)
	{
		m_ppObjectList[i] = new CAtumNode[m_NUM_BLOCK_Z];
	}
	// end 2014-12-03 by jwLee m_ppObjectList 변수 할당 방법 변경
...
//			else if(m_pTileInfo[(2*i+1)*(m_projectInfo.sYSize) + (2*j+1)].useWater)
			...
			m_bWaterRender[i*(m_projectInfo.sYSize/2) + j].waterTexNumber = rand()%WATER_TEXTURE_COUNT;
		}
	}
...
//	}
	...
	if(pHeader)
	{
		map<INT,EFFECTINFOCLIENT *>::iterator itEff;
		int nEffCont,nTemp;
		int nRegionCont1 = 0;
		int nRegionCont2 = 0;
		p = pHeader->m_pData + 20;
		...
		for(i=0;i<nEffCont;i++)
		{
			...
			if(eff->bRegion)
			{
				while(1)
				{
					...
					// end 2015-04-02 by jwlee 맵에 설정된 effect 리소스 관련 EFFECTINFOCLIENT 정보 메모리 관련 수정
				// 2015-04-02 by jwlee 맵에 설정된 effect 리소스 관련 EFFECTINFOCLIENT 정보 메모리 관련 수정
// 				}
// 				nRegionCont1++;
				nRegionCont1++;
...
*/
	// 2014-12-04 by jwLee 클라이언트 리포트 보내는 정보 수정
	// 2014-12-15 by jwLee m_pVBWater 버텍스 버퍼 생성 실패시 서버에 메세지 전송
#if defined(C_ERROR_REPORT_WATER_VERTEX)
	MEMORYSTATUS MemStatus;
	ZeroMemory(&MemStatus, sizeof(MEMORYSTATUS));
	MemStatus.dwLength = sizeof(MEMORYSTATUS);
	GlobalMemoryStatus(&MemStatus);
#endif
	// end 2014-12-15 by jwLee m_pVBWater 버텍스 버퍼 생성 실패시 서버에 메세지 전송
	// end 2014-12-04 by jwLee 클라이언트 리포트 보내는 정보 수정
	...
	for ( nCheckSizeX = 0;nCheckSizeX < m_projectInfo.sYSize / 2;nCheckSizeX++ )
	{
...
#if defined(C_ERROR_REPORT_WATER_VERTEX)
		HRESULT hrVB = NULL;
		if ( FAILED( hrVB = g_pD3dDev->CreateVertexBuffer( 2 * m_projectInfo.sXSize * sizeof( GROUNDVERTEX )
			, 0
			, D3DFVF_GROUNDVERTEX
			, D3DPOOL_MANAGED
...
#endif
		{
			...
			// 2014-12-15 by jwLee m_pVBWater 버텍스 버퍼 생성 실패시 해제 코드 추가
			for (int j = 0 ; j < nCheckSizeX ; j++)
			{
				SAFE_RELEASE(m_pVBWater[j]);
			}
			// end 2014-12-15 by jwLee m_pVBWater 버텍스 버퍼 생성 실패시 해제 코드 추가
			
			// 2014-12-15 by jwLee m_pVBWater 버텍스 버퍼 생성 실패시 서버에 메세지 전송
#if defined(C_ERROR_REPORT_WATER_VERTEX)
			MSG_FC_CLIENT_REPORT sMsg;
			memset(&sMsg, 0x00, sizeof(sMsg));
			strcpy(sMsg.CharacterName, g_pShuttleChild->m_myShuttleInfo.CharacterName);
			LPDIRECT3D9 pd3d9 = NULL;
			g_pD3dDev->GetDirect3D(&pd3d9);
			D3DADAPTER_IDENTIFIER9 id;
			pd3d9->GetAdapterIdentifier(D3DADAPTER_DEFAULT, 0, &id);
			SAFE_RELEASE(pd3d9);

			wsprintf(sMsg.Message,
				"[FailedToCreateVB] Pos:%s, ErrCode:%d, Map:%d, VGA:%s, AvPhM:%d, ToViM:%d, AvViM:%d", 
				"m_pVBWater[nCheckSizeX]",
				hrVB,																	// 에러 코드
				g_pShuttleChild->m_myShuttleInfo.MapChannelIndex.MapIndex,				// 맵 번호
				id.Description,															// VGA
				MemStatus.dwAvailPhys		/ 1024,										// 물리 메모리
				MemStatus.dwTotalVirtual	/ 1024, MemStatus.dwAvailVirtual	/ 1024	// 가상 메모리
				);
			
			g_pFieldWinSocket->SendMsg(T_FC_CLIENT_REPORT, (char*)&sMsg, sizeof(sMsg));
#endif
			// end 2014-12-15 by jwLee m_pVBWater 버텍스 버퍼 생성 실패시 서버에 메세지 전송
			return E_FAIL;
		}
	...
	{
		// 2014-08-26 by ymjoo 버텍스 버퍼 생성 실패시 서버에 메세지 전송
...
//    												 D3DUSAGE_WRITEONLY, //0,	// 2014-08-18 by ymjoo 버텍스 버퍼 옵션 수정
...
// 		}
		...
		// 2014-12-12 by jwLee 물 버텍스 버퍼 비정상적인 크기로 생성하는것 수정
		//HRESULT hrVB = g_pD3dDev->CreateVertexBuffer(4 * m_projectInfo.sXSize * m_projectInfo.sYSize * sizeof(WATERBUMPVERTEX), 
		HRESULT hrVB = g_pD3dDev->CreateVertexBuffer(4 * (m_projectInfo.sXSize / 2) * (m_projectInfo.sYSize / 2) * sizeof(WATERBUMPVERTEX), 
		// end 2014-12-12 by jwLee 물 버텍스 버퍼 비정상적인 크기로 생성하는것 수정
													 D3DUSAGE_WRITEONLY, 
													 D3DFVF_WATERBUMPVERTEX, 
													 D3DPOOL_MANAGED, 
													 &m_pToRenderWaterVB, 
													 NULL);
		// 2014-09-11 by ymjoo CreateVertexBuffer 실패시 재시도
		for(int i = 0 ; i < 10 ; ++i)
		{
			if(SUCCEEDED(hrVB))
			{
			...
			SAFE_RELEASE(m_pToRenderWaterVB);
			// 2014-12-12 by jwLee 물 버텍스 버퍼 비정상적인 크기로 생성하는것 수정
			//hrVB = g_pD3dDev->CreateVertexBuffer(4 * m_projectInfo.sXSize * m_projectInfo.sYSize * sizeof(WATERBUMPVERTEX), 
			hrVB = g_pD3dDev->CreateVertexBuffer(4 * (m_projectInfo.sXSize / 2) * (m_projectInfo.sYSize / 2) * sizeof(WATERBUMPVERTEX), 
			// end 2014-12-12 by jwLee 물 버텍스 버퍼 비정상적인 크기로 생성하는것 수정
												 D3DUSAGE_WRITEONLY, 
												 D3DFVF_WATERBUMPVERTEX, 
												 D3DPOOL_MANAGED, 
...
#else
			 wsprintf(sMsg.Message,
			 	"[FailedToCreateVB] ErrCode:%d, Map:%d, XSize:%d, YSize:%d, VGA:%s", 
					hrVB,																	// 에러 코드
			 	g_pShuttleChild->m_myShuttleInfo.MapChannelIndex.MapIndex,				// 맵 번호
			 	m_projectInfo.sXSize,													// XSize
			 	m_projectInfo.sYSize,													// YSize
			 	id.Description															// VGA
 				);
#endif
			// end 2014-12-04 by jwLee 클라이언트 리포트 보내는 정보 수정
		...
		// END 2014-08-26 by ymjoo 버텍스 버퍼 생성 실패시 서버에 메세지 전송
	}
	// end 2008. 12. 11 by ckPark 물렌더링
...
HRESULT CBackground::DeleteDeviceObjects()
{
	...
	// 2014-12-03 by jwLee m_ppObjectList 변수 삭제 방법 변경
//	SAFE_DELETE_ARRAY(m_ppObjectList[0]);
//	SAFE_DELETE_ARRAY(m_ppObjectList);
	if(m_ppObjectList)
	{
		...
		for (i = 0 ; i < x ; i++)
		{
			if (m_ppObjectList[i])
			{
				SAFE_DELETE_ARRAY(m_ppObjectList[i]);
			}
		}
	SAFE_DELETE_ARRAY(m_ppObjectList);
	}
	// end 2014-12-03 by jwLee m_ppObjectList 변수 삭제 방법 변경
//	if (m_ppObjectList)
//	{
//		SAFE_DELETE_ARRAY(m_ppObjectList[0]);
//		SAFE_DELETE_ARRAY(m_ppObjectList);
...
*/
	if(m_pMiniBigMapImage)
	{
		...
		if(m_pMiniBigMapCityImage)
		{
			...
			SAFE_DELETE(m_pMiniBigMapCityImage);
		}
	}
//	if(g_pD3dApp->m_pETCRender)
...
//		SAFE_DELETE(m_pObjectEvent);
	...
	// 2015-04-02 by jwlee 맵에 설정된 mon 리소스에 관련 MONSTERINFO 정보 메모리 관련 수정
	vector<MONSTERINFO*>::iterator itMon = m_vecMonsterResList.begin();
	while(itMon != m_vecMonsterResList.end())
	{
		MONSTERINFO * pMon = *itMon;
	...
	m_vecMonsterResList.clear();
	// end 2015-04-02 by jwlee 맵에 설정된 mon 리소스에 관련 MONSTERINFO 정보 메모리 관련 수정
