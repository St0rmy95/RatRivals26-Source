...
CMonsterData::~CMonsterData()
{
	...
	SAFE_DELETE(m_pMonsterMovePattern);
//	InvalidateShadow();
}
...
void CMonsterData::Init()
{
	...
	{
		int i; for(i=0; i < m_nindexSize; i++)
		{
...
void CMonsterData::ChangeBodyConditionFromServer(BodyCond_t hyBodyCondition)
{
		...
		COMPARE_BODYCON_BIT(hyBodyCondition,BODYCON_MON_AUTODESTROYED_MASK)))
	{
		...
		// 2014-06-17 by ymjoo 몬스터 바디컨디션 체인지 익셉션 예외 처리 2014-07-31 by ymjoo 롤백
		//CWeapon * pNode = (CWeapon *)g_pScene->m_pWeaponData->m_pChild;
		CWeapon *pNode = NULL;
		//if(g_pScene && (CWeapon*)g_pScene->m_pWeaponData)
			pNode = (CWeapon*)g_pScene->m_pWeaponData->m_pChild;
		// END 2014-06-17 by ymjoo 몬스터 바디컨디션 체인지 익셉션 예외 처리
		while(pNode)
		{
