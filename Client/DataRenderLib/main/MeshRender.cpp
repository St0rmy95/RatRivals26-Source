...
// 		}
		...
		nMeshIndex == g_pCharacterChild->m_nUnitNum)
	{
		...
		if(gameData->SetFile(strPath,FALSE, NULL,0)==TRUE)
		{
			...
			// 2014-03-28 by ymjoo 리소스 파일 에러일 때 팅기도록 수정
#if defined C_RESOURCE_FILE_ERROR_CLOSE_YMJOO
			// 2014-04-07 by ymjoo 리소스 파일 에러일 때 팅기도록 수정(재수정)
			//else
			else if(0 != myShuttleInfo.Race)
			{
				// 일반 유저는 팅긴다.
				wsprintf(buf, "%08d", nMeshIndex);
				if(gameData && gameData->Find(buf) == NULL)
				{
					DBGOUT("Resource File Error(%d)\n", nMeshIndex);	//리소스 파일 에러
					char ErrorMsgMissionList[256];
					wsprintf(ErrorMsgMissionList, "Resource File Error(%d)", nMeshIndex);
...
//					DBGOUT("PostMessage(g_pD3dApp->GetHwnd(), WM_CLOSE, 0, 0);");
					// 여기에서 메세지박스를 띄우자.
// 					MessageBox(NULL, STRERR_C_RESOURCE_0001, "Error", MB_OK);
// 					PostMessage(g_pD3dApp->GetHwnd(), WM_CLOSE, 0, 0);
					g_pD3dApp->NetworkErrorMsgBox(STRERR_C_RESOURCE_0001);//"서버와의 접속이 종료되었습니다. 클라이언트를 종료합니다. (FieldWinSocket WS_CLOSED)"
				}
			}
#endif
			// END 2014-03-28 by ymjoo 리소스 파일 에러일 때 팅기도록 수정
			int nObjectIdx = 0;
			if(FALSE == bCharacter)
			{
				nObjectIdx = nMeshIndex;			
