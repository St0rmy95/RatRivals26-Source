...
//end 2012-09-20 by jhahn 인비지블에서 파트너 아이템 사용가능
	...
	m_bShowInterface = FALSE;				// 2013-10-18 by ssjung A기어가 시즈모드 이거나 움직일때 인터페이스 띄우지 않도록 수정

	m_bToggleKeyS = FALSE;					// 2014-09-04 by ymjoo S키 토글
}
...
CShuttleChild::~CShuttleChild()
{
	...
	SAFE_DELETE( m_pCinemaUnitPattern );			// 2004.06.17 jschoi 스킬 패턴
	SAFE_DELETE( m_pCinemaCamera );			// 2004.06.17 jschoi 스킬 카메라

	SAFE_DELETE( m_pPrimaryWeapon );
	SAFE_DELETE( m_pSecondaryWeapon );

	SAFE_DELETE(m_pStoreData);	// m_pPrimaryWeapon, m_pSecondaryWeapon 뒤에 지워야 한다.

	SAFE_DELETE(m_pSkill);		// 2004-07-23 jschoi 스킬
	SAFE_DELETE(m_pClientParty);	// 2004-08-19 jschoi 파티
...
// 2013-10-18 by ssjung A기어가 시즈모드 이거나 움직일때 인터페이스 띄우지 않도록 수정
	if(!m_bIsAir && GetMoving())	// 2013-12-13 by ssjung M기어 리버스 스킬을 쓰면 MSGBOX가 사라지는 버그 수정
	{
		if(!m_bShowInterface)
		{
...
// end 2013-10-18 by ssjung A기어가 시즈모드 이거나 움직일때 인터페이스 띄우지 않도록 수정
}
	...
	if(i >= 0 && i < g_pGround->m_projectInfo.sXSize && j >= 0 && j < g_pGround->m_projectInfo.sYSize)
	{
			...
			&fDist))//, &fBary1, &fBary2 ))
		{
			...
			else
			{
				...
				// 2013-11-21 by ymjoo 경사가 급한 지형에서 벽을 뚫는 현상 해결
				...
				// (v0, v1, v2)와 orig(vPos)의 거리
				FLOAT fDet = D3DXVec3Dot( &tVec, &normalVec );
				
				if( fDet * fDet <= 4.0f )
					m_bCollMap = TRUE;
#endif
				// END 2013-11-21 by ymjoo 경사가 급한 지형에서 벽을 뚫는 현상 해결
...
void CShuttleChild::WndProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	if(!g_pD3dApp->m_bChatMode && FALSE == GetChatModeState())// && m_bMoveInputType == 0)		// 2013-11-14 by ssjung 비행 중 채팅 할시 조작키가 작동하는 문제 수정
	{	
		switch(uMsg)
		{
		case WM_KEYDOWN:
...
//	CheckMoveRate(fElapsedTime,0,nFBMove);
	...
	if(m_bOnObject)
	{
		...
		// 2013-12-04 by ymjoo A기어가 지상에서 뒤집어지는 현상 수정
		if( 0 > m_vUp.y )
		{
			m_vUp.y *= -1;
		}
		// END 2013-12-04 by ymjoo A기어가 지상에서 뒤집어지는 현상 수정
	}
	else
	{
		fHeight = CheckMap(m_vPos);
...
//		m_fShuttleSpeed = m_fRealShuttleSpeedMin>30 ? m_fRealShuttleSpeedMin : 30;
	...
	{// 좌측 부분 충돌
		...
		// 2013-11-25 by ymjoo 셔틀 좌우측 날개 충돌시 좀 더 과격하게 밀려나도록 개선
#ifdef C_SHUTTLE_WING_COLL_ADD_REFLECT
		m_vPos += (vRight - vLeft) * 1.0f;
#endif
		// END 2013-11-25 by ymjoo 셔틀 좌우측 날개 충돌시 좀 더 과격하게 밀려나도록 개선
...
// 2013-10-28 by ssjung A기어 지상에서 몬스터와 부딪치면 뒤집어지는 현상 수정
				if(m_bIsAir == FALSE && IS_DT(m_myShuttleInfo.UnitKind))			//A기어가 땅에 있을때
				{
					if(m_vUp.y < 0)
					{
...
// end 2013-10-28 by ssjung A기어 지상에서 몬스터와 부딪치면 뒤집어지는 현상 수정
...
void CShuttleChild::SetShuttleSpeed()
{
	...
	//	m_fRealShuttleSpeedMax = m_fShuttleSpeedMax*(1.0f + m_paramFactor.pfm_SPEED)+1000.0f;
		...
		// 스킬 데스 파람값도 적용
		// 2014-07-17 by ymjoo 튜토리얼에서 이동속도 개선
		...
		m_fRealShuttleSpeedMax = ( m_fShuttleSpeedMax + GetParamFactor_DesParam( m_paramFactor, DES_ENGINE_MAX_SPEED_UP ) ) * ( 1.0f + GetParamFactor_DesParam( m_paramFactor, DES_SPEED ) );
#endif
		// END 2014-07-17 by ymjoo 튜토리얼에서 이동속도 개선
	...
	//	m_fRealShuttleSpeedBoosterOn = m_fShuttleSpeedBoosterOn*(1.0f + m_paramFactor.pfm_SPEED)+1000.0f;
...
#ifdef C_TUTORIAL_SPEED_YMJOO		// 2014-07-17 by ymjoo 튜토리얼에서 이동속도 개선
		if(g_pTutorial->IsTutorialMode())
		{
		...
		else
		{
			...
			if(m_fRealShuttleSpeedMin <= 0.0f && !COMPARE_RACE(g_pShuttleChild->m_myShuttleInfo.Race,RACE_OPERATION|RACE_GAMEMASTER))		// 2014-08-07 by ymjoo GM 최소속력 음수 적용
				m_fRealShuttleSpeedMin = 0.0f;
...
//			if ( g_pShuttleChild->m_myShuttleInfo.CurrentSP < (temp2 * tempCurrentInfo->PetKitSP.TriggerValue ))
			...
			SpCharge = FALSE;			// 2014-08-06 by ymjoo 파트너 키트 자동 사용 소켓 설정 수치 까지만 회복되도록 수정
