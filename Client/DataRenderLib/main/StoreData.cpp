...
CStoreData::~CStoreData()
{
	...
	{
		SAFE_DELETE(itItem->second);
		itItem++;
	}
	// 2013-05-28 by bhsohn 아머 컬렉션 시스템
	if(m_pArmorCollectManager)
	{
		SAFE_DELETE(m_pArmorCollectManager);
		m_pArmorCollectManager = NULL;
	}
	// END 2013-05-28 by bhsohn 아머 컬렉션 시스템
	// 2014-02-07 by ymjoo 캡슐형 캐시 아이템 경고 메세지
	vector<SHOP_ITEM*>::iterator it = m_vecCashItemInfo.begin();
	while(it != m_vecCashItemInfo.end())
	{
		SHOP_ITEM* pItem = *it;
		delete pItem;
		pItem = NULL;
		...
		case ITEMKIND_PET_ITEM :
			{
				...
				// 2014-03-10 by ssjung 인첸트시 핵 유저 일 때 크래쉬 예외 처리
				if(g_pShuttleChild && g_pShuttleChild->GetPetManager())
					tempCurrentInfo = g_pShuttleChild->GetPetManager()->GetPtr_PetCurrentData( pItemInfo->UniqueNumber );
				// end 2014-03-10 by ssjung 인첸트시 핵 유저 일 때 크래쉬 예외 처리
				if(tempCurrentInfo)
				{
...
// 													  FALSE );
			...
			g_pShuttleChild->ShowWeapon(0, FALSE);
			...
			if( !g_pShuttleChild->IsRobotArmor() )			
			{
				...
				if(g_pShuttleChild->GetMonsterTransformer())
					...
					//g_pShuttleChild->ShowWeapon( TRUE );
					g_pShuttleChild->ShowWeapon(0, TRUE);
				// END 2014-08-01 by ymjoo A기어 로봇아머 기본무기 렌더 안되는 문제 수정
				//end 2010. 03. 18 by jskim 몬스터변신 카드
			}
			else
			{	
				...
				if( type == 2 && pPrimaryWeapon )
				{
					...
					{
						//g_pShuttleChild->CreateRobotAGearWeapon(pAGearWeapon, type);
						g_pShuttleChild->ShowWeapon(1, TRUE);
					}
					// END 2014-08-01 by ymjoo A기어 로봇아머 기본무기 렌더 안되는 문제 수정
				}
			}
...
void CStoreData::UpdateWearItemData( CItemInfo* pItemInfo, BOOL bWearItem)
{
	...
	if(it1 == m_mapItemWindowPosition.end() && g_pShuttleChild->m_pPrimaryWeapon)
	{
		...
		SAFE_DELETE(g_pShuttleChild->m_pPrimaryWeapon);
	...
	if(it2 == m_mapItemWindowPosition.end() && g_pShuttleChild->m_pSecondaryWeapon)
	{
		...
		SAFE_DELETE(g_pShuttleChild->m_pSecondaryWeapon);
...
//			g_pD3dApp->SetUnitArmorColorNum(max(1, nColorCode%100));
			...
			// 2010. 03. 18 by jskim 몬스터변신 카드
 			if( !g_pShuttleChild->IsRobotArmor() )
 			{
				...
				{
					// 2014-09-11 by ymjoo A기어 기본무기 없는 상태로 아머 변경시 문제 수정
					//ITEM* pItem = g_pShuttleChild->m_pPrimaryWeapon->GetRealItemInfo();
					//ITEM* pShapeItem = g_pDatabase->GetServerItemInfo(g_pShuttleChild->m_pPrimaryWeapon->GetItemGeneral()->GetShapeItemNum());
					ITEM* pItem	= NULL;
					ITEM* pShapeItem = NULL;
					if(g_pShuttleChild->m_pPrimaryWeapon)
					{
						pItem = g_pShuttleChild->m_pPrimaryWeapon->GetRealItemInfo();
						pShapeItem = g_pDatabase->GetServerItemInfo(g_pShuttleChild->m_pPrimaryWeapon->GetItemGeneral()->GetShapeItemNum());
					}
					// END 2014-09-11 by ymjoo A기어 기본무기 없는 상태로 아머 변경시 문제 수정
					g_pShuttleChild->CreatePrimaryWeaponItem(pItem, pShapeItem);
				}
				if(g_pShuttleChild->GetMonsterTransformer())
					//g_pShuttleChild->ShowWeapon( FALSE );
					g_pShuttleChild->ShowWeapon(0, FALSE);
...
// 2014-11-27 by jwLee 인벤 아이템 검색. ( 유니크 아이디 기준. )
const CItemInfo* CStoreData::FindItemInInventoryByUid ( UID64_t ItemUniqueNumber )
{
	
	if ( NULL == ItemUniqueNumber )
		return NULL;
	...
	while(it != m_mapItemUniqueNumber.end())
	{
		...
		if ( pcInvenItemInfo->UniqueNumber == ItemUniqueNumber )
		{
			return it->second;
		}
...
// end 2014-11-27 by jwLee 인벤 아이템 검색. ( 유니크 아이디 기준. )
...
{
	// 2014-08-07 by ymjoo 보급상점 총알 묶음 개수 버그 수정
// 	CMapItemInventoryIterator it = m_mapItemUniqueNumber.begin();
// 	while(it != m_mapItemUniqueNumber.end())
// 	{
// 		CItemInfo* pItemInfo = it->second;
...
// 	}
//	return NULL;
	return g_pDatabase->FindBulletItemToDB(bCaliber);
	// END 2014-08-07 by ymjoo 보급상점 총알 묶음 개수 버그 수정
}
...
{
	BOOL bArmor = ( nItemWear == WEAR_ATTACHED )&& (  nItemKind == ITEMKIND_DEFENSE);
	// 아머만 체크썸을 보내준다. 
	if(FALSE == bArmor)
	{
		return;
...
// 2014-02-07 by ymjoo 캡슐형 캐시 아이템 경고 메세지
bool CStoreData::IsCashShopItem(ITEM_BASE* pItem)
{
	UINT itemNum = (UINT)pItem->ItemNum;
	if(FindCashShopData(itemNum))
	{
		return true;
	}
	return false;
...
SHOP_ITEM* CStoreData::FindCashShopData(UINT ItemNum)
{
	vector<SHOP_ITEM*>::iterator it = m_vecCashItemInfo.begin();
	while(it != m_vecCashItemInfo.end())
	{
...
int CStoreData::InputCashItem(char* pMsg)
{
	if(pMsg)
	{
		...
		if(item)
		{
			...
			if(FindCashShopData(pItem->ItemNum) == NULL)
			{
				m_vecCashItemInfo.push_back(pItem);
			}
			else
			{
				SAFE_DELETE(pItem);
