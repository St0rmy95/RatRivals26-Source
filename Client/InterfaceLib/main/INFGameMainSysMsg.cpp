...
{
	m_pBoxImage = NULL;
// 	int i; for(i=0;i<MAX_BOX_IMAGE;i++)
// 	{
...
{
	// 2008. 11. 21 by ckPark 서버 시간 보여주기, 박스 수정
	SAFE_DELETE(m_pBoxImage);
// 	int i; for(i=0;i<MAX_BOX_IMAGE;i++)
// 		SAFE_DELETE(m_pBoxImage[i]);

	SAFE_DELETE(m_pFontTxt);
	// 2014-06-27 by ymjoo DrawText 성능 개선 작업 (DBG 텍스트)
#ifdef C_DRAWTEXT_UPGRADE_YMJOO
	SAFE_DELETE(m_pFontTxt2);	
#endif
	// END 2014-06-27 by ymjoo DrawText 성능 개선 작업 (DBG 텍스트)
}
...
// 	}
	...
	if(NULL == m_pFontTxt2)
	{
		m_pFontTxt2 = new CD3DHanFont(_T(g_pD3dApp->GetFontStyle()), 9, D3DFONT_ZENABLE, TRUE, 512, 32);
		m_pFontTxt2->InitDeviceObjects(g_pD3dDev);
	}
#endif
	// END 2014-06-27 by ymjoo DrawText 성능 개선 작업 (DBG 텍스트)
	return S_OK;
...
// 		m_pBoxImage[i]->RestoreDeviceObjects();
	...
	if(m_pFontTxt2)
	{
		m_pFontTxt2->RestoreDeviceObjects();
	}
#endif
	// END 2014-06-27 by ymjoo DrawText 성능 개선 작업 (DBG 텍스트)
...
// 	}
	...
	if(m_pFontTxt2)
	{
		m_pFontTxt2->DeleteDeviceObjects();
		SAFE_DELETE(m_pFontTxt2);
	}
#endif
	// END 2014-06-27 by ymjoo DrawText 성능 개선 작업 (DBG 텍스트)
...
// 	}
	...
	if(m_pFontTxt2)
	{
		m_pFontTxt2->InvalidateDeviceObjects();
	}
#endif
	// END 2014-06-27 by ymjoo DrawText 성능 개선 작업 (DBG 텍스트)
...
//	RenderCenterWindow(m_ptBkPos.x, m_ptBkPos.y, m_nWidth, m_nHeight);
	...
	for(nCnt =1; nCnt < m_nMaxLine;nCnt++)
	{
		...
		{
			m_pFontTxt2->DrawText(nPosX - m_nStringSize[nCnt].cx, nPosY, GUI_FONT_COLOR, m_szStringDesc[nCnt], 0L);
		}
#else
		m_pFontTxt->DrawText(nPosX - m_nStringSize[nCnt].cx, nPosY, GUI_FONT_COLOR,m_szStringDesc[nCnt],0L);
#endif
...
void CINFGameMainSysMsg::ShowWnd(BOOL bShowWnd, POINT *ptPos/*=NULL*/, int nWndWidth/*=0*/)
{
	...
	if(bShowWnd && ptPos)
	{
		...
		for(nCnt =0; nCnt < m_nMaxLine;nCnt++)
		{
			...
			if(nCnt == 0)
			{
				size = m_pFontTxt->GetStringSize(m_szStringDesc[nCnt]);
			}
			else
			{
				size = m_pFontTxt2->GetStringSize(m_szStringDesc[nCnt]);
			}
#else
