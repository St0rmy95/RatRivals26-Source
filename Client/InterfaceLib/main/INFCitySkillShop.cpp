...
CINFCitySkillShop::CINFCitySkillShop(CAtumNode* pParent, BUILDINGNPC* pBuilding)
{
	...
	int i; for(i=0;i<4;i++)
	{
		...
		// 2014-12-01 by jwLee 현재 사용하지 않는 리소스 주석 처리 (2006년도 ui 스킬 구입 버튼)
//		m_pBuyButton[i] = NULL;// [3]은 NULL
		//end 2014-12-01 by jwLee 현재 사용하지 않는 리소스 주석 처리 (2006년도 ui 스킬 구입 버튼)
		m_pItemTab[i] = NULL;
...
//{
//	vector<SHOP_ITEM*>::iterator it = m_vecItemInfo[m_nCurrentTab].begin();
//	int i; for(i=0;i<m_nCurrentScrollNumber[m_nCurrentTab]+nSlotNumber;i++)
//	{
//		if(it != m_vecItemInfo[m_nCurrentTab].end())
//		{
//			it++;
