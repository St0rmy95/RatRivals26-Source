...
#define		SEARCH_ITEMLIST_POS_WIDTH			452				// 2013-10-08 by ssjung 조합 검색창에서 조합식 선택 영역이 이미지 보다 큰 부분 수정
...
CINFItemMixWnd::~CINFItemMixWnd()
{
	...
	SAFE_DELETE(m_pEditItemName);
	SAFE_DELETE(m_pSearchBtn);	 
	//SAFE_DELETE(m_pComboKindEx);
	...
	case SEARCH_TYPE_ITEMNUM:
		{
			if(m_nShowItemNum > 0)
			{
				...
				if(pShowItem)
				{
					...
					{
						// 2013-10-10 by bhsohn 인게임 조합식 아이콘 잘못 나오는 현상 체크
//						sprintf( chIconName, "%08d", SKILL_BASE_SOURCEINDEX( pShowItem->SourceIndex ) );							
						sprintf( chIconName, "%08d", pShowItem->SourceIndex);	
						// END 2013-10-10 by bhsohn 인게임 조합식 아이콘 잘못 나오는 현상 체크
