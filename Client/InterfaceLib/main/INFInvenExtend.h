...
public:
	...
	CD3DHanFont*	GetFontWP();		// 2014-06-30 by ymjoo DrawText 성능 개선 작업 (인벤토리)
	...
	BOOL		IsPossibleUseCardItem(DestParam_t	desParam);		// 2013-08-22 by bhsohn 마을에서 서치아이 사용 못하게 처리

	// 2013-08-27 by bhsohn 미션 프리 패스 시스템
	void			DoModalMissionFreePass(ITEM_BASE* pItem);
	void			UseSendMissionFreePassItem(UID64_t UniqueNumber);
...
//	int					m_nSelectItemWindowPosition;// 클릭된 아이템의 Window에서의 위치 // 2006-06-21 by ispark, m_SelectItem->ItemWindowIndex 이 변수를 쓰기로 함
	...
	UID64_t				m_nUseCapsuleCashItemUniqueNum;		// 2014-02-07 by ymjoo 캡슐형 캐시 아이템 경고 메세지
	...
	UID64_t GetUseCapsuleCashItemUniqueNum()			{ return m_nUseCapsuleCashItemUniqueNum; }		// 2014-02-07 by ymjoo 캡슐형 캐시 아이템 경고 메세지
	void SetUseCapsuleCashItemUniqueNum(UID64_t num)	{ m_nUseCapsuleCashItemUniqueNum = num; }		// 2014-02-07 by ymjoo 캡슐형 캐시 아이템 경고 메세지
	
	// 2014-06-10 by ymjoo 세력 변경 아이템
	UID64_t GetUseByMessageItemUniqueNum()				{ return m_nUseByMessageItemUniqueNum; }
	void SetUseByMessageItemUniqueNum(UID64_t num)		{ m_nUseByMessageItemUniqueNum = num; }
...
protected:
	...
	BOOL				m_bTradeItemCenterState;				 // 2013-11-29 by ssjung 거래소 구현

	map<BYTE,float>		m_mapChangeWearItemCoolTime;	// 2014-04-16 by ymjoo 장비 교체시 쿨타임 추가

	UID64_t				m_nUseByMessageItemUniqueNum;	// 2014-06-10 by ymjoo 세력 변경 아이템		
};
