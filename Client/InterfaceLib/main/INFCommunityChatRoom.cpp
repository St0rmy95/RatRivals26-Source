...
#define CHATROOM_BTN_TIME_GAP				0.5f
	...
	#define CHATROOM_LIST_DATA_SUBJECT_FONT_X	(m_ptCommunityPos.x + 232)	// 2012-03-29 by mspark, 채팅방 정보창에서 제목 / 캐릭터 이름 등 내용 위치 수정 - 기존 210에서 209로 수정		
	#define CHATROOM_LIST_DATA_RM_FONT_X		(m_ptCommunityPos.x + 392)	// 2012-03-29 by mspark, 채팅방 정보창에서 제목 / 캐릭터 이름 등 내용 위치 수정 - 기존 350에서 346로 수정
	...
	#define CHATROOM_VOICE_OK_BTN_Y				(m_ptCommunityPos.y + 200)
	#define CHATROOM_VOICE_CAN_BTN_X			(m_ptCommunityPos.x + 389)
	#define CHATROOM_VOICE_CAN_BTN_Y			(m_ptCommunityPos.y + 200)
	...
	#define CHATROOM_EDIT_SUBJECT_X				(m_ptCommunityPos.x + 286)	// 2012-03-29 by mspark, 채팅방 정보창에서 제목 / 캐릭터 이름 등 내용 위치 수정 - 기존 281에서 278로 수정
	...
	#define CHATROOM_CREATE_OK_BTN_Y			(m_ptCreatePos.y + 158)
	#define CHATROOM_CREATE_CAN_BTN_X			(m_ptCreatePos.x + 226)
	#define CHATROOM_CREATE_CAN_BTN_Y			(m_ptCreatePos.y + 158)
	...
	#define CHATROOM_CREATE_EDIT_SUBJECT_X		(m_ptCreatePos.x + 43)	// 2012-03-29 by mspark, 채팅방 정보창에서 제목 / 캐릭터 이름 등 내용 위치 수정 - 기존 33에서 30으로 수정
	#define CHATROOM_CREATE_EDIT_SUBJECT_Y		(m_ptCreatePos.y + 69)
...
// 	m_pImgRoomBg->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize);
	...
	m_pBtnVoiceOk->InitDeviceObjects("okb0");
	if(NULL == m_pBtnVoiceCan)
	{
		m_pBtnVoiceCan = new CINFImageBtn;
	}
	m_pBtnVoiceCan->InitDeviceObjects("canb0");
#else
	if(NULL == m_pBtnVoiceOk)
	{
		m_pBtnVoiceOk = new CINFImageBtn;
...
void CINFCommunityChatRoom::RenderList()
{
	...
	for(int i = 0 ; i < CHATROOM_LIST_SCR_COUNT ; i++)
	{
		...
		if( m_pFontRoomNum->GetStringSize(it->ChatRoomName).cx > 229 )
		{
			memset(chResultBuf, 0x00, 512);
			strncpy(chResultBuf, it->ChatRoomName, strlen(it->ChatRoomName)-1);
			do 
			{
				chResultBuf[GetStringBuffPos(chResultBuf, GetStringBuffLen(chResultBuf)-1)] = '\0';
			} while ( m_pFontRoomNum->GetStringSize(chResultBuf).cx > 217 );
			sprintf(buf, "%s..", chResultBuf);
		}
 		else
 		{
...
//	m_pScrRoom->SetMouseBallRect(rtWhell);
	...
	SIZE size = {0, 0};
	int charNamePoint = 0;
#if defined C_CHATROOM_MEMBER_LEVEL_YMJOO
	int charLevelPoint = 0;		// 2014-01-28 by ymjoo 채팅방 lv 정보 추가
#endif
	...
	for(int i = 0 ; i < CHATROOM_SCR_COUNT ; i++)
	{
		...
		// 2014-01-28 by ymjoo 채팅방 lv 정보 추가
#if defined C_CHATROOM_MEMBER_LEVEL_YMJOO
		wsprintf(buf,"%d",it->CharacterLevel);
		size = m_pFontRoomNum->GetStringSize(buf);
		charLevelPoint = CHATROOM_DATA_NAME_X + 100 - (INT)(size.cx/2);

		m_pFontRoomNum->DrawText(charLevelPoint ,
			CHATROOM_DATA_START_Y + (CHATROOM_DATA_GAP_H * i),
			GUI_FONT_COLOR_W,
			buf);
#endif
		// END 2014-01-28 by ymjoo 채팅방 lv 정보 추가
...
void CINFCommunityChatRoom::RenderRoomInfo()
{
...
#if defined C_REMOVE_VOIP_YMJOO
	char chResultBuf[512];
	if( m_pFontRoomNum->GetStringSize(m_stOtherRoomInfo.ChatRoomName).cx > 186 )
	{
		memset(chResultBuf, 0x00, 512);
		strncpy(chResultBuf, m_stOtherRoomInfo.ChatRoomName, strlen(m_stOtherRoomInfo.ChatRoomName)-1);
		do 
		{
			chResultBuf[GetStringBuffPos(chResultBuf, GetStringBuffLen(chResultBuf)-1)] = '\0';
		} while ( m_pFontRoomNum->GetStringSize(chResultBuf).cx > 174 );
		sprintf(buf, "%s..", chResultBuf);
	}
	else
	{
	...
	for(int i = 0 ; i < CHATROOM_INFO_SCR_COUNT ; i++)
	{
		...
		wsprintf(buf,"%d",it->CharacterLevel);
		size = m_pFontRoomNum->GetStringSize(buf);
		charLevelPoint = CHATROOM_INFO_DATA_NAME_X + 100 - (INT)(size.cx/2);
		
		m_pFontRoomNum->DrawText(charLevelPoint ,
			CHATROOM_INFO_DATA_START_Y + 1 + (CHATROOM_INFO_DATA_GAP_H * i),
			GUI_FONT_COLOR_W,
			buf);
#endif
		// END 2014-01-28 by ymjoo 채팅방 lv 정보 추가
...
#if defined C_CHATROOM_MEMBER_LEVEL_YMJOO
	sMemberInfo.CharacterLevel = pMsg->CharacterLevel;	// 2014-01-28 by ymjoo 채팅방 lv 정보 추가
#endif
...
#if defined C_CHATROOM_MEMBER_LEVEL_YMJOO
		stRoom.CharacterLevel			= pRoom->CharacterLevel;	// 2014-01-28 by ymjoo 채팅방 lv 정보 추가
#endif
...
#if defined C_CHATROOM_MEMBER_LEVEL_YMJOO
	sMember.CharacterLevel = g_pShuttleChild->m_myShuttleInfo.Level;	// 2014-01-28 by ymjoo 채팅방 lv 정보 추가
#endif
