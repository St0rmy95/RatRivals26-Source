...
// 2014-01-15 by ssjung 파괴 경매장 구현
	...
	#define GAMEMAIN_BUTTON_RETURNAUCTION		4
	#define GAMEMAIN_BUTTON_SYSTEM				5
	#define GAMEMAIN_BUTTON_NUMBER				6
#else
...
class CINFBaseWarManager;										 // 2013-08-23 by ssjung 거점전 리뉴얼
class CINFFirstAnniversaryEvent;						// 2013-10-01 by ssjung 1주년 이벤트
class CINFWorldMap;									// 2014-03-11 by ssjung&ymjoo 월드맵 구현
...
//	BOOL SaveEnvironment();
	...
	BOOL IsShowing() { return	!(m_nLeftWindowInfo == LEFT_WINDOW_WEAPON && 
	...
	int GetMaxBulletItem(int nMaxBullet, int nColorCode = 0);
	...
	BOOL FindMotherShipMapIdx(MapIndex_t i_MapIndex, BOOL bCurrent = FALSE);					 //0이면 기본 , 1이면 현재 진행중		// 2014-03-11 by ssjung&ymjoo 월드맵 구현
	...
	BOOL StringCullingUserData_ToNormal( char* strTalk, int nCullPixel, vector<string> *vecDstString, CD3DHanFont* pFont, BOOL bChRt = FALSE, BOOL bColorState = TRUE);
	char StringCullingAddVector_ToNormal(char chColorSave, char* pString, vector<string> *vecDstString, BOOL bColorState = TRUE);
...
//	void SetItemInfoUser( CItemInfo* pItemInfo, int x, int y, BOOL bShowMyEq=TRUE);
	...
	void SetItemInfoNormal( ITEM* pITEM, int x, int y, BOOL bShop=TRUE, int nItemCount = 0, BOOL bShowMyEq=TRUE, int nLinkItem=0,  BOOL ArmorCollect = FALSE, BOOL bCanadaCashShop = FALSE);
...
// 2013-08-23 by ssjung 거점전 리뉴얼
	CINFBaseWarManager* GetINFBaseWarManager() {return m_pBaseWarManager;}
	void AddBaseWarInfo(MapIndex_t	MapIndex, float fSummonLiveTime, short MapInfluenceType = NULL);
	void DestroyBaseWarInfo(MapIndex_t MapIndex, SHORT MapInfluenceType);	
	void SetNgcBaseWarInfo(MSG_NGCSPWAR_DISPLAY* pinfo, BOOL bPopUp = FALSE);
	BOOL GetNGCWarState();
// end 2013-08-23 by ssjung 거점전 리뉴얼

	// 2013-08-22 by bhsohn 해피아워 리뉴얼
	void SetAllHappyHourInfo(EVENT_INFO* pAllHappyHourEventInfo);
	EVENT_INFO* GetAllHappyHourInfo(){ return &m_AllHappyHourEventInfo;}

	BOOL UserWarpRestriction();			// 2013-09-02 by ssjung 필드이동금지아이템

// 2013-10-01 by ssjung 1주년 이벤트
	void FirstAnniversaryEventItemUse(BOOL bShow);
	void RenderFirstAnniversaryEvent();
	int  WndProcFirstAnniversaryEvent(UINT uMsg, WPARAM wParam, LPARAM lParam);
// end 2013-10-01 by ssjung 1주년 이벤트
// 2014-03-11 by ssjung&ymjoo 월드맵 구현
	int WndProcWorldMap(UINT uMsg, WPARAM wParam, LPARAM lParam);	// 월드맵 처리
	void RenderWorldMap();
	BOOL IsShowWorldMap();
	void ShowWorldMapWnd(BOOL bShow);
...
// 2014-06-09 by ymjoo 프로파일러 구현
	void RenderProfiler();
	BOOL IsShowProfiler();
	void ShowProfiler(BOOL bShow);
// END 2014-06-09 by ymjoo 프로파일러 구현

	void PopupReturnAuctionWnd();							// 2014-01-15 by ssjung 파괴 경매장 구현

	BOOL GetChatModeState();		   // 2013-11-14 by ssjung 비행 중 채팅 할시 조작키가 작동하는 문제 수정
public:
	//좌, 우 윈도우 위치 계산한 결과값
	int		m_nLeftWindowY;
	int		m_nRightWindowY;
...
//#endif
	// 2014-05-07 by ymjoo 인터페이스를 꺼도 거점전 시간이 흐르도록 수정
	CINFBaseWarManager*			m_pBaseWarManager;						// 2013-08-23 by ssjung 거점전 리뉴얼`
	// END 2014-05-07 by ymjoo 인터페이스를 꺼도 거점전 시간이 흐르도록 수정
private:
	// 2007-07-04 by bhsohn 오퍼레이터 추가
	...
	EVENT_INFO  m_AllHappyHourEventInfo;	// 2013-08-22 by bhsohn 해피아워 리뉴얼

	CINFFirstAnniversaryEvent*		m_pFirstAnniversaryEvent;		// 2013-10-01 by ssjung 1주년 이벤트

	CINFWorldMap*					m_pWorldMap;			// 2014-03-11 by ssjung&ymjoo 월드맵 구현
...
public:
	...
	BOOL GetFirstAnniversaryEventIsShowWnd();		// 2013-10-01 by ssjung 1주년 이벤트
