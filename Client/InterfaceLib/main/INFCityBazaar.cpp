...
BOOL CINFCityBazaar::GetPointInType(POINT &pt,int type,int Num)
{
	...
	else if(type == BAZAAR_ICON)
	{
		if(pt.x >= BAZAAR_SHOP_ICON_X &&
				...
				pt.y <= BAZAAR_SHOP_ITEM_Y + (BAZAAR_SHOP_ITEM_H * BAZAAR_ITEM_SLOT_NUMBER))		// 2014-09-04 by ymjoo 컴파일 경고 제거 작업
		{
			return TRUE;
		}
		return FALSE;
...
CINFCityBazaarOpen::CINFCityBazaarOpen(CAtumNode* pParent, CGameData* pGameData, int nBazaarType)
{
	...
	m_nScrollMaxLine	= BAZAAR_ITEM_SLOT_NUMBER;		// 2014-09-04 by ymjoo 컴파일 경고 제거 작업
...
{
	// 2006-08-29 by dgwoo 바자샾이 활성화&비활성화인지 .
	if(!m_bBazaarMinAct)
	{//비활성화시.
		Render_Disable();
		return;
...
void CINFCityBazaarOpen::Render_Sell()
{
	...
	while(ite != m_vecOpenBazaarItem.end())
	{
		...
		if(nLineCount >= 0)
		{
			if(nLineCount >= BAZAAR_ITEM_SLOT_NUMBER)		// 2014-09-04 by ymjoo 컴파일 경고 제거 작업
			{
				break;
			}
#ifdef C_EPSODE4_UI_CHANGE_JSKIM    // 2011. 10. 10 by jskim UI시스템 변경
...
if(m_byFocusText == TEXT_COUNT && m_nSelectIndex == i)
	...
	if(j < 0)
		j = 0;
	for(j; j < BAZAAR_ITEM_SLOT_NUMBER; j++)		// 2014-09-04 by ymjoo 컴파일 경고 제거 작업
	{
...
void CINFCityBazaarOpen::Render_Buy()
{
	...
	if(nRenderIndex >= 0 &&
		nRenderIndex < BAZAAR_ITEM_SLOT_NUMBER)		// 2014-09-04 by ymjoo 컴파일 경고 제거 작업
	{
	...
	case WM_LBUTTONDOWN:
		{
			...
			for(i = 0; i < BAZAAR_ITEM_SLOT_NUMBER; i++)		// 2014-09-04 by ymjoo 컴파일 경고 제거 작업
			{
				//아이템의 버튼의 상태를 조정해준다.
...
{
	auto it1 = FindItemItr(pMsg->itemUID);
	OPEN_BAZAAR_ITEM * it = &*it1;
...
{
	// 삭제
	auto it1 = FindItemItr(pMsg->nBuyItemIndex0, 2);
	OPEN_BAZAAR_ITEM * it = &*it1;
	if(it)
	{
...
void CINFCityBazaarOpen::OnSellItem(CItemInfo* pSelectItem)
{
	...
	// 화면 라인을 넘어가면
	if(m_vecOpenBazaarItem.size() > m_nScrollLine + BAZAAR_ITEM_SLOT_NUMBER)		// 2014-09-04 by ymjoo 컴파일 경고 제거 작업
	{
		m_nScrollLine++;
...
void CINFCityBazaarOpen::Send_RegItem(int nSelectItem)
{
	if(m_nSelectIndex != -1)
	{
		...
		if(m_byBazaarType == 1)
		{
			...
			// 2014-06-25 by ymjoo 판매 상점 아이템 등록시 아머 컬렉션 해제
#ifdef SC_COLLECTION_ARMOR_JHSEOL_BCKIM
			CItemInfo* pItem = m_vecOpenBazaarItem[m_nSelectIndex].pItemBase;
...
#endif
			// END 2014-06-25 by ymjoo 판매 상점 아이템 등록시 아머 컬렉션 해제
...
void CINFCityBazaarOpen::Recv_SellBuyItemOK(MSG_FC_BAZAAR_SELL_BUY_ITEM_OK* pMsg)
{
	...
	if(pEnemy)
	{
		auto it = FindItemItr(pMsg->itemUID);
		OPEN_BAZAAR_ITEM* pBazaarItem = &*it;
		if(pBazaarItem)
		{
			char strLogMsg[1024];
			ATUM_DATE_TIME tmATime(TRUE);
	...
	{
		itEnemy = g_pScene->m_mapEnemyList.begin();
...
void CINFCityBazaarOpen::Recv_BuySellItemOK(MSG_FC_BAZAAR_BUY_SELL_ITEM_OK* pMsg)
{
	...
	if(pEnemy)
	{
		auto it = FindItemItr(pMsg->nBuyItemIndex0, 2);
		OPEN_BAZAAR_ITEM* pBazaarItem = &*it;
		if(pBazaarItem)
		{
			char strLogMsg[1024];
...
{
	// 확인용
	auto it = FindItemItr(pMsg->nBuyItemIndex0);
	VISIT_BAZAAR_ITEM* pBazaarVisitItem = &*it;
	if(pBazaarVisitItem)
	{
		pBazaarVisitItem->nAmount0 -= pMsg-> nAmount0;
