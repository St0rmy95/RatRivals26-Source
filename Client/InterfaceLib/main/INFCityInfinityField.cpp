...
CINFCityInfinityField::CINFCityInfinityField()
{
	...
	m_pFont			= NULL;		// 폰트
#endif
	// END 2014-07-01 by ymjoo DrawText 성능 개선 작업 (아레나 입장)
...
HRESULT CINFCityInfinityField::InitDeviceObjects()
{
	...
	for(int i = 0 ; i < 14 + 1 ; ++i)
	{
		m_pFont[i] = new CD3DHanFont(g_pD3dApp->GetFontStyle(), 8, D3DFONT_ZENABLE, TRUE, 1024, 32);
		m_pFont[i]->InitDeviceObjects(g_pD3dDev);
	}
#else
...
HRESULT CINFCityInfinityField::RestoreDeviceObjects()
{
	...
	for(int i = 0 ; i < 14 + 1 ; ++i)
	{
		if(m_pFont[i])
		{
			m_pFont[i]->RestoreDeviceObjects();
		}
	}
#else
 	if( m_pFont )
...
HRESULT CINFCityInfinityField::InvalidateDeviceObjects()
{
	...
	for(int i = 0 ; i < 14 + 1 ; ++i)
	{
		if(m_pFont[i])
		{
			m_pFont[i]->InvalidateDeviceObjects();
		}
	}
#else
 	if( m_pFont )
...
void	CINFCityInfinityField::Render()
{
	...
	// 2014-07-01 by ymjoo DrawText 성능 개선 작업 (아레나 입장)
#ifdef C_DRAWTEXT_UPGRADE_YMJOO
	STRING_CULL(STRMSG_C_091103_0301, 580, &vecMessage, m_pFont[14]);
#else
	...
	// 2014-07-01 by ymjoo DrawText 성능 개선 작업 (아레나 입장)
#ifdef C_DRAWTEXT_UPGRADE_YMJOO
	STRING_CULL(STRMSG_C_091103_0302, 580, &vecMessage, m_pFont[14]);
#else
	...
	// 2014-07-01 by ymjoo DrawText 성능 개선 작업 (아레나 입장)
#ifdef C_DRAWTEXT_UPGRADE_YMJOO
	STRING_CULL(STRMSG_C_091103_0303, 580, &vecMessage, m_pFont[14]);
#else
	...
	// 2014-07-01 by ymjoo DrawText 성능 개선 작업 (아레나 입장)
#ifdef C_DRAWTEXT_UPGRADE_YMJOO
	STRING_CULL(STRMSG_C_091103_0304, 580, &vecMessage, m_pFont[14]);
#else
	...
	for( i=0; i<vecTotalMessage.size(); ++i )
	{
		if( i >= m_pDescScroll->GetScrollStep() && nRenderCnt < 14 )
		{
			...
			// 2014-07-01 by ymjoo DrawText 성능 개선 작업 (아레나 입장)
#ifdef C_DRAWTEXT_UPGRADE_YMJOO
			m_pFont[nRenderCnt]->DrawText(bkPos.x + 35.0f, bkPos.y + (int)(fHeight), GUI_FONT_COLOR, szBuff);
			fHeight += m_pFont[nRenderCnt]->GetStringSize(szBuff).cy * 1.5f;
#else
...
// 2013-08-28 by ssjung 거점전 리뉴얼(NGC)
				if(g_pGameMain->GetNGCWarState())
				{
					g_pD3dApp->m_pChat->CreateChatChild(STRMSG_C_130828_0002,COLOR_ERROR);	//NGC거점전이 진행 중이므로 참가할 수 없습니다. 
					return INF_MSGPROC_BREAK;	
				}
...
CINFCityInfinityFieldPopUp::CINFCityInfinityFieldPopUp()
{
	...
	m_pFontMapName = NULL;
	m_pFontLevel = NULL;
	m_pFontAdmission = NULL;
	for(i = 0 ; i < 10 ; ++i)
	{
		for(int j = 0 ; j < 4 ; ++j)
		{
			m_pFontList[i][j] = NULL;
		}
	}
	m_pFontMapName2 = NULL;
	for(i = 0 ; i < 7 ; ++i)
	{
		m_pFontMapExplain[i] = NULL;
	}
	for(i = 0 ; i < 6 ; ++i)
	{
		m_pFontChat[i] = NULL;
	}
	m_pFontUserName = NULL;
	m_pFontUserLevel = NULL;
	m_pFontUserGear = NULL;
	m_pFontStrCull = NULL;
#else
	m_pFont	= NULL;
#endif
...
HRESULT CINFCityInfinityFieldPopUp::InitDeviceObjects()
{
	...
	int i;
#ifdef C_DRAWTEXT_UPGRADE_YMJOO
	m_pFontMapName = new CD3DHanFont(g_pD3dApp->GetFontStyle(), 8, D3DFONT_ZENABLE, TRUE, 1024, 32);
	m_pFontMapName->InitDeviceObjects(g_pD3dDev);
	m_pFontLevel = new CD3DHanFont(g_pD3dApp->GetFontStyle(), 8, D3DFONT_ZENABLE, TRUE, 1024, 32);
	m_pFontLevel->InitDeviceObjects(g_pD3dDev);
	m_pFontAdmission = new CD3DHanFont(g_pD3dApp->GetFontStyle(), 8, D3DFONT_ZENABLE, TRUE, 1024, 32);
	m_pFontAdmission->InitDeviceObjects(g_pD3dDev);
	for(i = 0 ; i < 10 ; ++i)
	{
		for(int j = 0 ; j < 4 ; ++j)
		{
			m_pFontList[i][j] = new CD3DHanFont(g_pD3dApp->GetFontStyle(), 8, D3DFONT_ZENABLE, TRUE, 1024, 32);
			m_pFontList[i][j]->InitDeviceObjects(g_pD3dDev);
		}
	}
	m_pFontMapName2 = new CD3DHanFont(g_pD3dApp->GetFontStyle(), 8, D3DFONT_ZENABLE, TRUE, 1024, 32);
	m_pFontMapName2->InitDeviceObjects(g_pD3dDev);
	for(i = 0 ; i < 7 ; ++i)
	{
		m_pFontMapExplain[i] = new CD3DHanFont(g_pD3dApp->GetFontStyle(), 8, D3DFONT_ZENABLE, TRUE, 1024, 32);
		m_pFontMapExplain[i]->InitDeviceObjects(g_pD3dDev);
	}
	for(i = 0 ; i < 6 ; ++i)
	{
		m_pFontChat[i] = new CD3DHanFont(g_pD3dApp->GetFontStyle(), 8, D3DFONT_ZENABLE, TRUE, 1024, 32);
		m_pFontChat[i]->InitDeviceObjects(g_pD3dDev);
	}
	m_pFontUserName = new CD3DHanFont(g_pD3dApp->GetFontStyle(), 8, D3DFONT_ZENABLE, TRUE, 1024, 32);
	m_pFontUserName->InitDeviceObjects(g_pD3dDev);
	m_pFontUserLevel = new CD3DHanFont(g_pD3dApp->GetFontStyle(), 8, D3DFONT_ZENABLE, TRUE, 1024, 32);
	m_pFontUserLevel->InitDeviceObjects(g_pD3dDev);
	m_pFontUserGear = new CD3DHanFont(g_pD3dApp->GetFontStyle(), 8, D3DFONT_ZENABLE, TRUE, 1024, 32);
	m_pFontUserGear->InitDeviceObjects(g_pD3dDev);
	m_pFontStrCull = new CD3DHanFont(g_pD3dApp->GetFontStyle(), 8, D3DFONT_ZENABLE, TRUE, 1024, 32);
	...
	char szTemp[ MAX_PATH ] = {0,};
	//int i;			// 2014-07-01 by ymjoo DrawText 성능 개선 작업 (아레나 입장)
...
HRESULT CINFCityInfinityFieldPopUp::RestoreDeviceObjects()
{
	...
	if(m_pFontAdmission)
	{
		m_pFontAdmission->RestoreDeviceObjects();
	}
	for(i = 0 ; i < 10 ; ++i)
	{
		for(int j = 0 ; j < 4 ; ++j)
		{
			if(m_pFontList[i][j])
			{
				m_pFontList[i][j]->RestoreDeviceObjects();
			}
		}
	}
	if(m_pFontMapName2)
	{
		m_pFontMapName2->RestoreDeviceObjects();
	}
	for(i = 0 ; i < 7 ; ++i)
	{
		if(m_pFontMapExplain[i])
		{
			m_pFontMapExplain[i]->RestoreDeviceObjects();
		}
	}
	for(i = 0 ; i < 6 ; ++i)
	{
		if(m_pFontChat[i])
		{
			m_pFontChat[i]->RestoreDeviceObjects();
		}
	}
	if(m_pFontUserName)
	{
		m_pFontUserName->RestoreDeviceObjects();
	}
	if(m_pFontUserLevel)
	{
		m_pFontUserLevel->RestoreDeviceObjects();
	}
	if(m_pFontUserGear)
	{
		m_pFontUserGear->RestoreDeviceObjects();
	}
	if(m_pFontStrCull)
	{
	...
	//int i=0;		// 2014-07-01 by ymjoo DrawText 성능 개선 작업 (아레나 입장)
	for( i=INFINITY_ZONESEL; i<INFINITY_MAX_STATE; ++i )
	{
		if( m_pInfinityBackGround[i] )
			m_pInfinityBackGround[i]->RestoreDeviceObjects();
...
HRESULT CINFCityInfinityFieldPopUp::DeleteDeviceObjects()
{
	...
	if(m_pFontAdmission)
	{
		m_pFontAdmission->DeleteDeviceObjects();
		SAFE_DELETE(m_pFontAdmission);
	}
	for(i = 0 ; i < 10 ; ++i)
	{
		for(int j = 0 ; j < 4 ; ++j)
		{
			if(m_pFontList[i][j])
			{
				m_pFontList[i][j]->DeleteDeviceObjects();
				SAFE_DELETE(m_pFontList[i][j]);
			}
		}
	}
	if(m_pFontMapName2)
	{
		m_pFontMapName2->DeleteDeviceObjects();
		SAFE_DELETE(m_pFontMapName2);
	}
	for(i = 0 ; i < 7 ; ++i)
	{
		if(m_pFontMapExplain[i])
		{
			m_pFontMapExplain[i]->DeleteDeviceObjects();
			SAFE_DELETE(m_pFontMapExplain[i]);
		}
	}
	for(i = 0 ; i < 6 ; ++i)
	{
		if(m_pFontChat[i])
		{
			m_pFontChat[i]->DeleteDeviceObjects();
			SAFE_DELETE(m_pFontChat[i]);
		}
	}
	if(m_pFontUserName)
	{
		m_pFontUserName->DeleteDeviceObjects();
		SAFE_DELETE(m_pFontUserName);
	}
	if(m_pFontUserLevel)
	{
		m_pFontUserLevel->DeleteDeviceObjects();
		SAFE_DELETE(m_pFontUserLevel);
	}
	if(m_pFontUserGear)
	{
		m_pFontUserGear->DeleteDeviceObjects();
		SAFE_DELETE(m_pFontUserGear);
	}
	if(m_pFontStrCull)
	{
...
HRESULT CINFCityInfinityFieldPopUp::InvalidateDeviceObjects()
{
	...
	if(m_pFontAdmission)
	{
		m_pFontAdmission->InvalidateDeviceObjects();
	}
	for(i = 0 ; i < 10 ; ++i)
	{
		for(int j = 0 ; j < 4 ; ++j)
		{
			if(m_pFontList[i][j])
			{
				m_pFontList[i][j]->InvalidateDeviceObjects();
			}
		}
	}
	if(m_pFontMapName2)
	{
		m_pFontMapName2->InvalidateDeviceObjects();
	}
	for(i = 0 ; i < 7 ; ++i)
	{
		if(m_pFontMapExplain[i])
		{
			m_pFontMapExplain[i]->InvalidateDeviceObjects();
		}
	}
	for(i = 0 ; i < 6 ; ++i)
	{
		if(m_pFontChat[i])
		{
			m_pFontChat[i]->InvalidateDeviceObjects();
		}
	}
	if(m_pFontUserName)
	{
		m_pFontUserName->InvalidateDeviceObjects();
	}
	if(m_pFontUserLevel)
	{
		m_pFontUserLevel->InvalidateDeviceObjects();
	}
	if(m_pFontUserGear)
	{
		m_pFontUserGear->InvalidateDeviceObjects();
	}
	if(m_pFontStrCull)
	{
...
void	CINFCityInfinityFieldPopUp::RenderZoneSel( void )
{
	...
	((*it).second)->Move( bkPos.x + 17, bkPos.y + 54 + nRenderCnt * 96 );
...
#ifdef C_DRAWTEXT_UPGRADE_YMJOO
			{
				m_pFontMapName->DrawText(bkPos.x + 340 - m_pFontMapName->GetStringSize(pMapInfo->MapName).cx / 2, 
										 bkPos.y + 82 + nRenderCnt * 96,
			...
			// 2014-07-01 by ymjoo DrawText 성능 개선 작업 (아레나 입장)
#ifdef C_DRAWTEXT_UPGRADE_YMJOO
			m_pFontLevel->DrawText(bkPos.x + 515 - m_pFontLevel->GetStringSize(szBuff).cx / 2,
								   bkPos.y + 82 + nRenderCnt * 96,
								   GUI_FONT_COLOR, szBuff);
#else
			...
			if( pInfo->Join )
			{
				// 2014-07-01 by ymjoo DrawText 성능 개선 작업 (아레나 입장)
#ifdef C_DRAWTEXT_UPGRADE_YMJOO
				m_pFontAdmission->DrawText(bkPos.x + 675 - m_pFontAdmission->GetStringSize(STRMSG_C_091103_0310).cx / 2, 
										   bkPos.y + 82 + nRenderCnt * 96, 
										   GUI_FONT_COLOR, STRMSG_C_091103_0310);
#else
			...
			else
			{
				...
				// 2014-07-01 by ymjoo DrawText 성능 개선 작업 (아레나 입장)
#ifdef C_DRAWTEXT_UPGRADE_YMJOO
				m_pFontAdmission->DrawText(bkPos.x + 675 - m_pFontAdmission->GetStringSize(STRMSG_C_091103_0311).cx / 2, 
										   bkPos.y + 82 + nRenderCnt * 96, 
										   GUI_FONT_COLOR, STRMSG_C_091103_0311);
#else
...
void	CINFCityInfinityFieldPopUp::RenderRoomSel( void )
{
	...
	if( pMapInfo )
	{
		...
		// 2014-07-01 by ymjoo DrawText 성능 개선 작업 (아레나 입장)
#ifdef C_DRAWTEXT_UPGRADE_YMJOO
		m_pFontMapName->DrawText(bkPos.x + 145 - m_pFontMapName->GetStringSize(pMapInfo->MapName).cx / 2, 
								 bkPos.y + 127, 
								 GUI_FONT_COLOR, pMapInfo->MapName);
#else
		...
		// 2014-07-01 by ymjoo DrawText 성능 개선 작업 (아레나 입장)
#ifdef C_DRAWTEXT_UPGRADE_YMJOO
		m_pFontLevel->DrawText(bkPos.x + 145 - m_pFontLevel->GetStringSize(szBuff).cx / 2, 
							   bkPos.y + 145, 
							   GUI_FONT_COLOR, szBuff);
#else
	...
	for( i=0; i<g_pD3dApp->GetInfinityManager()->GetInfinityReadyInfoCount(); ++i )
	{
		if( i >= m_pRoomSelRoomScroll->GetScrollStep() && nRenderCnt < 10 )
		{
			...
			// 2014-07-01 by ymjoo DrawText 성능 개선 작업 (아레나 입장)
#ifdef C_DRAWTEXT_UPGRADE_YMJOO
			m_pFontList[nRenderCnt][0]->DrawText(bkPos.x + 323 - m_pFontList[nRenderCnt][0]->GetStringSize(szBuffNum).cx / 2, 
												 bkPos.y + 111 + nRenderCnt * 31, 
												 GUI_FONT_COLOR, szBuffNum);
#else
			...
			// 2014-07-01 by ymjoo DrawText 성능 개선 작업 (아레나 입장)
#ifdef C_DRAWTEXT_UPGRADE_YMJOO
			m_pFontList[nRenderCnt][1]->DrawText(bkPos.x + 456 - m_pFontList[nRenderCnt][1]->GetStringSize(szStrBuff).cx / 2, 
												 bkPos.y + 111 + nRenderCnt * 31, 
												 GUI_FONT_COLOR, szStrBuff);
#else
			...
			// 2014-07-01 by ymjoo DrawText 성능 개선 작업 (아레나 입장)
#ifdef C_DRAWTEXT_UPGRADE_YMJOO
			m_pFontList[nRenderCnt][2]->DrawText(bkPos.x + 621 - m_pFontList[nRenderCnt][2]->GetStringSize(szBufftemp).cx / 2, 
												 bkPos.y + 111 + nRenderCnt * 31, 
												 GUI_FONT_COLOR, szBufftemp);
#else
			...
			// 2014-07-01 by ymjoo DrawText 성능 개선 작업 (아레나 입장)
#ifdef C_DRAWTEXT_UPGRADE_YMJOO
				m_pFontList[nRenderCnt][3]->DrawText(bkPos.x + 704 - m_pFontList[nRenderCnt][3]->GetStringSize(szBuff).cx / 2, 
												 bkPos.y + 111 + nRenderCnt * 31, 
												 GUI_FONT_COLOR, szBuff);
#else
	...
	if( pMapInfo )
	{
		...
		// 2014-07-01 by ymjoo DrawText 성능 개선 작업 (아레나 입장)
#ifdef C_DRAWTEXT_UPGRADE_YMJOO
		m_pFontMapName2->DrawText(bkPos.x + 146 - m_pFontMapName2->GetStringSize(pMapInfo->MapName).cx / 2, 
								  bkPos.y + 199, 
								  GUI_FONT_COLOR, pMapInfo->MapName);
#else
		...
		// 2014-07-01 by ymjoo DrawText 성능 개선 작업 (아레나 입장)
#ifdef C_DRAWTEXT_UPGRADE_YMJOO
		STRING_CULL(pMapInfo->MapDescription, 227, &vecMessage, m_pFontStrCull);
#else
		...
		for( i=0; i<vecMessage.size(); ++i )
		{
			if( i >= m_pRoomSelDescScroll->GetScrollStep() && nRenderCnt < 7 )
			{
				...
				// 2014-07-01 by ymjoo DrawText 성능 개선 작업 (아레나 입장)
#ifdef C_DRAWTEXT_UPGRADE_YMJOO
				m_pFontMapExplain[nRenderCnt]->DrawText(bkPos.x + 31, (int)(fHeight), GUI_FONT_COLOR, szBuff);
				fHeight += m_pFontMapExplain[nRenderCnt]->GetStringSize(szBuff).cy * 1.3f;
#else
	...
	for( i=0; i<g_pD3dApp->GetInfinityManager()->GetMemberCount(); ++i )
	{
		if( i >= m_pRoomPartyMemberScroll->GetScrollStep() && nRenderCnt < 6 )
		{
			...
			if( pMember->ClientIdx == g_pD3dApp->GetInfinityManager()->GetMyRoomInfo()->MasterIndex )
			{
				// 2014-07-01 by ymjoo DrawText 성능 개선 작업 (아레나 입장)
#ifdef C_DRAWTEXT_UPGRADE_YMJOO
				m_pRoomMaster->Move(bkPos.x + 425 - m_pFontList[nRenderCnt][0]->GetStringSize(pMember->CharacterName).cx / 2 - m_pRoomMaster->GetImgSize().x - 5, 
			...
			// 2014-07-01 by ymjoo DrawText 성능 개선 작업 (아레나 입장)
#ifdef C_DRAWTEXT_UPGRADE_YMJOO
			m_pFontList[nRenderCnt][0]->DrawText(bkPos.x + 425 - m_pFontList[nRenderCnt][0]->GetStringSize(pMember->CharacterName).cx / 2, 
												 bkPos.y + 93 + nRenderCnt * 31, 
												 GUI_FONT_COLOR, pMember->CharacterName);
#else
			...
			// 2014-07-01 by ymjoo DrawText 성능 개선 작업 (아레나 입장)
#ifdef C_DRAWTEXT_UPGRADE_YMJOO
			m_pFontList[nRenderCnt][1]->DrawText(bkPos.x + 594 - m_pFontList[nRenderCnt][1]->GetStringSize(szBuff).cx / 2, 
												 bkPos.y + 93 + nRenderCnt * 31, 
												 GUI_FONT_COLOR, szBuff);
#else
			...
			// 2014-07-01 by ymjoo DrawText 성능 개선 작업 (아레나 입장)
#ifdef C_DRAWTEXT_UPGRADE_YMJOO
			m_pFontList[nRenderCnt][2]->DrawText(bkPos.x + 697 - m_pFontList[nRenderCnt][2]->GetStringSize(szBuff).cx / 2, 
												 bkPos.y + 93 + nRenderCnt * 31, 
												 GUI_FONT_COLOR, szBuff);
#else
	...
	if( !m_ChatList.empty() )
	{
		...
		for( i=m_CullStringVec.size()-1; i>=0 && nRenderCnt < 6 ; --i )
		{
				...
				&& i > m_pRoomChatScroll->GetScrollStep() - 6 )
			{
				...
				// 2014-07-01 by ymjoo DrawText 성능 개선 작업 (아레나 입장)
#ifdef C_DRAWTEXT_UPGRADE_YMJOO
				m_pFontChat[nRenderCnt]->DrawText(bkPos.x + 315, (int)(fHeight), GUI_FONT_COLOR, szBuff);
				fHeight -= m_pFontChat[nRenderCnt]->GetStringSize(szBuff).cy * 1.3f;
#else
...
//		m_pJoinRequestBG->Render();																  
		...
		// 2014-07-01 by ymjoo DrawText 성능 개선 작업 (아레나 입장)
#ifdef C_DRAWTEXT_UPGRADE_YMJOO
		int StrSizeCenter = m_pFontUserName->GetStringSize(pRequest->ReQuestCharacterName).cx / 2;
#else
		...
		// 2014-07-01 by ymjoo DrawText 성능 개선 작업 (아레나 입장)
#ifdef C_DRAWTEXT_UPGRADE_YMJOO
		m_pFontUserName->DrawText(pt.x + 157 - StrSizeCenter, pt.y + 59, GUI_FONT_COLOR, pRequest->ReQuestCharacterName);
#else
		...
		// 2014-07-01 by ymjoo DrawText 성능 개선 작업 (아레나 입장)
#ifdef C_DRAWTEXT_UPGRADE_YMJOO
		StrSizeCenter = m_pFontUserLevel->GetStringSize(szBuff).cx / 2;
		m_pFontUserLevel->DrawText(pt.x + 157 - StrSizeCenter, pt.y + 82, GUI_FONT_COLOR, szBuff);
#else
		...
		// 2014-07-01 by ymjoo DrawText 성능 개선 작업 (아레나 입장)
#ifdef C_DRAWTEXT_UPGRADE_YMJOO
		StrSizeCenter = m_pFontUserGear->GetStringSize(szBuff).cx / 2;
		m_pFontUserGear->DrawText(pt.x + 157 - StrSizeCenter, pt.y + 106, GUI_FONT_COLOR, szBuff);
#else
 		StrSizeCenter = m_pFont->GetStringSize( szBuff ).cx / 2;
...
void	CINFCityInfinityFieldPopUp::RenderMemberBan( void )
{
	...
	if( g_pD3dApp->GetInfinityManager()->GetRejectClientIdx() )
	{
		...
		else
		{
			...
			// 2014-07-01 by ymjoo DrawText 성능 개선 작업 (아레나 입장)
#ifdef C_DRAWTEXT_UPGRADE_YMJOO
			m_pFontUserName->DrawText(pt.x + (m_pRejectBG->GetImgSize().x - m_pFontUserName->GetStringSize(pMember->CharacterName).cx) / 2, 
									  pt.y + 51, 
									  GUI_FONT_COLOR, pMember->CharacterName);
#else
...
void	CINFCityInfinityFieldPopUp::AddChat( const char* szChat )
{
		 ...
		 ++it )
	{
		...
		// 2014-07-01 by ymjoo DrawText 성능 개선 작업 (아레나 입장)
#ifdef C_DRAWTEXT_UPGRADE_YMJOO
		STRING_CULL((char*)((*it).c_str()), 390, &aLine, m_pFontStrCull);
#else
