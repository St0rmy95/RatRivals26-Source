...
#endif
	// END 2014-07-03 by ymjoo DrawText 성능 개선 작업 (보급상점)
...
//#endif
		...
		{
			nMaxCharge = g_pGameMain->GetMaxBulletItem(g_pShuttleChild->m_pPrimaryWeapon->GetRealItemInfo()->Charging, g_pShuttleChild->m_pSecondaryWeapon->GetItemGeneral()->ColorCode);
		} 
		else
		{
...
#ifdef SC_RENEWER_HAPPY_HOUR_BHSOHN_BCKIM
	INT nRenderPrice = (int)(CAtumSJ::GetItemSPIPrice(nItemRatePrice, m_fTexRate, g_pGameMain->GetAllHappyHourInfo()->fSPIDiscountRate));
	wsprintf(strtemp1, "%d", nRenderPrice);
#else
	// 각각 보급 가격
	nItemRatePrice = CAtumSJ::GetCityWarTex(nItemRatePrice, m_fTexRate)+nItemRatePrice;	 // 여기서 세금 적용 안시키고 전체 누적후 세금 적용
...
// 2013-08-22 by bhsohn 해피아워 리뉴얼 
// 전체 값 갱신
void CINFCitySupply::UpdateTotalPrice()
{
#ifdef SC_RENEWER_HAPPY_HOUR_BHSOHN_BCKIM
	m_nTotalPrice = (int)(CAtumSJ::GetItemSPIPrice(m_nTotalPrice, m_fTexRate, g_pGameMain->GetAllHappyHourInfo()->fSPIDiscountRate));
#endif
