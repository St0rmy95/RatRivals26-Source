...
CINFCityShop::CINFCityShop(CAtumNode* pParent, BUILDINGNPC* pBuilding)
{
	...
	for(i=0;i<SHOP_ITEM_SLOT_NUMBER;i++)
	{
		...
		// 2014-10-29 by ymjoo DrawText 성능 개선 작업 (상점)
#ifdef C_DRAWTEXT_UPGRADE_YMJOO
		for(int j = 0 ; j < 3 ; ++j)
		{
			m_pFontItemName[i][j] = NULL;
		}
#else
		m_pFontItemName[i] = NULL;
#endif
		// END 2014-10-29 by ymjoo DrawText 성능 개선 작업 (상점)
...
//		m_mapItemInfo[i].clear();
	...
	for(i=0;i<SHOP_ITEM_SLOT_NUMBER;i++)
	{
		...
		for(int j = 0 ; j < 3 ; ++j)
		{
			SAFE_DELETE(m_pFontItemName[i][j]);
		}
#else
...
HRESULT CINFCityShop::InitDeviceObjects()
{
	...
	for(i=0;i<SHOP_ITEM_SLOT_NUMBER;i++)
	{
		...
		for(int j = 0 ; j < 3 ; ++j)
		{
			m_pFontItemName[i][j] = new CD3DHanFont(_T(g_pD3dApp->GetFontStyle()),9, D3DFONT_ZENABLE, TRUE,256,32);
			m_pFontItemName[i][j]->InitDeviceObjects(g_pD3dDev);
		}
#else
...
HRESULT CINFCityShop::RestoreDeviceObjects()
{
	...
	for(i=0;i<SHOP_ITEM_SLOT_NUMBER;i++)
	{
		...
		for(int j = 0 ; j < 3 ; ++j)
		{
			m_pFontItemName[i][j]->RestoreDeviceObjects();
		}
#else
...
//		m_mapItemInfo[i].clear();
	...
	for(i=0;i<SHOP_ITEM_SLOT_NUMBER;i++)
	{
		...
		for(int j = 0 ; j < 3 ; ++j)
		{
			m_pFontItemName[i][j]->DeleteDeviceObjects();
			SAFE_DELETE(m_pFontItemName[i][j]);
		}
#else
...
HRESULT CINFCityShop::InvalidateDeviceObjects()
{
	...
	for(i=0;i<SHOP_ITEM_SLOT_NUMBER;i++)
	{
		...
		for(int j = 0 ; j < 3 ; ++j)
		{
			m_pFontItemName[i][j]->InvalidateDeviceObjects();
		}
#else
...
// 2011-12-08 by jhahn EP4 트리거 상점 UI 변경
				...
				if (pInfo->BuildingKind == BUILDINGKIND_TRIGGER_CRYSTAL)
				{
					if(m_pFontItemName[i][2]->GetStringSize(pItem->ItemName).cx <= SHOP_TEXT_SHOW_SIZE_TRI_X)
					{
						m_pFontItemName[i][0]->DrawText(nPosX + SHOP_ITEM_NAME_START_X, 
														nPosY + SHOP_ITEM_NAME_START_Y - SHOP_ITEM_SLOT_INTERVAL + SHOP_ITEM_SLOT_INTERVAL * i, 
														GUI_FONT_COLOR, pItem->ItemName, 0L);
					}
					else
					{
						StringCullingUserDataEx(pItem->ItemName, SHOP_TEXT_SHOW_SIZE_TRI_X, &vecMessage, m_pFontItemName[i][2]);	// 2011. 10. 10 by jskim UI시스템 변경
						for(int j = 0 ; j < SHOP_TEXT_SHOW ; ++j)
						{
							m_pFontItemName[i][j]->DrawText(nPosX + SHOP_ITEM_NAME_START_X, 
															nPosY + SHOP_ITEM_NAME_START_Y - SHOP_ITEM_SLOT_INTERVAL - 8 + SHOP_ITEM_SLOT_INTERVAL * i + (j * m_pFontItemName[i][2]->GetStringSize( pItem->ItemName ).cy), 
				...
				{
					if(m_pFontItemName[i][2]->GetStringSize(pItem->ItemName).cx <= SHOP_TEXT_SHOW_SIZE_X)
					{
						m_pFontItemName[i][0]->DrawText(nPosX + SHOP_ITEM_NAME_START_X, 
														nPosY + SHOP_ITEM_NAME_START_Y + SHOP_ITEM_SLOT_INTERVAL * i, 
														GUI_FONT_COLOR, pItem->ItemName, 0L);
					}
					else
					{
						StringCullingUserDataEx(pItem->ItemName, SHOP_TEXT_SHOW_SIZE_X, &vecMessage, m_pFontItemName[i][2], FALSE, TRUE );	// 2013-04-05 by ssjung 샾 일때 아이템 이름 글자 수 넘어 갈때 처리 보정
						for(int j = 0 ; j < SHOP_TEXT_SHOW ; ++j)
						{
							m_pFontItemName[i][j]->DrawText(nPosX + SHOP_ITEM_NAME_START_X, 
															nPosY + SHOP_ITEM_NAME_START_Y - 8 + SHOP_ITEM_SLOT_INTERVAL * i + (j * m_pFontItemName[i][2]->GetStringSize(pItem->ItemName).cy), 
															GUI_FONT_COLOR, (char*)vecMessage[j].c_str(), 0L);
						}
					}
				}
...
//				int i; for(i=0;i<SHOP_ITEM_SLOT_NUMBER;i++)
				int i; for(i=0;i<SHOP_ITEM_SLOT_NUMBER+nPlusNumber;i++)
				{
...
UINT CINFCityShop::GetTotalPrice()
{
		...
		while(it != m_vecItemInfo[tab].end())
		{
			if(m_pnSelectItemNumber[tab][i]>0)
			{
				...
				int tex = CAtumSJ::GetCityWarTex(price, m_fTexRate); 				
 				total += price + tex;				
#endif				
				// END END 2013-08-22 by bhsohn 해피아워 리뉴얼
			}
			it++;
			i++;
		}
	...
	// 2013-08-22 by bhsohn 해피아워 리뉴얼 
#ifdef SC_RENEWER_HAPPY_HOUR_BHSOHN_BCKIM
	total = (int)(CAtumSJ::GetItemSPIPrice(total, m_fTexRate, ((CINFGameMain*)m_pParent)->GetAllHappyHourInfo()->fSPIDiscountRate));
#endif
	// END 2013-08-22 by bhsohn 해피아워 리뉴얼 
...
//	g_pD3dApp->m_pFieldWinSocket->Write(buffer, SIZE_FIELD_TYPE_HEADER + sizeof(sMsg));
	...
	g_pD3dApp->CityShopLock(TRUE);		// 2014-07-04 by ymjoo 상점 아이템이 여러번 구입되는 현상 수정
...
{
	vector<SHOP_ITEM*>::iterator itStart, itEnd;
