...
typedef enum 
{
	...
	FUNCTIONTYPE_TIMELIMINT_ATTACK_CARD		// 2013-09-09 by ssjung 이달의 아머 - 이달의 아머 툴팁 표시(추가수정)
#endif
} FUNCTION_TYPE;
// end 2010. 02. 11 by ckPark 발동류 장착아이템
...
public:
	...
	void SetItemInfoNormal( ITEM* pITEM, int x, int y, BOOL bShop=TRUE, int nItemCount = 0, 
					int nLinkItem=0 , BOOL ArmorCollect = FALSE, BOOL bCanadaCashShop = FALSE);	// 2006-03-14 by ispark, nItemCount : 미션지급아이템 개수 추가
...
//	void SetParameterInfo( char * str, DestParam_t bType,float fValue, BOOL bDefEnchant = TRUE , BOOL bMonthlyArmor = FALSE); // 2013-06-12 by ssjung 아머 컬렉션 툴팁 표시
	void SetParameterInfo( char * str, DestParam_t bType,float fValue, BOOL bDefEnchant = TRUE , BOOL bMonthlyArmor = FALSE, FUNCTION_TYPE nFunctionType = FUNCTIONTYPE_NORMAL); // 2014-07-18 by ymjoo 프리미엄 아머컬렉션 구현
...
// 2013-06-26 by ssjung 인벤토리 추가 툴팁 
// 2014-05-07 by ssjung 캐시샾 툴팁 Transaction(거래)정보 추가
	void SetRadarItemInfo(BOOL bShop = FALSE, BOOL bInven = FALSE, BOOL bCanadaCashShop = FALSE);						// 2005-08-17 by ispark
	void SetPrimaryWeaponInfo(BOOL bShop=FALSE, BOOL bInven = FALSE, BOOL bCanadaCashShop = FALSE);
	void SetSecondaryWeaponInfo(BOOL bShop=FALSE, BOOL bInven = FALSE, BOOL bCanadaCashShop = FALSE);
	void SetSkillItemInfo(BOOL bShop=FALSE, BOOL bInven = FALSE);
	void SetDefenseItemInfo(BOOL bShop=FALSE, BOOL bInven = FALSE, BOOL bCanadaCashShop = FALSE);
	void SetSupportItemInfo(BOOL bShop=FALSE, BOOL bInven = FALSE, BOOL bCanadaCashShop = FALSE);
	void SetEnergyItemInfo(BOOL bShop=FALSE, BOOL bInven = FALSE, BOOL bCanadaCashShop = FALSE);
	void SetIngotItemInfo(BOOL bShop=FALSE, BOOL bInven = FALSE, BOOL bCanadaCashShop = FALSE);
	void SetCardItemInfo(BOOL bShop=FALSE, BOOL bInven = FALSE, BOOL bCanadaCashShop = FALSE);
	void SetEnchantItemInfo(BOOL bShop=FALSE, BOOL bInven = FALSE, BOOL bCanadaCashShop = FALSE);
//	void SetTankItemInfo(BOOL bShop=FALSE);							// 2006-04-24 by ispark, -> SetAccessoryUnLimitItemInfo()로 변경
	void SetBulletItemInfo(BOOL bShop=FALSE, BOOL bInven = FALSE, BOOL bCanadaCashShop = FALSE);
	void SetComputerItemInfo(BOOL bShop=FALSE, BOOL bInven = FALSE, BOOL bCanadaCashShop = FALSE);
	void SetArmorColorInfo(BOOL bShop=FALSE, BOOL bInven = FALSE, BOOL bCanadaCashShop = FALSE);
	void PrevetionDeleteItem(BOOL bShop=FALSE, BOOL bInven = FALSE, BOOL bCanadaCashShop = FALSE);
	void SetAccessoryUnLimitItemInfo(BOOL bShop=FALSE, BOOL bInven = FALSE, BOOL bCanadaCashShop = FALSE);				// 2006-04-24 by ispark

	void SetPetSoketItemInfo(BOOL bShop=FALSE, BOOL bInven = FALSE, BOOL bCanadaCashShop = FALSE);						//2011-10-06 by jhahn 파트너 성장형 시스템
	void SetAccessoryTimeLimitItemInfo(BOOL bShop = FALSE, BOOL bInven = FALSE, BOOL bCanadaCashShop = FALSE);			// 2006-03-30 by ispark
	void SetInfluenceBuffItemInfo(BOOL bShop = FALSE, BOOL bInven = FALSE, BOOL bCanadaCashShop = FALSE);				// 2006-04-25 by ispark
	void SetInfluenceGameEventItemInfo(BOOL bShop = FALSE, BOOL bInven = FALSE, BOOL bCanadaCashShop = FALSE);			// 2006-04-25 by ispark
	void SetRandomBoxItemInfo(BOOL bShop = FALSE, BOOL bInven = FALSE, BOOL bCanadaCashShop = FALSE);					// 2006-08-11 by dgwoo 랜덤박스.
	void SetExclusiveUser(int nParameterIndex);
	void SetMarkItemInfo(BOOL bShop = FALSE, BOOL bInven = FALSE, BOOL bCanadaCashShop = FALSE);						// 2006-08-21 by ispark
	void SetSkillSupportItem(BOOL bShop = FALSE, BOOL bInven = FALSE, BOOL bCanadaCashShop = FALSE);					// 2006-10-02 by ispark
	void SetPetItemInfo ( BOOL bShop = FALSE , BOOL bInven = FALSE, BOOL bCanadaCashShop = FALSE);						// 2010-06-15 by shcho&hslee 펫시스템
...
// end 2013-06-26 by ssjung 인벤토리 추가 툴팁 		
	...
	void SetItemAttribute( int nParameterIndex, BOOL bCanadaCashShop = FALSE);			// 2014-05-07 by ssjung 캐시샾 툴팁 Transaction(거래)정보 추가
	void SetItemSubAttribute(int nParameterIndex, BitFlag64_t ItemAttribute = NULL);	// 2014-05-07 by ssjung 캐시샾 툴팁 Transaction(거래)정보 추가
...
// 2013-06-10 by ssjung 아머 컬렉션 툴팁 표시
	...
	void SetMonthlyArmorValue(int* nParameterIndex, ATUM_DATE_TIME MonthlyEventEndDate, ItemNum_t MonthlyOptionItemNum, BOOL bCard = FALSE); // 2013-09-09 by ssjung 이달의 아머 - 이달의 아머 툴팁 표시(추가수정)
