...
//	m_pMeshArea = NULL;
	...
	m_pMiniMapUnitSightImage = NULL;
#endif
	// END 2014-07-17 by ymjoo 미니맵 앞부분 시야표시 적용
...
{
//	SAFE_DELETE(m_pMeshUnit);							// 2005-09-13 by ispark 별도 처리
//	SAFE_DELETE(m_pMeshParty);
//	SAFE_DELETE(m_pMeshArea);
	...
	// 2014-07-17 by ymjoo 미니맵 앞부분 시야표시 적용
#ifdef C_MINIMAP_SIGHT_IMG_YMJOO
	SAFE_DELETE(m_pMiniMapUnitSightImage);
#endif
	// END 2014-07-17 by ymjoo 미니맵 앞부분 시야표시 적용
	SAFE_DELETE(m_pMiniMapPartyImage);
	SAFE_DELETE(m_pMiniMapGuildImage);						// 2012-04-09 by isshin 레이더 및 미니맵 파티,여단원 표시
	SAFE_DELETE(m_pArenaMapInfImg);
	SAFE_DELETE(m_pImgQuestTarget);
	int i; for(i=0;i<3;i++)
	{
		SAFE_DELETE(m_pUserImage[i]);
		SAFE_DELETE(m_pPartyImage[i]);
		SAFE_DELETE(m_pMonsterImage[i]);
...
//		SAFE_DELETE(m_pButtonShowHide[BUTTON_HIDE][i]);
	...
	SAFE_DELETE(m_pFontPosition);
	SAFE_DELETE(m_pFontQuestTarget);
...
// 		m_pMiniMapFrame->RestoreDeviceObjects();					  
	...
	if(m_MiniMapSubDataHeader[1])
	{
		...
		// 2015-06-23 by jwlee 미니맵 관련 이미지 Init 부분에서 Restore 사용 오류 수정
		//m_pMiniMapUnitImage->RestoreDeviceObjects();
		// end 2015-06-23 by jwlee 미니맵 관련 이미지 Init 부분에서 Restore 사용 오류 수정
	}
	...
	// 2014-07-17 by ymjoo 미니맵 앞부분 시야표시 적용
#ifdef C_MINIMAP_SIGHT_IMG_YMJOO
	pDataHeader = FindResource("m_mmsite");
	if(pDataHeader)
	{
		m_pMiniMapUnitSightImage = new CINFImageEx;
		m_pMiniMapUnitSightImage->InitDeviceObjects(pDataHeader);
		// 2015-06-23 by jwlee 미니맵 관련 이미지 Init 부분에서 Restore 사용 오류 수정
		//m_pMiniMapUnitSightImage->RestoreDeviceObjects();
		// end 2015-06-23 by jwlee 미니맵 관련 이미지 Init 부분에서 Restore 사용 오류 수정
	}
#endif
	// END 2014-07-17 by ymjoo 미니맵 앞부분 시야표시 적용
	...
	// 레이더 미니맵
	// 2015-08-10 by jwlee 레이더 미니맵 익셉션 관련 수정
	// m_pRadarMiniMap = new CINFImageEx;
	m_pRadarMiniMap = new CINFImage;
	// end 2015-08-10 by jwlee 레이더 미니맵 익셉션 관련 수정
...
HRESULT CINFGameMainMiniMap::InvalidateDeviceObjects()
{
	if(m_bRestored)
	{
		...
		// 2014-07-17 by ymjoo 미니맵 앞부분 시야표시 적용
#ifdef C_MINIMAP_SIGHT_IMG_YMJOO
		m_pMiniMapUnitSightImage->InvalidateDeviceObjects();
#endif
	// END 2014-07-17 by ymjoo 미니맵 앞부분 시야표시 적용
...
HRESULT CINFGameMainMiniMap::DeleteDeviceObjects()
{
	...
	// 2014-07-17 by ymjoo 미니맵 앞부분 시야표시 적용
#ifdef C_MINIMAP_SIGHT_IMG_YMJOO
	m_pMiniMapUnitSightImage->DeleteDeviceObjects();
#endif
	// END 2014-07-17 by ymjoo 미니맵 앞부분 시야표시 적용
	...
	SAFE_DELETE(m_pMiniMapPartyImage);
	SAFE_DELETE(m_pArenaMapInfImg);
	SAFE_DELETE(m_pMiniMapGuildImage);						// 2012-04-09 by isshin 레이더 및 미니맵 파티,여단원 표시 - 맵 여단원 표시
	int i; for(i=0;i<3;i++)
	{
...
void CINFGameMainMiniMap::RenderSmallMap()
{
	...
	{
		POINT sizeUnit = m_pMiniMapUnitImage->GetImgSize();
		POINT sizeSight = m_pMiniMapUnitSightImage->GetImgSize();
		float len = (float)sizeUnit.y / 8 + (float)sizeSight.y / 8;
		m_pMiniMapUnitSightImage->Move(fMiniMapX + fMyPosX + (float)sizeUnit.x / 8 + len * sin(m_fAngle) - (float)sizeSight.x / 8, 
									   fMiniMapY + fMyPosY + (float)sizeUnit.y / 8 + len * cos(m_fAngle) - (float)sizeSight.y / 8);
		m_pMiniMapUnitSightImage->Rotate((float)sizeSight.x / 8, (float)sizeSight.y / 8, m_fAngle + PI);
		m_pMiniMapUnitSightImage->Render();
	}
#endif
	// END 2014-07-17 by ymjoo 미니맵 앞부분 시야표시 적용
...
void CINFGameMainMiniMap::RenderBigMap()
{
	...
	if(!IS_CITY_MAP_INDEX(g_pD3dApp->GetMyShuttleMapChannelIndex().MapIndex))
	{
		POINT sizeUnit = m_pMiniMapUnitImage->GetImgSize();
		POINT sizeSight = m_pMiniMapUnitSightImage->GetImgSize();
		float len = (float)sizeUnit.y / 2 + (float)sizeSight.y / 2;
		m_pMiniMapUnitSightImage->Move(fMiniMapX + fMyPosX + (float)sizeUnit.x / 2 + len * sin(m_fAngle) - (float)sizeSight.x / 2, 
									   fMiniMapY + fMyPosY + (float)sizeUnit.y / 2 + len * cos(m_fAngle) - (float)sizeSight.y / 2);
		m_pMiniMapUnitSightImage->Rotate((float)sizeSight.x / 2, (float)sizeSight.y / 2, m_fAngle + PI);
		m_pMiniMapUnitSightImage->Render();
	}
#endif
	// END 2014-07-17 by ymjoo 미니맵 앞부분 시야표시 적용
...
void CINFGameMainMiniMap::RenderEnemy()
{
	...
	while(itEnemy != g_pScene->m_mapEnemyList.end())
	{
		...
		if( fLength < fRangeTemp && itEnemy->second->m_bSetPosition == TRUE)
		{
			...
			{
				// 2014-01-20 by ymjoo 레이더 좌측 유저 위치 수정
//#ifdef C_RADAR_LEFT_SIDE_USER_POSITION_YMJOO
				int nMinPosX = BACK_START_X - 23 ;
// #else
// 				int nMinPosX = BACK_START_X + 25 ;
// #endif
				// END 2014-01-20 by ymjoo 레이더 좌측 유저 위치 수정
	...
	case WM_KEYDOWN:
		{
			...
			{
				if(g_pGameMain && TRUE == g_pGameMain->GetChatModeState())			   // 2013-11-14 by ssjung 비행 중 채팅 할시 조작키가 작동하는 문제 수정
					break;
...
void CINFGameMainMiniMap::InitMapSet()
{
	...
	if(pMiniBigMapImageTEMP)			
	{
		if(m_bZoomState == BUTTON_ZOOM_IN)
		{
			...
			// 2014-07-17 by ymjoo 미니맵 앞부분 시야표시 적용
#ifdef C_MINIMAP_SIGHT_IMG_YMJOO
			m_pMiniMapUnitSightImage->SetScale(1.0f, 1.0f);
#endif
			// END 2014-07-17 by ymjoo 미니맵 앞부분 시야표시 적용
		...
		else if(m_bZoomState == BUTTON_ZOOM_OUT)
		{
			...
			// 2014-07-17 by ymjoo 미니맵 앞부분 시야표시 적용
#ifdef C_MINIMAP_SIGHT_IMG_YMJOO
			m_pMiniMapUnitSightImage->SetScale(SMALLMAP_SCALE, SMALLMAP_SCALE);
#endif
			// END 2014-07-17 by ymjoo 미니맵 앞부분 시야표시 적용
...
void CINFGameMainMiniMap::SetMonsterMiniMap()
{
	...
	while(it != g_pQuestData->m_mapCharacterQuest.end())
	{
		if(it->second->QuestState == QUEST_STATE_IN_PROGRESS )
		{
			...
			//if(!quest_temp) continue;		// 2014-06-09 by ymjoo 미니맵 몬스터 익셉션 예외 처리 2014-07-31 by ymjoo 롤백
...
void CINFGameMainMiniMap::TickRadarMap()
{
	...
	if (m_pRadarMiniMap == NULL)
	{
		strcpy(sMsg.CharacterName, g_pShuttleChild->m_myShuttleInfo.CharacterName);
		wsprintf(sMsg.Message,"[TickRaderErr] m_pRadarMiniMap == NULL");
		g_pFieldWinSocket->SendMsg(T_FC_CLIENT_REPORT, (char*)&sMsg, sizeof(sMsg));
	}
	else if (m_pRadarMiniMap->GetTexture() == NULL)
	{
		strcpy(sMsg.CharacterName, g_pShuttleChild->m_myShuttleInfo.CharacterName);
		wsprintf(sMsg.Message,"[TickRaderErr] m_pRadarMiniMap->GetTexture() == NULL");
		g_pFieldWinSocket->SendMsg(T_FC_CLIENT_REPORT, (char*)&sMsg, sizeof(sMsg));
	}
	...
	{
		strcpy(sMsg.CharacterName, g_pShuttleChild->m_myShuttleInfo.CharacterName);
		wsprintf(sMsg.Message,"[TickRaderErr] pMiniBigMapImageTEMP == NULL");
		g_pFieldWinSocket->SendMsg(T_FC_CLIENT_REPORT, (char*)&sMsg, sizeof(sMsg));
	}
	else if ( pMiniBigMapImageTEMP->GetTexture() == NULL)
	{
		strcpy(sMsg.CharacterName, g_pShuttleChild->m_myShuttleInfo.CharacterName);
		wsprintf(sMsg.Message,"[TickRaderErr] pMiniBigMapImageTEMP->GetTexture() == NULL");
		g_pFieldWinSocket->SendMsg(T_FC_CLIENT_REPORT, (char*)&sMsg, sizeof(sMsg));
	}
	// end 2015-08-10 by jwlee 레이더 미니맵 익셉션 관련 수정
