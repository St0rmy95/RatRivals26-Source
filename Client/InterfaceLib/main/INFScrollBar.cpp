	...
	case WM_MOUSEMOVE:
		{
			if(m_bLockMouse)
			{
				...
				if(m_nScrollStyle == INFSCROLL_TYPE_VERTICAL)
				{
					...
					{
						// 2014-01-16 by ymjoo 스크롤 바와 커서의 위치가 일관되도록 개선
//#ifdef C_SCROLLBAR_CURSOR_POSITION_YMJOO
						m_nCurrentScrollIndex = (pt.y - (m_nStartY - (INTERVAL_PIXEL_PER_LINE - 23)/2)) / INTERVAL_PIXEL_PER_LINE;
						m_nCurrentScrollIndex = max(m_nCurrentScrollIndex, 0);
						m_nCurrentScrollIndex = min(m_nCurrentScrollIndex, m_nNumberOfData);
...
// 						}
// #endif
						// END 2014-01-16 by ymjoo 스크롤 바와 커서의 위치가 일관되도록 개선
...
HRESULT CINFScrollBarEX::DeleteDeviceObjects()
{
	...
	if(m_ImgScrollBar)
	{
		...
		SAFE_DELETE(m_ImgScrollBar);
	}
	
	if(m_ImgScrollLine)
	{
		m_ImgScrollLine->DeleteDeviceObjects();
		SAFE_DELETE(m_ImgScrollLine);
	}
	...
	for(i = 0; i < 4; i++)
	{
		if(m_ImgScrollTopBtn[i])
		{
			...
			SAFE_DELETE(m_ImgScrollTopBtn[i]);
		}

		if(m_ImgScrollBottomBtn[i])
		{
			m_ImgScrollBottomBtn[i]->DeleteDeviceObjects();
			SAFE_DELETE(m_ImgScrollBottomBtn[i]);
		}
	}
