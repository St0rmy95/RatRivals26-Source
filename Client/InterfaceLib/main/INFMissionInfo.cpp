...
CINFMissionInfo::~CINFMissionInfo()
{
	...
	// Image
	for(nCount=0; nCount<IMAGE_MAX_COUNT; nCount++)
		SAFE_DELETE(m_pMissionImg[nCount]);	
	
	// Font
	for(nCount=0;nCount<LEFTFONT_MAX_LINE_COUNT;nCount++)
		SAFE_DELETE(m_pMissionFontLeft[nCount]);
	for(nCount=0;nCount<RIGHTFONT_MAX_LINE_COUNT;nCount++)
		SAFE_DELETE(m_pMissionFontRight[nCount]);
	for(nCount=0;nCount<MISSION_TARGET_LIST;nCount++)
		SAFE_DELETE(m_pMissionTarget[nCount]);
	for(nCount=0;nCount<MISSION_SCROLL_FLUENCE_MAX_LINE;nCount++)
	{
		SAFE_DELETE(m_pMissionFontfluenceL[nCount]);
		SAFE_DELETE(m_pMissionFontfluenceR[nCount]);
	}

	SAFE_DELETE(m_pCompensation);
	SAFE_DELETE(m_pScrollLeftWindow);
	SAFE_DELETE(m_pScrollRightWindow);
	SAFE_DELETE(m_pScrollLeftfluence);
	SAFE_DELETE(m_pScrollRightfluence);

	int i;
	for(i = 0; i < 4; i++)
	{
		SAFE_DELETE(m_pImgOkButton[i]);
		SAFE_DELETE(m_pImgCancelButton[i]);
	}
	for(i = 0; i < 3; i++)
	{	
		SAFE_DELETE(m_pInfluenceWarning[i]);
	}
	SAFE_DELETE(m_pImgWarning);
	m_vecMissionName.clear();
}
...
void CINFMissionInfo::RenderMissionList()
{
	...
	// 미션 이름 출력
	int i; for(i=0; i<LEFTFONT_MAX_LINE_COUNT; i++)
	{
	...
	if(NULL != pQuestInfo)
	{
		...
		while(it != pQuestInfo->TargetItemVector.end())
		{
			...
			{				
				...
				//wsprintf(buf, STRMSG_C_050726_0002, pItem->ItemName, (*it).Count); //"목표:%s %d개"
		...
		while(itMonC != pQuestInfo->TargetMonsterVector.end())
		{
			...
			ZERO_MEMORY(buf);
			// 2009-01-12 by bhsohn 일본 추가 수정사항
			//wsprintf( buf, STRMSG_C_050726_0002,pTempIn->MonsterName, (*itMonC).Count ); //"\\e%s\\e \\w%d개/%d개\\w"
		...
		while(it2 != pQuestInfo->TargetMonsterItemVector.end())
		{
			...
			if(pItem)
			{
				...
				if(pQuestInfo->vecQuestDropItem.size()<0)
				{
					ZERO_MEMORY(buf);
					// 2009-01-12 by bhsohn 일본 추가 수정사항
					//wsprintf(buf, STRMSG_C_050726_0002, pItem->ItemName, it2->ItemWithCount.Count); //"목표:%s %d개"
...
int CINFMissionInfo::CheckProgressMission(CQuest *pQuestInfo)
{
	...
	if (!IS_IN_RANGE(pQuestInfo->ReqAttackPart.Min, g_pShuttleChild->m_myShuttleInfo.TotalGearStat.AttackPart, pQuestInfo->ReqAttackPart.Max))
	{
		m_vecErrPart.push_back(MISSION_ERR_STAT_AT);
		err = FALSE;
	}

	if (!IS_IN_RANGE(pQuestInfo->ReqDefensePart.Min, g_pShuttleChild->m_myShuttleInfo.TotalGearStat.DefensePart, pQuestInfo->ReqDefensePart.Max))
	{
		m_vecErrPart.push_back(MISSION_ERR_STAT_DE);
		err = FALSE;
	}

	if (!IS_IN_RANGE(pQuestInfo->ReqFuelPart.Min, g_pShuttleChild->m_myShuttleInfo.TotalGearStat.FuelPart, pQuestInfo->ReqFuelPart.Max))
	{
		m_vecErrPart.push_back(MISSION_ERR_STAT_FU);
		err = FALSE;
	}

	if (!IS_IN_RANGE(pQuestInfo->ReqSoulPart.Min, g_pShuttleChild->m_myShuttleInfo.TotalGearStat.SoulPart, pQuestInfo->ReqSoulPart.Max))
	{
		m_vecErrPart.push_back(MISSION_ERR_STAT_SO);
		err = FALSE;
	}

	if (!IS_IN_RANGE(pQuestInfo->ReqShieldPart.Min, g_pShuttleChild->m_myShuttleInfo.TotalGearStat.ShieldPart, pQuestInfo->ReqShieldPart.Max))
	{
		m_vecErrPart.push_back(MISSION_ERR_STAT_SH);
		err = FALSE;
	}

	if (!IS_IN_RANGE(pQuestInfo->ReqDodgePart.Min, g_pShuttleChild->m_myShuttleInfo.TotalGearStat.DodgePart, pQuestInfo->ReqDodgePart.Max))
	{
		m_vecErrPart.push_back(MISSION_ERR_STAT_DO);
		err = FALSE;
	}

	if (!IS_IN_RANGE(pQuestInfo->ReqLevel.Min, g_pShuttleChild->m_myShuttleInfo.Level, pQuestInfo->ReqLevel.Max))	// 필요레벨
	{
...
void CINFMissionInfo::RenderInfluenceMission()
{
	...
	int i; for(i=0; i<MISSION_SCROLL_FLUENCE_MAX_LINE; i++)
	{
