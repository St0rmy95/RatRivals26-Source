...
CINFTrade::~CINFTrade()
{
	int i; for(i=0;i<3;i++)
	{
		...
		SAFE_DELETE(m_pTradeCancelButton[i]);
	}
	SAFE_DELETE(m_pTralog );
	SAFE_DELETE(m_pTrame );
	SAFE_DELETE(m_pTrayou );
	// 2008-11-21 by bhsohn 거래 완료 시스템 처리
	SAFE_DELETE(m_pTrameComple);
	SAFE_DELETE(m_pTrayouComple);
	// end 2008-11-21 by bhsohn 거래 완료 시스템 처리


	SAFE_DELETE(m_pFontCharacterName[0] );
	SAFE_DELETE(m_pFontCharacterName[1] );
	for(i=0;i<2;i++)
	{
		SAFE_DELETE(m_pFontItemNum[i]);
	}

	SAFE_DELETE(m_pFontStoreSpi[0] );
	SAFE_DELETE(m_pFontStoreSpi[1] );
...
//	}
	...
	int count = RENEW_TRADE_INVEN_NUM*m_nOtherStoreScroll;
	int j;
	for(j=0;j<count;j++)
	{
	...
	int i=0;
	char buf[20];
	// 2015-06-03 by jwlee 아이템 갯수가 너무 많을 시 생기는 오류 수정
// 	char bufCnt[8];
// 	memset(bufCnt, 0x00, 8);
	char bufCnt[20];
	memset(bufCnt, 0x00, 20);
	// end 2015-06-03 by jwlee 아이템 갯수가 너무 많을 시 생기는 오류 수정
	while(it != m_vecOtherStore.end())
	{
		CItemInfo* pItem = *it;
		...
		pIcon->SetIcon( buf, TRADE_MYSTORE_START_X+i%RENEW_TRADE_INVEN_NUM*TRADE_SLOT_INTERVAL_X+1+RENEW_TRADE_WIDTH, 
			nWindowPosY+TRADE_OTHERSTORE_START_Y+i/RENEW_TRADE_INVEN_NUM*TRADE_SLOT_INTERVAL_Y+1, 1.0f );
		pIcon->Render();
	...
	while(it != m_vecMyStore.end())
	{
		...
		pIcon->SetIcon( buf, TRADE_MYSTORE_START_X+i%RENEW_TRADE_INVEN_NUM*TRADE_SLOT_INTERVAL_X+1+RENEW_TRADE_WIDTH, 
			nWindowPosY+TRADE_MYSTORE_START_Y+i/RENEW_TRADE_INVEN_NUM*TRADE_SLOT_INTERVAL_Y+1, 1.0f );
		pIcon->Render();
...
{	
	...
	if( CharacterUniqueNumber == myShuttle.CharacterUniqueNumber)
	{
		// 나 자신이다.	
		m_bTraMeComple = TRUE;
	}
