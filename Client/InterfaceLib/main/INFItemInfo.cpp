...
CINFItemInfo::~CINFItemInfo()
{
	...
	for(i=0;i<ITEMINFO_DESC_LINE_NUMBER;i++)
	{
		SAFE_DELETE(m_pFontExtendItemInfo[i]);								 // 2013-06-26 by ssjung 인벤토리 추가 툴팁 
	}
...
void CINFItemInfo::SetItemName(int nParameterIndex)
{
	...
	memset( m_strItemName, 0x00, ITEMINFO_ITEM_FULL_NAME);
	// 2015-05-08 by jwlee 인챈트 횟수 아이템 이름 앞에 출력하도록 수정
...
// 	}
	if(m_pRefItemInfo && m_pRefItemInfo->GetEnchantNumber())
	{
		wsprintf( m_strItemName, "+%d", m_pRefItemInfo->GetEnchantNumber());
		wsprintf( m_strItemInfo[nParameterIndex], "\\e+%d\\e", m_pRefItemInfo->GetEnchantNumber());
	}
	if(m_pRefPrefixRareInfo)
	{
		wsprintf( m_strItemName, "%s %s" ,m_strItemName , m_pRefPrefixRareInfo->Name );
		wsprintf( m_strItemInfo[nParameterIndex], "%s \\g%s\\g", m_strItemInfo[nParameterIndex], m_pRefPrefixRareInfo->Name );
	}
	// end 2015-05-08 by jwlee 인챈트 횟수 아이템 이름 앞에 출력하도록 수정
	if(m_bEnableItem)
	{
...
// 2013-06-12 by ssjung 아머 컬렉션 툴팁 표시
			...
			{
				wsprintf(buf, STRMSG_C_070607_0107, m_pRefItemInfo->FixedTermShape.nStatLevel);		//"%d회"   //2013-05-20 by ssjung +숫자를 +회로 변경 
...
#ifdef SC_PREMIUM_COLLECTION_ARMOR_YMJOO_BCKIM		// 2014-07-18 by ymjoo 프리미엄 아머컬렉션 구현
	if(bType1 != 0)
	{
		SetParameterInfo(buf1, bType1, fValue1, bDefEnchant, bMonthlyArmor, nFunctionType);
	}
	if(bType2 != 0)
	{
		SetParameterInfo(buf2, bType2, fValue2, bDefEnchant, FALSE, nFunctionType);
	}
#else
	if(bType1 != 0)
...
//  				sprintf(buf1,STRMSG_C_130419_0005, (int)fValue1);								//"\\y[모든스텟:+%d]\\y"
				...
				if(strlen(buf2) > 0)
				{
					if(FUNCTIONTYPE_TIMELIMINT_ATTACK_CARD == nFunctionType)
					{
						wsprintf( m_strItemInfo[nParameterIndex], STRMSG_C_130909_0001, buf1, buf2);		// "기능획득기간 : \\m%s\\m %s"
					}
					else
					{
...
//2013-05-20 by ssjung 이달의 아머 - 이달의 아머 툴팁 표시
		...
		case FUNCTIONTYPE_TIMELIMINT_ATTACK_CARD:		// 2013-09-09 by ssjung 이달의 아머 - 이달의 아머 툴팁 표시(추가수정)
		{
			if(fValue2 > 0)
			{
				int nTempTimeSec = (int)fValue2;
			...
			if( strlen(buf1) > 0 && strlen(buf2) > 0 )	
			{
#ifdef C_MONTHLY_ARMOR_ADD_CHANGE
				if(FUNCTIONTYPE_TIMELIMINT_ATTACK_CARD == nFunctionType)		// 2013-09-09 by ssjung 이달의 아머 - 이달의 아머 툴팁 표시(추가수정)
					wsprintf( m_strItemInfo[nParameterIndex], STRMSG_C_130909_0001, buf1, buf2);		//"기능획득기간 : \\m%s\\m %s"	
				else
#endif
					wsprintf( m_strItemInfo[nParameterIndex], STRMSG_C_130516_0001, buf1, buf2);		//"기간제 기능 : \\m%s\\m %s"	
...
//	}
	...
	// 2015-02-12 by jwLee 아이템 설명 짤리는 현상 수정
...
// 		}
//	}
	int nCullingSize = SIZE_MAX_ITEM_DESCRIPTION / ITEMINFO_DESC_LINE_NUMBER;
	::StringCullingUserData(strBuf, nCullingSize, &vectempCulling);	
	int nItemStringLen = 0;
	if(vectempCulling.size() > 0)
	{
		nItemStringLen = m_pFontDescInfo[0]->GetStringSize((char*)vectempCulling[0].c_str()).cx;
	...
	{
		nItemStringLen = GetItemStringLen();
		if(nItemStringLen < nCullingSize)
		{
			nItemStringLen = nCullingSize;
		}
	}
	// end 2015-02-12 by jwLee 아이템 설명 짤리는 현상 수정
...
void CINFItemInfo::SetPrimaryWeaponInfo(BOOL bShop, BOOL bInven, BOOL bCanadaCashShop)							  // 2014-05-07 by ssjung 캐시샾 툴팁 Transaction(거래)정보 추가
// 2013-06-26 by ssjung 인벤토리 추가 툴팁 
{
	int index = 0;
	SetItemName( index++ );
	...
	if(bShop == FALSE)
	{
		...
		SetItemAttribute(index++, bCanadaCashShop);			// 2014-05-07 by ssjung 캐시샾 툴팁 Transaction(거래)정보 추가
	}
...
void CINFItemInfo::SetSecondaryWeaponInfo(BOOL bShop, BOOL bInven, BOOL bCanadaCashShop)				  // 2014-05-07 by ssjung 캐시샾 툴팁 Transaction(거래)정보 추가
// 2013-06-26 by ssjung 인벤토리 추가 툴팁 
{
	int index = 0;
	SetItemName( index++);
...
void CINFItemInfo::SetDefenseItemInfo(BOOL bShop, BOOL bInven, BOOL bCanadaCashShop)										  // 2014-05-07 by ssjung 캐시샾 툴팁 Transaction(거래)정보 추가
// 2013-06-26 by ssjung 인벤토리 추가 툴팁 
{
	int index = 0;
	SetItemName( index++);
...
void CINFItemInfo::SetSupportItemInfo(BOOL bShop, BOOL bInven, BOOL bCanadaCashShop)								  // 2014-05-07 by ssjung 캐시샾 툴팁 Transaction(거래)정보 추가
// 2013-06-26 by ssjung 인벤토리 추가 툴팁 
{
	int index = 0;
	SetItemName( index++);
...
void CINFItemInfo::SetEnergyItemInfo(BOOL bShop, BOOL bInven, BOOL bCanadaCashShop)								 // 2014-05-07 by ssjung 캐시샾 툴팁 Transaction(거래)정보 추가
 // 2013-06-26 by ssjung 인벤토리 추가 툴팁 
{
	int index = 0;
	SetItemName( index++ );
...
void CINFItemInfo::SetIngotItemInfo(BOOL bShop, BOOL bInven, BOOL bCanadaCashShop)							// 2014-05-07 by ssjung 캐시샾 툴팁 Transaction(거래)정보 추가
// 2013-06-26 by ssjung 인벤토리 추가 툴팁 
{
	int index = 0;
	SetItemName( index++ );
...
void CINFItemInfo::SetCardItemInfo(BOOL bShop, BOOL bInven, BOOL bCanadaCashShop)							// 2014-05-07 by ssjung 캐시샾 툴팁 Transaction(거래)정보 추가
// 2013-06-26 by ssjung 인벤토리 추가 툴팁 
{
...
// 2013-06-10 by ssjung 아머 컬렉션 툴팁 표시
	...
	if(m_pRefItemInfo)
	{
		SetArmorCollectionParameter(&index);
		if(m_pRefItemInfo->nMonthlyOptionItemNum)
			SetMonthlyArmorValue(&index, m_pRefItemInfo->atMonthlyEventEndDate,m_pRefItemInfo->nMonthlyOptionItemNum, TRUE);
// 		if(m_pRefItemInfo->ItemInfo->SkillLevel)
// 		{
// 			SetLookChangeSystem(&index,m_pRefItemInfo->ItemInfo->SkillLevel);
// 		}
...
void CINFItemInfo::PrevetionDeleteItem(BOOL bShop, BOOL bInven, BOOL bCanadaCashShop)							 // 2014-05-07 by ssjung 캐시샾 툴팁 Transaction(거래)정보 추가
// 2013-06-26 by ssjung 인벤토리 추가 툴팁 
{
	int index = 0;
	SetItemName( index++ );
...
void CINFItemInfo::SetArmorColorInfo(BOOL bShop, BOOL bInven, BOOL bCanadaCashShop)												 // 2014-05-07 by ssjung 캐시샾 툴팁 Transaction(거래)정보 추가
// 2013-06-26 by ssjung 인벤토리 추가 툴팁 
{
	int index = 0;
	SetItemName( index++ );
...
void CINFItemInfo::SetEnchantItemInfo(BOOL bShop, BOOL bInven, BOOL bCanadaCashShop)								// 2014-05-07 by ssjung 캐시샾 툴팁 Transaction(거래)정보 추가
// 2013-06-26 by ssjung 인벤토리 추가 툴팁 
{
	int index = 0;
	SetItemName( index++ );
...
void CINFItemInfo::SetAccessoryUnLimitItemInfo(BOOL bShop, BOOL bInven, BOOL bCanadaCashShop)					 // 2014-05-07 by ssjung 캐시샾 툴팁 Transaction(거래)정보 추가
// 2013-06-26 by ssjung 인벤토리 추가 툴팁 
{
	int index = 0;
	SetItemName( index++ );
...
//2011-10-06 by jhahn 파트너 성장형 시스템
void CINFItemInfo::SetPetSoketItemInfo(BOOL bShop, BOOL bInven, BOOL bCanadaCashShop)							// 2014-05-07 by ssjung 캐시샾 툴팁 Transaction(거래)정보 추가
// 2013-06-26 by ssjung 인벤토리 추가 툴팁 
{
	int index = 0;
	SetItemName( index++ );
...
//end 2011-10-06 by jhahn 파트너 성장형 시스템
void CINFItemInfo::SetBulletItemInfo(BOOL bShop, BOOL bInven, BOOL bCanadaCashShop)									  // 2014-05-07 by ssjung 캐시샾 툴팁 Transaction(거래)정보 추가
// 2013-06-26 by ssjung 인벤토리 추가 툴팁 
{
	int index = 0;
	SetItemName( index++ );
...
void CINFItemInfo::SetComputerItemInfo(BOOL bShop, BOOL bInven, BOOL bCanadaCashShop)							  // 2014-05-07 by ssjung 캐시샾 툴팁 Transaction(거래)정보 추가
// 2013-06-26 by ssjung 인벤토리 추가 툴팁 
{
	int index = 0;
	SetItemName( index++ );
...
void CINFItemInfo::SetItemInfoNormal( ITEM* pITEM, int x, int y, BOOL bShop, int nItemCount, int nLinkItem/*=0*/, BOOL ArmorCollect/*FALSE*/, BOOL bCanadaCashShop/*FALSE*/)//(ITEM_GENERAL* pItem, ITEM* pItemInfo,ITEM_ENCHANT* pEnchant, int x, int y)
// 2014-05-07 by ssjung 캐시샾 툴팁 Transaction(거래)정보 추가
// 2013-06-10 by ssjung 아머 컬렉션 툴팁 표시
{
	m_ptItemInfo.x = x;
	m_ptItemInfo.y = y;// + ITEMINFO_BIGICON_GAB;
...
// 2014-05-07 by ssjung 캐시샾 툴팁 Transaction(거래)정보 추가
			...
			{
				SetPrimaryWeaponInfo(bShop,FALSE,bCanadaCashShop);	
			}
			else if(IS_SECONDARY_WEAPON(pITEM->Kind))
			{
				SetSecondaryWeaponInfo(bShop,FALSE,bCanadaCashShop);
			}
			...
			{
				SetDefenseItemInfo(bShop,FALSE,bCanadaCashShop);
			}
			else if(ITEMKIND_SUPPORT == pITEM->Kind)
			{
				SetSupportItemInfo(bShop,FALSE,bCanadaCashShop);
			}
			else if(ITEMKIND_ENERGY == pITEM->Kind)
			{
				SetEnergyItemInfo(bShop,FALSE,bCanadaCashShop);
			}
			else if(ITEMKIND_INGOT == pITEM->Kind)
			{
				SetIngotItemInfo(bShop,FALSE,bCanadaCashShop);
			}
			else if(ITEMKIND_CARD == pITEM->Kind)
			{
				SetCardItemInfo(bShop,FALSE,bCanadaCashShop);
			}
			else if(ITEMKIND_ENCHANT == pITEM->Kind || ITEMKIND_GAMBLE == pITEM->Kind)
			{
				SetEnchantItemInfo(bShop,FALSE,bCanadaCashShop);
			}
			else if(ITEMKIND_TANK == pITEM->Kind)
			{
				SetAccessoryUnLimitItemInfo(bShop,FALSE,bCanadaCashShop);
			}
			else if(ITEMKIND_BULLET == pITEM->Kind)
			{
				SetBulletItemInfo(bShop,FALSE,bCanadaCashShop);
			}
			else if(ITEMKIND_QUEST == m_pRefITEM->Kind)
			{
				SetIngotItemInfo(bShop,FALSE,bCanadaCashShop);
			...
			{
				SetRadarItemInfo(bShop,FALSE,bCanadaCashShop);
				//SetEnchantItemInfo();
			}
			else if(ITEMKIND_COMPUTER == m_pRefITEM->Kind)
			{
				SetComputerItemInfo(bShop,FALSE,bCanadaCashShop);
				//SetEnchantItemInfo();
			}
			else if(ITEMKIND_PREVENTION_DELETE_ITEM == m_pRefITEM->Kind)
			{
				PrevetionDeleteItem(bShop,FALSE,bCanadaCashShop);
			}
			else if(ITEMKIND_COLOR_ITEM == m_pRefITEM->Kind)
			{
				SetArmorColorInfo(bShop,FALSE,bCanadaCashShop);
			}
			// 2006-03-30 by ispark 악세사리 추가 (ex. 유료 아이템)
			else if(ITEMKIND_ACCESSORY_UNLIMITED == m_pRefITEM->Kind)
			{
			...
			{
				SetAccessoryTimeLimitItemInfo(bShop,FALSE,bCanadaCashShop);
			}
			else if(ITEMKIND_INFLUENCE_BUFF == m_pRefITEM->Kind)
			{
				SetInfluenceBuffItemInfo(bShop,FALSE,bCanadaCashShop);
			}
			else if(ITEMKIND_INFLUENCE_GAMEEVENT == m_pRefITEM->Kind)
			{
				SetInfluenceGameEventItemInfo(bShop,FALSE,bCanadaCashShop);
			}
			else if(ITEMKIND_RANDOMBOX == m_pRefITEM->Kind)
			{
				SetRandomBoxItemInfo(bShop,FALSE,bCanadaCashShop);
			}
			else if(ITEMKIND_MARK == m_pRefITEM->Kind)
			{
				if(!bCanadaCashShop)
					...
					SetMarkItemInfo(FALSE,FALSE,bCanadaCashShop);
			}
			else if(ITEMKIND_SKILL_SUPPORT_ITEM == pITEM->Kind)
			{
				SetSkillSupportItem(bShop,FALSE,bCanadaCashShop);
			}
			else if(ITEMKIND_PET_ITEM == m_pRefITEM->Kind)
			{
				SetPetItemInfo(bShop,FALSE,bCanadaCashShop);
			}
			//2011-10-06 by jhahn 파트너 성장형 시스템
			else if ( ITEMKIND_PET_SOCKET_ITEM == m_pRefITEM->Kind )
			{	// 2010-06-15 by shcho&hslee 펫시스템
				if (!bCanadaCashShop)
					...
					SetPetSoketItemInfo(FALSE,FALSE,bCanadaCashShop);
			}								  
			//end 2011-10-06 by jhahn 파트너 성장형 시스템
// end 2014-05-07 by ssjung 캐시샾 툴팁 Transaction(거래)정보 추가
		}
	}
	else
	{
...
void CINFItemInfo::SetParameterInfo(char * str, 
									DestParam_t bType, 
								    float fValue, 
									BOOL bDefEnchant /*= TRUE*/, 
									BOOL bMonthlyArmor /*= FALSE*/, 
									FUNCTION_TYPE nFunctionType /*= FUNCTIONTYPE_NORMAL*/)		// 2014-07-18 by ymjoo 프리미엄 아머컬렉션 구현
// end 2010. 02. 11 by ckPark 발동류 장착아이템
...
// 2005-11-22 by ispark 새로 구현된 스킬 정보
	...
	case DES_DROP_EXP:
		...
		if(bMonthlyArmor)
		{
			sprintf(str, STRMSG_C_140718_0000, (int)(fValue));	//"\\y[경험치:+%d%%]\\y"
			return;
		}
#endif
		// END 2014-07-18 by ymjoo 프리미엄 아머컬렉션 구현
	...
	case DES_DROP_ITEM:
		...
		if(bMonthlyArmor)
		{
			sprintf(str, STRMSG_C_140718_0001, (int)(fValue));	//"\\y[아이템 드랍율:+%d%%]\\y"
			return;
		}
#endif
		// END 2014-07-18 by ymjoo 프리미엄 아머컬렉션 구현
	...
	case DES_ITEM_ADDATTACK:
		{
			if(!bMonthlyArmor)
				...
				sprintf( str, STRMSG_C_130617_0001, (int)fValue );	//"[몬스터 기본무기 추가 타격치:+%d]"		// 2013-06-12 by ssjung 아머 컬렉션 툴팁 표시
#endif
			return;
		}
		break;
	case DES_ITEM_ADDATTACK_SEC:
		{
			if(!bMonthlyArmor)
				...
				sprintf( str, STRMSG_C_130617_0002, (int)fValue );	//"[몬스터 고급무기 추가 타격치:+%d]"			// 2013-06-12 by ssjung 아머 컬렉션 툴팁 표시
#endif
			return;
		}
		break;
	...
	// 2014-06-19 by ymjoo 세력 불균형 역전의 기회 WP 획득량 증가
	case DES_PLUS_WARPOINT_RATE_SHOW_TOOLTIP:
		{
			//fValue = (0.3f + 0.7f * ((TURN_AROUND_BUFF_SKILL_100P_VALUE * g_pD3dApp->GetPVPBuffPercent() + 15.0f ) / 15.0f - 1.0f));
			fValue = (0.3f + 0.7f * ((TURN_AROUND_BUFF_SKILL_100P_VALUE * g_pD3dApp->GetPVPBuffPercent() + 14.0f ) / 15.0f - 1.0f));		// 2014-08-27 by ymjoo 세력 불균형 역전의 기회 WP 획득량 증가 세부 오차 수정
			fValue = min(fValue, 1.0f);
			sprintf(str, STRMSG_C_100518_0401, fValue + 1);	// "[WarPoint :%.2f배 증가]"
			return;
		}
		break;
	// END 2014-06-19 by ymjoo 세력 불균형 역전의 기회 WP 획득량 증가
	...
	case DES_SKILL_BUFF_MON_ATTACK_POWER:			// 몬스터 공격시 - 공격력 증가 : Value 증가 %
		{
			...
			nValueType = FUNCTION_VALUE_TYPE_PROB100;
			// 2014-06-19 by ymjoo 세력 불균형 역전의 기회 WP 획득량 증가
			if(0.0f <= TURN_AROUND_BUFF_SKILL_100P_VALUE * g_pD3dApp->GetPVPBuffPercent())
			{
				fValue = 0.1f;
			}
			// END 2014-06-19 by ymjoo 세력 불균형 역전의 기회 WP 획득량 증가
	...
	case DES_SKILL_BUFF_PVP_DEFENCE :				// PVP - 방어력 증가 : Value 증가 %
		{
			...
			fValue = min(TURN_AROUND_BUFF_SKILL_100P_VALUE * g_pD3dApp->GetPVPBuffPercent(), 10.00f);	//2014-04-02 by ssjung 역전의 버프 리뉴얼(방어/회피) 최대치 조절
#else
			fValue = TURN_AROUND_BUFF_SKILL_100P_VALUE * g_pD3dApp->GetPVPBuffPercent();
#endif
		}
...
/// \return		
///////////////////////////////////////////////////////////////////////////////
void CINFItemInfo::SetRadarItemInfo(BOOL bShop, BOOL bInven, BOOL bCanadaCashShop)	// 2014-05-07 by ssjung 캐시샾 툴팁 Transaction(거래)정보 추가
// 2013-06-26 by ssjung 인벤토리 추가 툴팁 
{
	int index = 0;
	SetItemName( index++ );
...
/// \return		
///////////////////////////////////////////////////////////////////////////////
void CINFItemInfo::SetAccessoryTimeLimitItemInfo(BOOL bShop, BOOL bInven, BOOL bCanadaCashShop)					   // 2014-05-07 by ssjung 캐시샾 툴팁 Transaction(거래)정보 추가
// 2013-06-26 by ssjung 인벤토리 추가 툴팁 
{
	int index = 0;
	SetItemName( index++ );
...
/// \return		
///////////////////////////////////////////////////////////////////////////////
void CINFItemInfo::SetInfluenceBuffItemInfo(BOOL bShop, BOOL bInven, BOOL bCanadaCashShop)										// 2014-05-07 by ssjung 캐시샾 툴팁 Transaction(거래)정보 추가
	// 2013-06-26 by ssjung 인벤토리 추가 툴팁 
{
	int index = 0;
	SetItemName( index++ );
...
/// \return		
///////////////////////////////////////////////////////////////////////////////
void CINFItemInfo::SetInfluenceGameEventItemInfo(BOOL bShop, BOOL bInven, BOOL bCanadaCashShop)						   // 2014-05-07 by ssjung 캐시샾 툴팁 Transaction(거래)정보 추가
// 2013-06-26 by ssjung 인벤토리 추가 툴팁 
{
	int index = 0;
	SetItemName( index++ );
...
/// \return		
///////////////////////////////////////////////////////////////////////////////
void CINFItemInfo::SetRandomBoxItemInfo(BOOL bShop, BOOL bInven, BOOL bCanadaCashShop)								// 2014-05-07 by ssjung 캐시샾 툴팁 Transaction(거래)정보 추가
 // 2013-06-26 by ssjung 인벤토리 추가 툴팁 
{
	int index = 0;
	SetItemName( index++ );
...
/// \return		
///////////////////////////////////////////////////////////////////////////////
void CINFItemInfo::SetMarkItemInfo(BOOL bShop, BOOL bInven, BOOL bCanadaCashShop)										// 2014-05-07 by ssjung 캐시샾 툴팁 Transaction(거래)정보 추가
	 // 2013-06-26 by ssjung 인벤토리 추가 툴팁 
{
	int index = 0;
	SetItemName( index++ );
...
// 	// end 2008-09-26 by bhsohn 신규 인첸트 처리
	...
	if( m_pRefItemInfo )
	{
		...
		{
			BOOL bEnchantShow = FALSE;			// 2013-10-17 by ssjung 아머에 접두접미 하면 인천트옵션이 두번 나오는 문제 수정
...
// 				&& vecAdditionalFunctionList.end() == std::find( vecAdditionalFunctionList.begin(), vecAdditionalFunctionList.end(), (*it).m_nDesParam ) )
			...
			{
// 2013-10-17 by ssjung 아머에 접두접미 하면 인천트옵션이 두번 나오는 문제 수정
				int i; for(i=0; i < SIZE_MAX_DESPARAM_COUNT_IN_ITEM; i++)
				{
					if(m_pRefItemInfo->GetPrefixRareInfo())
					{
						if(m_pRefItemInfo->GetPrefixRareInfo()->DesParameter[i] == (*it).m_nDesParam)
							bEnchantShow = TRUE;
					}
					if(m_pRefItemInfo->GetSuffixRareInfo())
					{
						if(m_pRefItemInfo->GetSuffixRareInfo()->DesParameter[i] == (*it).m_nDesParam)
							bEnchantShow = TRUE;
					}
				}
				if(!bEnchantShow)
...
// end 2013-10-17 by ssjung 아머에 접두접미 하면 인천트옵션이 두번 나오는 문제 수정
			}
...
/// \return		
///////////////////////////////////////////////////////////////////////////////
void CINFItemInfo::SetSkillSupportItem(BOOL bShop, BOOL bInven, BOOL bCanadaCashShop)						// 2014-05-07 by ssjung 캐시샾 툴팁 Transaction(거래)정보 추가
// 2013-06-26 by ssjung 인벤토리 추가 툴팁 
{
	int index = 0;
	SetItemName( index++ );
...
**
*********************************************************************************/
void CINFItemInfo :: SetPetItemInfo ( BOOL bShop /*= FALSE*/ , BOOL bInven, BOOL bCanadaCashShop)					// 2014-05-07 by ssjung 캐시샾 툴팁 Transaction(거래)정보 추가
	// 2013-06-26 by ssjung 인벤토리 추가 툴팁 
{
	int iIndex = 0;
	...
	if ( FALSE == bShop )
		SetItemAttribute( iIndex++ , bCanadaCashShop);		// 2014-05-07 by ssjung 캐시샾 툴팁 Transaction(거래)정보 추가
...
/// \return		
///////////////////////////////////////////////////////////////////////////////
void CINFItemInfo::SetItemAttribute( int nParameterIndex, BOOL bCanadaCashShop)		// 2014-05-07 by ssjung 캐시샾 툴팁 Transaction(거래)정보 추가
{
// 2014-05-07 by ssjung 캐시샾 툴팁 Transaction(거래)정보 추가
	if(!bCanadaCashShop)
	{
		if(m_pRefItemInfo && m_pRefItemInfo->ItemInfo)
			SetItemSubAttribute(nParameterIndex, m_pRefItemInfo->ItemInfo->ItemAttribute);
		else
			SetItemSubAttribute(nParameterIndex, NULL);
	}
	else
	{
		if(m_pRefITEM)
			SetItemSubAttribute(nParameterIndex, m_pRefITEM->ItemAttribute);
		else
			SetItemSubAttribute(nParameterIndex, NULL);
	}
// end 2014-05-07 by ssjung 캐시샾 툴팁 Transaction(거래)정보 추가
...
int CINFItemInfo::GetItemStringLen()
{
	...
	int nMaxLen,nLen;
	nMaxLen = nLen = 0;
	//int i; for(i=0;i<ITEMINFO_PARAMETER_NUMBER;i++)
	for(i=1;i<m_nDescIndex;i++)
	{
...
BOOL CINFItemInfo::IsShowFunction(int nType)		
{
	...
	case DES_SKILL_BUFF_PVP_DEFENCE:				// PVP - 방어력 증가 : Value 증가 %
	case DES_SKILL_BUFF_PVP_DEFENCE_PROBABILITY:	// PVP - 회피력 증가 : Value 증가 %
	case DES_PLUS_WARPOINT_RATE_SHOW_TOOLTIP:		// 2014-06-19 by ymjoo 세력 불균형 역전의 기회 WP 획득량 증가
	// end 2013-08-02 by ssjung 역전의 버프 툴팁표시
		// END 2013-05-15 by bhsohn 세력포인트 개선안 버프 시스템
		{
			return TRUE;
...
BOOL CINFItemInfo::SingofDecimalDecision(float fVal)
{
	int nTemp = fVal*100;
	nTemp = nTemp%100;
	if(0 < abs(nTemp)) return TRUE;				 // 2014-04-18 by ssjung 값이 -일때 0으로 표시되는 이슈 수정
	else return FALSE;
}
//end 2013-04-11 by ssjung 소숫점이 있을때만 소숫점 표시
...
{
#ifndef C_MONTHLY_ARMOR_ADD_CHANGE
 	struArmorCollectionInfo*						pAmorCollectItemData = NULL;
...
// 2013-09-09 by ssjung 이달의 아머 - 이달의 아머 툴팁 표시(추가수정)
	...
	if(pAmorCollectItemData)
		SetMonthlyArmorValue(nParameterIndex, pAmorCollectItemData->struCollectItemInfo.DurationTime, pAmorCollectItemData->struCollectItemInfo.nOptionItemNum);
#endif
// end 2013-09-09 by ssjung 이달의 아머 - 이달의 아머 툴팁 표시(추가수정)

}
void CINFItemInfo::SetMonthlyArmorValue(int* nParameterIndex, ATUM_DATE_TIME MonthlyEventEndDate, ItemNum_t MonthlyOptionItemNum, BOOL bCard) //보통의 이달의 아머 함수 
// 2013-09-09 by ssjung 이달의 아머 - 이달의 아머 툴팁 표시(추가수정)
{
	...
	for (int nArrParamCnt = 0; nArrParamCnt < SIZE_MAX_DESPARAM_COUNT_IN_ITEM; nArrParamCnt++)
	{
		...
		if (nRemainSecond > 0)
		{
			if (DES_ITEM_ADDATTACK == pLOOKITEM->ArrDestParameter[nArrParamCnt] || DES_ITEM_ADDATTACK_SEC == pLOOKITEM->ArrDestParameter[nArrParamCnt])
			{
				...
				if (pLOOKITEM->ArrDestParameter[0] == DES_OLD_OPTION_ITEM_DEFAULT_DESPARAM) {
					if (bCard)
					{
						SetFunction((*nParameterIndex)++, pLOOKITEM->ArrDestParameter[nArrParamCnt], pLOOKITEM->ArrParameterValue[nArrParamCnt], 0, (float)nRemainSecond, FUNCTIONTYPE_TIMELIMINT_ATTACK_CARD, 0, TRUE);
					}
					else
					{
						struArmorCollectionInfo* pAmorCollectItemData;
						int nParamValue;
						if (m_pRefItemInfo)
						{
							// 아머컬렉션 창을 최소 1회 연 뒤
							INT nUnitCollectType = g_pGameMain->GetArmorCollectType();
							INT nShapeItemNum = m_pRefItemInfo->ShapeItemNum;
							pAmorCollectItemData = g_pStoreData->GetArmorCollectInfo_SourceIndex(nUnitCollectType, nShapeItemNum);
							if (NULL == pAmorCollectItemData)
							{
								// 아머컬렉션 창을 최소 1회 열기 전
								ItemNum_t nStatShapeItemNum = m_pRefItemInfo->FixedTermShape.nStatShapeItemNum;
								ITEM* pTempItem = g_pDatabase->GetServerItemInfo(nStatShapeItemNum);
								if (pTempItem)
								{
									nUnitCollectType = g_pStoreData->Convert_UNITKIND_To_UnitCollectType(pTempItem->ReqUnitKind);
									pAmorCollectItemData = g_pStoreData->GetArmorCollectInfo(nUnitCollectType, nStatShapeItemNum);
								}
							}
						}
						else
						...
						if (ARENA_STATE_ARENA_GAMING == g_pD3dApp->GetArenaState())
						{
							nParamValue = m_pRefItemInfo->FixedTermShape.nStatLevel;
						}
						else
						...
						if (nParamValue)
						{
							SetFunction((*nParameterIndex)++, pLOOKITEM->ArrDestParameter[nArrParamCnt], pLOOKITEM->ArrParameterValue[nArrParamCnt] * nParamValue, 0, (float)nRemainSecond, FUNCTIONTYPE_TIMELIMINT_ATTACK, 0, TRUE);
						}
					}
				}
				else {
#endif // end 2014-12-29 by shchoi 이달의 구 아머
#ifdef C_MONTHLY_ARMOR_ADD_CHANGE
					if (bCard)	// 2013-09-09 by ssjung 이달의 아머 - 이달의 아머 툴팁 표시(추가수정)
						SetFunction((*nParameterIndex)++, pLOOKITEM->ArrDestParameter[nArrParamCnt], pLOOKITEM->ArrParameterValue[nArrParamCnt], 0, (float)nRemainSecond, FUNCTIONTYPE_TIMELIMINT_ATTACK_CARD, 0, TRUE);
					else
#endif
			...
			// 2014-07-18 by ymjoo 프리미엄 아머컬렉션 구현
#ifdef SC_PREMIUM_COLLECTION_ARMOR_YMJOO_BCKIM
			if (DES_DROP_EXP == pLOOKITEM->ArrDestParameter[nArrParamCnt]
				|| DES_DROP_ITEM == pLOOKITEM->ArrDestParameter[nArrParamCnt])
			{
				if (bCard)
				{
					SetFunction((*nParameterIndex)++, pLOOKITEM->ArrDestParameter[nArrParamCnt], (int)(pLOOKITEM->ArrParameterValue[nArrParamCnt] * 100), 0, (float)nRemainSecond, FUNCTIONTYPE_TIMELIMINT_ATTACK_CARD, 0, TRUE);
				}
				else
				{
					struArmorCollectionInfo* pAmorCollectItemData;
					// 2014-08-19 by ymjoo 프리미엄 아머컬렉션 툴팁 오류 수정
// 					if(m_pRefItemInfo)
// 					{
// 						pAmorCollectItemData = g_pStoreData->GetArmorCollectInfo_SourceIndex(g_pGameMain->GetArmorCollectType(), m_pRefItemInfo->ShapeItemNum);
// 					}
// 					else
// 					{
// 						pAmorCollectItemData = g_pStoreData->GetArmorCollectInfo_SourceIndex(g_pGameMain->GetArmorCollectType(), m_pRefITEM->ItemNum);
...
// 						continue;
// 					}
//					int nParamValue = pAmorCollectItemData->struCollectItemInfo.EnchantLevel;
					int nParamValue;
					if (m_pRefItemInfo)
					{
						// 아머컬렉션 창을 최소 1회 연 뒤
					...
					// END 2014-08-19 by ymjoo 프리미엄 아머컬렉션 툴팁 오류 수정
					if (nParamValue)
					{
						SetFunction((*nParameterIndex)++, pLOOKITEM->ArrDestParameter[nArrParamCnt], (int)(pLOOKITEM->ArrParameterValue[nArrParamCnt] * 100) * nParamValue, 0, (float)nRemainSecond, FUNCTIONTYPE_TIMELIMINT_ATTACK, 0, TRUE);
					}
				}
			}
#endif
...
// 2014-05-07 by ssjung 캐시샾 툴팁 Transaction(거래)정보 추가
void CINFItemInfo::SetItemSubAttribute(int nParameterIndex, BitFlag64_t ItemAttribute)
{
	if(NULL == ItemAttribute || 0 >= ItemAttribute)
	{
		sprintf( m_strItemInfo[nParameterIndex], STRMSG_C_070907_0204, STRMSG_C_070907_0203) ;//"거래가능, 삭제가능, 창고가능"
		return;
	}
	// 2011-06-07 by shcho, 베트남 요청 창고에만 저장 불가 속성 구현
	if(COMPARE_BIT_FLAG(ItemAttribute, ITEM_ATTR_NOT_STORE_SAVE))
	{
		sprintf( m_strItemInfo[nParameterIndex], STRMSG_C_070907_0204, STRMSG_C_110608_0001) ;// "\\r거래불가\\r, \\r삭제불가\\r, \\r창고불가\\r"
	}
	else if(COMPARE_BIT_FLAG(ItemAttribute, ITEM_ATTR_NO_TRANSFER))
		// end 2011-06-07 by shcho, 베트남 요청 창고에만 저장 불가 속성 구현
	{
		sprintf( m_strItemInfo[nParameterIndex], STRMSG_C_070907_0204, STRMSG_C_070907_0201) ;// "\\r거래불가\\r, \\r삭제불가\\r, \\r창고불가\\r"
	}
	// 2007.09.19 by bhsohn 아이템정보 표시 추가 기획안 처리
	else if((COMPARE_BIT_FLAG(ItemAttribute, ITEM_ATTR_KILL_MARK_ITEM))
		||(COMPARE_BIT_FLAG(ItemAttribute, ITEM_ATTR_ACCOUNT_POSSESSION)))
	{
		sprintf( m_strItemInfo[nParameterIndex], STRMSG_C_070907_0204, STRMSG_C_070907_0202) ;//"\\r거래불가\\r, 삭제가능, 창고가능"
	}
	else
