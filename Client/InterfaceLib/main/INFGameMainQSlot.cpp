...
//			g_pD3dApp->m_pChat->CreateChatChild(msg,COLOR_SKILL_USE);
			...
			if(pItemInfo)
			{
				...
				else
				{
					...
					if((ITEMKIND_RANDOMBOX == pItemInfo->Kind) && (g_pStoreData->IsCashShopItem(pItemInfo)))
					{	// 캐시샵에 판매중인 캡슐 아이템
						g_pGameMain->m_pInven->SetUseCapsuleCashItemUniqueNum(pItemInfo->UniqueNumber);
						char buf[256];
						wsprintf(buf, STRMSG_C_140210_0003, pItemInfo->ItemInfo->ItemName);		// "%s 을 개봉할 경우 교환 및 환불이 불가능합니다.\\n그래도 사용하시겠습니까?"
...
void CINFGameMainQSlot::SetQSlotInfo(int nTab, int nNumber, ITEM_BASE* pItem)
{
	...
	if(pItem)
	{
		...
		// END 2012-12-21 by bhsohn [드레인모듈] 아이템 슬롯 등록 안되게 수정
		#if defined(C_QUICKSLOT_CHECK_DESPARAM_JWLEE)
		// 2015-01-06 by jwLee 아이템 데스파람값으로 퀵슬롯 등록 가능 여부 확인
		for (int nCheckDesparam = 0 ; nCheckDesparam < SIZE_MAX_DESPARAM_COUNT_IN_ITEM ; nCheckDesparam++)
		{
			if (!IsPossibleSetQuickSlotDesParam(pItem->ItemInfo->ArrDestParameter[nCheckDesparam]))
		{
			if(g_pGameMain && g_pGameMain->m_pInfWindow)
			{
				g_pGameMain->m_pInfWindow->AddMsgBox(STRMSG_C_060728_0003, _MESSAGE);	// "등록을 할 수 없는 아이템 입니다."
		...
		#endif
		// end 2015-01-06 by jwLee 아이템 데스파람값으로 퀵슬롯 등록 가능 여부 확인
...
//	InitQSlotItem() ;
	...
	int i; for(i=0;i<QSLOT_TAB_NUMBER;i++)
	{
		for(int j=0;j<QSLOT_NUMBER;j++)
		{
			...
			for (int nCheckDesparam = 0 ; nCheckDesparam < SIZE_MAX_DESPARAM_COUNT_IN_ITEM ; nCheckDesparam++)
			{
				if(!IsPossibleSetQuickSlotDesParam(pITEM->ArrDestParameter[nCheckDesparam]))
			{
					bPass = true;
					break;
				}
...
// 2015-01-06 by jwLee 아이템 데스파람값으로 퀵슬롯 등록 가능 여부 확인
#if defined(C_QUICKSLOT_CHECK_DESPARAM_JWLEE)
BOOL CINFGameMainQSlot::IsPossibleSetQuickSlotDesParam(DestParam_t nItemDesParam)
{
	switch(nItemDesParam)
	{
	case DES_WARP:
