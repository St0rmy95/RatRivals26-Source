...
#include "INFBaseWarManager.h"										  // 2013-08-23 by ssjung 거점전 리뉴얼
#include "INFFirstAnniversaryEvent.h"								// 2013-10-01 by ssjung 1주년 이벤트
#include "INFWorldMap.h"											// 2014-03-11 by ssjung&ymjoo 월드맵 구현
...
//#define	VCN_FIRST_QUEST_IDX		113		// 바이제니유 첫번쨰 퀘스트
//#define	ANI_FIRST_QUEST_IDX		501		// 알링턴 첫번쨰 퀘스트
...
//	memset(m_strNPCName, 0x00, sizeof(m_strNPCName));
	...
	m_pBaseWarManager = NULL;						  // 2013-08-23 by ssjung 거점전 리뉴얼
...
//#endif
	...
	memset(&m_AllHappyHourEventInfo,0x00,sizeof(EVENT_INFO));	// 2013-08-22 by bhsohn 해피아워 리뉴얼

	m_pFirstAnniversaryEvent = NULL;				// 2013-10-01 by ssjung 1주년 이벤트

#ifdef C_WORLD_MAP_SSJUNG_YMJOO
	m_pWorldMap = NULL;									 // 2014-03-11 by ssjung&ymjoo 월드맵 구현
#endif
	g_pD3dApp->m_pProfiler = NULL;							// 2014-06-09 by ymjoo 프로파일러 구현
}
...
//#ifdef C_EPSODE4_UI_CHANGE_JSKIM					        // 2011. 10. 10 by jskim UI시스템 변경
	SAFE_DELETE(m_GruopImagemanager);															  
//#endif
	SAFE_DELETE(m_pUnitInfoBar) ;
	SAFE_DELETE(m_pInfWindow) ;
//	SAFE_DELETE(m_pParty);
	...
	SAFE_DELETE(m_pIcon) ;
//	SAFE_DELETE(m_pShop) ;
	SAFE_DELETE(m_pUnitNameInfo);
	SAFE_DELETE(m_pQuest) ;
	SAFE_DELETE(m_pMission) ;
	SAFE_DELETE(m_pItemInfo) ;
	// 2009-02-03 by bhsohn 장착 아이템 비교 툴팁
	SAFE_DELETE(m_pEquipItemInfo);	// 현재 장착한 아이템 툴팁
	// end 2009-02-03 by bhsohn 장착 아이템 비교 툴팁

//	SAFE_DELETE(m_pNotice) ;
	SAFE_DELETE(m_pUnitState) ;
	SAFE_DELETE(m_pAttackDirection) ;
	SAFE_DELETE(m_pGameCountDown) ;
	SAFE_DELETE(m_pDamageView) ;
	SAFE_DELETE(m_pOtherCharInfo) ;
	SAFE_DELETE(m_pInfGameMainFaq) ;
	...
	// 2007-06-12 by bhsohn 미션 인터페이스 수정안
	//SAFE_DELETE(m_pMissionInfo);
	SAFE_DELETE(m_pMissionMain);
	
	// 2009-02-13 by bhsohn 월드 랭킹 시스템
	SAFE_DELETE(m_pINFWorldRankWnd);
	// end 2009-02-13 by bhsohn 월드 랭킹 시스템
	
	// 2013-02-20 by bhsohn 인게임 조합창 처리
	SAFE_DELETE(m_pINFItemMixWnd);
	// END 2013-02-20 by bhsohn 인게임 조합창 처리
	// 2013-05-28 by bhsohn 아머 컬렉션 시스템
	SAFE_DELETE(m_pINFArmorCollectWnd);		
	// END 2013-05-28 by bhsohn 아머 컬렉션 시스템

	// 2007-08-28 by dgwoo 전진기지전 class추가.
	SAFE_DELETE(m_pOutPost);
	SAFE_DELETE(m_pChangeCharactor);
	SAFE_DELETE(m_pCouponWindow);

	// 2008-11-3 by bhsohn 서버 시간 보기 버튼 추가
	SAFE_DELETE(m_pINFGameMainSysMsg);

	// 2008-11-13 by bhsohn 조이스틱 작업
	SAFE_DELETE(m_pINFOpJoystick);	
	// end 2008-11-13 by bhsohn 조이스틱 작업

	// 2007-08-07 by bhsohn 스피커 아이템 추가
	SAFE_DELETE(m_pINFSpeakeMsg);	
	
	SAFE_DELETE(m_pCommunityVOIP);
...
//	SAFE_DELETE(m_pImgUnitLock) ;
	...
	// 2005-03-03 by jschoi
	SAFE_DELETE(m_pRequestEnableBack);
	SAFE_DELETE(m_pRequestEnableTriangle);
	SAFE_DELETE(m_pAltiMeterGage);
	SAFE_DELETE(m_pAltiMeterPin);
	
	for(i=0;i<SIZE_MAX_POS;i++)
	{
		if(m_pItemInfoWear[i] != NULL)	{
		SAFE_DELETE( m_pItemInfoWear[i]);
		}
		if(m_pITEMG[i] != NULL)	{
			SAFE_DELETE( m_pITEMG[i]);
		}
	}

	SAFE_DELETE(m_pCityBoard);
	...
	// 2007-07-04 by bhsohn 오퍼레이터 추가
	SAFE_DELETE(m_pFaqBtn);	 
	SAFE_DELETE(m_pCouponBtn);
	// end 2007-07-04 by bhsohn 오퍼레이터 추가
	// 2009. 10. 14 by jskim 프리스카 제거
	SAFE_DELETE(m_pTut_SelBtn);
	//end 2009. 10. 14 by jskim 프리스카 제거

	// 2008-04-04 by bhsohn 모선전 관리 클래스 추가
	SAFE_DELETE(m_pMotherShipC);

	if(m_pBaseWarManager)
		SAFE_DELETE(m_pBaseWarManager);										 // 2013-08-23 by ssjung 거점전 리뉴얼
...
// 2013-10-01 by ssjung 1주년 이벤트
	if(m_pFirstAnniversaryEvent)
		SAFE_DELETE(m_pFirstAnniversaryEvent);
// end 2013-10-01 by ssjung 1주년 이벤트
...
// 2014-06-09 by ymjoo 프로파일러 구현
#ifdef C_PROFILER_YMJOO
	if(g_pD3dApp->m_pProfiler)
		SAFE_DELETE(g_pD3dApp->m_pProfiler);
#endif
// END 2014-06-09 by ymjoo 프로파일러 구현
}
...
// 2014-01-15 by ssjung 파괴 경매장 구현
#ifdef SC_DESTROY_AUCTION_JHSEOL_BCKIM_SSJUNG
	m_pImgButton[GAMEMAIN_BUTTON_RETURNAUCTION] =	new CINFImageBtn;
#endif
// end 2014-01-15 by ssjung 파괴 경매장 구현
...
// 2014-01-15 by ssjung 파괴 경매장 구현
#ifdef SC_DESTROY_AUCTION_JHSEOL_BCKIM_SSJUNG
	m_pImgButton[GAMEMAIN_BUTTON_RETURNAUCTION]->InitDeviceObjects( "mnbtn53" , "mnbtn50", "mnbtn50", "mnbtn50","STRTOOLTIP100" );
#endif
// 2014-01-15 by ssjung 파괴 경매장 구현
...
// 2013-08-23 by ssjung 거점전 리뉴얼
	...
	{
		m_pBaseWarManager = new CINFBaseWarManager(m_pGameData);
		m_pBaseWarManager->InitDeviceObjects();
	}
#endif
...
// 2013-10-01 by ssjung 1주년 이벤트
	...
	{
		m_pFirstAnniversaryEvent = new CINFFirstAnniversaryEvent();
		m_pFirstAnniversaryEvent->SetGameData(m_pGameData);
		m_pFirstAnniversaryEvent->InitDeviceObjects();
	}
#endif
// end 2013-10-01 by ssjung 1주년 이벤트
...
// 2014-03-11 by ssjung&ymjoo 월드맵 구현
	...
	if(NULL == m_pWorldMap)
	{
		m_pWorldMap = new CINFWorldMap();
		m_pWorldMap->SetGameData(m_pGameData);
		m_pWorldMap->InitDeviceObjects();
	}
#endif
// end 2014-03-11 by ssjung&ymjoo 월드맵 구현
...
// 2014-06-09 by ymjoo 프로파일러 구현
#ifdef C_PROFILER_YMJOO
	if(NULL == g_pD3dApp->m_pProfiler)
	{
		g_pD3dApp->m_pProfiler = new CProfiler();
		g_pD3dApp->m_pProfiler->InitDeviceObjects();
	}
#endif
// END 2014-06-09 by ymjoo 프로파일러 구현
...
//			m_pImgUnitLock->RestoreDeviceObjects();
		...
		if(m_pBaseWarManager)
			m_pBaseWarManager->RestoreDeviceObjects();			 // 2013-08-23 by ssjung 거점전 리뉴얼
...
//		m_pMissionInfo->RestoreDeviceObjects();
	...
	if(m_pCommunityVOIP)
		m_pCommunityVOIP->RestoreDeviceObjects();

// 2013-10-01 by ssjung 1주년 이벤트
	if(m_pFirstAnniversaryEvent)
		m_pFirstAnniversaryEvent->RestoreDeviceObjects();
// end 2013-10-01 by ssjung 1주년 이벤트
...
#ifdef C_WORLD_MAP_SSJUNG_YMJOO
	if(m_pWorldMap)
		m_pWorldMap->RestoreDeviceObjects();
#endif
// end 2014-03-11 by ssjung&ymjoo 월드맵 구현
...
#ifdef C_PROFILER_YMJOO
	if(g_pD3dApp->m_pProfiler)
		g_pD3dApp->m_pProfiler->RestoreDeviceObjects();
#endif
// END 2014-06-09 by ymjoo 프로파일러 구현
...
// 2013-08-23 by ssjung 거점전 리뉴얼
	if(m_pBaseWarManager)
	{
		m_pBaseWarManager->DeleteDeviceObjects();
		SAFE_DELETE(m_pBaseWarManager);
	}								 
// end 2013-08-23 by ssjung 거점전 리뉴얼
...
// 2013-10-01 by ssjung 1주년 이벤트
	if(m_pFirstAnniversaryEvent)
	{
		m_pFirstAnniversaryEvent->DeleteDeviceObjects();
		SAFE_DELETE(m_pFirstAnniversaryEvent);
	}
// end 2013-10-01 by ssjung 1주년 이벤트
...
// 2014-03-11 by ssjung&ymjoo 월드맵 구현
	...
	if(m_pWorldMap)
	{
		m_pWorldMap->DeleteDeviceObjects();
		SAFE_DELETE(m_pWorldMap);
	}
#endif
// end 2014-03-11 by ssjung&ymjoo 월드맵 구현	
...
// 2014-06-09 by ymjoo 프로파일러 구현
	...
	if(g_pD3dApp->m_pProfiler)
	{
		g_pD3dApp->m_pProfiler->DeleteDeviceObjects();
		SAFE_DELETE(g_pD3dApp->m_pProfiler);
	}
#endif
// END 2014-06-09 by ymjoo 프로파일러 구현
...
// 2013-08-23 by ssjung 거점전 리뉴얼
		if(m_pBaseWarManager)
			m_pBaseWarManager->InvalidateDeviceObjects();

// 2013-10-01 by ssjung 1주년 이벤트
		if(m_pFirstAnniversaryEvent)
			m_pFirstAnniversaryEvent->InvalidateDeviceObjects();
// 2013-10-01 by ssjung 1주년 이벤트
...
#ifdef C_WORLD_MAP_SSJUNG_YMJOO
		if(m_pWorldMap)
			m_pWorldMap->InvalidateDeviceObjects();
#endif
// end 2014-03-11 by ssjung&ymjoo 월드맵 구현
...
#ifdef C_PROFILER_YMJOO
		if(g_pD3dApp->m_pProfiler)
			g_pD3dApp->m_pProfiler->InvalidateDeviceObjects();
#endif
// END 2014-06-09 by ymjoo 프로파일러 구현
...
//	}	
	...
	if(m_pBaseWarManager)
		m_pBaseWarManager->Tick();						// 2013-08-23 by ssjung 거점전 리뉴얼
...
// 			if(m_pUnitInfoBar->GetRestCount() > 0)// 휴식 경험치 카운트가 있다면, 튜토리얼 버튼을 밑으로
// 			{
// 				nAddPosY += TUTORIAL_BUTTON_Y_ADDCOUNT;
// 			}
// 			if(m_pUnitInfoBar->GetKillCount() > 0)// 킬 카운트가 있다면, 튜토리얼 버튼을 밑으로
// 			{
// 				nAddPosY += TUTORIAL_BUTTON_Y_ADDCOUNT;
// 			}
// 			m_pTut_SelBtn->SetBtnPosition(TUTORIAL_BUTTON_X, TUTORIAL_BUTTON_Y+nAddPosY);
// #endif // S_BONUSEXPSYSTEM_RENEWAL
// 			// end 2012-10-12 by jhjang 해피 아워 경험치 보너스 시스템 리뉴얼
			m_pTut_SelBtn->Render();
		}		
...
//	if(m_pMenuList && m_bMenuListFlag && g_pInterface->m_pTarget->GetShowTargetCodition() == FALSE)
//	{
//		if(m_pMenuList->WndProc(uMsg, wParam, lParam) == INF_MSGPROC_BREAK)
//			return INF_MSGPROC_BREAK;
//	}
	//
	/////////////////////////////////////////////////////////////////////////////////////////////////
...
// 	if(m_pChangeCharactor)
// 	{
// 		if(m_pChangeCharactor->WndProc(uMsg, wParam, lParam) == INF_MSGPROC_BREAK)
// 		{
// 			return INF_MSGPROC_BREAK;
// 		}
// 	}
...
// 2013-08-23 by ssjung 거점전 리뉴얼
	...
	if(m_pBaseWarManager)
	{
		if((m_pBaseWarManager->GetInfluenceState() || m_pBaseWarManager->m_bNGCbaseWar) && m_pBaseWarManager->WndProc(uMsg,wParam,lParam) == INF_MSGPROC_BREAK)
		{
			return INF_MSGPROC_BREAK;
		}
	}								 
...
//							}
						...
						break;
// 2014-03-11 by ssjung&ymjoo 월드맵 구현
					case WM_KEYDOWN_LPARAM_N:
						{
#ifdef C_WORLD_MAP_SSJUNG_YMJOO
							BOOL bShowWorldMap = IsShowWorldMap();
							if(bShowWorldMap)
							{
								ShowWorldMapWnd(FALSE);
							}
							else
							{
								// 2014-09-04 by ymjoo 아레나 서버에서 월드맵 안열리게 수정
// 								g_pInterface->SetWindowOrder(WNDWorldMap);
// 								ShowWorldMapWnd(TRUE);
								if(FALSE == g_pInterface->IsArenaStart() && ARENA_STATE_ARENA_GAMING != g_pD3dApp->GetArenaState())
								{
									g_pInterface->SetWindowOrder(WNDWorldMap);
									ShowWorldMapWnd(TRUE);
								}
								// END 2014-09-04 by ymjoo 아레나 서버에서 월드맵 안열리게 수정
							}
#endif
						}
						break;	   
...
// 				|| g_pD3dApp->m_bCharacter)	// 2012-11-23 by mspark, 비행시에 메인버튼 클릭 방지
			...
			if(g_pShuttleChild->GetIsUseInterface())	// 2013-12-18 by ssjung 비행시에 메인머튼 클릭 방지 루틴 수정
			{
				int i; for(i=0; i < GAMEMAIN_BUTTON_NUMBER; i++)
				{
...
// 							m_bKnowON[LOW_LEVEL_MISSION_HELP] = FALSE;
			...
			if(m_bMenuListFlag)
			{
				...
				{
					// 버튼
					int i; for(i=0; i<MENU_LIST_ALL_COUNT; i++)
					{
...
// 2014-01-15 by ssjung 파괴 경매장 구현
#ifdef SC_DESTROY_AUCTION_JHSEOL_BCKIM_SSJUNG
	case GAMEMAIN_BUTTON_RETURNAUCTION:
		{
			PopupReturnAuctionWnd();
		}
		break;	
#endif
// end 2014-01-15 by ssjung 파괴 경매장 구현
...
DataHeader * CINFGameMain::FindResource(char* szRcName)
{
...
//	RightWindowShow(FALSE, RIGHT_WINDOW_OPTION);
	...
	// 2007-06-12 by bhsohn 미션 인터페이스 수정안
//	if(g_pTutorial->GetTutorialEd() == FALSE)			// 2005-10-17 by ispark 방금 튜토리얼을 끝낸 상태라면 미션창을 닫으면 안됀다.
//		m_pMissionInfo->MissionProcFlag(FALSE);
	// 방금 튜토리얼을 끝낸 상태라면 미션창을 닫으면 안됀다.
...
//		g_pD3dApp->SetUnitArmorColorNum(nUnitColor);
	...
	{	// 1형 정보 수정
		SAFE_DELETE( m_pItemInfoWear[POS_PROWOUT]);
		m_pItemInfoWear[POS_PROWOUT] = NULL;
		SAFE_DELETE( m_pITEMG[POS_PROWOUT]);
	...
	{
		// 2형 정보 수정
		SAFE_DELETE( m_pItemInfoWear[POS_WINGOUT]);
		m_pItemInfoWear[POS_WINGOUT] = NULL;
		SAFE_DELETE( m_pITEMG[POS_WINGOUT]);
	...
	{
		// 아머 정보 수정
		SAFE_DELETE( m_pItemInfoWear[POS_CENTER]);
		m_pItemInfoWear[POS_CENTER] = NULL;
		SAFE_DELETE( m_pITEMG[POS_CENTER]);
	...
	if(nPos == -1)
{
	...
	{
		SAFE_DELETE( m_pItemInfoWear[nPos]);
		m_pItemInfoWear[nPos] = NULL;
		SAFE_DELETE( m_pITEMG[nPos]);
		m_pITEMG[nPos] = NULL;
...
// 캐릭정보
CHARACTER CINFGameMain::GetMyShuttleInfo()
{
...
/// \return		
///////////////////////////////////////////////////////////////////////////////
int CINFGameMain::GetMaxBulletItem(int nBullet, int nColorCode/* = 0*/)
{
...
#endif
// 2014-12-08 by bckim, jwLee 고급무기 이펙트 장탄수 확장 기능추가
#ifdef SC_INCREASE_BULLETS_OF_ADVANCED_WEAPONS_BY_EFFECT
	if(nColorCode)
	{
		nMaxCharge += nBullet * 0.2;
	}
#endif
// end 2014-12-08 by bckim, jwLee 고급무기 이펙트 장탄수 확장 기능추가
	return nMaxCharge;
...
void CINFGameMain::PopupSystemOptinWnd()
{
	...
	else
	{
		...
		int nNewVol = -10000;
		if(g_pSOption->sMusicVolume > 0)
		{
			float fPercent = (float)g_pSOption->sMusicVolume*0.01f;
...
{
#ifdef SC_STRATEGYPOINTWAR_RENEWAL_JHSEOL_SSJUNG
	if(pWarInfo->DisplatType && m_pBaseWarManager)
		return m_pBaseWarManager->SetBaseWarPointMapInfo(pWarInfo);			// 2013-08-23 by ssjung 거점전 리뉴얼
	else
#endif
...
// 2014-03-11 by ssjung&ymjoo 월드맵 구현
BOOL CINFGameMain::FindMotherShipMapIdx(MapIndex_t i_MapIndex, BOOL bCurrent) //0이면 기본 , 1이면 현재 진행중
{
	if(m_pMotherShipC)
		return 	m_pMotherShipC->FindMotherShipMapIdx(i_MapIndex, bCurrent);
	return FALSE;
}
// end 2014-03-11 by ssjung&ymjoo 월드맵 구현
///////////////////////////////////////////////////////////////////////////////
...
void CINFGameMain::UpdatePlayingMotherShipInfo()
{
	...
	for(nCnt =0;nCnt < m_pMotherShipC->GetSummonMotherShipCnt();nCnt++)
	{
		...
		if(pMonsterInfo)
		{		
			STRNCPY_MEMSET(buf,pMonsterInfo->MonsterName,SIZE_MAX_MONSTER_NAME);			
		}
...
{	
	...
	if(IS_SKILL_SHOP_TYPE(nBuildingKind)
		...
		|| (BUILDINGKIND_AUCTION == nBuildingKind)					// 2013-11-29 by ssjung 거래소 구현
		|| (BUILDINGKIND_DESTROY_AUCTION == nBuildingKind))		 // 2014-01-15 by ssjung 파괴 경매장 구현
	{
...
/// \return		
///////////////////////////////////////////////////////////////////////////////
BOOL CINFGameMain::StringCullingUserData_ToNormal( char* strTalk, int nCullPixel, vector<string> *vecDstString, CD3DHanFont* pFont, BOOL bChRt /*= FALSE*/, BOOL bColorState /*TRUE*/)
...
// 2013-09-11 by ssjung 편지 읽기에서 글자 수 넘어 갈때 처리 보정
		BOOL bShopNextLine = FALSE;	
		int nBufSize = pFont->GetStringSize(chCullingBuff).cx;
		{
			int nListChar = strlen(chCullingBuff) - 1;
			if(' ' == chCullingBuff[nCullingLen-1] || '+' == chCullingBuff[nCullingLen-1])
			{
				char* strTemp = NULL;
				...
				{
					strTemp = CharNext(&pNextStr[nCount]);
					if(' ' == pNextStr[nCount] || '+' == pNextStr[nCount] || ']' == pNextStr[nCount] || NULL == pNextStr[nCount])
					{
						n_pStrMinusstrTemp  = pFont->GetStringSize(pNextStr).cx -  pFont->GetStringSize(strTemp).cx;
						if(nBufSize+n_pStrMinusstrTemp >= nCullPixel)
						{
							bShopNextLine = TRUE;
						}
						break;
...
// end 2013-09-11 by ssjung 편지 읽기에서 글자 수 넘어 갈때 처리 보정

		if(nBufSize >= nCullPixel || bFindCrtn || bShopNextLine)
		{			
			// 길이 추가
			int nStringSize = strlen(chCullingBuff);
			memset(chCullingBuff, 0x00, nStringSize+1);
			if(bShopNextLine){nLastCullingLen = nCullingLen;	nCullingPos = nCheckPoint;}			// 2013-09-11 by ssjung 편지 읽기에서 글자 수 넘어 갈때 처리 보정
			if(nLastCullingLen)
			{
				// 다시 메모리를 다시 복사해온다.
			...
			chColorSave = StringCullingAddVector_ToNormal(chColorSave, chCullingBuff, vecDstString, bColorState);		  // 2013-11-13 by ssjung 캐나다 세력 공지사항에서 \관련 이슈 수정
			ZERO_MEMORY(chCullingBuff);				
...
/// \return		
///////////////////////////////////////////////////////////////////////////////
char CINFGameMain::StringCullingAddVector_ToNormal(char chColorSave, char* pString, vector<string> *vecDstString, BOOL bColorState /*TRUE*/)
// 2013-11-13 by ssjung 캐나다 세력 공지사항에서 \관련 이슈 수정
{
	char chCullingTmpBuff[2048];
	// 2008-07-24 by bhsohn 컬링 관련 버그 수정
	// 2013-11-13 by ssjung 캐나다 세력 공지사항에서 \관련 이슈 수정
	char chFindColor = NULL;
	if(bColorState) GetFindColorOnTxt(pString);						
...
/// \return		
///////////////////////////////////////////////////////////////////////////////
void CINFGameMain::SetItemInfoNormal( ITEM* pITEM, int x, int y, BOOL bShop, int nItemCount /*= 0*/, BOOL bShowMyEq/*=TRUE*/, int nLinkItem/*=0*/, BOOL ArmorCollect /*FALSE*/, BOOL bCanadaCashShop /*FALSE*/)
...
//	m_pItemInfo->SetItemInfoNormal( pITEM, x, y, bShop, nItemCount );
	m_pItemInfo->SetItemInfoNormal( pITEM, x, y, bShop, nItemCount, nLinkItem,ArmorCollect, bCanadaCashShop); 
	// 2014-05-07 by ssjung 캐시샾 툴팁 Transaction(거래)정보 추가
	// 2013-06-10 by ssjung 아머 컬렉션 툴팁 표시
...
void CINFGameMain::RefreshItemTooltipPos()
{
	...
	
	{
		...
		// 2014-07-29 by ymjoo 인벤토리 장착 아이템과 비교 툴팁이 화면 밖으로 나가는 현상 수정 (주석처리)
...
// 		}
		// END 2014-07-29 by ymjoo 인벤토리 장착 아이템과 비교 툴팁이 화면 밖으로 나가는 현상 수정 (주석처리)
...
// 2013-10-01 by ssjung 1주년 이벤트
void CINFGameMain::FirstAnniversaryEventItemUse(BOOL bShow)
{
	if(NULL == m_pFirstAnniversaryEvent)
	{
		return;
	}		
	POINT ptInvenItem = {0,0};
	...
	{		
		if(g_pD3dApp->GetArenaState() == ARENA_STATE_ARENA_GAMING)
		{
			m_pFirstAnniversaryEvent->ShowWnd(FALSE);
			return;
		}
		
		const int FIRSTANNIVERSARYEVENT_WIDTH	= 722;
		const int FIRSTANNIVERSARYEVENT_HEIGHT	= 662;
...
int CINFGameMain::WndProcFirstAnniversaryEvent(UINT uMsg, WPARAM wParam, LPARAM lParam)
{	
	if(NULL == m_pFirstAnniversaryEvent)
	{
		return INF_MSGPROC_NORMAL;
	}
	return m_pFirstAnniversaryEvent->WndProc(uMsg, wParam, lParam);
}

void CINFGameMain::RenderFirstAnniversaryEvent()
{
	if(NULL == m_pFirstAnniversaryEvent)
	{
		return;
	}
	m_pFirstAnniversaryEvent->Render();
}

BOOL CINFGameMain::GetFirstAnniversaryEventIsShowWnd()
{
#ifdef C_FIRST_ANNIVERSARY_EVENT
	if(m_pFirstAnniversaryEvent) 
		return m_pFirstAnniversaryEvent->IsShowWnd();
	else 
#endif
		return FALSE;
}
...
// 2014-03-11 by ssjung&ymjoo 월드맵 구현
int CINFGameMain::WndProcWorldMap(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	if(NULL == m_pWorldMap)
		return INF_MSGPROC_NORMAL;
	return m_pWorldMap->WndProc(uMsg, wParam, lParam);			// 월드맵 처리
}
void CINFGameMain::RenderWorldMap()
{
	if(NULL == m_pWorldMap)
		return;
	m_pWorldMap->Render();
}
BOOL CINFGameMain::IsShowWorldMap()
{
	if(NULL == m_pWorldMap)
		return FALSE;
	return m_pWorldMap->IsShowWorldMap();
}
void CINFGameMain::ShowWorldMapWnd(BOOL bShow)
{
	if(m_pWorldMap)
		m_pWorldMap->ShowWnd(bShow);
}
// end 2014-03-11 by ssjung&ymjoo 월드맵 구현

// 2014-06-09 by ymjoo 프로파일러 구현
void CINFGameMain::RenderProfiler()
{
	if(g_pD3dApp->m_pProfiler)
		g_pD3dApp->m_pProfiler->Render();
}
BOOL CINFGameMain::IsShowProfiler()
{
	if(g_pD3dApp->m_pProfiler)
		return g_pD3dApp->m_pProfiler->IsShowProfiler();
	return FALSE;
}
void CINFGameMain::ShowProfiler(BOOL bShow)
{
	if(g_pD3dApp->m_pProfiler)
		g_pD3dApp->m_pProfiler->ShowWnd(bShow);
}
// END 2014-06-09 by ymjoo 프로파일러 구현
...
void CINFGameMain::SetKillCount(INT nCount) // 킬 카운트 설정
{
	...
	if(nCount == KILLCOUNT_MAX)// 킬 카운트가 300이라면 이펙트 재생
	{
...
#endif
// 2015-05-08 by jwlee 아이템 삭제 편의성 강화
BOOL CINFGameMain::IsShowMixWnd()
{
	if (m_pINFItemMixWnd == NULL)
	{
		return FALSE;
	}
	return m_pINFItemMixWnd->IsShowWnd();
}
// end 2015-05-08 by jwlee 아이템 삭제 편의성 강화

// 2013-08-23 by ssjung 거점전 리뉴얼
void CINFGameMain::AddBaseWarInfo(MapIndex_t MapIndex, float fSummonLiveTime, short MapInfluenceType)
{
	if(m_pBaseWarManager)
		m_pBaseWarManager->AddBaseInfo(MapIndex, fSummonLiveTime, MapInfluenceType);
}
void CINFGameMain::DestroyBaseWarInfo(MapIndex_t MapIndex, SHORT MapInfluenceType)
{
	if(m_pBaseWarManager)
		m_pBaseWarManager->DestroyBaseWarInfo(MapIndex,MapInfluenceType);
}
void CINFGameMain::SetNgcBaseWarInfo(MSG_NGCSPWAR_DISPLAY* pinfo,BOOL bPopUp)
{
	if(m_pBaseWarManager)
		m_pBaseWarManager->AdventBaseNGC(pinfo, bPopUp);
}		  
// end 2013-08-23 by ssjung 거점전 리뉴얼
...
/// \fn			
/// \brief		
/// \author		// 2013-08-22 by bhsohn 해피아워 리뉴얼
/// \date		2013-08-22 ~ 2013-08-22
/// \warning	
///
/// \param		
/// \return		
///////////////////////////////////////////////////////////////////////////////
void CINFGameMain::SetAllHappyHourInfo(EVENT_INFO* pAllHappyHourEventInfo)
{
	memcpy(&m_AllHappyHourEventInfo,pAllHappyHourEventInfo,sizeof(EVENT_INFO));	
}

// 2013-09-02 by ssjung 필드이동금지아이템
BOOL CINFGameMain::UserWarpRestriction()
{
	if(m_pInfSkill && m_pInfSkill->IsDoingSkillExistDesParam(DES_MOVEMENT_RESTRICTION_TAKE))
	{
		g_pD3dApp->m_pChat->CreateChatChild(STRMSG_C_130902_0001,COLOR_ITEM);
		return FALSE;
	}
	return TRUE;
}
...
BOOL CINFGameMain::GetNGCWarState()
{
	BOOL bState = FALSE;
#ifdef SC_STRATEGYPOINTWAR_RENEWAL_JHSEOL_SSJUNG
	if(m_pBaseWarManager)
	{
		bState = m_pBaseWarManager->GetNGCWarState();
	}
#endif
	return bState;
}										  
// end 2013-08-28 by ssjung 거점전 리뉴얼(NGC)
 ...
 // 2013-11-14 by ssjung 비행 중 채팅 할시 조작키가 작동하는 문제 수정
BOOL CINFGameMain::GetChatModeState()
{
	if(m_pChat)
		return m_pChat->m_bChatMode;

	return FALSE;
}																	  
// end 2013-11-14 by ssjung 비행 중 채팅 할시 조작키가 작동하는 문제 수정

// 2014-03-11 by ssjung&ymjoo 월드맵 구현
BOOL CINFGameMain::FindBaseWarMapIdx(MapIndex_t i_MapIndex, BOOL bCurrent)	 //0이면 기본 , 1이면 현재 진행중)
{
	if(m_pBaseWarManager)
		return m_pBaseWarManager->FindBaseWarMapIdx(i_MapIndex, bCurrent);
	return FALSE;
}								   
// end 2014-03-11 by ssjung&ymjoo 월드맵 구현

// 2014-01-15 by ssjung 파괴 경매장 구현
void CINFGameMain::PopupReturnAuctionWnd()
{
#ifdef SC_DESTROY_AUCTION_JHSEOL_BCKIM_SSJUNG
	
 	MSG_FC_EVENT_CHARACTERMODE_ENTER_BUILDING sMsg;
 	sMsg.nBuildingIndex0 = 9395;
