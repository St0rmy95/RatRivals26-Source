...
#include "INFTradeItemCenter.h"		 // 2013-11-29 by ssjung 거래소 구현
#include "INFTradeItemBuy.h"		 // 2013-11-29 by ssjung 거래소 구현

#include "INFReturnAuctionCenter.h"				  // 2014-01-15 by ssjung 파괴 경매장 구현
...
//								 x == BUILDINGKIND_AUCTION				||			\		// 2013-11-29 by ssjung 거래소 구현
...
CINFCityBase::~CINFCityBase()
{
	...
	int i; for(i=0;i<9;i++)
	{
		SAFE_DELETE(m_pNPCBoxImage[i]);
	}
	for(i=0;i<4;i++)
	{
		SAFE_DELETE( m_pOutDoorButtonImage[i] );
	}
	...
	SAFE_DELETE(m_pDownBoxImage[0]);
	SAFE_DELETE(m_pDownBoxImage[1]);
	SAFE_DELETE(m_pUpBoxImage[0]);
	SAFE_DELETE(m_pUpBoxImage[1]);
	SAFE_DELETE(m_pMission);
	for(i=0;i<CITY_NPCTALK_LINE_FONT_NUMBER;i++)
	{
		SAFE_DELETE(m_pFontNPCTalk[i]);
	}
	...
	// 2012-02-29 by mspark, 상점 타이틀 넣기
	SAFE_DELETE(m_pImgTitleWeapon);
	SAFE_DELETE(m_pImgTitleParts);
	SAFE_DELETE(m_pImgTitleGear);
	SAFE_DELETE(m_pImgTitleCard);
	SAFE_DELETE(m_pImgTitleWarPoint);
	SAFE_DELETE(m_pImgTitleLab);
	SAFE_DELETE(m_pImgTitleFactory);
	// end 2012-02-29 by mspark, 상점 타이틀 넣기
...
//	}
	...
	for(i = 0 ; i < CITY_BUTTON_NUMBER ; i++)
	{
		...
		SAFE_DELETE(m_pImgAppB[i]);
		SAFE_DELETE(m_pImgCancelB[i]);
		SAFE_DELETE(m_pImgQuickAppB[i]);
...
//2011-12-08 by jhahn EP4 트리거 상점 UI 변경	
	if(m_pRenewTirShopImage)
	{
		...
		SAFE_DELETE(m_pRenewTirShopImage);
 	}
//end 2011-12-08 by jhahn EP4 트리거 상점 UI 변경
...
// 2014-01-15 by ssjung 파괴 경매장 구현
		...
		case BUILDINGKIND_DESTROY_AUCTION : 
			{
				if(
					g_pD3dApp->m_dwGameState == _CITY ||
					g_pD3dApp->m_dwGameState == _SHOP ||
...
// 2013-11-29 by ssjung 거래소 구현
#ifdef SC_MARKET_JHSEOL_BCKIM_SSJUNG
			if(m_pCurrentBuildingNPC->buildingInfo.BuildingKind == BUILDINGKIND_AUCTION)
			{
				CMapCityShopIterator it = m_mapCityShop.find(m_pCurrentBuildingNPC->buildingInfo.BuildingKind);
				if(it != m_mapCityShop.end())
					((CINFTradeItemCenter*)it->second)->BlackBackImgRender();
			}
#endif
// end 2013-11-29 by ssjung 거래소 구현
...
// 2014-01-15 by ssjung 파괴 경매장 구현
#ifdef SC_DESTROY_AUCTION_JHSEOL_BCKIM_SSJUNG
			if(m_pCurrentBuildingNPC->buildingInfo.BuildingKind == BUILDINGKIND_DESTROY_AUCTION)
			{
				CMapCityShopIterator it = m_mapCityShop.find(m_pCurrentBuildingNPC->buildingInfo.BuildingKind);
				if(it != m_mapCityShop.end())
					((CINFReturnAuctionCenter*)it->second)->BlackBackImgRender();
			}
#endif
// end 2014-01-15 by ssjung 파괴 경매장 구현
			...
			int i; for(i=0;i<CITY_NPCTALK_LINE_FONT_NUMBER;i++)
			{
...
#ifdef SC_MARKET_JHSEOL_BCKIM_SSJUNG
					 RenderBuildingItemTradeCenter(it->second);
#endif
// end 2013-11-29 by ssjung 거래소 구현
				}
			}
		...
		case BUILDINGNPC_RENDER_SHOP:
			{
				...
				if(it != m_mapCityShop.end())
				{
					...
					// 2014-01-14 by ymjoo 워프 상점 툴팁이 남아있는 현상 수정
					if(!m_pCurrentBuildingNPC)
					{
						break;
					}
					// END 2014-01-14 by ymjoo 워프 상점 툴팁이 남아있는 현상 수정
...
// 				}
		...
		g_pD3dApp->m_dwGameState != _CITY)
	{
		...
		if(WM_KEYDOWN == uMsg && wParam == VK_ESCAPE && m_pCurrentBuildingNPC)
		{
			...
			if(TRUE == g_pD3dApp->GetWeb())
			{
				g_pFieldWinSocket->SendMsg( T_FC_CHARACTER_GET_CASH_MONEY_COUNT, NULL, 0 );
				g_pD3dApp->PopupWebWindow(FALSE);

				OnCityNPCButtonDown(CITY_NPC_BUTTON_CLOSE);   // 2013-05-31 by ssjung 웹 캐쉬샾을 열고 ESC키를 두번 눌러야 꺼지는 현상 한번으로 수정
			}
			else
			{
				OnCityNPCButtonDown(CITY_NPC_BUTTON_CLOSE);
	...
	case WM_MOUSEMOVE:
		{
			...
			{
				int i; for(i=0;i<m_mapButtonState.size();i++)
				{
					if(m_mapButtonState[i] != BUTTON_STATE_DISABLE)
					{
	...
	case CITY_NPC_BUTTON_CLOSE:
		{
			...
			// 2015-05-29 by jwlee 연구소 UI 변경
#if defined(C_LABORATORY_UI_CHANGE_JWLEE)
			if(BUILDINGNPC_RENDER_LABORATORY == m_nCurrentBuildingNPCState)
			{
				CMapCityShopIterator it = m_mapCityShop.find(BUILDINGKIND_LABORATORY);
				if(it != m_mapCityShop.end())
				{
					if(!((CINFCityLab*)it->second)->ISStateNormal())
					{
						return;
					}
				}		
...
//			m_pCurrentBuildingNPC = NULL;
			...
			// 2013-11-29 by ymjoo 상점에서 클릭했던 아이템의 정보가 적용되는 현상 수정
...
// 			m_nCurrentBuildingNPCState = BUILDINGNPC_RENDER_SHOP;
			...
			{
				BUILDINGNPC* pInfo = g_pDatabase->GetServerBuildingNPCInfo(pBuilding->buildingInfo.BuildingIndex);				
				CINFTradeItemCenter* pTradeCenter = new CINFTradeItemCenter(g_pGameMain, pInfo);
				pTradeCenter->SetGameData( g_pGameMain->m_pGameData );
				pTradeCenter->InitDeviceObjects();
				pTradeCenter->RestoreDeviceObjects();
				m_mapCityShop[pBuilding->buildingInfo.BuildingKind] = pTradeCenter;	
			}
			m_nCurrentBuildingNPCState = BUILDINGNPC_RENDER_SHOP;
// end 2013-11-29 by ssjung 거래소 구현
#endif
	...
	case BUILDINGKIND_CASH_SHOP:
		{
			...
			if(it == m_mapCityShop.end())
			{
				...
				#ifdef SC_WEB_CASHSHOP_SSJUNG_JHSEOL
			...
			((CINFCityCashShop*)it2->second)->InitScrollbar();
...
// 2014-01-15 by ssjung 파괴 경매장 구현
	case BUILDINGKIND_DESTROY_AUCTION:
		{	
			...
			{
				BUILDINGNPC* pInfo = g_pDatabase->GetServerBuildingNPCInfo(pBuilding->buildingInfo.BuildingIndex);				
				CINFReturnAuctionCenter* pAuctionCenter = new CINFReturnAuctionCenter();
				pAuctionCenter->SetGameData( g_pGameMain->m_pGameData );
				pAuctionCenter->InitDeviceObjects();
				pAuctionCenter->RestoreDeviceObjects();
				m_mapCityShop[pBuilding->buildingInfo.BuildingKind] = pAuctionCenter;	   
			}
			m_nCurrentBuildingNPCState = BUILDINGNPC_RENDER_SHOP;
#endif
		}
...
// 2013-11-29 by ssjung 거래소 구현
#ifdef SC_MARKET_JHSEOL_BCKIM_SSJUNG
		if(BUILDINGKIND_AUCTION == m_pCurrentBuildingNPC->buildingInfo.BuildingKind)
		{
			if(it2 != m_mapCityShop.end())
			{
				((CINFTradeItemCenter*)it2->second)->RestoreDeviceObjects();
// 				CINFTradeItemBuy* pTradeBuyView = ((CINFTradeItemBuy*)((CINFTradeItemCenter*)it2->second)->GetCurrentView());
// 				if(pTradeBuyView->RefreshSendMsg())
				{
...
// 2014-01-15 by ssjung 파괴 경매장 구현
#ifdef SC_DESTROY_AUCTION_JHSEOL_BCKIM_SSJUNG
		if(BUILDINGKIND_DESTROY_AUCTION == m_pCurrentBuildingNPC->buildingInfo.BuildingKind)
		{
			if(it2 != m_mapCityShop.end())
			{
				g_pFieldWinSocket->SendMsg( T_FC_DESTROY_AUCTION_BASE_INFO_REQUEST, NULL , 0);
				((CINFReturnAuctionCenter*)it2->second)->ShowWnd(TRUE);
			}
		}	
#endif
// end 2014-01-15 by ssjung 파괴 경매장 구현
...
int CINFCityBase::WebWndProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	switch(uMsg)
	{
	case WM_LBUTTONDOWN:
		{
			...
			const int nCloseButtonSize = 18;

			if(	pt.x >= WEB_START_X + WEB_WIDTH - nCloseButtonSize	&&
				pt.x <= WEB_START_X + WEB_WIDTH			&&
				pt.y >=	WEB_START_Y - nCloseButtonSize	&&
				pt.y <=	WEB_START_Y )
			{
...
// 2013-11-29 by ssjung 거래소 구현
#ifdef SC_MARKET_JHSEOL_BCKIM_SSJUNG
	CMapCityShopIterator itAuction = m_mapCityShop.find(BUILDINGKIND_AUCTION);
	if(itAuction != m_mapCityShop.end()) 
	{
		((CINFTradeItemCenter*)itAuction->second)->OnCloseInfWnd();
	}
#endif
// end 2013-11-29 by ssjung 거래소 구현
...
// 2014-01-15 by ssjung 파괴 경매장 구현
#ifdef SC_DESTROY_AUCTION_JHSEOL_BCKIM_SSJUNG
	CMapCityShopIterator itReturnAuction = m_mapCityShop.find(BUILDINGKIND_DESTROY_AUCTION);
	if(itReturnAuction != m_mapCityShop.end()) 
	{
		((CINFReturnAuctionCenter*)itReturnAuction->second)->OnCloseInfWnd();
	}
#endif
// end 2014-01-15 by ssjung 파괴 경매장 구현
...
// 2013-11-29 by ssjung 거래소 구현
void CINFCityBase::RenderBuildingItemTradeCenter(CINFBase* pShop)
{
#ifdef SC_MARKET_JHSEOL_BCKIM_SSJUNG
	pShop->Render();
#endif
