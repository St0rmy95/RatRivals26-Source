	...
	for(int tab=0;tab<4;tab++)
	{
		...
		while(it != m_vecItemInfo[tab].end())
		{
			if(m_pnSelectItemNumber[tab][i]>0)
			{
				...
				int nItemPrice = pItem->Price;
				nItemPrice = (int)(CAtumSJ::GetItemWPPrice(pItem->Price, ((CINFGameMain*)m_pParent)->GetAllHappyHourInfo()->fWPDiscountRate));
				int Cashprice = nItemPrice * m_pnSelectItemNumber[tab][i] / (pItem->MinTradeQuantity == 0 ? 1.0f : pItem->MinTradeQuantity);
#else
...
CINFCityWarPointShop::CINFCityWarPointShop(CAtumNode* pParent, BUILDINGNPC* pBuilding, CGameData *pGameData)
{
	...
	for(i=0;i<SHOP_ITEM_SLOT_NUMBER;i++)
	{
		...
		// 2014-07-17 by ymjoo DrawText 성능 개선 작업 (워포인트샵)
...
#endif
		// END 2014-07-17 by ymjoo DrawText 성능 개선 작업 (워포인트샵)
	...
	if(m_pnSelectItemNumber[m_nCurrentTab])
	{
		for(i=0;i<SHOP_ITEM_SLOT_NUMBER;i++)
		{
			...
			if(it != m_vecItemInfo[m_nCurrentTab].end())
			{
				...
				// 2013-08-22 by bhsohn 해피아워 리뉴얼 
#ifdef SC_RENEWER_HAPPY_HOUR_BHSOHN_BCKIM
				wsprintf( temp1, "%d", (int)(CAtumSJ::GetItemWPPrice(pItem->Price, ((CINFGameMain*)m_pParent)->GetAllHappyHourInfo()->fWPDiscountRate)));
#else
...
#ifdef C_DRAWTEXT_UPGRADE_YMJOO			// 2014-07-17 by ymjoo DrawText 성능 개선 작업 (워포인트샵)
				vector<string> vecMessage;
				if(m_pFontItemName[i][2]->GetStringSize(pItem->ItemName).cx <= SHOP_TEXT_SHOW_SIZE_X)
				{
					m_pFontItemName[i][0]->DrawText(nPosX + SHOP_ITEM_NAME_START_X, nPosY + SHOP_ITEM_NAME_START_Y + SHOP_ITEM_SLOT_INTERVAL * i, GUI_FONT_COLOR, pItem->ItemName, 0L);
				}
				else
				{
					StringCullingUserDataEx(pItem->ItemName, SHOP_TEXT_SHOW_SIZE_X, &vecMessage, m_pFontItemName[i][2], FALSE, TRUE);
					for(int j = 0 ; j < SHOP_TEXT_SHOW ; ++j)
					{
						m_pFontItemName[i][j]->DrawText(nPosX + SHOP_ITEM_NAME_START_X, 
														nPosY + SHOP_ITEM_NAME_START_Y - 8 + SHOP_ITEM_SLOT_INTERVAL * i + (j * 14), 
