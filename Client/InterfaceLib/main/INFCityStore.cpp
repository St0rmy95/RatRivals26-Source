...
// 2006-09-15 by dgwoo 여단 창고 관련 탭 DEFINE
	...
	#define STORE_SPI_TEXT_Y		(STORE_RESIZE_Y + 290)
...
//	m_pSelectIcon	 = NULL;
	...
	// 2014-08-01 by ymjoo DrawText 성능 개선 작업 (창고)
	...
	m_pFontItemNum = NULL;
#endif
	// END 2014-08-01 by ymjoo DrawText 성능 개선 작업 (창고)
	m_pFontLog	= NULL;
	m_pScroll = NULL;
	m_nItemSpi = 0;
	m_nLineCount = 0;
...
// 2007-10-10 by dgwoo 예당은 멤버쉽이 아니더라도 여단창고 이용가능.
	...
	// 2013-10-07 by bhsohn 창고에서 툴팁 깨져 나오는 현상 수정
	if(g_pGameMain)
	{
		g_pGameMain->SetItemInfoUser( NULL, 0 , 0 );
	}
	// END 2013-10-07 by bhsohn 창고에서 툴팁 깨져 나오는 현상 수정
			...
			if(pIcon)
			{
				...
				if( IS_COUNTABLE_ITEM(pItemInfo->Kind) )
				{
					...
					if( pItemInfo->CurrentCount > 1 )
					{
						...
						// 2014-08-01 by ymjoo DrawText 성능 개선 작업 (창고)
#ifdef C_DRAWTEXT_UPGRADE_YMJOO
						m_pFontItemNum[i][j]->DrawText(STORE_ICON_START_X + STORE_INTERVAL * j + 21 - len * 6, 		// 여기서 6은 영문 숫자 텍스트 간격이다.,
...
//	if(nButton == 0) // OK
//	{
//		int i; for(i=0;i<m_vecItemInfo.size();i++)
//		{
