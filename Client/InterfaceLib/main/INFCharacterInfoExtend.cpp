...
#define PARTNER_SOCKET_POS_X		63
...
//end 2011-10-06 by jhahn 파트너 성장형 시스템
	...
	for(i = 0 ; i < 6 ; ++i)
	{
		m_StatInfoBoldFont[i] = NULL;
		m_StatOverInfoFont[i] = NULL;
	}
#else
	m_StatOverInfoFont = NULL;
	m_StatInfoBoldFont = NULL;
#endif
	// END 2014-07-02 by ymjoo DrawText 성능 개선 작업 (캐릭터 정보)
...
//	}
	...
	m_pFontStratum = NULL;
	m_pFontRecord = NULL;
	m_pFontWinningRate = NULL;
	m_pFontForcedTermination = NULL;
	m_pFontAccWP = NULL;
	m_pFontCurWP = NULL;
	m_pFontFuelConsum = NULL;
	m_pFontBoosterTime = NULL;
	m_pFontMoveSpeed = NULL;
#else
	m_pFontGeneral = NULL;
#endif
	// END 2014-07-02 by ymjoo DrawText 성능 개선 작업 (캐릭터 정보)
...
//		m_pFontQuestState[i] = NULL;
	...
	// 2014-07-03 by ymjoo DrawText 성능 개선 작업 (캐릭터 스킬정보)
#ifdef C_DRAWTEXT_UPGRADE_YMJOO
	for(i = 0 ; i < CHAR_UTC_TYPE_NUMBER ; ++i)
	{
		for(int j = 0 ; j < MAX_CHAR_UTC_SLOT_NUMBER ; ++j)
		{
			for(int k = 0 ; k < 2 ; ++k)
			{
				m_pFontItemNum[i][j][k] = NULL;
			}
		}
	}
#else
...
#endif
	// END 2014-07-03 by ymjoo DrawText 성능 개선 작업 (캐릭터 스킬정보)
...
//	SAFE_DELETE(m_pScrollBar);
	...
	// 2014-07-02 by ymjoo DrawText 성능 개선 작업 (캐릭터 정보)
#ifdef C_DRAWTEXT_UPGRADE_YMJOO
	SAFE_DELETE(m_pFontStratum);
	SAFE_DELETE(m_pFontRecord);
	SAFE_DELETE(m_pFontWinningRate);
	SAFE_DELETE(m_pFontForcedTermination);
	SAFE_DELETE(m_pFontAccWP);
	SAFE_DELETE(m_pFontCurWP);
	SAFE_DELETE(m_pFontFuelConsum);
	SAFE_DELETE(m_pFontBoosterTime);
	SAFE_DELETE(m_pFontMoveSpeed);
#else
...
//2011-10-06 by jhahn 파트너 성장형 시스템
	...
	for (count = 0 ; count < 9 ; count++)
	{
		SAFE_DELETE(m_pPartnerSkillShop[count]);
	}

	for ( count = 0 ; count < 9 ; count++)
	{
		SAFE_DELETE(m_pOkSocket[count]);
	}
//end 2011-10-06 by jhahn 파트너 성장형 시스템
	...
	for(i = 0 ; i < 6 ; ++i)
	{
		SAFE_DELETE(m_StatInfoBoldFont[i]);
		SAFE_DELETE(m_StatOverInfoFont[i]);
	}
#else
	...
	SAFE_DELETE(m_SocketDescInfoFont);			   //2011-10-06 by jhahn 파트너 성장형 시스템
...
//		SAFE_DELETE(m_pFontQuestState[i]);
	...
	for(i = 0 ; i < CHAR_UTC_TYPE_NUMBER ; ++i)
	{
		for(int j = 0 ; j < MAX_CHAR_UTC_SLOT_NUMBER ; ++j)
		{
			for(int k = 0 ; k < 2 ; ++k)
			{
				SAFE_DELETE(m_pFontItemNum[i][j][k]);
			}
		}
	}
#else
...
//2011-10-06 by jhahn 파트너 성장형 시스템
	SAFE_DELETE(m_pPartnerItemLine);

	SAFE_DELETE(m_pPartnerItemUseSelect);
	
	SAFE_DELETE(m_pPartnerSkillSocket);
	SAFE_DELETE(m_pPartnerSkillshopSocket);

	

	for(i=0; i < 4; i++)
	{
		SAFE_DELETE(m_pItemSocketHP[i]);		
		SAFE_DELETE(m_pItemSocketShield[i]);		
	//	SAFE_DELETE(m_pItemSocketSp[i]);			
	}
	
	SAFE_DELETE(m_pUseSkillSocket);

	SAFE_DELETE(m_pBigSlotSocket);
//end 2011-10-06 by jhahn 파트너 성장형 시스템	
...
// end 2012-06-15 by isshin 키트 자동 사용 소켓 설정 스크롤 변경 - c_scrlbt -> scrl_b
	...
	// 2013-11-26 by ymjoo 키트 자동 사용 소켓 파트너창에 D급 SP 키트를 추가
#ifdef C_AUTO_USE_KIT_SOCKET_ADD_SP_KIT_D
	m_pItemSocketSp[0] = new CINFImageEx;
	pDataHeader = FindResource("PN_usespd");
	...
	m_pItemSocketSp[3]->InitDeviceObjects(pDataHeader) ;
#endif
	// END 2013-11-26 by ymjoo 키트 자동 사용 소켓 파트너창에 D급 SP 키트를 추가
...
//	m_pImgArenaTab->InitDeviceObjects(pDataHeader->m_pData,pDataHeader->m_DataSize) ;
	...
	for(i = 0 ; i < 6 ; ++i)
	{
		m_StatInfoBoldFont[i] = new CD3DHanFont(_T(g_pD3dApp->GetFontStyle()), 9, D3DFONT_ZENABLE, FALSE, 256, 32);
		m_StatInfoBoldFont[i]->InitDeviceObjects(g_pD3dDev);

		m_StatOverInfoFont[i] = new CD3DHanFont(_T(g_pD3dApp->GetFontStyle()), 9, D3DFONT_ZENABLE|D3DFONT_BOLD, FALSE, 256, 32);
		m_StatOverInfoFont[i]->InitDeviceObjects(g_pD3dDev);
	}
#else
...
//	}
	...
	// 2014-07-02 by ymjoo DrawText 성능 개선 작업 (캐릭터 정보)
#ifdef C_DRAWTEXT_UPGRADE_YMJOO
	m_pFontStratum = new CD3DHanFont(_T(g_pD3dApp->GetFontStyle()), 9, D3DFONT_ZENABLE, FALSE, 256, 32);
	m_pFontStratum->InitDeviceObjects(g_pD3dDev);
	m_pFontRecord = new CD3DHanFont(_T(g_pD3dApp->GetFontStyle()), 9, D3DFONT_ZENABLE, FALSE, 256, 32);
	m_pFontRecord->InitDeviceObjects(g_pD3dDev);
	m_pFontWinningRate = new CD3DHanFont(_T(g_pD3dApp->GetFontStyle()), 9, D3DFONT_ZENABLE, FALSE, 256, 32);
	m_pFontWinningRate->InitDeviceObjects(g_pD3dDev);
	m_pFontForcedTermination = new CD3DHanFont(_T(g_pD3dApp->GetFontStyle()), 9, D3DFONT_ZENABLE, FALSE, 256, 32);
	m_pFontForcedTermination->InitDeviceObjects(g_pD3dDev);
	m_pFontAccWP = new CD3DHanFont(_T(g_pD3dApp->GetFontStyle()), 9, D3DFONT_ZENABLE, FALSE, 256, 32);
	m_pFontAccWP->InitDeviceObjects(g_pD3dDev);
	m_pFontCurWP = new CD3DHanFont(_T(g_pD3dApp->GetFontStyle()), 9, D3DFONT_ZENABLE, FALSE, 256, 32);
	m_pFontCurWP->InitDeviceObjects(g_pD3dDev);
	m_pFontFuelConsum = new CD3DHanFont(_T(g_pD3dApp->GetFontStyle()), 9, D3DFONT_ZENABLE, FALSE, 256, 32);
	m_pFontFuelConsum->InitDeviceObjects(g_pD3dDev);
	m_pFontBoosterTime = new CD3DHanFont(_T(g_pD3dApp->GetFontStyle()), 9, D3DFONT_ZENABLE, FALSE, 256, 32);
	m_pFontBoosterTime->InitDeviceObjects(g_pD3dDev);
	m_pFontMoveSpeed = new CD3DHanFont(_T(g_pD3dApp->GetFontStyle()), 9, D3DFONT_ZENABLE, FALSE, 256, 32);
	m_pFontMoveSpeed->InitDeviceObjects(g_pD3dDev);
#else
...
//		m_pFontQuestState[i]->InitDeviceObjects(g_pD3dDev) ;
	...
	for(i = 0 ; i < CHAR_UTC_TYPE_NUMBER ; ++i)
	{
		for(int j = 0 ; j < MAX_CHAR_UTC_SLOT_NUMBER ; ++j)
		{
			for(int k = 0 ; k < 2 ; ++k)
			{
				m_pFontItemNum[i][j][k] = new CD3DHanFont(_T(g_pD3dApp->GetFontStyle()), 9, D3DFONT_ZENABLE, TRUE, 256, 32);
				m_pFontItemNum[i][j][k]->InitDeviceObjects(g_pD3dDev);
			}
		}
	}
#else
...
// 	m_pPartnerBtnOK->RestoreDeviceObjects();
	...
	// 2014-07-02 by ymjoo DrawText 성능 개선 작업 (캐릭터 정보)
#ifdef C_DRAWTEXT_UPGRADE_YMJOO
	m_pFontStratum->RestoreDeviceObjects();
	m_pFontRecord->RestoreDeviceObjects();
	m_pFontWinningRate->RestoreDeviceObjects();
	m_pFontForcedTermination->RestoreDeviceObjects();
	m_pFontAccWP->RestoreDeviceObjects();
	m_pFontCurWP->RestoreDeviceObjects();
	m_pFontFuelConsum->RestoreDeviceObjects();
	m_pFontBoosterTime->RestoreDeviceObjects();
	m_pFontMoveSpeed->RestoreDeviceObjects();
#else
	...
	for(i = 0 ; i < 6 ; ++i)
	{
		m_StatInfoBoldFont[i]->RestoreDeviceObjects();
		m_StatOverInfoFont[i]->RestoreDeviceObjects();
	}
#else
...
//		m_pFontQuestState[i]->RestoreDeviceObjects() ;
	...
	for(i = 0 ; i < CHAR_UTC_TYPE_NUMBER ; ++i)
	{
		for(int j = 0 ; j < MAX_CHAR_UTC_SLOT_NUMBER ; ++j)
		{
			for(int k = 0 ; k < 2 ; ++k)
			{
				m_pFontItemNum[i][j][k]->RestoreDeviceObjects();
			}
		}
	}
#else
...
//	SAFE_DELETE(m_pScrollBar );
	...
	SAFE_DELETE(m_pHpItemScroll);
	SAFE_DELETE(m_pDpItemScroll);
	SAFE_DELETE(m_pSpItemScroll);
	//end 2011-10-06 by jhahn 파트너 성장형 시스템
...
// 	SAFE_DELETE(m_pPartnerBtnOK);
	...
	// 2014-07-02 by ymjoo DrawText 성능 개선 작업 (캐릭터 정보)
#ifdef C_DRAWTEXT_UPGRADE_YMJOO
	m_pFontStratum->DeleteDeviceObjects();
	SAFE_DELETE(m_pFontStratum );
	m_pFontRecord->DeleteDeviceObjects();
	SAFE_DELETE(m_pFontRecord );
	m_pFontWinningRate->DeleteDeviceObjects();
	SAFE_DELETE(m_pFontWinningRate );
	m_pFontForcedTermination->DeleteDeviceObjects();
	SAFE_DELETE(m_pFontForcedTermination );
	m_pFontAccWP->DeleteDeviceObjects();
	SAFE_DELETE(m_pFontAccWP );
	m_pFontCurWP->DeleteDeviceObjects();
	SAFE_DELETE(m_pFontCurWP );
	m_pFontFuelConsum->DeleteDeviceObjects();
	SAFE_DELETE(m_pFontFuelConsum );
	m_pFontBoosterTime->DeleteDeviceObjects();
	SAFE_DELETE(m_pFontBoosterTime );
	m_pFontMoveSpeed->DeleteDeviceObjects();
	SAFE_DELETE(m_pFontMoveSpeed );
#else
	...
	for(i = 0 ; i < 6 ; ++i)
	{
		if(m_StatInfoBoldFont[i])
		{
			m_StatInfoBoldFont[i]->DeleteDeviceObjects();
			SAFE_DELETE(m_StatInfoBoldFont[i]);
		}
		if(m_StatOverInfoFont[i])
		{
			m_StatOverInfoFont[i]->DeleteDeviceObjects();
			SAFE_DELETE(m_StatOverInfoFont[i]);
		}
	}
#else
	...
	if(m_SocketDescInfoFont)
	{
		...
		SAFE_DELETE(m_SocketDescInfoFont );
	}
	//end 2011-10-06 by jhahn 파트너 성장형 시스템
...
//		SAFE_DELETE(m_pFontQuestState[i] );
	...
	for(i = 0 ; i < CHAR_UTC_TYPE_NUMBER ; ++i)
	{
		for(int j = 0 ; j < MAX_CHAR_UTC_SLOT_NUMBER ; ++j)
		{
			for(int k = 0 ; k < 2 ; ++k)
			{
				m_pFontItemNum[i][j][k]->DeleteDeviceObjects();
				SAFE_DELETE(m_pFontItemNum[i][j][k]);
			}
		}
	}
	...
	{		
		...
		SAFE_DELETE(m_pPartnerSocketSettingOk);
	}	
...
//2011-12-120 by jhahn 성장형 파트너 시스템 
	...
	SAFE_DELETE(m_pImgPnUseHp);

	m_pImgPnUseSh->DeleteDeviceObjects();
	SAFE_DELETE(m_pImgPnUseSh);

	m_pImgPnUseSp->DeleteDeviceObjects();
	SAFE_DELETE(m_pImgPnUseSp);
//end 2011-12-120 by jhahn 성장형 파트너 시스템 
...
//2011-10-06 by jhahn 파트너 성장형 시스템
	...
	for(i = 0 ; i < 6 ; ++i)
	{
		m_StatInfoBoldFont[i]->InvalidateDeviceObjects();
		m_StatOverInfoFont[i]->InvalidateDeviceObjects();
	}
#else
...
//	}
	...
	// 2014-07-02 by ymjoo DrawText 성능 개선 작업 (캐릭터 정보)
#ifdef C_DRAWTEXT_UPGRADE_YMJOO
	m_pFontStratum->InvalidateDeviceObjects();
	m_pFontRecord->InvalidateDeviceObjects();
	m_pFontWinningRate->InvalidateDeviceObjects();
	m_pFontForcedTermination->InvalidateDeviceObjects();
	m_pFontAccWP->InvalidateDeviceObjects();
	m_pFontCurWP->InvalidateDeviceObjects();
	m_pFontFuelConsum->InvalidateDeviceObjects();
	m_pFontBoosterTime->InvalidateDeviceObjects();
	m_pFontMoveSpeed->InvalidateDeviceObjects();
#else
...
//		m_pFontQuestState[i]->InvalidateDeviceObjects() ;
	...
	for(i = 0 ; i < CHAR_UTC_TYPE_NUMBER ; ++i)
	{
		for(int j = 0 ; j < MAX_CHAR_UTC_SLOT_NUMBER ; ++j)
		{
			for(int k = 0 ; k < 2 ; ++k)
			{
				m_pFontItemNum[i][j][k]->InvalidateDeviceObjects();
			}
		}
	}
#else
...
void CINFCharacterInfoExtend::ItemRender()
{
	...
	// 2014-07-02 by ymjoo DrawText 성능 개선 작업 (캐릭터 정보)
#ifdef C_DRAWTEXT_UPGRADE_YMJOO
	szSizeTmp = m_pPartnerItem->GetStringSize(buff);
#else
	...
	{
		// 2013-11-26 by ymjoo 키트 자동 사용 소켓 파트너창 UI 개선
		//m_pPartnerItemLine->Move( m_ptBkPos.x + 95 + 22,	m_ptBkPos.y + 442 + 26 + 10 );
		m_pPartnerItemLine->Move( m_ptBkPos.x + 95 + 22,	m_ptBkPos.y + 442 + 26 + 10 - 1 );
...
//end 2011-12-120 by jhahn 성장형 파트너 시스템 
	...
	for (int i = 0 ; i < 4 ; i++)
	{
		if(psPetLevelDataPrev->KitLevelHP > i)
		{
			...
			m_pItemSocketHP[i]->Move(m_ptBkPos.x + PARTNER_SOCKET_KIT_ITEM_POS_X + i * 13 , m_ptBkPos.y +PARTNER_SOCKET_KIT_ITEM_POS_Y );
		...
		if(psPetLevelDataPrev->KitLevelShield > i)
		{
			...
			m_pItemSocketShield[i]->Move(m_ptBkPos.x + PARTNER_SOCKET_KIT_ITEM_POS_X + i * 13 , m_ptBkPos.y +PARTNER_SOCKET_KIT_ITEM_POS_Y + 35 );
			m_pItemSocketShield[i]->Render();
			
		}
		// 2013-11-26 by ymjoo 키트 자동 사용 소켓 파트너창에 D급 SP 키트를 추가
#ifdef C_AUTO_USE_KIT_SOCKET_ADD_SP_KIT_D
		if(psPetLevelDataPrev->KitLevelSP >= i && psPetLevelDataPrev->KitLevelSP != 0)
#else
	...
	// END 2013-11-26 by ymjoo 키트 자동 사용 소켓 파트너창에 D급 SP 키트를 추가
		{
			...
			m_pItemSocketSp[i]->Move(m_ptBkPos.x + PARTNER_SOCKET_KIT_ITEM_POS_X + i * 13 , m_ptBkPos.y +PARTNER_SOCKET_KIT_ITEM_POS_Y + 69 );
...
//		strcat( strBuf, STRMSG_C_120712_0001 );			
	...
	{
		int i; for(i=0; i<strlen(strBuf); i++)
		{
			if(strBuf[i]=='$')
				strBuf[i] = ' ';
		}
...
//	StaminaBar( psPetCurInfo->Stamina , psPetLevelData->Stamina );
	...
	*/
	...
	// 2014-07-02 by ymjoo DrawText 성능 개선 작업 (캐릭터 정보)
#ifdef C_DRAWTEXT_UPGRADE_YMJOO
	szSize = m_pPartnerLvBar->GetStringSize(szTemp);
#else
...
void CINFCharacterInfoExtend::ItemScrollUpdate()
{
	...
	m_pDpItemScroll->SetPosition( nWindowPosX + PARTNER_ITEM_SCROLL_POS_X, nWindowPosY + PARTNER_ITEM_SCROLL_POS_Y + 35 , PARTNER_ITEM_SCROLL_POS_W, PARTNER_ITEM_SCROLL_POS_H);
	RECT Dpitemrect = {
		nWindowPosX + PARTNER_ITEM_SCROLL_POS_X,
			// 2013-11-26 by ymjoo 키트 자동 사용 소켓 파트너창 UI 개선
			//nWindowPosY + PARTNER_ITEM_SCROLL_POS_Y + 34,
			nWindowPosY + PARTNER_ITEM_SCROLL_POS_Y + 35,
			nWindowPosX + PARTNER_ITEM_SCROLL_POS_X + PARTNER_ITEM_SCROLL_POS_W,
			nWindowPosY + PARTNER_ITEM_SCROLL_POS_Y + 10
	};
	...
	m_pSpItemScroll->SetPosition( nWindowPosX + PARTNER_ITEM_SCROLL_POS_X, nWindowPosY + PARTNER_ITEM_SCROLL_POS_Y + 35 * 2 , PARTNER_ITEM_SCROLL_POS_W, PARTNER_ITEM_SCROLL_POS_H);
	RECT Spitemrect = {
		nWindowPosX + PARTNER_ITEM_SCROLL_POS_X,
			// 2013-11-26 by ymjoo 키트 자동 사용 소켓 파트너창 UI 개선
			//nWindowPosY + PARTNER_ITEM_SCROLL_POS_Y + 34 * 2,
			nWindowPosY + PARTNER_ITEM_SCROLL_POS_Y + 35 * 2,
			nWindowPosX + PARTNER_ITEM_SCROLL_POS_X + PARTNER_ITEM_SCROLL_POS_W,
			nWindowPosY + PARTNER_ITEM_SCROLL_POS_Y + 10
	};
...
//	}
	// end 2008-08-22 by bhsohn EP3 인벤토리 처리
	CINFIcon* pIcon = ((CINFGameMain*)m_pParent)->m_pIcon;
	int i; for(i=0;i<CHAR_UTC_TYPE_NUMBER;i++)
	{
...
#ifdef C_DRAWTEXT_UPGRADE_YMJOO						// 2014-07-03 by ymjoo DrawText 성능 개선 작업 (캐릭터 스킬정보)
						g_pGameMain->TextReduce(m_pFontItemNum[i][j][1], 73, buf);
#else
			...
			{				
					...
					&& pt.y < nWindowPosY + PARTNER_SOCKET_KIT_POS_Y + 30 + j * PARTNER_SOCKET_KIT_POS_GAP ))
				{
					...
					else if (j == 2 && tempCurrentInfo)
					{
						// 2013-11-26 by ymjoo 키트 자동 사용 소켓 파트너창 UI 개선
//						SoketItem = g_pStoreData->FindItemInInventoryByUniqueNumber(tempCurrentInfo->PetKitHP.ItemNum ); 
						SoketItem = g_pStoreData->FindItemInInventoryByUniqueNumber(tempCurrentInfo->PetKitSP.ItemNum );
					}
	...
	case WM_LBUTTONDOWN:
		{
			...
			{
				// 2014-04-30 by ymjoo 파트너 경험치 설정 스크롤 되는 버그 수정
				if(NULL == g_pStoreData->FindItemInInventoryByWindowPos( POS_PET ))
				{
					return INF_MSGPROC_BREAK;
				}
				// END 2014-04-30 by ymjoo 파트너 경험치 설정 스크롤 되는 버그 수정
	...
	case WM_LBUTTONUP:
		{
			...
			if(m_pExpScroll->GetMouseMoveMode())
			{
				...
				// 2014-04-09 by ymjoo 파트너 경험치 설정 버그 수정
				if(pAllItem)
				{
...
// 				msg.ExpRatio = 	m_pExpScroll->GetScrollStep();
// 				g_pFieldWinSocket->SendMsg( T_FC_ITEM_PET_SET_EXP_RATIO, (char*)(&msg), sizeof(MSG_FC_ITEM_PET_SET_EXP_RATIO) );
				// END 2014-04-09 by ymjoo 파트너 경험치 설정 버그 수정
				//end 2011-10-06 by jhahn 파트너 성장형 시스템
				return INF_MSGPROC_BREAK;
			}	  
			//2011-10-06 by jhahn 파트너 성장형 시스템
	...
	if(pShuttle)
	{
		...
		{			
			...
			if(m_nShowOverStat[STAT_SHUTTLEINFO_ATTACK] > 0)
			{
				...
				if(m_StatOverInfoFont[0])
				{				
					// 빨간색으로 그린다. 
					m_StatOverInfoFont[0]->DrawText(fPosX+OVER_STAT_CAPX, fPosY, GUI_FONT_COLOR_R, szOvrbuff, 0 );
				}
				
				// 두꺼운 글씨로 그려준다.
				wsprintf(buff, "\\e%4d", (int)pShuttle->m_myShuttleInfo.TotalGearStat.AttackPart);//+(int)pShuttle->m_paramFactor.pfp_ATTACK_PART) ;
				if(m_StatInfoBoldFont[0])
				{
					m_StatInfoBoldFont[0]->DrawText(fPosX, fPosY,GUI_FONT_COLOR, buff, 0 );
				}
		...
		{			
			...
			if(m_nShowOverStat[STAT_SHUTTLEINFO_DEFENCE] > 0)
			{
				...
				if(m_StatOverInfoFont[1])
				{				
					// 빨간색으로 그린다. 
					m_StatOverInfoFont[1]->DrawText(fPosX+OVER_STAT_CAPX, fPosY, GUI_FONT_COLOR_R, szOvrbuff, 0 );
				}
				...
				if(m_StatInfoBoldFont[1])
				{
					m_StatInfoBoldFont[1]->DrawText(fPosX, fPosY,GUI_FONT_COLOR, buff, 0 );
				}
			}
		...
		{			
			...
			if(m_nShowOverStat[STAT_SHUTTLEINFO_FUEL] > 0)
			{
				...
				if(m_StatOverInfoFont[2])
				{				
					// 빨간색으로 그린다. 
					m_StatOverInfoFont[2]->DrawText(fPosX+OVER_STAT_CAPX, fPosY, GUI_FONT_COLOR_R, szOvrbuff, 0 );
				}
				...
				if(m_StatInfoBoldFont[2])
				{
					m_StatInfoBoldFont[2]->DrawText(fPosX, fPosY,GUI_FONT_COLOR, buff, 0 );
				}
			}
		...
		{			
			...
			if(m_nShowOverStat[STAT_SHUTTLEINFO_SOUL] > 0)
			{
				...
				if(m_StatOverInfoFont[3])
				{				
					// 빨간색으로 그린다. 
					m_StatOverInfoFont[3]->DrawText(fPosX+OVER_STAT_CAPX, fPosY, GUI_FONT_COLOR_R, szOvrbuff, 0 );
				}
				...
				if(m_StatInfoBoldFont[3])
				{
					m_StatInfoBoldFont[3]->DrawText(fPosX, fPosY,GUI_FONT_COLOR, buff, 0 );
				}
			}
		...
		{			
			...
			if(m_nShowOverStat[STAT_SHUTTLEINFO_DODGE] > 0)
			{
				...
				if(m_StatOverInfoFont[4])
				{				
					// 빨간색으로 그린다. 
					m_StatOverInfoFont[4]->DrawText(fPosX+OVER_STAT_CAPX, fPosY, GUI_FONT_COLOR_R, szOvrbuff, 0 );
				}
				...
				if(m_StatInfoBoldFont[4])
				{
					m_StatInfoBoldFont[4]->DrawText(fPosX, fPosY,GUI_FONT_COLOR, buff, 0 );
				}
			}
		...
		{			
			...
			if(m_nShowOverStat[STAT_SHUTTLEINFO_SHIELD] > 0)
			{
				...
				if(m_StatOverInfoFont[5])
				{				
					// 빨간색으로 그린다. 
					m_StatOverInfoFont[5]->DrawText(fPosX+OVER_STAT_CAPX, fPosY, GUI_FONT_COLOR_R, szOvrbuff, 0 );
				}
				...
				if(m_StatInfoBoldFont[5])
				{
					m_StatInfoBoldFont[5]->DrawText(fPosX, fPosY,GUI_FONT_COLOR, buff, 0 );
				}
			}
...
void CINFCharacterInfoExtend::RenderCommandInfo()
{
	...
	{
		wsprintf(buff, "%s", g_pShuttleChild->m_myShuttleInfo.GuildName );//"소속:%s"					
		// 2013-08-22 by bhsohn 여단명 너무 길떄 여단마크 어긋나는 버그 수정
		g_pGameMain->TextReduce(m_pFontGuild, 70, buff); // 2013-03-28 by bhsohn 여단 이름이 너무 길 경우 정보창에서 필드 부족
	...
	wsprintf(buff, "%s", (LPTSTR)CAtumSJ::GetStatusByLevel(g_pD3dApp->m_pShuttleChild->m_myShuttleInfo.Level) );//"신분:%s"	
	// 2014-07-02 by ymjoo DrawText 성능 개선 작업 (캐릭터 정보)
#ifdef C_DRAWTEXT_UPGRADE_YMJOO
	szSize = m_pFontStratum->GetStringSize(buff);
	m_pFontStratum->DrawText(nWindowPosX + GENERAL_START_X - szSize.cx,
	...
	wsprintf(buff,"%d",g_pD3dApp->m_pShuttleChild->m_myShuttleInfo.Propensity);//"명성:%d"	
...
//		}
		...
		
		{
			sprintf( buff, STRMSG_C_080923_0200, ((float)CAtumSJ::GetDefenseC( &g_pShuttleChild->m_myShuttleInfo, 
							...
							&paramCharctorParam, TRUE ) / PROB100_MAX_VALUE)*100.0f );//"방어력 기본무기 : %.2f%%"
			...
			sprintf( buff, STRMSG_C_080923_0200, ((float)CAtumSJ::GetDefenseC( &g_pShuttleChild->m_myShuttleInfo, 
							...
							&paramCharctorParam, FALSE ) / PROB100_MAX_VALUE)*100.0f );//"       고급무기 : %.2f%%"
			szSize = m_CharacInfoFont[0]->GetStringSize(buff);
...
// 					TRUE, g_pShuttleChild->m_myShuttleInfo.TotalGearStat.AttackPart )*100)/255);	// 캐릭터의 공격 확률
				...
				sprintf( buff, STRMSG_C_080923_0200,//"공격확율 : %.2f"
						// 2010-07-28 by dgwoo 확률 수식 변경 (255 => 100%)
						 ...
						 (CAtumSJ::GetAttackProbabilityC(g_pShuttleChild->m_pPrimaryWeapon->GetRealItemInfo(),&paramPrimaryParam,
						 TRUE, g_pShuttleChild->m_myShuttleInfo.TotalGearStat.AttackPart, g_pShuttleChild->m_myShuttleInfo.Level )*100)/PROB100_MAX_VALUE);	// 캐릭터의 공격 확률
			...
			{			
				...
				m_pFontAmmoWeapon[1]->DrawText(nWindowPosX + AMMO_START_X - szSize.cx, 
...

			{
				...
				m_pFontAmmoWeapon[2]->DrawText(nWindowPosX + AMMO_START_X - szSize.cx, 
			...
			}
			{
				...
				m_pFontAmmoWeapon[3]->DrawText(nWindowPosX + AMMO_START_X - szSize.cx, 
...
// 					&paramSecondParam, FALSE, g_pShuttleChild->m_myShuttleInfo.TotalGearStat.AttackPart)*100)/255 );	// 캐릭터의 공격 확률
				...
				sprintf( buff, STRMSG_C_080923_0200,//"공격확율 : %.2f"
						...
						(CAtumSJ::GetAttackProbabilityC(g_pShuttleChild->m_pSecondaryWeapon->GetRealItemInfo(),
						 &paramSecondParam, FALSE, g_pShuttleChild->m_myShuttleInfo.TotalGearStat.AttackPart, g_pShuttleChild->m_myShuttleInfo.Level)*100)/PROB100_MAX_VALUE);	// 캐릭터의 공격 확률
				...
				m_pFontAmmoWeapon[4]->DrawText(nWindowPosX + AMMO_START_X + AMMO_START_INTERVAL_X - szSize.cx, 
											   nWindowPosY + AMMO_START_Y, 
			...
			{				
				...
				m_pFontAmmoWeapon[5]->DrawText(nWindowPosX + AMMO_START_X + AMMO_START_INTERVAL_X - szSize.cx, 
											   nWindowPosY + AMMO_START_Y + AMMO_INTERVAL, 
											   GUI_FONT_COLOR, buff, 0L);
#else
				szSize = m_pFontAmmoWeapon[2]->GetStringSize(buff);
				m_pFontAmmoWeapon[2]->DrawText( nWindowPosX+AMMO_START_X+AMMO_START_INTERVAL_X-szSize.cx, 
...

			{
				...
				// 2014-07-02 by ymjoo DrawText 성능 개선 작업 (캐릭터 정보)
#ifdef C_DRAWTEXT_UPGRADE_YMJOO
				szSize = m_pFontAmmoWeapon[6]->GetStringSize(buff);
				m_pFontAmmoWeapon[6]->DrawText(nWindowPosX + AMMO_START_X + AMMO_START_INTERVAL_X - szSize.cx, 
											   nWindowPosY + AMMO_START_Y + (AMMO_INTERVAL * 2), 
											   GUI_FONT_COLOR, buff, 0L);
#else
				szSize = m_pFontAmmoWeapon[2]->GetStringSize(buff);
				m_pFontAmmoWeapon[2]->DrawText( nWindowPosX+AMMO_START_X+AMMO_START_INTERVAL_X-szSize.cx, 
			...
			
			{
				...
				// 2014-07-02 by ymjoo DrawText 성능 개선 작업 (캐릭터 정보)
#ifdef C_DRAWTEXT_UPGRADE_YMJOO
				szSize = m_pFontAmmoWeapon[7]->GetStringSize(buff);
				m_pFontAmmoWeapon[7]->DrawText(nWindowPosX + AMMO_START_X + AMMO_START_INTERVAL_X - szSize.cx, 
											   nWindowPosY + AMMO_START_Y + AMMO_INTERVAL * 3, 
											   GUI_FONT_COLOR, buff, 0L);
#else
				szSize = m_pFontAmmoWeapon[3]->GetStringSize(buff);
		...
		// 2014-07-02 by ymjoo DrawText 성능 개선 작업 (캐릭터 정보)
#ifdef C_DRAWTEXT_UPGRADE_YMJOO
		szSize = m_pFontAmmoWeapon[8]->GetStringSize(buff);
		m_pFontAmmoWeapon[8]->DrawText(nWindowPosX + AMMO_WEIGHT_START_X - szSize.cx, 
...
#ifdef C_DRAWTEXT_UPGRADE_YMJOO
		szSize = m_pFontAmmoWeapon[9]->GetStringSize(buff);
		m_pFontAmmoWeapon[9]->DrawText(nWindowPosX + INVEN_WEIGHT_START_X - szSize.cx,
									   nWindowPosY + INVEN_WEIGHT_START_Y, 
									   GUI_FONT_COLOR, buff, 0L);
#else
		szSize = m_pFontAmmoWeapon[5]->GetStringSize(buff);
...
//		m_pImgArenaTab->Render();
		...
		// 2014-07-02 by ymjoo DrawText 성능 개선 작업 (캐릭터 정보)
#ifdef C_DRAWTEXT_UPGRADE_YMJOO
		szSize = m_pFontRecord->GetStringSize(buff);
		m_pFontRecord->DrawText(ARENA_SCORE_X - szSize.cx, ARENA_SCORE_Y, GUI_FONT_COLOR, buff);
#else
		szSize = m_pFontGeneral->GetStringSize(buff);
		...
		// 2014-07-02 by ymjoo DrawText 성능 개선 작업 (캐릭터 정보)
#ifdef C_DRAWTEXT_UPGRADE_YMJOO
		szSize = m_pFontWinningRate->GetStringSize(buff);
		m_pFontWinningRate->DrawText(ARENA_SCORE_PERCENTAGE_X - szSize.cx, ARENA_SCORE_PERCENTAGE_Y, GUI_FONT_COLOR, buff);
#else
		szSize = m_pFontGeneral->GetStringSize(buff);
		...
		// 2014-07-02 by ymjoo DrawText 성능 개선 작업 (캐릭터 정보)
#ifdef C_DRAWTEXT_UPGRADE_YMJOO
		szSize = m_pFontForcedTermination->GetStringSize(buff);
		m_pFontForcedTermination->DrawText(ARENA_COMPULSION_END_X - szSize.cx, ARENA_COMPULSION_END_Y, GUI_FONT_COLOR, buff);
#else
		szSize = m_pFontGeneral->GetStringSize(buff);
		...
		// 2014-07-02 by ymjoo DrawText 성능 개선 작업 (캐릭터 정보)
#ifdef C_DRAWTEXT_UPGRADE_YMJOO
		szSize = m_pFontAccWP->GetStringSize(buff);
		m_pFontAccWP->DrawText(ARENA_COMMULATION_POINT_X - szSize.cx, ARENA_COMMULATION_POINT_Y, GUI_FONT_COLOR, buff);
#else
		szSize = m_pFontGeneral->GetStringSize(buff);
		...
		// 2014-07-02 by ymjoo DrawText 성능 개선 작업 (캐릭터 정보)
#ifdef C_DRAWTEXT_UPGRADE_YMJOO
		szSize = m_pFontCurWP->GetStringSize(buff);
		m_pFontCurWP->DrawText(ARENA_FUSIBLE_POINT_X - szSize.cx, ARENA_FUSIBLE_POINT_Y, GUI_FONT_COLOR, buff);
#else
		szSize = m_pFontGeneral->GetStringSize(buff);
...
//				wsprintf(buf, "%08d", SKILL_BASE_NUM(m_pCharacterSkillDisplayInfo[i][j].pItem->ItemInfo->SourceIndex));
				...
				if(m_pCharacterSkillDisplayInfo[i][j].pItem->ItemInfo->SkillLevel > 0)
				{
					char buf[256];
					{
						...
						// 2014-07-03 by ymjoo DrawText 성능 개선 작업 (캐릭터 스킬정보)
#ifdef C_DRAWTEXT_UPGRADE_YMJOO
						m_pFontItemNum[i][j][0]->DrawText(CHAR_UTC_START_X + CHAR_UTC_INTERVAL_X * i + 33, 
					...
					}
					{
						...
						m_pFontItemNum[i][j][1]->DrawText(CHAR_UTC_START_X + CHAR_UTC_INTERVAL_X * i + 33,
...
void CINFCharacterInfoExtend::RenderPartnerInfo()
{
	...
	// 2014-07-02 by ymjoo DrawText 성능 개선 작업 (캐릭터 정보)
#ifdef C_DRAWTEXT_UPGRADE_YMJOO
	szSizeTmp = m_pPartnerGiveExp->GetStringSize(buff);
#else
	szSizeTmp = m_pFontGeneral->GetStringSize(buff);
#endif
	// END 2014-07-02 by ymjoo DrawText 성능 개선 작업 (캐릭터 정보)
...
void CINFCharacterInfoExtend::GetItemAttackInfo(CItemInfo *pItemInfo, char* o_pDstBuff)
{
	...
	{
		int i; for(i=0; i<SIZE_DES_PARAM_PER_RARE_ITEM_INFO; i++)
		{
...
void CINFCharacterInfoExtend::RenderWeightStats()
{
	...
	{		
		...
		// 2014-07-02 by ymjoo DrawText 성능 개선 작업 (캐릭터 정보)
#ifdef C_DRAWTEXT_UPGRADE_YMJOO
		szSize = m_pFontFuelConsum->GetStringSize(chFuel);
		m_pFontFuelConsum->DrawText(nWindowPosX + WEIGHT_FUEL_START_X - szSize.cx, 
									nWindowPosY + WEIGHT_FUEL_START_Y, 
									GUI_FONT_COLOR, chFuel, 0L);
#else
	...
	{		
		...
		// 2014-07-02 by ymjoo DrawText 성능 개선 작업 (캐릭터 정보)
#ifdef C_DRAWTEXT_UPGRADE_YMJOO
		szSize = m_pFontBoosterTime->GetStringSize(chBooster);
		m_pFontBoosterTime->DrawText(nWindowPosX + WEIGHT_BOOSTER_START_X - szSize.cx, 
									 nWindowPosY + WEIGHT_BOOSTER_START_Y, 
									 GUI_FONT_COLOR, chBooster, 0L);
#else
	...
	{		
		...
		// 2014-07-02 by ymjoo DrawText 성능 개선 작업 (캐릭터 정보)
#ifdef C_DRAWTEXT_UPGRADE_YMJOO
		szSize = m_pFontMoveSpeed->GetStringSize(chSpeed);
		m_pFontMoveSpeed->DrawText(nWindowPosX + WEIGHT_SPEED_START_X - szSize.cx, 
								   nWindowPosY + WEIGHT_SPEED_START_Y, 
								   GUI_FONT_COLOR, chSpeed, 0L);
#else
