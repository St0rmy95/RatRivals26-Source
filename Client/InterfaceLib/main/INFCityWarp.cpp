...
CINFCityWarp::CINFCityWarp(CAtumNode* pParent)
{
	...
	m_pImgBack = NULL;
	m_pImgTitle = NULL;
	int i; for(i=0;i<CITY_WARP_BUTTON_NUMBER;i++)
	{
...
HRESULT CINFCityWarp::InitDeviceObjects()
{
	...
	// 2014-11-21 by ymjoo 워프상점 Restore 하고나면 팅기는 현상 수정 (주석 처리)
	//m_pInfluenceTex->RestoreDeviceObjects();
	// END 2014-11-21 by ymjoo 워프상점 Restore 하고나면 팅기는 현상 수정 (주석 처리)
	return S_OK;
}
...
HRESULT CINFCityWarp::RestoreDeviceObjects()
{
	...
	// 2014-11-21 by ymjoo 워프상점 Restore 하고나면 팅기는 현상 수정
	m_pInfluenceTex->RestoreDeviceObjects();
	// END 2014-11-21 by ymjoo 워프상점 Restore 하고나면 팅기는 현상 수정
	return S_OK;
}
				...
				nCurrentSelectRealIndex < m_vecWarpTargetInfo.size())
			{
				...
				if(pWarpInfo)
				{
					...
					if(pBuilding)
					{
						...
						g_pInterface->m_pCityBase->LeaveBuildingNPC();
						g_pInterface->m_pToolTip->m_bToolTipState = FALSE;
