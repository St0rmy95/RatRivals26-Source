...
//	m_bChatCurrentSelect[CHAT_TAB_TRADE]	= FALSE;
	...
	// 2014-07-01 by ymjoo DrawText 성능 개선 작업 (도움말)
	...
	m_pFontGM = NULL;
#endif
	// END 2014-07-01 by ymjoo DrawText 성능 개선 작업 (도움말)
...
#endif
// end 2015-02-04 by jwLee 일본 캐쉬샵 삭제
...
//	}
	...
	for(i=0; i<CHAT_BUTTON_NUMBER3; i++)
	{
		SAFE_DELETE(m_pNChatFAQ[i]);								// 채팅 FAQ버튼
		SAFE_DELETE(m_pNChatMenuView[i]);							// 채팅 메뉴 보이기 버튼
...
// end 2015-02-04 by jwLee 일본 캐쉬샵 삭제
	for(i=0; i<INPUT_LANGUAGE_END; i++)
	{
		SAFE_DELETE(m_pNChatInputLanguage[i]);						// 채팅 입력 언어
	}
	for(i=0; i<CHAT_MENULIST_NUMBER; i++)
	{
		SAFE_DELETE(m_pNChatMenu[i]);								// 채팅 메뉴 리스트
	}
	SAFE_DELETE(m_pNChatInput);									// 채팅 입력창
	SAFE_DELETE(m_pNChatCheck);									// 채팅 채크마크
	...
	case WM_LBUTTONDOWN:
		{
				...
				&& bShowMenu)
			{
				...
				if(g_pD3dApp->GetTestServerFlag() == FALSE &&
				   ...
				   && g_pD3dApp->GetArenaState() != ARENA_STATE_ARENA_GAMING )	// 2008-02-27 by bhsohn 통합 아레나 수정)
				{
#if defined(C_CASH_SHOP_NOT_USE_JWLEE) // 2015-02-04 by jwLee 일본 캐쉬샵 삭제
#else
					// 메뉴 선택 유료 아이템
...
// 						memcpy(chatOrder,m_strInputMessage+1,n-1);
						...
						int re = stricmp(chatOrder,STRCMD_C_COMMAND_CALL);	//"/통화"
						if(re==0)
						{
...
// 								g_pD3dApp->m_pChat->CreateChatChild(STRMSG_C_080225_0203,COLOR_ERROR);//"아레나에서는 이용할 수 없습니다."
							...
							{
								g_pD3dApp->m_pChat->CreateChatChild(STRMSG_C_VOIP_0008, COLOR_VOIP);//"교신을 할 수 없는 상태입니다."
							}
							else
							{
								g_pD3dApp->m_pChat->CreateChatChild(STRMSG_C_VOIP_0016, COLOR_VOIP);//"통화중입니다."
							}
							bSendChat = TRUE;
						}
						if(!bSendChat)
						{
							re = stricmp(chatOrder,STRCMD_C_COMMAND_PARTYCALL);//"/편대통화"
							if(re==0)
							{
								// 2008-02-27 by bhsohn 통합 아레나 수정
								if(g_pD3dApp->GetArenaState() == ARENA_STATE_ARENA_GAMING )
...
// 									g_pD3dApp->m_pChat->CreateChatChild(STRMSG_C_080225_0203,COLOR_ERROR);//"아레나에서는 이용할 수 없습니다."
									...
									g_pGameMain->m_pCommunity->GetGuild()->GetNtoNCallState() == FALSE )
								{
									...
									// 편대통화 인원수 1(50이하 숫자는 1명), 전체 선택(안쓰거나 50명이상은 전체(50))
									int nConnectCount = 0;
									...
									g_pGameMain->m_pInfWindow->AddMsgBox(STRMSG_C_VOIP_0014, _Q_PARTYMASTER_VOIP, 0,0, DELETE_MASSAGEBOX_TIME);//"편대간 통화를 하시겠습니까?"
									g_pD3dApp->m_VOIPState.bCaller = TRUE;
									g_pD3dApp->m_VOIPState.nVOIPState = _VOIP_TRY_REGISTER;
//									g_pIMSocket->WriteMessageType(T_IC_VOIP_NtoN_CREATEROOM_REQUEST);
								...
								{
									g_pD3dApp->m_pChat->CreateChatChild(STRMSG_C_VOIP_0009, COLOR_VOIP);//"이미 통화중입니다."
								}
								bSendChat = TRUE;
							}
						}
						if(!bSendChat)
						{
							re = stricmp(chatOrder,STRCMD_C_COMMAND_PARTYCALLEND);//"/편대통화종료"
							if(re==0)
							{
								// 2008-02-27 by bhsohn 통합 아레나 수정
								if(g_pD3dApp->GetArenaState() == ARENA_STATE_ARENA_GAMING )
...
//									DBGOUT("편대통화를 종료합니다\n");
									g_pD3dApp->m_pChat->CreateChatChild(STRMSG_C_VOIP_0017, COLOR_VOIP);//"편대통화를 종료합니다"
								}
								bSendChat = TRUE;
							}
						}
						if(!bSendChat)
						{
							re = stricmp(chatOrder,STRCMD_C_COMMAND_GUILDCALL);//"/여단통화"
							if(re==0)
							{
								// 2008-02-27 by bhsohn 통합 아레나 수정
								if(g_pD3dApp->GetArenaState() == ARENA_STATE_ARENA_GAMING )
...
// 									g_pD3dApp->m_pChat->CreateChatChild(STRMSG_C_080225_0203,COLOR_ERROR);//"아레나에서는 이용할 수 없습니다."
								...
								if( g_pD3dApp->m_VOIPState.nVOIPState == _VOIP_NOTLOGIN &&
									...
									g_pGameMain->m_pCommunity->GetGuild()->GetNtoNCallState() == FALSE )
								{
									g_pGameMain->m_pInfWindow->AddMsgBox(STRMSG_C_VOIP_0007, _Q_GUILDMASTER_VOIP,0,0,DELETE_MASSAGEBOX_TIME);//"여단 통화를 하시겠습니까?"
									g_pD3dApp->m_VOIPState.bCaller = TRUE;
									g_pD3dApp->m_VOIPState.nVOIPState = _VOIP_TRY_REGISTER;
//									g_pIMSocket->WriteMessageType(T_IC_VOIP_NtoN_CREATEROOM_REQUEST);
								}
						...
						if(!bSendChat)
						{
							re = stricmp(chatOrder,STRCMD_C_COMMAND_GUILDCALLEND);//"/여단통화종료"
							if(re==0)
							{
								// 2008-02-27 by bhsohn 통합 아레나 수정
								if(g_pD3dApp->GetArenaState() == ARENA_STATE_ARENA_GAMING )
...
//									DBGOUT("여단통화를 종료합니다\n");
									g_pD3dApp->m_pChat->CreateChatChild(STRMSG_C_VOIP_0006, COLOR_VOIP);//"여단통화를 종료합니다"
						...
						if(!bSendChat)
						{
							if( stricmp(chatOrder,STRCMD_C_COMMAND_CALLEND) == 0 &&	//"/통화종료"
								g_pD3dApp->m_VOIPState.nVOIPState != _VOIP_NOTLOGIN &&
								g_pShuttleChild->m_pClientParty->GetNtoNCallState() == FALSE &&
								g_pGameMain->m_pCommunity->GetGuild()->GetNtoNCallState() == FALSE)
							{
...
//#endif
						...
						// 2014-12-19 by jwLee "/데미지출력" 명령어 추가
						if(!bSendChat)
						{
							if(!stricmp(chatOrder,"/데미지출력")) //"/데미지출력"
								if(COMPARE_RACE(g_pD3dApp->m_pShuttleChild->m_myShuttleInfo.Race,RACE_OPERATION|RACE_GAMEMASTER))
								{
									bSendChat = TRUE;
									...
									{
									case TRUE:
										AddBuffer(CHAT_TAB_SYSTEM, "데미지 출력이 활성화 되었습니다.", COLOR_MONSTER_ID04, NULL, NULL);
										break;
									case FALSE:
										AddBuffer(CHAT_TAB_SYSTEM, "데미지 출력이 비활성화 되었습니다.", COLOR_MONSTER_ID04, NULL, NULL);
										break;
									}
								}
						}
						// end 2014-12-19 by jwLee "/데미지출력" 명령어 추가
						...
						if(!bSendChat)
						{
							...
							else if( NULL == stricmp( chatOrder, "/서버정보전송" ) )
							{
								char cBuf[SIZE_MAX_PACKET] = "예시덤프";
...
//									stricmp( chatOrder, STRCMD_CS_COMMAND_USERSINFOTOTAL ) == 0 ||		//"/서버"
...
//									stricmp( chatOrder, STRCMD_CS_COMMAND_CHATFORBIDRELEASE ) == 0 ||	//"/귓말"
//									stricmp( chatOrder, STRCMD_CS_COMMAND_CHATPTOPFLAG_1 ) == 0 ||		//"/whisperChat"
...
//									stricmp( chatOrder, STRCMD_CS_COMMAND_PREMEUM ) == 0 ||				//"/프리미엄"
//									stricmp( chatOrder, STRCMD_CS_COMMAND_PREMEUM_1 ) == 0 ||			//"/premeum"
//									stricmp( chatOrder, STRCMD_CS_COMMAND_CITYWAR ) == 0 ||				//"/도시점령전"
//									stricmp( chatOrder, STRCMD_CS_COMMAND_CITYWAR_1 ) == 0 ||			//"/citywar"
//									stricmp( chatOrder, STRCMD_CS_COMMAND_HAPPYHOUREVENT ) == 0 ||		//"/해피아워이벤트"
...
// 2005-10-25 by ispark									stricmp( chatOrder, STRCMD_CS_COMMAND_ITEMINSERTALL ) == 0 ||		//"/모든아템" 
									...
									stricmp( chatOrder, STRCMD_CS_COMMAND_GOUSER ) == 0 ||				//"/가기"
									stricmp( chatOrder, STRCMD_CS_COMMAND_GOUSER_1 ) == 0 ||			//"/go"
// 2005-10-25 by ispark									stricmp( chatOrder, STRCMD_CS_COMMAND_COMEON ) == 0 ||				//"/오기"
									stricmp( chatOrder, STRCMD_CS_COMMAND_CHATPTOPFLAG ) == 0 ||		//"/귓말"
									stricmp( chatOrder, STRCMD_CS_COMMAND_CHATPTOPFLAG_1 ) == 0 ||		//"/whisperChat"
									stricmp( chatOrder, STRCMD_CS_COMMAND_MENT_0 ) == 0 ||				//"/멘트"
									stricmp( chatOrder, STRCMD_CS_COMMAND_MENT_1 ) == 0 ||				//"/맨트"
							...
							{
								// 일반 계정
								if( stricmp( chatOrder, STRCMD_CS_COMMAND_WHOAREYOU ) == 0 ||			//"/누구"
									stricmp( chatOrder, STRCMD_CS_COMMAND_WHOAREYOU_1 ) == 0 ||			//"/whoareYou"
									stricmp( chatOrder, STRCMD_CS_COMMAND_MENT_0 ) == 0 ||				//"/멘트"
									stricmp( chatOrder, STRCMD_CS_COMMAND_MENT_1 ) == 0 ||				//"/맨트" 
									stricmp( chatOrder, STRCMD_CS_COMMAND_MENT_2 ) == 0 ||				//"/ment" 
									...
									stricmp( chatOrder, STRCMD_CS_COMMAND_SERVERINFO ) == 0)					// 2011-11-01 by jhahn 암호화 처리
...
// 						
// 						//memset(m_strInputMessage,0x00,sizeof(m_strInputMessage));
// 						// end 2009. 07. 15 by ckPark 채팅박스에서 전부 삭제시 한글자 남는 현상 수정
// 						SetChatMsgBuff(g_pD3dApp->m_inputkey.m_full_str);
// 						strncpy(m_strInputMessage, m_strBkInputMessage, SIZE_MAX_CHAT_MESSAGE);
// 						// end 2009. 07. 30 by jsKim 채팅창 버그 수정 및 기능 보안
// 					}
...
//	else if(g_pD3dApp->m_VOIPState.nCallState == _CALLSTATE_PARTY)	// 편대 통화종료
//	{
//		if( g_pD3dApp->m_VOIPState.nVOIPState != _VOIP_NOTLOGIN &&
//			g_pShuttleChild->m_pClientParty->GetNtoNCallState() == TRUE)
//		{
...
{
	// 2010. 02. 23 by jskim 채팅버그 수정 및 최적화
// 	if((m_strInputMessage[0] != '~' &&			// 공지 채팅
// 		m_strInputMessage[0] != '%' &&			// 아레나 채팅
// 		m_strInputMessage[0] != '@' &&			// 길드 채팅
// 		m_strInputMessage[0] != '#' &&			// 편대 채팅
// 		m_strInputMessage[0] != '^' &&			// 전쟁 채팅
// 		m_strInputMessage[0] != '!' &&			// 맵전체 채팅
// 		m_strInputMessage[0] != '$' &&			// 매매 채팅
// 		// 2008-11-19 by bhsohn 지도자 채팅 저장 안되는 문제
// 		m_strInputMessage[0] != '*' &&			// 지도자 공지	
// 		// end 2008-11-19 by bhsohn 지도자 채팅 저장 안되는 문제
// 		m_strInputMessage[0] != '&') ||			// 유료 맵전체 채팅
// 		//wParam == '\b' )						// 벡스페이스		
// 	{
// 		return FALSE;
// 	}
...
// 		g_pD3dApp->m_pChat->CreateChatChild(STRMSG_C_080225_0203,COLOR_ERROR);//"아레나에서는 이용할 수 없습니다."
	...
	{
		// 2008-10-20 by bhsohn VOIP 버그 수정
		// 내가 편대원이라면 편대장이 음성을 안키면 음성 통신 불가능
		// 편대원은 시작을 못시킴
	...
	{
		g_pGameMain->m_pInfWindow->AddMsgBox(STRMSG_C_VOIP_0003, _Q_JOIN_PARTY_VOIP);	//"편대간 통화에 참여하시겠습니까?"
	}
	else
	{
		if( g_pD3dApp->m_VOIPState.nVOIPState != _VOIP_NOTLOGIN &&
...
{
	if(g_pGameMain->m_pCommunity->GetGuild()->GetNtoNCallState())//여단통화중인경우
	{
		// 2008-10-20 by bhsohn VOIP 버그 수정
		g_pGameMain->m_pCommunity->GetGuild()->ReleaseVOIPGuildInfo();	// IM서버에서 오는 정보는 초기화 안함
...
{
	if(g_pShuttleChild->m_pClientParty->GetNtoNCallState())//편대통화중인경우
	{
		// 2008-10-20 by bhsohn VOIP 버그 수정
		//g_pShuttleChild->m_pClientParty->InitMyShuttlemVoipType();
		//g_pGameMain->ReleaseVOIPMode();	// 마이크 강제로 종료
		g_pShuttleChild->m_pClientParty->ReleaseVOIPAllUserInfo();	// IM서버에서 오는 정보는 초기화 안함
		// end 2008-10-20 by bhsohn VOIP 버그 수정
...
// 2012-11-21 by mspark, 관리자 복사, 붙여넣기 허용
...
//			}
			...
			return INF_MSGPROC_BREAK;
#endif
// end 2012-11-21 by mspark, 관리자 복사, 붙여넣기 허용
		}
		break;
#endif
