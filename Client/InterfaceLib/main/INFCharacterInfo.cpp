...
#ifdef KOR_CHARACTER_INTERFACE_POS_HSSON
...
//			m_pFontAmmoWeapon[4]->DrawText( POS_RIGHT_WINDOW_X+AMMO_DEFENSE_START_X, nWindowPosY + AMMO_START_Y+AMMO_INTERVAL*4, GUI_FONT_COLOR, buff, 0L );
			...
			sprintf( buff, STRMSG_C_INTERFACE_0010, ((float)CAtumSJ::GetDefenseC( &g_pShuttleChild->m_myShuttleInfo, &g_pShuttleChild->m_paramFactor, TRUE ) / PROB100_MAX_VALUE)*100.0f);//"방어력 기본무기 : %.2f%%"
			m_CharacInfoFont[0]->DrawText(POS_RIGHT_WINDOW_X+INFO_START_X, nWindowPosY+OTHER_INFO_START_Y,GUI_FONT_COLOR, buff, 0 );
			...
			// 2010-07-28 by dgwoo 확률 수식 변경 (255 => 100%)
			sprintf( buff, STRMSG_C_INTERFACE_0011, ((float)CAtumSJ::GetDefenseC( &g_pShuttleChild->m_myShuttleInfo, &g_pShuttleChild->m_paramFactor, FALSE ) / PROB100_MAX_VALUE)*100.0f);//"       고급무기 : %.2f%%"
...
// 						 (CAtumSJ::GetAttackProbabilityC(g_pShuttleChild->m_pPrimaryWeapon->GetRealItemInfo(), &g_pShuttleChild->m_paramFactor, TRUE, g_pShuttleChild->m_myShuttleInfo.TotalGearStat.AttackPart )*100)/255);	// 캐릭터의 공격 확률
				...
				sprintf( buff, STRMSG_C_INTERFACE_0013,//"공격확율 : %.2f"
						// 2010-07-28 by dgwoo 확률 수식 변경 (255 => 100%)
						 ...
						 (CAtumSJ::GetAttackProbabilityC(g_pShuttleChild->m_pPrimaryWeapon->GetRealItemInfo(), &g_pShuttleChild->m_paramFactor, TRUE, g_pShuttleChild->m_myShuttleInfo.TotalGearStat.AttackPart, g_pShuttleChild->m_myShuttleInfo.Level )*100)/PROB100_MAX_VALUE);	// 캐릭터의 공격 확률
...
// 				&g_pShuttleChild->m_paramFactor, FALSE, g_pShuttleChild->m_myShuttleInfo.TotalGearStat.AttackPart)*100)/255 );	// 캐릭터의 공격 확률
				...
				sprintf( buff, STRMSG_C_INTERFACE_0013,//"공격확율 : %.2f"
						...
						// (CAtumSJ::GetAttackProbabilityC(g_pShuttleChild->m_pSecondaryWeapon->GetRealItemInfo(),
						 ...
						 (CAtumSJ::GetAttackProbabilityC(g_pShuttleChild->m_pSecondaryWeapon->GetRealItemInfo(),
						 &g_pShuttleChild->m_paramFactor, FALSE, g_pShuttleChild->m_myShuttleInfo.TotalGearStat.AttackPart, g_pShuttleChild->m_myShuttleInfo.Level)*100)/PROB100_MAX_VALUE);	// 캐릭터의 공격 확률
