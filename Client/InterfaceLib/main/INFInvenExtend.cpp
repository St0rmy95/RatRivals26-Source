...
#include "QuestData.h"	// 2013-08-27 by bhsohn 미션 프리 패스 시스템
...
#define CHANGE_WEAR_ITEM_COOLTIME	20.0f		// 2014-04-16 by ymjoo 장비 교체시 쿨타임 추가
...
//	m_nSelectItemWindowPosition = -1;
	...
	m_bTradeItemCenterState = FALSE;						// 2013-11-29 by ssjung 거래소 구현

	m_nUseCapsuleCashItemUniqueNum = 0;						// 2014-02-07 by ymjoo 캡슐형 캐시 아이템 경고 메세지

	m_mapChangeWearItemCoolTime.clear();	// 2014-04-16 by ymjoo 장비 교체시 쿨타임 추가
}
...
HRESULT CINFInvenExtend::InitDeviceObjects()
{
	...
	// 2014-06-30 by ymjoo DrawText 성능 개선 작업 (인벤토리)
#ifdef C_DRAWTEXT_UPGRADE_YMJOO
	m_pFontWP = new CD3DHanFont(_T(g_pD3dApp->GetFontStyle()), 9, D3DFONT_ZENABLE, FALSE, 256, 32);
	m_pFontWP->InitDeviceObjects(g_pD3dDev);
#endif
	// END 2014-06-30 by ymjoo DrawText 성능 개선 작업 (인벤토리)
...
HRESULT CINFInvenExtend::RestoreDeviceObjects()
{
	...
	// 2014-06-30 by ymjoo DrawText 성능 개선 작업 (인벤토리)
#ifdef C_DRAWTEXT_UPGRADE_YMJOO
	m_pFontWP->RestoreDeviceObjects();		
#endif
	// END 2014-06-30 by ymjoo DrawText 성능 개선 작업 (인벤토리)
...
HRESULT CINFInvenExtend::InvalidateDeviceObjects()
{
	...
	// 2014-06-30 by ymjoo DrawText 성능 개선 작업 (인벤토리)
#ifdef C_DRAWTEXT_UPGRADE_YMJOO
	m_pFontWP->InvalidateDeviceObjects();		
#endif
	// END 2014-06-30 by ymjoo DrawText 성능 개선 작업 (인벤토리)
...
void CINFInvenExtend::RenderWp(int x, int y)
{
	char temp1[64];
	char temp2[64];
...
void CINFInvenExtend::InitWearIconInfo()
{
	...
	// 2010-06-15 by shcho&hslee 펫시스템 - 장착품 최대 개수 변경.
	//int i; for(i=0;i<WEAR_ITEM_NUMBER;i++)
...
// 2013-10-01 by ssjung 1주년 이벤트
			...
			case DES_WEBPAGE_OPEN : 
				{
					// 2014-10-31 by ymjoo 상점 열려있을 때 n주년 카드 열리지 않게 수정
					if(_SHOP == g_pD3dApp->m_dwGameState)
					{
						break;
					}
					// END 2014-10-31 by ymjoo 상점 열려있을 때 n주년 카드 열리지 않게 수정
					bSend = TRUE;
					g_pGameMain->FirstAnniversaryEventItemUse(TRUE);
		...
		{	
			ITEM* teml = g_pGameMain->m_pInfSkill->FindItemIcon(pItem->ItemNum);
 			...
 			if (teml)
 			{
 				...
 				sMsg.ClientIndex = g_pD3dApp->m_pShuttleChild->m_myShuttleInfo.ClientIndex;
				// 2014-11-26 by jwLee 퀵슬롯 파트너 소켓 아이템 사용시 현재 장착중인 파트너의 아이템으로 사용하도록 변경
				//sMsg.UniqueNumber = pItem->UniqueNumber;
				...
				if (pInvenDisplayInfo)
				{
					...
					sMsg.ItemUniqueNumber = NULL;
					int i;
					for(i = 0 ; i < SIZE_MAX_PETSOCKET ; ++i)
					{
						if(pItem->UniqueNumber == pPetCurrentInfo->PetSocketItemUID[i])
						{
							sMsg.ItemUniqueNumber = pPetCurrentInfo->PetSocketItemUID[i];
							break;
						}
					}
					if (!sMsg.ItemUniqueNumber)
					{
						
						for(i = 0 ; i < SIZE_MAX_PETSOCKET ; ++i)
						{
							const CItemInfo* pSocketItem = g_pStoreData->FindItemInInventoryByUid(pPetCurrentInfo->PetSocketItemUID[i]);
							if(!pSocketItem) continue;
							if(pItem->ItemNum == pSocketItem->ItemNum)
							{
								sMsg.ItemUniqueNumber = pSocketItem->UniqueNumber;
								break;
							}
						}
					}
				}
				// end 2014-11-26 by jwLee 퀵슬롯 파트너 소켓 아이템 사용시 현재 장착중인 파트너의 아이템으로 사용하도록 변경
...
//#endif
			...
			// 2013-08-22 by bhsohn 마을에서 서치아이 사용 못하게 처리
			if(!IsPossibleUseCardItem(pIteminfo->ArrDestParameter[0]))
			{
				return;
			}
			// END 2013-08-22 by bhsohn 마을에서 서치아이 사용 못하게 처리
			...
			case DES_QUEST_MISSION_FREE_PASS:
				{
					DoModalMissionFreePass(pItem);
				}
				break;
			...
			case DES_MOVEMENT_RESTRICTION_GIVE:
				{
					g_pGameMain->m_pInfWindow->AddMsgBox( STRMSG_C_130902_0002, _Q_USEITEM_LEADER_BAN_WARP,0,0,0, pItem->UniqueNumber);
				}
				break;
			...
			case DES_FIELD_STORE:
				{
					...
					// 2014-07-04 by ymjoo 필드창고 아이템 사용시 세력 변경 되는 버그 수정
					...
					// END 2014-07-04 by ymjoo 필드창고 아이템 사용시 세력 변경 되는 버그 수정
				}
				break;		// 2014-07-04 by ymjoo 필드창고 아이템 사용시 세력 변경 되는 버그 수정
			// end 2010. 10. 15 by jskim 시즈모드시 필드창고 아이템 사용 되는 버그 수정

			// 2014-06-10 by ymjoo 세력 변경 아이템
			case DES_CASH_CHANGE_INFLUENCE:
				{
					if(!IS_CITY_MAP_INDEX(g_pD3dApp->GetMyShuttleMapChannelIndex().MapIndex))
					{
						g_pD3dApp->m_pChat->CreateChatChild(STRMSG_C_140610_0005, COLOR_SYSTEM);	// "도시 외부에서는 사용이 불가능합니다."
					}
					else
					{
						SetUseByMessageItemUniqueNum(pItem->UniqueNumber);
						g_pGameMain->m_pInfWindow->AddMsgBox(STRMSG_C_140610_0001, _Q_CHANGE_INFLUENCE_MESSAGE, 0, 0, 0, 0, NULL, TRUE);	// "세력이 변경 됩니다. 정말 사용하시겠습니까?"
					}
				}
				break;
...
//  2006-08-10 by dgwoo 판도라의 상자 이벤트
	case ITEMKIND_RANDOMBOX:
		{
			// 2014-09-18 by ymjoo 인피니티에서 랜덤박스 개봉불가
			if(g_pInterface->IsArenaStart() || ARENA_STATE_ARENA_GAMING == g_pD3dApp->GetArenaState())
			{
				g_pD3dApp->m_pChat->CreateChatChild(STRMSG_C_100618_0406, COLOR_ERROR);	// "해당 아이템은 인피니티 필드내에서 사용이 불가능 합니다."
				break;
			}
			// END 2014-09-18 by ymjoo 인피니티에서 랜덤박스 개봉불가
...
//	if(m_pSelectItem && m_pSelectItem->ItemWindowIndex != nWindowPosition)
	if(pSelectItem && pSelectItem->ItemWindowIndex != nWindowPosition)
	{
		...
		{
			// 2014-04-16 by ymjoo 장비 교체시 쿨타임 추가
#if defined C_CHANGE_WEAR_ITEM_COOLTIME_YMJOO
			DWORD fTime = timeGetTime();
			if(FALSE == g_pD3dApp->m_bCharacter && FALSE == COMPARE_RACE(g_pShuttleChild->GetMyShuttleInfo().Race,RACE_OPERATION|RACE_GAMEMASTER))
			{
				if(m_mapChangeWearItemCoolTime.find(nWindowPosition) == m_mapChangeWearItemCoolTime.end())
				{
					m_mapChangeWearItemCoolTime[nWindowPosition] = fTime;
				}
				else if(fTime - m_mapChangeWearItemCoolTime[nWindowPosition] < CHANGE_WEAR_ITEM_COOLTIME * 1000.0f)
				{
					float remainCoolTime = (CHANGE_WEAR_ITEM_COOLTIME * 1000.0f - (fTime - m_mapChangeWearItemCoolTime[nWindowPosition])) / 1000.0f;
					char buf[128];
					sprintf(buf, STRMSG_C_051207_0002, remainCoolTime);	// "재사용 시간 : %.2fsec"
					g_pD3dApp->m_pChat->CreateChatChild(buf, COLOR_ERROR);
					return;
				}
				else
...
#endif
			// END 2014-04-16 by ymjoo 장비 교체시 쿨타임 추가
...
#ifndef C_COLLECTION_ARMOR_NO_STAT				// 2013-10-28 by ssjung 캐나다에서 아머 교체가 되지 않는 문제 수정
...
// 2014-06-30 by ymjoo DrawText 성능 개선 작업 (인벤토리)
CD3DHanFont* CINFInvenExtend::GetFontWP()
{
#ifdef C_DRAWTEXT_UPGRADE_YMJOO
	return m_pFontWP;
#else
...
/// \fn			
/// \brief		
/// \author		// 2013-08-22 by bhsohn 마을에서 서치아이 사용 못하게 처리
...
/// \return		
///////////////////////////////////////////////////////////////////////////////
BOOL CINFInvenExtend::IsPossibleUseCardItem(DestParam_t	desParam)
{
	switch(desParam)
	{
	case DES_SKILL_SCANNING:
		{
			if(g_pD3dApp && IS_CITY_MAP_INDEX(g_pD3dApp->GetMyShuttleMapChannelIndex().MapIndex))
			{
				return FALSE;
			}
		}
...
/// \fn			
/// \brief		
/// \author		// 2013-08-27 by bhsohn 미션 프리 패스 시스템
/// \date		2013-08-27 ~ 2013-08-27
/// \warning	
///
/// \param		
/// \return		
///////////////////////////////////////////////////////////////////////////////
void CINFInvenExtend::DoModalMissionFreePass(ITEM_BASE* pItem)
{	
	if(!pItem)
	{
		return;
	...
	{
		char buf[256] = {0,};
		wsprintf(buf,STRMSG_C_130827_0004);
		g_pD3dApp->m_pChat->CreateChatChild(buf,COLOR_ERROR);

		return; // 현재 퀘스트를 진행중이 아니다.
	}
	

	if((pItem->ItemInfo->ReqMinLevel <= pProgressMission->ReqLevel.Min)
		&&(pProgressMission->ReqLevel.Min <= pItem->ItemInfo->ReqMaxLevel))
	{		
	}
	else
	{
		// "현재 진행중인 미션의 레벨이 미션 프리패스의 레벨 범위 밖일 경우"
		char buf[256] = {0,};
		wsprintf(buf,STRMSG_C_130827_0002);
		g_pD3dApp->m_pChat->CreateChatChild(buf,COLOR_ERROR);
	...
	char *pItemName = pItem->ItemInfo->ItemName;
	UID64_t uUniqueNumber = pItem->UniqueNumber;



	char chMsg[256] = {0,};							
	wsprintf(chMsg, STRMSG_C_130827_0001, pItemName);
	if(g_pGameMain && g_pGameMain->m_pInfWindow)
	{
		g_pGameMain->m_pInfWindow->AddMsgBox( chMsg, 
												_Q_MISSON_FREEPASS_ITEM,
												0,
												0,
												0, 
												uUniqueNumber);	
...
void CINFInvenExtend::UseSendMissionFreePassItem(UID64_t UniqueNumber)
{
	if( m_bShopConcurrent + CINFInvenExtend::SHOP_CONCURRENT_DELAY < timeGetTime() )
	{
		m_bShopConcurrent = timeGetTime();
		...
		char buffer[SIZE_MAX_PACKET];
		sMsg.ClientIndex = g_pD3dApp->m_pShuttleChild->m_myShuttleInfo.ClientIndex;
		sMsg.ItemUniqueNumber = UniqueNumber;
		int nType = T_FC_ITEM_USE_ITEM;
		memcpy(buffer, &nType, SIZE_FIELD_TYPE_HEADER);
		memcpy(buffer+SIZE_FIELD_TYPE_HEADER, &sMsg, sizeof(sMsg));
		g_pD3dApp->m_pFieldWinSocket->Write(buffer, SIZE_FIELD_TYPE_HEADER + sizeof(sMsg));
...
// 2013-11-29 by ssjung 거래소 구현
void CINFInvenExtend::SetTradeItemCenterState(BOOL bState)
{
#ifdef SC_MARKET_JHSEOL_BCKIM_SSJUNG
	m_bTradeItemCenterState = bState;
	m_pINFInvenItem->SetTradeItemCenterState(bState);
	m_pINFInvenEquip->SetTradeItemCenterState(bState);
#endif
} 
// end 2013-11-29 by ssjung 거래소 구현