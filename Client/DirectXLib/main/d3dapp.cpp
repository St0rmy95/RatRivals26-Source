...
#include "ExploreHost.h" // // 2013-05-23 by bhsohn 익스폴로워 모드로 업그레이드

#include "CharacterInfo.h"
#include "MemPoolClient.h"
#include "CustomOptimizer.h"
#include "ShuttleChild.h"	// 2014-04-24 by ymjoo 전체화면에서 최소화시 프레임 진행이 되지 않는 현상 수정
#include "MeshInitThread.h"	// 2014-08-18 by ymjoo 싱글스레드로 변경
...
CD3DApplication::CD3DApplication()
{
	...
	m_pExploreHost = NULL; // 2013-05-23 by bhsohn 익스폴로워 모드로 업그레이드
...
HRESULT CD3DApplication::Initialize3DEnvironment()
{
	...
	// Create the device
	// 2014-08-06 by ymjoo Direct3D Device Multithreaded Option
...
//                                &m_pd3dDevice );
	hr = m_pD3D->CreateDevice(m_dwAdapter, pDeviceInfo->DeviceType,
		...
		&m_pd3dDevice);
	// END 2014-08-06 by ymjoo Direct3D Device Multithreaded Option
...
#ifdef C_GAME_SINGLE_THREAD_YMJOO
			if (g_pD3dApp && g_pD3dApp->m_pMeshInitThread)
			{
				g_pD3dApp->m_pMeshInitThread->Run();
			}
#endif
...
//	{
	...
	// 2014-04-24 by ymjoo 전체화면에서 최소화시 프레임 진행이 되지 않는 현상 수정
#if defined C_FRAME_MOVE_LAST_ELAPSED_TIME_YMJOO
	if (g_pShuttleChild)
	{
		if (0.0f != g_pShuttleChild->m_timeProcess.m_fFrameFlowTime && m_bDeviceLost)
		{
			fElapsedAppTime = g_pShuttleChild->m_timeProcess.m_fFrameFlowTime;
			g_pShuttleChild->m_timeProcess.m_fFrameFlowTime = 0.0f;
		}
		else if ((0.0f == fElapsedAppTime) && m_bFrameMoving)
			return S_OK;
	}
	else if ((0.0f == fElapsedAppTime) && m_bFrameMoving)
		return S_OK;
	...
	if ((0.0f == fElapsedAppTime) && m_bFrameMoving)
		return S_OK;
#endif
	// END 2014-04-24 by ymjoo 전체화면에서 최소화시 프레임 진행이 되지 않는 현상 수정
...
//void CD3DApplication::CreateWebWindow(char* strWebSite,char* strWebParameter) // 2012-11-28 by jhjang 게임포지 웹 상점 통합 추가 작업
void CD3DApplication::CreateWebWindow(char* strWebSite, char* strWebParameter, int nWebPosX, int nWebPosY, int nWebWidth, int nWebHeight)
{
	// 2009-02-24 by bhsohn 캐쉬샵 충전 웹페이지 추가
// 	LPSTR lpszClass = "WebWindow";
...
// 	m_pHost = new Host(m_hWebWnd,strWebAddress,strParameter,strHeader);
	...
	// 2013-03-22 by bhsohn [일본] 웹페이지 캐쉬상점 안뜨는 문제 해결
	// 2012-11-28 by jhjang 게임포지 웹 상점 통합 추가 작업
	if (nWebPosX == -1)
	{
		nWebPosX = WEB_START_X;
	}
	if (nWebPosY == -1)
	{
		nWebPosY = WEB_START_Y;
	}
	if (nWebWidth == -1)
	{
		nWebWidth = WEB_WIDTH;
	...
	if (NULL == m_hWebWnd)
	{
		...
		m_hWebWnd = CreateWindow(lpszClass, lpszClass,
			...
			//WEB_WIDTH,WEB_HEIGHT,
			nWebPosX, nWebPosY,
			//			nWebWidth,nWebHeight,
			nWebWidth + 30, nWebHeight + 30,										// 2013-07-18 by ssjung 신규 웹 캐쉬 상점 적용
			// end 2012-11-28 by jhjang 게임포지 웹 상점 통합 추가 작업
			m_hWnd, (HMENU)0,
			m_hInstance, NULL);
	}
	...
	if (NULL == m_pHost)
	{
		...
		// 2012-11-23 by jhjang 게임포지 웹 상점 통합 작업
		if (NULL == strWebSite || NULL == strWebParameter)
		{
			char strParameter[MAX_PATH];
			wsprintf(strParameter, "userid=%s", g_pD3dApp->m_strUserID);
			char strWebAddress[512] = { 0, };
		...
		{
			m_pHost = new Host(m_hWebWnd, strWebSite, strWebParameter, strHeader);
		}
...
void CD3DApplication::DestroyWebWindow()
{
	...
	// 2014-10-31 by ymjoo 웹 상점 닫을 때 클라이언트 종료되는 현상 수정
// 	if(m_pHost) 
// 	{
// 		m_pHost->Release();
// 		SAFE_DELETE(m_pHost);
// 	}
	...
	// END 2014-10-31 by ymjoo 웹 상점 닫을 때 클라이언트 종료되는 현상 수정
	// end 2009-02-24 by bhsohn 캐쉬샵 충전 웹페이지 추가
}


// 2012-11-28 by jhjang 게임포지 웹 상점 통합 추가 작업
//void CD3DApplication::ShowWebWindow(int nWebPosX,int nWebPosY)
void CD3DApplication::ShowWebWindow(int nWebPosX, int nWebPosY, int nWebWidth, int nWebHeight)		// 2013-07-18 by ssjung 신규 웹 캐쉬 상점 적용
{
	// 2013-03-22 by bhsohn [일본] 웹페이지 캐쉬상점 안뜨는 문제 해결
	// 2012-11-28 by jhjang 게임포지 웹 상점 통합 추가 작업
	if (nWebPosX == -1)
	...
	if (m_hWebWnd)
	{
		...
		// 가운데 정렬
		// 2012-11-28 by jhjang 게임포지 웹 상점 통합 추가 작업
		// 2012-12-17 by mspark, 캐쉬샵 충전 웹페이지 전체화면에서도 위치 벗어나지 않도록 수정
//		if(!g_pD3dApp->IsWindowMode())
//		{
//			rcWnd.left	+= (WEB_START_X - 3);
//			rcWnd.top	+= (WEB_START_Y - 29);		
//		}
//		else
//		// end 2012-12-17 by mspark, 캐쉬샵 충전 웹페이지 전체화면에서도 위치 벗어나지 않도록 수정
//		{
//		rcWnd.left	+= WEB_START_X;
//		rcWnd.top	+= WEB_START_Y;		
//		}
// 2013-05-31 by ssjung 전체화면에서 윂페이지 위치가 벗어나는 현상 수정 
#ifdef C_MASANG_WEB_CASHSHOP
		{
			// 2013-07-18 by ssjung 신규 웹 캐쉬 상점 적용
			//			rcWnd.left	+= nWebPosX; 
			//			rcWnd.top	+= nWebPosY;		
			rcWnd.left += (nWebPosX + 3);
			rcWnd.top += (nWebPosY + 3);
			//end 2013-07-18 by ssjung 신규 웹 캐쉬 상점 적용
		}
		// end 2013-05-31 by ssjung 전체화면에서 윂페이지 위치가 벗어나는 현상 수정 
#else
#ifdef SC_WEB_CASHSHOP_SSJUNG_JHSEOL
		if (!g_pD3dApp->IsWindowMode())
		{
			rcWnd.left += (nWebPosX - 3);
			rcWnd.top += (nWebPosY - 29);
		}
		else
			// end 2012-12-17 by mspark, 캐쉬샵 충전 웹페이지 전체화면에서도 위치 벗어나지 않도록 수정
		{
		...
		// 2013-07-18 by ssjung 신규 웹 캐쉬 상점 적용
#ifdef C_MASANG_WEB_CASHSHOP
		SetWindowPos(m_hWebWnd, NULL, rcWnd.left, rcWnd.top, nWebWidth, nWebHeight, SWP_NOZORDER);
#endif
		// end 2013-07-18 by ssjung 신규 웹 캐쉬 상점 적용
	}
	if (g_pD3dApp)
...
/// \return		
///////////////////////////////////////////////////////////////////////////////
void CD3DApplication::PopupExplore(char* strWebSite, char* strWebParameter, int nWebPosX, int nWebPosY, int nWebWidth, int nWebHeight)
{
	if (nWebPosX == -1)
	{
		nWebPosX = WEB_START_X;
	...
	if (m_pExploreHost)
	{
		m_pExploreHost->ReleaseExplore();
		SAFE_DELETE(m_pExploreHost);
	}
	if (NULL == m_hWebWnd)
	{
		LPSTR lpszClass = "WebWindow";
	...
	m_pExploreHost = new CExploreHost();
	m_pExploreHost->RefreshExplore(m_hWebWnd, strWebSite, nWebPosX, nWebPosY, nWebWidth, nWebHeight, g_pD3dApp->IsWindowMode());

	ShowWebWindow(nWebPosX, nWebPosY);
...
void CD3DApplication::ReleaseExplore()
{
	if (m_pd3dDevice)
	{
		m_pd3dDevice->SetDialogBoxMode(FALSE);
	...
	if (m_pExploreHost)
	{
		m_pExploreHost->ReleaseExplore(m_hWnd);
		SAFE_DELETE(m_pExploreHost);
	}
	if (m_hWebWnd != NULL)
	{
...
void CD3DApplication::ShowWebWindow_FirstAnniversaryEvent(int nWebPosX, int nWebPosY, int nWebWidth, int nWebHeight)
{
	if (nWebPosX == -1)
	{
		nWebPosX = WEB_START_X;
