...
//                                        HANDLE hNotifyEvent )
//{
//	FLOG( "CSoundManager::CreateStreaming( CStreamingSound** ppStreamingSound, LPTSTR strWaveFileName, DWORD dwCreationFlags, GUID guid3DAlgorithm, DWORD dwNotifyCount, DWORD dwNotifySize, HANDLE hNotifyEvent )" );
//    HRESULT hr;
//
//    if( m_pDS == NULL )
//        return CO_E_NOTINITIALIZED;
...
CSound::~CSound()
{
    ...
    SAFE_DELETE_ARRAY(m_pSCDirect3DSound); 
...
{
	FLOG( "CSound::RestoreBuffer( LPDIRECTSOUNDBUFFER pDSB, BOOL* pbWasRestored )" );
    HRESULT hr;
...
//-----------------------------------------------------------------------------
SCDIRECT_3DSOUND * CSound::GetFreeSCDirect3DSound(void)
{
	...
	SCDIRECT_3DSOUND *pRet = &m_pSCDirect3DSound[m_nCurrentIndex];
	m_nCurrentIndex = ++m_nCurrentIndex%m_dwNumBuffers;
	return pRet;
}
...
// Desc: 
//-----------------------------------------------------------------------------
SCDIRECT_3DSOUND * CSound::GetBuffer( DWORD dwIndex )
{
	FLOG( "CSound::GetBuffer( DWORD dwIndex )" );
    if( m_pSCDirect3DSound == NULL
		|| dwIndex >= m_dwNumBuffers)
	{
        return NULL;
	}
    
    return &m_pSCDirect3DSound[dwIndex];
}
...
HRESULT CSound::Play(DWORD dwPriority/* = 0*/, DWORD dwFlags/* = 0*/, LONG lVolume/*=0*/)
{
	...
	SCDIRECT_3DSOUND * pSC3DSound = GetFreeSCDirect3DSound();
...
void CSound::SetAllParameter(void)
{
//	if(m_pDS3DBuffer)
//	{
//		D3DXVECTOR3 tmTar = m_Vec3TargetPos - m_Vec3CurPos;
//		float fDist = D3DXVec3Length(&tmTar);
//		D3DXVec3Normalize(&tmTar, &tmTar);
//		D3DXVECTOR3 tmCurPos = m_Vec3CurPos + tmTar * fDist/100;
//		if(D3DXVec3Length(&(m_Vec3CurPos - tmCurPos)) > fDist)
//		{
//			tmCurPos = m_Vec3TargetPos;
//		}
//		//m_dsBufferParams.vPosition = tmCurPos;		
//		m_dsBufferParams.vPosition.x = 500.0f;
//		m_dsBufferParams.vPosition.y = 0.0f;
//		m_dsBufferParams.vPosition.z = 0.0f;//= tmCurPos;		
//		m_pDS3DBuffer->SetPosition(m_dsBufferParams.vPosition.x, m_dsBufferParams.vPosition.y, m_dsBufferParams.vPosition.z, DS3D_IMMEDIATE);
//	}
}
...
HRESULT CWaveFile::Open( LPTSTR strFileName, 
						...
						DWORD dwFlags )
{
	FLOG( "CWaveFile::Open( LPTSTR strFileName, WAVEFORMATEX* pwfx, DWORD dwFlags )" );
    HRESULT hr;
