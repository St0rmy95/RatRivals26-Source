...
BOOL CWinSocket::Connect(LPCSTR strPeerIP, int nPort)
{
	...
	m_byHostSequenceNumber = (BYTE)(GetTickCount() % SEQNO_VAR_C);
	m_bPeerSequenceNumberInitFlag = FALSE;

	if (m_byHostSequenceNumber == 116)
		m_byHostSequenceNumber++;

	hostent* sHostEnt = gethostbyname(strPeerIP);
	DWORD dwError;
	int i = 0;
	m_SockAddrRemote.sin_port = htons(nPort);

	if (sHostEnt == NULL) {
		DBGOUT("[CWinSocket] Connecting to %s:%d \r\n", strPeerIP, nPort);
		dwError = WSAGetLastError();
		if (dwError != 0) {
			if (dwError == WSAHOST_NOT_FOUND) {
				DBGOUT("Host not found\n");
				return 1;
			}
			else if (dwError == WSANO_DATA) {
				DBGOUT("No data record found\n");
				return 1;
			}
			else {
				DBGOUT("Function failed with error: %ld\n", dwError);
				return 1;
			}
		}
	}
	...
	{
		DBGOUT("[CWinSocket] Connecting by domain : %s", strPeerIP);
		while (sHostEnt->h_addr_list[i] != 0) {
			m_SockAddrRemote.sin_addr.s_addr = *(u_long*)sHostEnt->h_addr_list[i++];
		}
...
#if defined(SERVICE_TYPE_RUSSIAN_SERVER_1)                             // 2009-06-01 by cmkwon, 러시아 프로스트 시스템 수정(무시 패킷) - 러시아만 이렇게 처리
