...
DECLARE @AccTotal INT
SET @AccTotal = (SELECT TOP 1 (AccountUniqueNumber) FROM td_Account ORDER BY AccountUniqueNumber DESC)
SELECT @AccTotal - (@AccTotal - COUNT(*))/datediff(dd, min(registereddate), getdate()) AS '현 잠정 회원수'
	FROM td_Account
--
-------------------------------------------------------------------------------
...
--[윤탁수]
--캐릭터별
select  level, unitkind,possess as character_uniquenumber,  avg( cast(case when prefixcodenum <> 0 then 1 else 0 end +case when suffixcodenum <> 0 then 1 else 0 end as float ))/2*100 as rareitem퍼센트  from td_store, td_character
...
--[윤탁수]
--계정별 레어아이템 평균 개수
select td_store.accountuniquenumber,  avg( cast(case when prefixcodenum <> 0 then 1 else 0 end +case when suffixcodenum <> 0 then 1 else 0 end as float ))/2*100 as rareitem퍼센트 from td_store, td_character
