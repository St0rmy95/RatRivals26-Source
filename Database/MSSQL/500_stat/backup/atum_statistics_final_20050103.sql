...
select top 50 c.accountname, s.*
from td_store s, td_character c
where s.itemnum=7000022 and s.accountuniquenumber = c.accountuniquenumber and c.race < 128
...
select * from atum_log_total_user
where MgameServerID=0
order by time desc
