...
as
	...
	where c.accountuniquenumber = c2.accountuniquenumber and c.experience = c2.maxexp
			...
			and a.accountuniquenumber = c.accountuniquenumber and a.LastLoginDate > '2004-11-18 14:00'
GO
...
select * from td_character where accountuniquenumber in (
	select accountuniquenumber from td_account a where a.lastlogindate > '2004-11-18 14:00' and a.accountname not in (select accountname from levelranking)
)
...
where mapindex = 0 and time > '2004-11-18 14:00' and time < '2004-12-12 23:59'
order by time desc
GO
...
as
	...
	from td_Account
	where LastLoginDate >= '2004-11-18 14:00' and RegisteredDate >= '2004-11-18 14:00' and accounttype = 0
go
...
where race < 128  and charactername not like 'loadtest%' and laststartedtime > '2004-11-18 14:00'
--order by level desc
go
...
AS
	...
	from td_character c, (select top 10000000 characteruniquenumber, sum(playtime) as TotalPlayTime
						...
						where logtype = 20 and time > '2004-11-18 14:00' and time < '2004-12-12 23:59'
...
from (select time, atum.atum_getonlydate(time) as date
		...
		where logtype = 22 and time > '2004-11-18 14:00' and time < '2004-12-12 23:59') ll
group by date
order by date
...
where logtype = 22 and time > '2004-11-18 14:00' and time < '2004-12-12 23:59' and l.characteruniquenumber = c1.uniquenumber and l.param1 = c2.uniquenumber
order by time

select time, c1.charactername, c1.level,  c2.charactername, c2.level, l.mapindex, atum.atum_getonlydate(time)
from atum.atum_log_user_quest_pk_dead l, atum.td_character c1, atum.td_character c2
where (logtype = 22 or logtype = 23) and time > '2004-11-18 14:00' and time < '2004-12-12 23:59' and l.characteruniquenumber = c1.uniquenumber and l.param1 = c2.uniquenumber
order by time

-- 캐릭터별 1:1 전투 회수
select c1.charactername, (select level from atum.td_character where charactername = c1.charactername) as level, count(*) as count
from atum.atum_log_user_quest_pk_dead l, atum.td_character c1, atum.td_character c2
where (logtype = 22 or logtype = 23) and time > '2004-11-18 14:00' and time < '2004-12-12 23:59' and l.characteruniquenumber = c1.uniquenumber and c1.uniquenumber = c2.uniquenumber
...
from (select time, atum.atum_getonlydate(time) as date
		...
		where logtype = 21 and time > '2004-11-18 14:00' and time < '2004-12-12 23:59') ll
group by date
order by date
...
where l.characteruniquenumber = c.uniquenumber and logtype = 21 and time > '2004-11-18 14:00' and time < '2004-12-12 23:59'
...
from playtime p, levelranking l,
	...
	where logtype = 16 and time > '2004-11-18 14:00' and time < '2004-12-12 23:59'
	group by characteruniquenumber) ll
where p.accountuniquenumber = l.accountuniquenumber and ll.characteruniquenumber = p.uniquenumber and l.registereddate < '2004-11-18 14:00'
order by p.accountname

-- 기존 유저 통계
select (level-1)/5, count(*)
from levelranking
where registereddate < '2004-11-18 14:00'
group by (level-1)/5
...
where time > '2004-11-18 14:00' and time < '2004-12-12 23:59'
GO
